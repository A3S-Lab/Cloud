...
| Durable Cells | `CELL0.1-C1/C2/C3` freeze the application foundation; component-only `CELL0.2-C1` adds the first shared S0 conditional-object port/destructive CAS probe, one exact Secret-version reference, digest-locked namespace credential binding, and plaintext-free Durable Cell storage correlation without another object client | Active-Secret admission/materialization, sealed recovery/retention/deletion, real S0 provider evidence, Runtime/Box provider adapter, orchestration, Gateway publication, and `CELL0.5` availability remain |
