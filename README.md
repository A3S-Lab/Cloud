...
| Durable Cells | `CELL0.1-C1/C2/C3` freeze the canonical Service/application ACL, digest-locked shared fixtures, BuildRun/bundle/profile bindings, ordered Cell classes, state-schema migration/rollback rules, immutable revision lineage, desired-state aggregate, and deterministic S0/Workloads/Operations projection identities using existing owners | S0 binding, Runtime/Box provider adapter, persisted orchestration correlation, Gateway publication, real fault evidence, and `CELL0.5` availability remain |
