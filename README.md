...
| Durable Cells | `CELL0.1-C1/C2/C3` freeze the application foundation; component-only `CELL0.2-C1/C2` reuse the sole object client and Secrets authority for CAS probing, exact active-version credential admission/materialization, sealed S0 recovery lineage, isolated restore verification, retention policy, grace-delayed deletion evidence, and exact Durable Cell storage correlation | Real S0/provider execution and retained evidence, Runtime/Box provider adapter, orchestration, Gateway publication, and `CELL0.5` availability remain |
