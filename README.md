...
- **Source Delivery Slices**: Resolve exact GitHub commits, run isolated
  BuildKit tasks, validate and publish OCI graphs, and hand successful builds
  to the existing workload deployment path
...
| `G0` — External source delivery | Pinned Git sources, isolated builds, OCI validation and publication, provenance, and deployment handoff | In progress |
...
complete OCI graph validation, digest-only registry publication, replay
adoption, cleanup, and explicit deployment handoff. It remains in progress
until provenance, SBOM, signing, cache-trust, and external private-provider
certification pass.
...
source revision and lineage. Node-local Artifact locations and provider
credentials are not part of the public BuildRun state.
