...
The Artifacts context can now reserve one deterministic durable build run for
this revision and reconcile the source-commit-to-`cloud.build@1` operation
crash gap exactly once. That reconciler is covered by in-memory and isolated
PostgreSQL gates, but it is not enabled in the production worker until the
Build Flow runtime can execute the operation as an isolated Runtime Task.
The prerequisite Artifact data plane is implemented independently: the
control plane and node agent stream raw directory archives over mTLS, bind each
request to the assigned unexpired command and exact Runtime spec digest, and
persist content-addressed receipts. Docker consumes materialized inputs only as
read-only bind mounts and captures declared successful Task outputs back into
the same verified cache. This transport is not the Build Flow itself; the next
G0 slice must register and execute `cloud.build@1` through it.
...
| G0 — External source delivery | Pinned Git commits, isolated builds, OCI publication, provenance, and deployment through the existing workload path | In progress (immutable source/recipe contract, anonymous-first and installation-token GitHub resolution, credential-safe exact-commit checkout, signed lifecycle-aware subscription fanout, deterministic durable build intent/crash-gap reconciliation, command-bound mTLS Artifact transport, Docker Artifact mounts/outputs, and rootless BuildKit OCI-output validation implemented; Build Flow Runtime execution, authoritative polling, registry publication, and external private-repository evidence pending) |
