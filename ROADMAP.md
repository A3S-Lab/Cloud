...
| `MCP0.3` | Cloud | Foundation in progress | Closed contract values, canonical immutable Service-profile and separately mutable Edge route-policy ACL admission, migrations 052-054 plus typed A3S ORM persistence, exact AssetRelease/profile-bound WorkloadRevision identity, automatic opaque Runtime Service-spec binding, and deterministic Gateway ACL compilation exist; credential authority resolution, healthy target reconciliation, lifecycle surfaces, recovery, audit, and the real PostgreSQL gate remain |
