...
| `AUT0.3` | In progress; component-only calendar/misfire/concurrency/due-window/lease, durable cursor/lease, exact invocation-envelope, and bounded schedule-dispatch boundaries | Timezone-aware schedules, catch-up rules, timer ownership, and P0 scheduled Task adaptation through the existing Boot task rail remain the full gate |
