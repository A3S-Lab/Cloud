...
| `AUT0` — Automations and Connectors | Schedule, webhook, plugin/source-event triggers and reusable outbound HTTP/business connections with exact targets, deduplication, Secret/egress policy, and recovery | In progress and unavailable; Automations C1-C6 plus AUT0.2-C7 deterministic webhook admission, the durable immutable definition/revision catalog in migration 188, AUT0.3-C1-C12 calendar/misfire/concurrency/due-window/lease/state/envelope, bounded schedule dispatch, injectable schedule-worker boundary, P0 Task adaptation, idempotent invocation admission, and digest-verified invocation handoff components are implemented. AUT0.4-C1/C2 normalized event-envelope and bounded fan-out components are implemented; AUT0.4-C3 adds the strict durable-ack A3S Event consumer boundary for already normalized events, and C4 composes external candidate selection, deterministic fan-out, and idempotent invocation admission. Connector C1-C13 plus Flow-owned attempt/wait, immutable-response, typed JSON interpretation, exact revision revocation, and terminal-indeterminate recovery are component foundations; live PostgreSQL recovery, production candidate discovery/owner composition, timer process registration/listener/Gateway wiring, and public availability remain open |
