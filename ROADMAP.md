...
| `AUT0.5` | Planned; `C1`-`C8` foundation only | Complete Workflow Flow scheduling and immutable response-object composition, remaining provider/Event-consumer wiring, revocation/recovery operations, and retained integration evidence over the existing profile/revision, materializer, egress, executor, attempt, evidence, management-surface, and Workflow-attempt authorities; Flow remains the retry/backoff scheduler |
