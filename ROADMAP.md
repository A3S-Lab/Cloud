...
- REST major version 1 publishes one unauthenticated raw OpenAPI 3.0.3 snapshot
  ...
  responses, envelopes, and compatibility metadata; current contract `1.70.0`
  adds the closed immutable Harness invocation profile and typed digest-only
  Agent Tool request/result content without Tool payload or Secret material. It
  retains `1.69.0`'s provider selection and immutable provider evidence, the
