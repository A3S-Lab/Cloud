...
| `CELL0.4` | In progress | Component-only `C1` persists application heads and immutable canonical-ACL revisions through migration `116`, the existing A3S ORM Migrator, and the shared idempotency/Outbox/audit transaction path; its PostgreSQL gate is checked in but has no retained pass. `C2` authorized CQRS, `C3` deployment correlation plus Workload/S0/Operations composition, `C4` Gateway publication, `C5` REST/client/CLI/Management MCP, and `C6` retained recovery evidence remain; Web stays deferred |
