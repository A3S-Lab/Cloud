...
| `A0.5` | In progress | Exact hosted Git archives publish as immutable content-addressed Skill bundles, and active Agent Workloads bind, rebind, or unbind exact releases through new revisions, read-only Runtime Artifact mounts, migration 067 persistence, rollback-safe history, and tenant-authorized REST/client/CLI/Management MCP surfaces. CI now retains a REST/PostgreSQL/local-object gate for exact archive bytes, receipt identity, replay, yanking, and cleanup; real Box lifecycle/binding evidence still blocks verification; no generic forge surface is added |
