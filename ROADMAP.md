...
| `AUT0.5-C2` | Implemented locally; PostgreSQL 17 gate wired | One environment-scoped `ConnectorProfile` head advances through immutable, digest-linked `ConnectorRevision` rows. The owner parser admits canonical `cloud.connector.http.v1` A3S ACL only; migration `109` persists exact Secret ID/version bindings with scope/version foreign keys, immutable triggers, optimistic concurrency, shared idempotency, Outbox, audit, and A3S ORM. No plaintext, copied Secret state, execution attempt, retry rail, scheduler, or product surface is added. |
| `AUT0.5` | Planned; `C1`-`C2` foundation only | Add authorized application commands/queries, just-in-time Secret materialization with active-state rechecks, production egress/SSRF enforcement, execution evidence, revocation/recovery, and Workflow HTTP/service ports over the existing profile/revision authority and execution port; Flow remains the retry/backoff scheduler |
