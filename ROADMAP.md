...
| `C0.3-N2g` | Implemented; retained CI evidence pending (`2026-08-15`) | The existing PostgreSQL notification fixture now has an opt-in real JetStream path driven by the same transactional Outbox relay, exact-subject A3S Event durable/manual-ack consumer, and PostgreSQL delivery repository. It commits one C6-shaped terminal receipt before ACK, restarts the same durable consumer, injects an exact terminal replay, and proves the replay is ACK-only without another dispatcher call. CI starts the repository's existing digest-pinned NATS image and reruns the same PostgreSQL gate with that path enabled; it adds no product queue, retry loop, repository, table, parser, or configuration format. |
