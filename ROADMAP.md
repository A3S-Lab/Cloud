...
- REST major version 1 publishes one unauthenticated raw OpenAPI 3.0.3 snapshot
  ...
  responses, envelopes, and compatibility metadata; current contract `1.54.0`
  retains the exact-owner redacted recipient-contact self-service lifecycle and
  SMTP-only outbound-subscription v4 target union, then adds alert-policy v2 for
  the single closed Fleet Node-availability source and one exact Node target.
...
The current catalog contains 132 administrator tools and 72 read-only tools:
...
investigation, audit retention/export policy, and product usage-fact attribution
remain planned. `C0.3-PA2a` verifies the audit-attribution prerequisite to
signed export through the [PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/32632245460/job/97176670880)
and [complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/32632245460).
Therefore `C0.3` is still in progress rather than fully verified.
