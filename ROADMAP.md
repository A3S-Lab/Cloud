...
| `MCP0.3` | Cloud | Foundation in progress | Closed contract values, canonical immutable Service-profile and separately mutable Edge route-policy ACL admission, migrations 052-055 plus typed A3S ORM persistence, exact AssetRelease/profile-bound WorkloadRevision identity, automatic opaque Runtime Service-spec binding, hosted credential lifecycle and persistence, scope-complete healthy-target planning, deterministic release-bound projection, and Gateway ACL compilation exist; one-time credential issuance, grant/credential authority resolution, durable snapshot reconciliation/publication, lifecycle surfaces, recovery, audit, and the real PostgreSQL gate remain |
