...
pass. The retained clean A3S Box/PostgreSQL gate passed the pre-template
catalog and must be rerun for the 77/47 catalog before the new slice is
verified. Its strict `W0.2` evidence already proves exact Ontology persistence, rejected-write
...
| `W0.3` | In progress; planning, native Form, WorkflowRun, HumanTask loop, and finite Execution step implemented | Migrations `076`, `079`, `080`, `081`, `096`, and `097` retain the immutable Workflow definition/Goal/Plan, native Form, exact Goal/Plan-bound WorkflowRun, HumanTask decision/resume, automatic-expiry, and parent-cancellation authorities described above. Migrations `098` and `099` add Executions-owned immutable ACL-native ExecutionTemplate revisions plus exact WorkflowRun/Plan/step/attempt/template/digest columns, composite foreign keys, and one child uniqueness index on the existing Execution aggregate. One A3S Flow run executes Workflow-local `input`, `transform`, `branch`, `human_decision`, `execution`, and `output` steps. An `execution` step requires one exact environment and `executions/execution_template/execution.run` capability. The worker coordinator calls one typed Executions application port, creates or adopts the replay-safe ordinary Execution, links its existing Operation as the Flow child, resumes only from an authority-bound terminal result, and waits for cleanup-first child cancellation before parent cancellation or timeout. The retained PostgreSQL `SIGKILL` fixture now covers seven boundaries: the four existing WorkflowRun/HumanTask boundaries plus child-Execution commit before Operation enqueue, exact child link before parent projection, and terminal child resume before parent projection. The Management MCP provider gate reuses one shared `contracts/w0.3/execution-template.acl` fixture across REST and MCP and asserts accepted/rejected idempotency, Outbox, audit, migration `098`, immutability, and tenant non-disclosure. REST/OpenAPI `1.24.0`, the maintained client, CLI, and 32 planning/Form/run/task/template Management MCP tools reuse the same CQRS, Resource Grant, idempotency, A3S ORM, Outbox, and audit authorities. Focused finite-step tests and provider-gate sources pass local compilation/static checks; the retained clean Linux PostgreSQL/provider runs must turn green before this slice is verified. Business-service and remaining Agent/MCP/model/Tool dispatch, compensation, expanded clean provider evidence, and public availability remain required |
...
10. retain the implemented `W0.1` contracts, backend `W0.2` Ontology lifecycle,
    ...
    compatibility lock and native submitted-value parity, then expand the real
    PostgreSQL/provider cross-surface and process-death evidence for the exact
    ExecutionTemplate child binding before closing `W0.3` without waiting for
