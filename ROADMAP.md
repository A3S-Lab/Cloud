...
| `A1.6` | Implemented component; retained provider/Box certification pending | Migration `168` persists digest/size/path projections for bounded canonical logical trajectory snapshots in the shared `agent-checkpoints` object namespace and adds immutable parent execution/checkpoint lineage. Authorization-first capture/list/read/snapshot, paged trajectory, and fork APIs use REST/OpenAPI `1.73.0` and the maintained client; a fork creates a new execution and materializes one verified provider-neutral prompt without mutating its parent. Exact PostgreSQL/real-Box crash, cleanup, and provider-private checkpoint capability/fallback certification remain open; unsupported private suspend/resume is not emulated or advertised |
