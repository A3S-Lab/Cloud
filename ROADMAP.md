...
| `C0.3-RG2` | In progress; Workloads, external-source BuildRun, and ordinary Route slices implemented | A typed route-metadata contract admits indirect project-owned requests only when the caller has coarse project visibility. Workloads resolves Workload, Deployment, and workload-log IDs through its existing repository; Artifacts resolves BuildRun detail, evidence, logs, cancellation, and retry through the BuildRun repository; Edge resolves ordinary Route detail through the Edge repository. Each owner derives the canonical environment scope and calls the shared evaluator at the application boundary across REST and Management MCP. Denied and missing IDs share the same `404` contract, and mutation authorization runs before idempotency replay so revocation applies on the next request. Hosted Asset-release BuildRuns are organization-scoped today and therefore remain available to organization-wide roles while restricted memberships fail closed; no synthetic project ownership is inferred. MCP Route Policy, DomainClaim, and Credential remain separate Edge aggregates and must resolve their own canonical scopes rather than piggybacking on ordinary Route identity. Remaining Asset, Form, Workflow, Secret, Agent execution, and Operation boundaries must follow the same pattern. No Identity-owned cross-context ownership table, presentation-only filter, or context-local grant evaluator is allowed. |
