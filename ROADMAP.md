...
| `AUT0.2-C2` | Implemented; component-only | `control-plane/modules/automations` adds one application-owned admission service and atomic in-memory repository. Endpoint creation pins the exact revision and scoped opaque key; lifecycle changes use generation compare-and-swap; signature verification and schema evaluation cross explicit infrastructure ports; delivery identity/body replay, duplicate conflicts, lifecycle rejection receipts, redacted audit records, and admitted Outbox messages are committed under one repository lock. The repository also resolves opaque endpoint keys only within the complete organization/project/environment scope, ready for a future transport adapter without widening tenant lookup. Focused tests cover scope collisions, scoped key lookup, replay/body drift, lifecycle recovery fences, required ports, and secret-reference redaction. PostgreSQL durability is supplied by C3; no HTTP listener, Gateway route, worker, or public availability is claimed. |
