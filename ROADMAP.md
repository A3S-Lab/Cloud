...
| `AUT0.3-C10` | Implemented; application and PostgreSQL admission boundary; runtime evidence pending | `AutomationInvocationAdmissionService` validates one exact envelope. The in-memory adapter and migration `187`/`PostgresAutomationInvocationRepository` fence invocation identity and policy-derived deduplication keys, replay exact evidence as `AlreadyAdmitted`, and atomically retain the immutable envelope, redacted Audit, and one Outbox fact only on first admission. The isolated PostgreSQL integration gate covers first admission, exact replay, immutable identity and deduplication conflicts, reconnect recovery, and organization isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured; a retained live PostgreSQL result remains pending. `AutomationsDispatchServices` composes schedule and normalized-event dispatch behind this same admission port, and PostgreSQL webhook delivery admission now enters that transaction instead of maintaining a second invocation Outbox writer. No timer, queue, target execution, mutable revision lookup, production worker composition, or P0 handoff is introduced. |
