...
| `C0.5-MT2-C1` | Verified persistence; atomic public bootstrap implemented, recertification pending (`2026-08-29`) | Migration `177` and the sole `IPlatformRbacRepository` persist immutable accepted policy revisions, one current head, and versioned bindings. One Installation mutation lock, optimistic CAS, active-Principal checks, self-escalation denial, owner-only owner administration, and deferred database last-owner/Principal-disable constraints serialize every replica while reusing shared idempotency, Audit, and Outbox. The dedicated `IIdentityBootstrapRepository` now creates the first Organization/Principal/owner Membership/API token and matching baseline policy/`PlatformOwner` binding in one PostgreSQL transaction; it checks replay after the bootstrap lock and reuses the exact transaction-local platform writer. The retained provider gate injects a platform-fact failure and requires zero partial rows, then races/replays successful bootstrap and requires one matching authority root. Commit `459effcb` passes local architecture, application, domain, compile, formatting, and clippy gates; main PostgreSQL 17 recertification is pending. The prior [PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33224399567/job/99025035853) proves competing standalone platform bootstrap, policy-head advancement, owner revocation, and direct-SQL rejection. A controlled recovery transition for installations created before this atomic root and maintained administrator interfaces remain open. |
...
| `C0.5-MT2-C3` | In progress; atomic issuance verified, protected-mutation composition implemented (`2026-08-29`) | The sole transaction-local privileged authorization issuer share-locks the active Principal, exact API-token version, current policy/binding, and exact optional tenant-support grant, then commits one digest-bound allow through shared scoped Audit. The internal `AuthorizePrivilegedAccess` command and all seven non-bootstrap platform-RBAC/support mutations reuse it: write DTOs carry only actor Principal plus exact credential ID, each concrete use case fixes permission/action/scope/resource, server-derived credential evidence enters proposals/approvals/revocations, and the protected business Audit fact links the exact decision in the same PostgreSQL transaction. Role, token, grant, and business writes retain one database order; the PostgreSQL gate also races a protected binding mutation against exact-token revocation and requires decision plus business fact to commit or roll back together. No Redis/Lane lock, cache truth, dedicated decision table, or authorization Outbox is added. The prior [complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/33226790289) and its [PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33226790289/job/99031980422) verify standalone issuance/revocation serialization; retained certification of the new protected-mutation race is pending. Completion still requires maintained concrete Application/REST/OpenAPI/client/CLI/MCP interfaces to derive actor/credential from verified request context, followed by `MT3` cross-surface bypass removal; no generic client-authored evaluator will be exposed. |
...
Main PostgreSQL recertification and an operator-controlled recovery transition
for installations created without that root remain open. The retained
[PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33220123607/job/99012267599)
...
business Audit fact. A retained PostgreSQL gate races a binding mutation
against exact-token revocation and admits only an atomic decision-plus-write or
an atomic denial. Maintained concrete interfaces remain before `MT2-C3`
completes; they must derive identity from verified request context and cannot
expose a caller-authored generic evaluator.
