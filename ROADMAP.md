...
| `AUT0.2-C3` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI step wired | Migration `183` and `PostgresAutomationWebhookRepository` persist the exact endpoint projection plus canonical revision ACL/digest, one immutable first-delivery record, and append-only replay/conflict/lifecycle receipts. Endpoint lifecycle updates are generation-fenced and identity/revision immutable; delivery/receipt rows reject mutation; restore reparses and validates canonical contracts before admission. Shared Audit and Outbox tables are written in the same transaction, while Secret plaintext, retry/scheduler state, HTTP transport, worker, and public routes remain out of scope. The isolated PostgreSQL integration gate now covers scoped lookup, first admission, exact replay, reconnect recovery, wrong-environment isolation, and concurrent lifecycle generation fencing; it executes when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. A live PostgreSQL 17 recovery/concurrency result remains before availability. |
...
| `AUT0.3-C6` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI step wired | Migration `184` and the in-memory/PostgreSQL schedule-state repositories persist one exact Automation revision digest, exclusive due cursor, monotonic lease generation, and owner/token/timestamp fence. Acquisition and cursor commit lock the one state row and reject stale, active, expired, non-rotated, or out-of-order fences. Migration `186` also permits an exact owner to release a fence without cursor progress. The isolated PostgreSQL integration gate now covers concurrent lease fencing, cursor commit, reconnect recovery, generation rotation, cursor-preserving release, and wrong-scope isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured; a retained live PostgreSQL result remains pending. The state owner stores no definition copy, invocation, queue, retry, worker, or scheduler; invocation admission, timer ownership, and P0 handoff remain open. |
...
| `AUT0.3-C10` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI step wired | `AutomationInvocationAdmissionService` validates one exact envelope. The in-memory adapter and migration `187`/`PostgresAutomationInvocationRepository` fence invocation identity and policy-derived deduplication keys, replay exact evidence as `AlreadyAdmitted`, and atomically retain the immutable envelope, redacted Audit, and one Outbox fact only on first admission. The isolated PostgreSQL integration gate now covers first admission, exact replay, immutable identity and deduplication conflicts, reconnect recovery, and organization isolation; it executes when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. A live PostgreSQL 17 recovery/concurrency result remains before availability. `AutomationsDispatchServices` composes schedule and normalized-event dispatch behind this same admission port, and PostgreSQL webhook delivery admission now enters that transaction instead of maintaining a second invocation Outbox writer. No timer, queue, target execution, mutable revision lookup, production worker composition, or P0 handoff is introduced. |
