...
| `AUT0.4` | In progress; component-only event-envelope boundary | Plugin-trigger subscriptions and normalized event dispatch while Sources retains provider connection/revision authority and U0 retains package authority; C1 freezes the normalized event identity/filter port and C2 adds bounded deterministic fan-out over already selected exact revisions |
