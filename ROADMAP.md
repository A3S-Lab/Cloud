...
| `AUT0` — Automations and Connectors | Schedule, webhook, plugin/source-event triggers and reusable outbound HTTP/business connections with exact targets, deduplication, Secret/egress policy, and recovery | In progress and unavailable; Automations C1-C6 plus the AUT0.2-C7 deterministic webhook invocation factory, AUT0.3-C1 calendar, C2 misfire, C3 concurrency, C4 due-window, C5 lease, C6 durable cursor/lease, C7 deterministic due-time envelope, and AUT0.4-C1/C2 normalized event-envelope and bounded fan-out components are implemented. AUT0.4-C3 adds the strict durable-ack A3S Event consumer boundary for already normalized events, and C4 composes external candidate selection, deterministic fan-out, and idempotent invocation admission. Connector C1-C13 plus Flow-owned attempt/wait, immutable-response, typed JSON interpretation, exact revision revocation, and terminal-indeterminate recovery are component foundations; live PostgreSQL recovery, production candidate/admission owners, listener/Gateway wiring, and public availability remain open |
...
| `AUT0.3` | In progress; component-only calendar/misfire/concurrency/due-window/lease, durable cursor/lease, and exact invocation-envelope boundaries | Timezone-aware schedules, catch-up rules, scheduler/event ownership, and P0 scheduled Task adaptation through the existing Boot task rail remain the full gate |
