...
| `F0` — Foundation | Boot control plane and PostgreSQL task queue, PostgreSQL, tenancy, identity, ORM-backed Flow operations, outbox, projections, and API | In progress; re-certification required because the last complete pass used Flow `0.12.0`, while the current exact lock pins Flow `0.13.1` for the shared DAG compiler, startup-validates one exact workflow/step runtime registry, and pins one ACL `0.3.0` source with Boot `0.2.0` and ORM `0.3.0`; upstream ACL `0.2.2` and Flow `0.11.0` constraints must converge |
...
The shared execution substrate now pins A3S Flow `0.13.1`, A3S Boot `0.2.0`
