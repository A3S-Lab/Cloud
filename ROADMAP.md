...
| `AUT0.5` | Planned; `C1` foundation only | Add reusable immutable Connector profiles/revisions, A3S ACL admission, Secret references and just-in-time materialization, production egress/SSRF enforcement, authorization, persistence, execution evidence, revocation/recovery, and Workflow HTTP/service ports over the existing execution port; Flow remains the retry/backoff scheduler |
