...
| `C0.3-N5a` | Frozen | Identity owns one exact human-Principal-bound email `RecipientContact` and its short-lived, one-time `RecipientContactVerification` challenge. A pending contact is not usable for delivery. Only possession of a cryptographically verified, exact-contact/address/version/expiry-bound proof may atomically verify it; OIDC claims, Membership data, administrator assertion, presentation input, and Notifications cannot do so. Reissue invalidates older pending challenges, completion consumes one exact challenge, and contact revocation applies on the next resolution. Verification-requested, verified, and revoked Outbox facts plus shared audit records carry only opaque contact/challenge IDs, state, address digest, and timestamps—never the mailbox or proof. Identity exposes an internal exact-owner resolver that returns the canonical mailbox only for an active verified contact; Notifications may later pin that opaque contact ID but cannot copy or query a directory. `N5a` is component-only: migration `136`, the repository/application boundary, token signer/verifier port, PostgreSQL evidence, SMTP challenge delivery, REST/client/CLI/MCP surfaces, and notification composition remain open. No second identity/contact store, email inference, plaintext proof persistence, address/token event, queue, scheduler, provider configuration, or SMTP client is authorized. |
