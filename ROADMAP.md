...
| `C0.3-OIDC3` | Planned | Compose `OIDC1` persistence and `OIDC2` verification through Identity application commands, then expose bounded login/link begin and callback routes through REST/OpenAPI and the maintained client. Prove exact state consumption, configuration drift, replay/concurrency, invitation or closed-policy admission, tenant non-disclosure, token redaction, and PostgreSQL atomicity through the production application before advertising external sign-in. |
