...
| `AUT0.2` | In progress; component-only admission boundary | Signed webhook endpoints, bounded capture, schema evaluation, exact target, replay, disable/revoke, durable recovery, and Gateway recovery remain the full gate. C1-C6 contracts, admission, PostgreSQL persistence, exact-version HMAC verification, digest-bound evaluation, and exact-digest registry selection adapter are implemented; live PostgreSQL recovery evidence, production registry wiring, listener/Gateway wiring, and public availability remain open |
