...
**Pragmatic delivery authority:** critical-path GAs, `software` / `tee` / `ha`
profiles, and which portfolio items are deferred off the first exits live in
[architecture-optimization-roadmap.md](docs/architecture-optimization-roadmap.md).
This file still publishes every gate's public state; it does not treat the full
portfolio matrix as one Definition of Done.
...
The cumulative product **aspiration** is an A3S-native platform that covers
...
That aspiration is **not** the first Verified EXIT. Mission EXIT follows the
pragmatic GAs in
[architecture-optimization-roadmap.md](docs/architecture-optimization-roadmap.md):
software Box path (GA-0), one Agent vertical (GA-1), one Workflow/Application
vertical (GA-2), then day-two HA and deferred verticals. Reference-product
breadth (`APP0.6`, full `CELL0`, distributed `I0`, `EV0`) stays in the
portfolio without blocking those exits.
...
| --- | --- | --- |
| `BX0` — Box-only platform | Sole A3S Box execution/build path and Box re-certification of the complete Runtime, deployment, source-delivery, recovery, and cleanup baseline | **`BX0.software` Verified (`2026-09-17`)** — retained LIVE evidence [docs/evidence/bx0-software-exit-2026-09-17](docs/evidence/bx0-software-exit-2026-09-17/): `A3S_CLOUD_BX0_CLEAN_HOST_EXIT_CERTIFIED … profile=software power_revision=UNBOUND loop=included receipts=included` on Docker-free Linux (`docker.sock` absent); pins `cloud=140a62d04f1309d549f8764426328fc92bbbf473` `runtime=4c5fbd56bedd84d1007a7d9cd046a9f7083bbdcd` `box=8b2804c585d2f31bce06f213990407606a475634` `gateway=e92896769953aee28ef69261f77265e427f9d396`; CREATE enroll→OCI→deploy→health→HTTPS→logs→update→rollback→cleanup via `run_bx0_software_exit_live_chain.sh`. Checklist [ga0-bx0-software-checklist.md](docs/ga0-bx0-software-checklist.md). **`BX0.tee`** remains In progress / blocked on physical SEV capacity (not Verified). Retained Box provider conformance evidence on [34737151481](https://github.com/A3S-Lab/Cloud/actions/runs/34737151481). Software EXIT does not wait on TEE or PW0 |
| `PW0` — Power inference boundary | ACL-native immutable Power Service profile, Box-hosted health, inference, recovery, and cleanup | Planned; `PW0.software` follows `BX0.software` for ordinary inference availability; `PW0.tee` remains a stronger confidential profile and does not gate GA-0 |
...
| `K0` — Knowledge and Knowledge Pipeline | User files, Knowledge Bases, document/chunk lifecycle, multi-source ingestion, General/Parent-child/Q&A and multimodal processing, indexing/retrieval/rerank/citations, external Knowledge, and Flow-backed Knowledge Pipelines | In progress and unavailable; `K0.1-C1/C2` implement strong identities, one canonical Files admission ACL/lifecycle, exact upload/scan receipts, shared streaming objects, atomic quota/persistence, authorization-first CQRS, shared audit/Outbox/idempotency, REST/OpenAPI, client, CLI, and Management MCP tools. `K0.1-C2b1` adds authorized public `PUT .../content` (OpenAPI `1.89.0`) over the same upload command with local object-store tests; `K0.1-C2b2` adds admitted-only public `GET .../content` (OpenAPI `1.90.0`); `K0.1-C2b3` adds metadata-only public `POST .../scan` admit/reject decisions (OpenAPI `1.91.0`) and a sixth Management MCP tool; `K0.1-C2b4` adds metadata-only public `POST .../expire` for awaiting uploads (OpenAPI `1.92.0`) and a seventh Management MCP tool. Live object cleanup execution remains open. A retained [PostgreSQL 17 H0 persistence step](https://github.com/A3S-Lab/Cloud/actions/runs/33159659047/job/98810769471) verifies rollback, concurrent organization-quota serialization, lifecycle replay, quota release, and atomic side effects through the production owner ports. `K0.1-C3` freezes Knowledge contracts; `K0.1-C4a`/`C4b1`/`C4b2a` add catalog persistence, owner services, and authorized writes; `K0.1-C4b2b1`/`C4b2b2` expose KnowledgeBase/Pipeline lifecycle through REST/OpenAPI `1.88.0`, client, CLI, and Management MCP. `K0.1-C5` persists immutable KnowledgeDocument/Chunk catalogs through migration `202` with local/PostgreSQL adapters and no public surface; `K0.1-C6` adds application owner document/chunk catalog services over those repositories. `K0.1-C7` adds authorized idempotent KnowledgeDocument/Chunk create writes with audit and Outbox side effects over the C5 repositories without public surfaces. `K0.1-C8` exposes that document/chunk boundary over REST/OpenAPI `1.93.0` without client/CLI/MCP or live MinIO/scanner/SEV claims. `K0.1-C9` adds maintained client, CLI, and four Management MCP tools over the same C8 handlers without bumping OpenAPI or claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C10` adds authorized bounded KnowledgeDocument/Chunk list through REST/OpenAPI `1.94.0`, client, CLI, and two Management MCP read tools without claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C11` persists immutable KnowledgeIndexRevision/RetrievalPolicyRevision/ExternalKnowledgeBinding catalogs through migration `203` with local/PostgreSQL adapters and no public surface. `K0.1-C12` adds application owner index/policy/binding catalog services over those repositories. `K0.1-C13` adds authorized idempotent index/policy/binding create writes with audit/Outbox. `K0.1-C14` exposes that index/policy/binding boundary over REST/OpenAPI `1.95.0` without client/CLI/MCP or live MinIO/scanner/SEV claims. `K0.1-C15` adds maintained client, CLI, and six Management MCP tools over the same C14 handlers without bumping OpenAPI or claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C16` adds authorized bounded index/policy/binding list through REST/OpenAPI `1.96.0`, client, CLI, and three Management MCP read tools without live MinIO/scanner/SEV/cleanup claims. Live MinIO/S3 provider wiring, live scanner/cleanup execution, ingestion, retrieval, and Flow-backed Knowledge Pipelines remain open |
| `AUT0` — Automations and Connectors | Schedule, webhook, plugin/source-event triggers and reusable outbound HTTP/business connections with exact targets, deduplication, Secret/egress policy, and recovery | In progress and unavailable; Automations C1-C6 plus AUT0.2-C7 deterministic webhook admission, the durable immutable definition/revision catalog in migration 188 (H0 CI PostgreSQL recovery wired), AUT0.3-C1-C12 calendar/misfire/concurrency/due-window/lease/state/envelope, bounded schedule dispatch, repository-backed owner-port candidate discovery, injectable schedule-worker boundary, P0 Task adaptation, idempotent invocation admission, and digest-verified invocation handoff components are implemented. AUT0.4-C1/C2 normalized event-envelope and bounded fan-out components are implemented; AUT0.4-C3 adds the strict durable-ack A3S Event consumer boundary for already normalized events, and C4 composes external candidate selection, deterministic fan-out, and idempotent invocation admission. Connector C1-C13 plus Flow-owned attempt/wait, immutable-response, typed JSON interpretation, exact revision revocation, and terminal-indeterminate recovery are component foundations; live PostgreSQL recovery, production process-role composition, timer process registration/listener/Gateway wiring, and public availability remain open |
...
| `I0` — Distributed inference service | Accelerator-backed Power serving, independent replicas, gang-distributed model replicas, typed prefill/decode role pools, cache/load-aware Gateway dispatch, scoped keys, routing/fallback, durable usage, and governed self-service | Planned; Track A control plane continues now. Single-node Track B follows `BX0.software`+`PW0.software`. Gang/PD-pool distributed serving is P2 and not GA-0..2 EXIT |
| `EV0` — Governed self-evolution | Authorized evidence datasets, reproducible evaluation and reward policy, Agentic RL candidate jobs, approval-gated promotion, canary observation, and exact rollback | Planned; **P2 only** — retained in portfolio, explicitly off the critical path until GA-2 and safety foundations exist |
| `AR0` — Governed Agent Runtime experience | One simplified projection over existing Agent, Workload, Deployment, Operation, Runtime, Box, Secret, and evidence authorities; bounded egress, brokered credentials, context-cost evidence, idle policy, and checkpoint/fork experience without a parallel lifecycle | Planned; after GA-1 Agent availability; `AR0.1` waits for `A1.3` and `BX0.software` |
...
5. `BX0.5` has removed the retired build executors and adds an architecture
   ...
   staying `OPEN`, with steps 1–9 enroll…stop/cleanup preflight only
   ...
   `A3S_CLOUD_BX0_EXECUTE=1` may record steps 1–9 `*=executed` only with real
...
certification. Hardware SEV remains the **`BX0.tee` / confidential** gate, not
the sole Verified door for Box-backed product EXIT. **Physical-capacity rule
(2026-09-17):** with zero `sev-snp` runners and no `/dev/sev` on available
hosts, **skip** `BX0.tee`, `PW0.tee`, confidential-workload claims, and any
post-TEE-only features. Do **not** skip `BX0.software`, GA-0 clean-host
enroll→deploy→HTTPS→logs→update/rollback, or ordinary Agent/Workflow
availability work that only needs software Box. Skipping TEE slices is
required honesty; it is not Verified tee and it must not freeze the software
exit. WSL2 and ordinary
GitHub-hosted runners have KVM at best and cannot supply `/dev/sev`. ACL-native
simulation stays distinct from hardware evidence. Do not promote `PW0.tee` or
tee-profile EXIT until that isolation evidence exists. Cloud binds tee evidence
only through
...
`simulate=false` certification line cites a green Box hardware SEV run. Prefer `collect_bx0_tee_isolation_evidence.sh` / `verify_bx0_tee_box_hardware_run.sh`, which re-check the Box job remotely (`success` on the pinned `box-revision`) and reject hand-edited skipped-run certs. Operator sequence: `tools/box-conformance/OPERATOR_TEE.md`. Sandbox re-cert tip remains `dbe34158` / run `34737151481`; later TEE-binder commits on this branch do not invent Verified tee.

`PW0.1` / `PW0.software` may follow `BX0.software` for ordinary
Box-hosted Power serving without waiting on SEV. `PW0.tee` follows the
required `BX0.tee` isolation evidence and remains **unstarted** under the
physical-capacity stop (no online `sev-snp` runner); do not implement tee
slices to simulate roadmap motion. Power
...
attestation where claimed, process/VM recovery, update, rollback, and cleanup.
...
Execution sequencing (Wave 0 architecture integrity → GA-0 `BX0.software` →
GA-1 Agent vertical → GA-2 Workflow/Application vertical → GA-3 day-two `ha` →
GA-4+ deferred verticals, plus dual-track I0) is specified in
[architecture-optimization-roadmap.md](docs/architecture-optimization-roadmap.md).
This section retains horizon outcomes; it does not duplicate wave checklists.

**Critical path vs portfolio:** the table below still lists the full product
portfolio. Only the first three rows plus the named Agent/Workflow narrow exits
are GA-0…GA-2. Horizons marked **deferred** must not block software EXIT.

| Horizon | Required gates | Product outcome | Path |
| --- | --- | --- | --- |
| Usable service platform | `BX0.software` plus Box re-cert of `R0` through `E0` | One operator can deploy, reach, observe, update, roll back, and stop one Box-hosted stateless Service on one Linux node **without TEE** | **GA-0** |
| Heterogeneous Agent platform (narrow) | `A0` + Code-path `A1` availability | One immutable Agent release executes through Gateway with durable events and retained recovery evidence | **GA-1** |
| Ontology-driven Workflow / one Application (narrow) | Narrow `W0` + one `APP0` experience | One Goal→Plan→Run and one Application experience publish through Gateway | **GA-2** |
| Developer platform | `G0`, `P0`, `C0`, and `A0` | Source-to-release workflows, previews, stable automation, team operations, and A3S assets reuse the verified deployment path | After GA-0 |
| Continuous delivery platform | `CD0`, `G0`, `P0`, product Release gates, `H0`, `E0`, and observability gates | Every Runtime profile and Cloud system service builds once, binds verifiable evidence, promotes exact immutable releases, observes declared SLOs, and rolls back only when state compatibility permits | **GA-3** (`ha`) |
| Stateful production platform | `S0` and `H0` | Stateful resources, multi-node placement, HA, measured scaling, backup, and disaster recovery are production-operable | **GA-3** / GA-4+ |
| Plugin-managed cognitive platform | `U0`, `C0.3`, the required A3S Use gates, and named `BX0`/`H0` host foundations | Tenants assign signed multi-surface A3S Use packages to authorized workspaces without another package manager, scheduler, or node channel | GA-4+ |
| Hosted MCP platform | `A0.3`, `MCP0.1` through `MCP0.5`, and their named `BX0`/`H0` foundations | One immutable modern MCP release runs as a Box-hosted Runtime Service through an authorized conforming Gateway | GA-4+ |
| Function platform | `FN0`, `R0`, `A1`, `W0`, `AUT0.5`, and named `H0`/Gateway gates | Finite hosted, stateless hosted, and external Functions share one release/invocation authority across APIs, Agent Tools, and Workflow nodes | GA-4+ |
| Static Web platform | `WEB0`, `G0`, shared objects, Edge/Gateway, and named `H0` gates | React/Vue Agent/Application UIs build through Task/Box and serve as immutable object releases without per-site Services; Cloud ships no management Dashboard | GA-4+ |
| Single-node inference | `PW0.software`, I0 Track B minimal, Gateway dispatch | Governed keys/routes serve one Box-hosted Power replica | GA-4+ |
| Distributed inference platform | Full `I0`, `PW0`, `H0.3`/`H0.5`, model-supply, Gateway gates | Independent or gang-distributed Power replicas and phase-disaggregated cohorts | **P2 deferred** |
| AI application platform (full matrix) | `APP0.6`, `K0`, `AUT0`, `W0`, and named dependencies | Six application experiences, 23-node matrix, Knowledge Pipelines, six plugin outcomes, multi-channel publication, monitoring, enterprise policy | **P2 deferred** (one experience is GA-2) |
| Durable collaboration platform | `CELL0.1` through `CELL0.5` plus named foundations | Named SQLite-backed rooms/blackboards with alarms, WebSockets, fenced single-writer ownership, RPO=0 acknowledgement | GA-4+ (after `S0`; not GA-0..2) |
| Governed evolution platform | `EV0`, `W0`, `A1.6`, `I0`, and named safety foundations | Authorized evidence produces reproducible evaluations and immutable candidates | **P2 deferred** |
...
| `C0.5-MT2-C1` | Verified (`2026-08-29`) | Migration `177` and the sole `IPlatformRbacRepository` persist immutable accepted policy revisions, one current head, and versioned bindings. One Installation mutation lock, optimistic CAS, active-Principal checks, self-escalation denial, owner-only owner administration, and deferred database last-owner/Principal-disable constraints serialize every replica while reusing shared idempotency, Audit, and Outbox. The dedicated `IIdentityBootstrapRepository` creates the first Organization/Principal/owner Membership/API token and matching baseline policy/`PlatformOwner` binding in one PostgreSQL transaction; it checks replay after the bootstrap lock and reuses the exact transaction-local platform writer. The provider gate injects a platform-fact failure and requires zero partial rows, then races/replays successful bootstrap and requires one matching authority root. The [complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/33249012696) and its [PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33249012696/job/99091360732) recertify atomic public bootstrap, competing standalone platform bootstrap, policy-head advancement, owner revocation, and direct-SQL rejection. Only an explicit operator-controlled recovery transition for installations created before this atomic root remains open. |
...
| `C0.5-MT2-C3` | Verified (`2026-08-29`) | The sole transaction-local privileged authorization issuer share-locks the active Principal, exact API-token version, current policy/binding, and exact optional tenant-support grant, then commits one digest-bound allow through shared scoped Audit. The internal `AuthorizePrivilegedAccess` command and all seven non-bootstrap platform-RBAC/support mutations reuse it: write DTOs carry only actor Principal plus exact credential ID, each concrete use case fixes permission/action/scope/resource, server-derived credential evidence enters proposals/approvals/revocations, and the protected business Audit fact links the exact decision in the same PostgreSQL transaction. Maintained REST/OpenAPI, TypeScript client, CLI, and Management MCP interfaces derive actor and credential from verified request context and expose no generic caller-authored evaluator. Organization catalog reads now carry exact Installation/Principal/credential/request identity through `ReadOrganizationCatalog`: PostgreSQL evaluates `TenantLifecycleRead` in the same Identity transaction, returns the installation catalog only after a persisted allow, otherwise narrows a still-active exact `cloud:read` credential to its own Organization, and denies invalid credentials. Token verification and controllers no longer mint or inspect an ambient platform-role string. Role, token, binding, grant, catalog, and protected-business races retain one database order. API-token revocation now takes the canonical Installation shared authorization-evidence mutation fence before idempotency, token update, scope-lineage resolution, and shared Outbox persistence; this removes the reverse `Token -> Installation` wait without deadlock retry or a second coordination mechanism. An architecture ratchet and the retained platform-RBAC/workload-trust PostgreSQL races protect the order. The [complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/33251290420) and its [PostgreSQL 17 H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33251290420/job/99097293875) verify revocation fencing, the protected-mutation race, the cross-surface interfaces, and concurrent catalog-read/binding-revocation behavior. No Redis/Lane lock, cache truth, dedicated decision table, or authorization Outbox is added. `MT3` remains the broader system/organization role matrix, owner-port cleanup, and adversarial cross-tenant enforcement gate. |
...
also passes. `C0.3-PA2d` is verified on PostgreSQL 17 in main CI for the complete transient multi-page
manifest bundle by the [H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566/job/101250990670), [Management MCP job](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566/job/101250990708), [TypeScript client and CLI job](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566/job/101250990747), and [complete run](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566). `C0.3` remains in progress
...
| `A0.4` | Verified | Exact published Agent releases bind immutably to ordinary Workload revisions through migration 066 and the existing Deployment, Operation, Flow, Fleet, and Runtime path. Server-side OCI publication injection, replay, update, rollback, Secret restart, persistence, REST, client, CLI, and Management MCP projections are implemented. The retained [PostgreSQL 17/real A3S Box provider gate](https://github.com/A3S-Lab/Cloud/actions/runs/33686237668/job/100434300332) and [complete Cloud CI](https://github.com/A3S-Lab/Cloud/actions/runs/33686237772) pass against Box `65f3d3fc7c1e0e2cb1ba2d409a79f7357314f5ae` and OCI Runtime `878f8414cef3b85bef1b51fe6735017b25828252`, including published-release recovery, Secret rematerialization, cancellation, and cleanup. Hosted MCP deployment is owned by `MCP0` |
| `A0.5` | In progress | Exact hosted Git archives publish as immutable content-addressed Skill bundles, and active Agent Workloads bind, rebind, or unbind exact releases through new revisions, read-only Runtime Artifact mounts, migration 067 persistence, rollback-safe history, and tenant-authorized REST/client/CLI/Management MCP surfaces. CI retains a REST/PostgreSQL/local-object gate for exact archive bytes, receipt identity, replay, yanking, and cleanup, plus real Box gates for typed Skill-bundle hydration and the complete Workload bind/rebind/unbind/rollback lifecycle, including exact bytes, read-only mount enforcement, revision history, one download per content digest, and Runtime-remove cleanup. Retained external-provider evidence still blocks verification; no generic forge surface is added |
...
releases; exact replay and rollback preserve a pinned identity. The retained
[real-provider lifecycle gate](https://github.com/A3S-Lab/Cloud/actions/runs/33686237668/job/100434300332)
now verifies `A0.4` on PostgreSQL 17 and the exact Box/OCI Runtime pair,
including published-release recovery, Secret rematerialization, cancellation,
and cleanup. Hosted MCP deployment and traffic conformance proceed only through
`MCP0`.
...
| Sub-gate | State | Outcome |
| --- | --- | --- |
| `K0.1` | In progress and unavailable; `C1/C2` implemented with retained PostgreSQL persistence evidence, `C3` component-only contracts implemented, `C4a` catalog persistence implemented, `C4b1` application owner ports implemented, `C4b2a` authorized idempotent writes implemented, `C4b2b1` REST/OpenAPI implemented, `C4b2b2` client/CLI/MCP implemented, `C5`/`C6`/`C7` document/chunk catalog and authorized writes implemented, `C8` document/chunk REST/OpenAPI `1.93.0` implemented, `C9` document/chunk client/CLI/MCP implemented, `C10` document/chunk authorized list REST/OpenAPI `1.94.0` + client/CLI/MCP implemented, `C11` index/policy/binding catalog persistence implemented, `C12` index/policy/binding owner catalog services implemented, `C13` authorized index/policy/binding writes implemented, `C14` index/policy/binding REST/OpenAPI `1.95.0` implemented, `C15` index/policy/binding client/CLI/MCP implemented, `C16` index/policy/binding list REST/OpenAPI `1.96.0` implemented | `C1` freezes strong Files/Knowledge/KnowledgePipeline identities, canonical `cloud.user-file.v1` ACL, distinct upload expiry/retention, exact write and scan receipts, optimistic admission lifecycle, metadata-only events, and a `user-files` streaming adapter over the shared immutable-object client without whole-file buffering or raw cleanup. `C2` adds migration `170`, one atomic metadata/quota repository, authorization before replay, shared idempotency/audit/Outbox, REST/OpenAPI `1.77.0`, maintained client/CLI, and Management MCP tools over the same handlers; MCP remains metadata-only. `C2b1` adds authorized public `PUT .../content` (OpenAPI `1.89.0`) over the same upload command with local object-store tests; `C2b2` adds admitted-only public `GET .../content` (OpenAPI `1.90.0`); `C2b3` adds metadata-only public `POST .../scan` admit/reject decisions (OpenAPI `1.91.0`) and a sixth Management MCP tool; live provider scan/cleanup stay open. The retained [PostgreSQL 17 H0 persistence step](https://github.com/A3S-Lab/Cloud/actions/runs/33159659047/job/98810769471) verifies rollback, concurrent organization-quota serialization, identity fencing, exact reservation/upload/scan/tombstone replay, quota release, and atomic Outbox/audit/idempotency effects. Live MinIO/S3 provider wiring and live scanner/cleanup execution remain open. `C3` freezes KnowledgeBase/document/chunk/index/retrieval/external-binding and KnowledgePipeline release contracts over exact Files/object/Workflow references without persistence or providers. `C4a` adds migration `201` plus PostgreSQL/in-memory KnowledgeBase revision and KnowledgePipeline release catalogs with CAS head fencing and focused H0 recovery gates; no search index, provider client, DAG engine, worker, or public surface. `C4b1` adds application owner catalog services over those repositories. `C4b2a` adds authorized idempotent writes with audit/Outbox. `C4b2b1` adds REST/OpenAPI `1.88.0` over the same handlers. `C4b2b2` adds maintained client/CLI/Management MCP over those handlers. `C5` persists immutable KnowledgeDocument/Chunk catalogs through migration `202`. `C6` adds application owner document/chunk catalog services. `C7` adds authorized idempotent document/chunk create writes with audit/Outbox. `C8` exposes that document/chunk boundary over REST/OpenAPI `1.93.0`. `C9` adds maintained client, CLI, and Management MCP over the same handlers without OpenAPI bump or live MinIO/scanner/SEV/cleanup claims. `C10` adds authorized bounded document/chunk list through REST/OpenAPI `1.94.0`, client, CLI, and two Management MCP read tools without live MinIO/scanner/SEV/cleanup claims. `C11` persists immutable index/retrieval-policy/external-binding catalogs through migration `203` without public surfaces or live MinIO/scanner/SEV/cleanup claims. `C12` adds application owner index/policy/binding catalog services over those repositories without auth/REST/MCP or live MinIO/scanner/SEV/cleanup claims. `C13` adds authorized idempotent index/policy/binding create writes with audit/Outbox without REST/MCP or live MinIO/scanner/SEV/cleanup claims. `C14` exposes that index/policy/binding boundary over REST/OpenAPI `1.95.0` without client/CLI/MCP or live MinIO/scanner/SEV/cleanup claims. `C15` adds maintained client, CLI, and Management MCP over the same handlers without OpenAPI bump or live MinIO/scanner/SEV/cleanup claims. `C16` adds authorized bounded index/policy/binding list through REST/OpenAPI `1.96.0`, client, CLI, and three Management MCP read tools without live MinIO/scanner/SEV/cleanup claims. No K0 product availability is claimed |
...
| `AUT0.2` | In progress; component-only admission boundary | Signed webhook endpoints, bounded capture, deterministic invocation construction, schema evaluation, exact target, replay, disable/revoke, durable recovery, and Gateway recovery remain the full gate. C1-C6 contracts, admission, PostgreSQL persistence, exact-version HMAC verification, digest-bound evaluation, exact-digest registry selection adapter, and C7's exact-revision webhook invocation factory are implemented; live PostgreSQL recovery evidence, production registry wiring, listener/Gateway wiring, and public availability remain open |
| `AUT0.2-C1` | Implemented; component-only | `a3s-cloud-contracts` now defines an opaque endpoint projection bound to one immutable Automation revision, Secret identity/version and schema digest; bounded canonical JSON capture; canonical HMAC-SHA256 signature facts; monotonic disable/enable/revoke generations; and admitted, replayed, or lifecycle-bound rejected delivery receipts. Exact target and authorization continue through the AUT0.1 invocation envelope. No HTTP listener, schema registry/evaluator, Gateway route, persistence, worker, or public availability is claimed. |
| `AUT0.2-C2` | Implemented; component-only | `control-plane/modules/automations` adds one application-owned admission service and atomic in-memory repository. Endpoint creation pins the exact revision and scoped opaque key; lifecycle changes use generation compare-and-swap; signature verification and schema evaluation cross explicit infrastructure ports; delivery identity/body replay, duplicate conflicts, lifecycle rejection receipts, redacted audit records, and admitted Outbox messages are committed under one repository lock. The repository and application query service resolve opaque endpoint keys only within the complete organization/project/environment scope, validate restored endpoint invariants, and reject nil-scope or malformed-key queries before a future transport adapter can capture or verify a request. Focused tests cover scope collisions, scoped key lookup, key validation, replay/body drift, lifecycle recovery fences, required ports, and secret-reference redaction. PostgreSQL durability is supplied by C3; no HTTP listener, Gateway route, worker, or public availability is claimed. |
| `AUT0.2-C3` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI steps wired | Migration `183` and `PostgresAutomationWebhookRepository` persist the exact endpoint projection plus canonical revision ACL/digest, one immutable first-delivery record, and append-only replay/conflict/lifecycle receipts. Endpoint lifecycle updates are generation-fenced and identity/revision immutable; delivery/receipt rows reject mutation; restore reparses and validates canonical contracts before admission. Shared Audit and Outbox tables are written in the same transaction, while Secret plaintext, retry/scheduler state, HTTP transport, worker, and public routes remain out of scope. The isolated `automation_postgres_integration` gate covers scoped lookup, first admission, exact replay, reconnect recovery, wrong-environment isolation, and concurrent lifecycle generation fencing when `A3S_CLOUD_TEST_POSTGRES_URL` is configured and is wired in main CI H0. Public lifecycle APIs, Gateway registration, and product availability remain open. |
| `AUT0.2-C4` | Implemented; component-only | `HmacSha256AutomationWebhookSignatureVerifier` resolves only the endpoint's exact Secret ID/version through the published Secrets materializer, computes HMAC-SHA256 over the captured raw body, and compares the canonical lowercase-hex fact in constant time. Secret plaintext remains transient and zeroized by Secrets; materialization failures are redacted. No schema registry/evaluator, HTTP listener, Gateway route, worker, or public availability is claimed. |
| `AUT0.2-C5` | Implemented; component-only | `DigestBoundJsonSchemaValidator` compiles one already-selected canonical JSON Schema and requires its sorted-JSON SHA-256 to equal the endpoint/request schema digest. It bounds schema and payload shape, permits only in-document references, rejects external URL/file retrieval, and returns redacted validation failures. It owns no registry, HTTP listener, Gateway route, worker, persistence, or public availability; live PostgreSQL recovery and registry selection remain open. |
| `AUT0.2-C6` | Implemented; component-only | `IAutomationWebhookSchemaRegistry` and `RegistryBackedAutomationWebhookSchemaValidator` select one immutable schema by the endpoint's exact digest, then delegate to the digest-bound evaluator. Missing, invalid, or mismatched documents fail closed without URL/file retrieval, mutable-latest caching, schema persistence, listener, Gateway route, worker, or public availability; the in-memory registry is test/local composition only. |
| `AUT0.2-C7` | Implemented; component-only | `AutomationWebhookInvocationFactory` converts a captured, endpoint-bound request into one exact-revision invocation envelope. It binds the delivery ID/body digest/received timestamp to the `automation.webhook.received` origin, derives the policy-scoped deduplication key, validates authorization, input, endpoint/revision identity, and timestamp ordering, and carries correlation/causation without allowing event, target, time, or authorization drift. `AutomationWebhookReceiver` now composes complete-scope lookup, bounded capture, a non-mutating signature/schema preflight, an injected authorization-snapshot owner port, exact invocation construction for active endpoints, and the single admission service; transport input cannot manufacture a grant snapshot, unauthenticated requests cannot trigger authorization resolution, and inactive endpoints bypass external verification, authorization resolution, and invocation construction while retaining lifecycle rejection semantics. It owns no HTTP listener, Gateway route, registry, persistence, queue, worker, target execution, or public availability. |
| `AUT0.2-C8` | Planned | Expose authorized Automation webhook endpoint lifecycle (create/get/disable/enable/revoke) and bounded definition/revision catalog reads through REST/OpenAPI, maintained client/CLI, and Management MCP over existing C2/C3 authorities. No Gateway public receive registration, production schema-registry wiring, or product availability. |
| `AUT0.3` | In progress; component-only calendar/misfire/concurrency/due-window/lease, durable cursor/lease, exact invocation-envelope, bounded schedule-dispatch, P0 Task-target adaptation, idempotent invocation-admission, injectable schedule-worker, and digest-verified invocation-handoff boundaries | Timezone-aware schedules, catch-up rules, production candidate discovery/owner composition, durable production invocation admission, and the production P0 scheduled Task handoff through the existing Boot task rail remain the full gate |
| `AUT0.3-C1` | Implemented; component-only | `AutomationScheduleCalculator` parses the canonical seven-field expression, requires a canonical IANA timezone, returns UTC instants for the next occurrence or a bounded `(start, end]` window, and preserves both instants during a fall-back DST transition. It owns no misfire/concurrency policy, lease, scheduler state, persistence, worker, or P0 handoff. |
| `AUT0.3-C2` | Implemented; component-only | `AutomationScheduleMisfireEvaluator` validates a bounded, ordered due window and applies the canonical grace, `skip`, `fire_once`, or `fire_latest` policy while returning explicit selected and skipped occurrences. It owns no cursor, lease, concurrency counter, scheduler state, persistence, worker, or P0 handoff. |
| `AUT0.3-C3` | Implemented; component-only | `AutomationConcurrencyEvaluator` validates the bounded active count and returns `admit`, `queue`, `drop`, or `replace` at the exact policy capacity. It owns no counter, queue, cancellation, lease, scheduler state, persistence, worker, or P0 handoff. |
| `AUT0.3-C4` | Implemented; component-only | `AutomationScheduleDueEvaluator` composes the calendar and misfire components over an exclusive cursor and bounded observation window, returning explicit selected/skipped occurrences plus `evaluated_through` for a future atomic owner update. It owns no cursor persistence, lease, concurrency counter, scheduler state, worker, or P0 handoff. |
| `AUT0.3-C5` | Implemented; component-only | `AutomationScheduleLease` and `AutomationScheduleLeaseEvaluator` define a canonical half-open lease fence, exact owner/token checks, a five-minute maximum, and pre/post due-evaluation expiry checks. They own no lease row, cursor mutation, invocation admission, scheduler state, worker, or P0 handoff. |
| `AUT0.3-C6` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI steps wired | Migration `184` and the in-memory/PostgreSQL schedule-state repositories persist one exact Automation revision digest, exclusive due cursor, monotonic lease generation, and owner/token/timestamp fence. Acquisition and cursor commit lock the one state row and reject stale, active, expired, non-rotated, or out-of-order fences. Migration `186` also permits an exact owner to release a fence without cursor progress. The isolated `automation_postgres_integration` gate covers concurrent lease fencing, cursor commit, reconnect recovery, generation rotation, cursor-preserving release, and wrong-scope isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured and is wired in main CI H0. The state owner stores no definition copy, invocation, queue, retry, worker, or scheduler; production timer ownership and P0 handoff remain open. |
| `AUT0.3-C7` | Implemented; component-only | `AutomationScheduleInvocationFactory` binds one already-selected due timestamp and caller-supplied authorization snapshot to the exact schedule revision, validates the common invocation envelope, and derives the policy key for deterministic deduplication. It owns no calendar selection, cursor/lease commit, invocation persistence, queue, worker, or P0 handoff. |
| `AUT0.3-C8` | Implemented; component-only | `AutomationScheduleDispatchService` composes exact revision validation, row-fenced lease acquisition/release, bounded due/misfire evaluation, stable UUIDv5 occurrence identities, exact schedule envelopes, idempotent admission outcomes, and cursor commit. No timer loop, candidate discovery, queue, target execution, or P0 handoff is introduced; failures release the fence while preserving the exclusive cursor for safe redelivery. |
| `AUT0.3-C9` | Implemented; component-only | `AutomationsScheduledTaskProfileAdapter` is the P0 anti-corruption boundary: it emits the exact Task profile/revision UUIDs, immutable profile digest, and canonical schedule policy into one typed Automations binding. Developer Workflows carries that binding beside the existing Execution template and validates it against the accepted revision; no Automation persistence, timer, queue, invocation admission, or second scheduler is introduced. |
| `AUT0.3-C10` | Implemented; local PostgreSQL 17.5 recovery gate passes; H0 CI steps wired | `AutomationInvocationAdmissionService` validates one exact envelope. The in-memory adapter and migration `187`/`PostgresAutomationInvocationRepository` fence invocation identity and policy-derived deduplication keys, replay exact evidence as `AlreadyAdmitted`, and atomically retain the immutable envelope, redacted Audit, and one Outbox fact only on first admission. The isolated `automation_postgres_integration` gate covers first admission, exact replay, immutable identity and deduplication conflicts, reconnect recovery, and organization isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured and is wired in main CI H0. `AutomationsDispatchServices` composes schedule and normalized-event dispatch behind this same admission port, and PostgreSQL webhook delivery admission now enters that transaction instead of maintaining a second invocation Outbox writer. No timer, queue, target execution, mutable revision lookup, production worker composition, or P0 handoff is introduced. |
| `AUT0.3-C11` | Implemented; application schedule-worker boundary | `AutomationScheduleWorker` owns one bounded polling loop, stable process owner identity, deterministic candidate ordering/deduplication, bounded lease timing, graceful shutdown, and per-candidate failure reporting over injected `IAutomationScheduleCandidateProvider` and `IAutomationScheduleDispatchService` ports. `ControlPlaneWorkers` accepts the worker as an optional owner-composed process and supervises graceful shutdown. It owns no definition/revision storage, candidate discovery, target execution, queue, retry rail, public route, or P0 handoff; failed dispatches remain recoverable through the durable cursor/lease service. |
| `AUT0.3-C12` | Implemented; application invocation handoff boundary | `A3sEventAutomationInvocationConsumer` validates the exact published Outbox envelope, scope, event identity, and immutable invocation digest, reloads the committed record through `IAutomationInvocationReader`, and hands it to an injected `IAutomationInvocationHandler`. Missing durable records and handler failures remain pending for provider-owned redelivery; malformed or drifted provider facts are acknowledged without target execution. `ControlPlaneWorkers` accepts this consumer as an optional owner-composed process and supervises graceful shutdown. No target lookup, execution, retry queue, or public handoff is added. |
| `AUT0.4` | In progress; component-only event-envelope boundary | Plugin-trigger subscriptions and normalized event dispatch while Sources retains provider connection/revision authority and U0 retains package authority; C1 freezes the normalized event identity/filter port, C2 adds bounded deterministic fan-out over already selected exact revisions, C3 adds the strict durable-ack provider consumer, and C4 composes external candidate selection with idempotent invocation admission |
| `AUT0.4-C1` | Implemented; component-only | `AutomationNormalizedEventV1` validates one canonical digest-bound event identity, observation time, and bounded normalized input. `AutomationEventInvocationFactory` binds it to the exact PluginEvent/SourceEvent subscription and Automation revision, validates event-key/observation ordering and the common invocation envelope, and derives policy-scoped deduplication. The application boundary consumes the revision's exact `filter_digest` through `IAutomationEventFilterEvaluator`; a non-match produces no envelope and evaluator failures are fail-closed. Sources/Use still own provider normalization, while Automations owns filter evaluation. This slice owns no provider normalization, filter storage/publication, subscription persistence, queue, worker, scheduler, or P0 handoff. |
| `AUT0.4-C2` | Implemented; component-only | `AutomationEventInvocationFanoutService` evaluates one validated normalized event against an already selected, bounded set of exact revisions. It sorts candidates by immutable revision digest and invocation ID, rejects duplicate identities, carries one caller-supplied request timestamp, and returns only filter-matching envelopes. It owns no candidate discovery, subscription persistence, event publication, queue, worker, scheduler, or P0 handoff. |
| `AUT0.4-C3` | Implemented; application durable-ack boundary | `A3sEventAutomationNormalizedEventConsumer` binds one exact durable provider subject/source to `AutomationNormalizedEventV1`. It ACKs malformed or successfully handled events, leaves handler failures unacknowledged for provider-owned redelivery, and `ControlPlaneWorkers` accepts it as an optional owner-composed process with graceful shutdown. It owns no local retry, queue, subscription discovery, invocation persistence, or target execution. |
| `AUT0.4-C4` | Implemented; component-only | `AutomationEventInvocationDispatchService` composes an external candidate provider, the bounded deterministic fan-out service, and an explicit idempotent invocation-admission port. It validates the normalized event before candidate lookup, admits only filter-matching exact envelopes, returns provider-redelivery errors without local retry, and treats duplicate admission as success. Candidate storage, subscription persistence, invocation durability, queues, target execution, and public availability remain owned by injected authorities. |
...
| Sub-gate | Outcome |
| --- | --- |
| `FN0.1` | Implemented component-only: canonical Function release/profile ACL, closed mode/owner matrix, invocation authority, errors, bounds, fixtures, and architecture fitness tests; no Function lifecycle or availability is claimed |
...
The default portfolio priority follows the pragmatic GAs in
[architecture-optimization-roadmap.md](docs/architecture-optimization-roadmap.md):

1. ~~finish `BX0.software` / GA-0 clean-host evidence~~ **Done (`2026-09-17`)** —
   retained [docs/evidence/bx0-software-exit-2026-09-17](docs/evidence/bx0-software-exit-2026-09-17/);
2. land GA-1 Agent availability (Code path) and GA-2 one Workflow/Application
   experience — checklist [ga1-agent-availability-checklist.md](docs/ga1-agent-availability-checklist.md);
3. continue Wave 0 integrity and I0 Track A fail-closed work in parallel;
4. treat `CD0.1`, `H0.4-WI1`, `H0.5-OBS1`, and `C0.4-COMP1` as useful
   contract-only freezes that do **not** redefine GA-0 EXIT;
5. defer full `ha` (`H0.3`–`H0.5`), `CELL0` production, distributed `I0`,
   `APP0.6`, and `EV0` until after those exits.

Before product-specific **breadth** advances past GA-2, the platform should
still freeze
...
commit atomically. Fresh-install authority now uses the dedicated
`IIdentityBootstrapRepository`: one locked transaction creates the tenant
identity root, API-token digest, accepted baseline policy, matching
`PlatformOwner` binding, and shared facts. Idempotency is checked after the
bootstrap lock, both bootstrap entry points reuse one transaction-local
platform writer, and fault injection requires the entire root to roll back.
The [complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/33249012696)
and its [PostgreSQL 17 H0
job](https://github.com/A3S-Lab/Cloud/actions/runs/33249012696/job/99091360732)
recertify atomic fresh bootstrap, racing standalone bootstrap, policy
acceptance, owner revocation and direct-SQL bypass rejection. Only an explicit
operator-controlled recovery transition for installations created without
that root remains open. `C0.5-MT2-C2` is also verified: migration `178` and the sole
...
approver rollback, terminal history and replay. Verified `MT2-C3` now
...
Audit, and conflict with role/token/grant revocation. The same transaction-local
issuer is now composed by every non-bootstrap platform-RBAC and support-grant
mutation. Their closed write DTOs accept the actor and exact verified credential
identity, never caller-authored permission/scope/resource or authentication
evidence; the concrete use case selects those values, derives credential
evidence from the issued decision, and links that decision from the protected
business Audit fact. Maintained REST/OpenAPI, TypeScript client, CLI and
Management MCP interfaces all derive actor and exact credential identity from
verified request context and expose only closed use cases. Organization catalog
reads carry the exact Installation, Principal, credential and request through
`ReadOrganizationCatalog`; PostgreSQL authorizes the full catalog through the
same `TenantLifecycleRead` decision and otherwise narrows a still-active exact
`cloud:read` credential to its own Organization. Invalid credentials fail
closed, and no verifier or controller maps an ambient platform role. The
[complete main CI run](https://github.com/A3S-Lab/Cloud/actions/runs/33251290420)
and its [PostgreSQL 17 H0
job](https://github.com/A3S-Lab/Cloud/actions/runs/33251290420/job/99097293875)
pass the protected-write/token-revocation, catalog-read/binding-revocation,
and cross-surface multi-replica gates. `MT3` remains responsible for the wider
system/organization-role matrix, internal owner ports and adversarial tenant
evidence; it does not reopen `MT2-C3` or permit a caller-authored evaluator.
With this authority in place,
`WI1-C2` may persist installation trust state;
...
`4c5fbd56bedd84d1007a7d9cd046a9f7083bbdcd` and Box `3.2.6` at
`774c57abd1d70b227c45b4dbc1ce50a7027308aa`. They preserve one opaque Identity
...
macOS MicroVMs a mount-free, guest-owned raw ext4 rootfs by default while
...
`402949c2d73cabdf5b959113806db9108e918cf1`, then runs the complete Rust,
...
- the claimed **delivery profile** (`software`, `tee`, or `ha`) is named in
  the gate evidence and matching ROADMAP state;
- the documentation capability-preservation check retains the native Cloud,
  ...
  **or deferral** (deferred portfolio items must not be implied by GA-0…GA-2);
- an `APP0`, `K0`, or `AUT0` **availability** claim for GA-2 names one experience
  (or Files foundation) with owner, verified dependencies, and retained
  evidence; full `APP0.6` matrix parity remains a separate P2 claim;
...
- a backend/interface slice lands its domain invariants, commands, queries,
  ...
  consume these contracts and static delivery must pass `WEB0` before advertising
  static hosting;
...
- real-provider happy path, failure, process-death, replay, corruption, and
  cleanup gates pass from a clean environment **for the claimed profile**;
- the owning installation gate passes on clean supported Linux with A3S Box and
  ...
  daemon when the capability is part of the native software EXIT;
...
Hardware TEE, multi-node HA, distributed inference, full application-matrix
parity, Durable Cell RPO=0 production, and `EV0` are **additional** profile or
P2 claims. Their absence must not prevent marking a software-profile gate
Verified when its named software evidence passes.

See the [development plan](docs/development-plan.md),
[architecture optimization roadmap](docs/architecture-optimization-roadmap.md),
[Workflow and evolution plan](docs/workflow-evolution-plan.md),
