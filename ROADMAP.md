...
| `MCP0.3` | Cloud | Foundation in progress | Closed contract values, canonical immutable Service-profile and separately mutable Edge route-policy ACL admission, migrations 052-059 plus typed A3S ORM persistence, exact verified DomainClaim binding, exact AssetRelease/profile-bound WorkloadRevision identity, automatic opaque Runtime Service-spec binding, hosted credential lifecycle/persistence and an internal one-shot Argon2id issuer, scope-complete healthy-target validation, exact grant/generation authority resolution, deterministic receiving-node route planning, conflict-safe complete-projection assembly, retained policy/DomainClaim/Workload/credential CAS evidence, pure ordinary-plus-MCP snapshot composition, durable atomic staging, automatic desired-state convergence, restart-safe Fleet dispatch, and exact acknowledgement/expiry projection now exist. Migration 058 records the canonical desired logical-scope set on each immutable marker and maintains one latest-MCP head per physical Gateway. Migration 059 assigns each marker to either the MCP reconciler or its originating ordinary flow. Single-member, replicated, deployment-cutover, exact rollback, and certificate-convergence ordinary Route publication now consume the same node desired-state planner, preserve all active MCP policy in their complete snapshots, and validate ordinary plus MCP authority under one staging transaction. Rollback retains observed physical ordinary state and composes current MCP desired state. Certificate renewal, provider or DomainClaim revocation, projection repair, and validity renewal classify every pre-write ordinary Route while preserving current MCP ingress; they reuse a certificate only when its request and stored Claim set cover the complete projection, otherwise stage a deterministic replacement. Owner-aware dispatch and acknowledgement keep the ordinary Route/rollout/cutover/rollback/convergence projection authoritative while advancing the shared MCP head. The cursor-fair worker treats logical scopes only as discovery triggers, deduplicates physical nodes, loads and independently plans every active scope on each node, rejects cross-scope identity/ownership/authority conflicts, then emits one complete node snapshot. PostgreSQL staging locks and compares the complete active scope set, memberships, policies, ordinary routes, Claims, Workloads, credentials, and physical revision before atomically advancing publication, certificate, marker, node head, scope, and secret-free Outbox evidence. Changed state, route-less expiry or membership removal, due terminal retry, displacement, and a revoked or renewal-due certificate installed by an MCP-owned snapshot are repaired without physical-identity or observation-time churn. The last case gives MCP-only nodes proactive certificate rotation; the shared obsolete-certificate phase revokes the superseded provider serial only after the replacement revision is installed. An exact Applied zero-route acknowledgement removes the mutable head while retaining immutable replay history, so cleanup runs once and ordinary-only changes are no longer MCP-owned. Focused tests cover two-scope composition, one-publication node deduplication, ordinary Route, cutover, rollback, and DomainClaim-revocation preservation of MCP policy/certificate authority, membership-removal cleanup, no-op convergence, cursor fairness, pending deferral, retry, displacement, certificate-renewal decisions, stable digest identity, dispatch interruption/restart, replay, expiry, and future-clock fail-closed behavior. The compiled integration fixture additionally checks desired scope-set/head persistence and post-acknowledgement no-churn alongside stale-policy rejection, final-Outbox rollback, Fleet replay, certificate readiness, and Applied acknowledgement. Revoked-credential cleanup, public idempotent credential delivery/rotation, lifecycle surfaces, control-plane audit, an executed real PostgreSQL gate for the new certificate path, and joint Runtime/Gateway recovery evidence remain |
