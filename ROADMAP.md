...
| `W0.4` | In progress; immutable Connector response-object composition implemented | Bind typed Agent, MCP, model, Tool, and business-service steps with exact revisions, approvals, compensation, and bounded evidence references. WorkflowRun v6 now stores an accepted Connector body through the Connectors-owned typed child of the shared immutable-object client before C6 terminal settlement, then retains only its exact reference, digest, and length in Flow; response consumption and the remaining typed capability steps stay open. |
...
response consumption and the remaining W0.4 gates still block HTTP Request.
...
composite claim false. The twenty accepted authority decisions live under
...
| `AUT0.5` | In progress; Flow interpretation and immutable response-object composition implemented (`2026-08-20`) | WorkflowRun input/runtime/Flow v6 binds exact Connector hook creation and resume history, delegates every attempt to C6 through the C8 port, owns bounded durable observation/retry waits, honors bounded `Retry-After` before the C9 fallback delay, retries only retryable terminal evidence, re-observes deferred attempts without spending the provider budget, and fails indeterminate attempts closed. C10 stores accepted response bytes through the sole shared immutable-object authority before terminal evidence and exposes only an exact opaque reference, digest, and length to Workflow; historic v5 stays body-free. Complete response consumption, remaining provider/Event-consumer wiring, revocation/recovery operations, and retained integration evidence before product availability. |
