...
| `MCP0.3` | Cloud | Foundation in progress | Closed profile and route-policy ACL admission, migrations 052-060, typed persistence, exact release/profile/Workload binding, Runtime Service projection, credential authority, complete node planning, ordinary-plus-MCP snapshot composition, atomic staging, Fleet delivery, acknowledgement, expiry cleanup, and unified certificate convergence now exist. Credential material generation is separated from persistence; issuance, rotation, revocation, encrypted one-time recovery, secret-free Outbox/idempotency evidence, and expired-ciphertext cleanup share one fail-closed lifecycle repository transaction. Registered application commands perform tenant-scoped replay before generation and decrypt plus Argon2id-verify only the committed delivery. The versioned REST/OpenAPI surface exposes issue, rotate, revoke, list, and get with `route:write` mutation authorization, tenant guards, zeroizing one-time-secret serialization, redacted metadata, closed request/response schemas, and mandatory no-store/no-cache success and error responses. The shared TypeScript client requires those cache protections, reconstructs only allowlisted fields, and rejects misplaced secret or internal material; the compiled CLI provides metadata-only list/get/revoke plus explicit one-time issue/rotate delivery. A worker-role loop performs bounded expired-ciphertext cleanup while retaining the idempotency reference. HTTP and client/CLI tests prove same-secret replay, rotation fencing, revocation, metadata redaction, non-cacheability, exact tenant paths, unsafe-input rejection, and sanitized output. A replay can return only the exact current generation during its bounded recovery window and can never mint a second secret. Every issue, rotation, and revocation appends one validated actor- and request-bound secret-free audit record in the same aggregate, delivery, Outbox, and idempotency transaction. Every ordinary publication, rollout, cutover, rollback, and certificate-repair path consumes the same node desired-state plan and preserves current MCP policy. The cursor-fair worker composes all active logical scopes for one physical Gateway, while owner-aware dispatch preserves the originating publication authority. Revoked, expired, or generation-mismatched credentials are resolved before Runtime health; only affected MCP routes are suppressed, unrelated routes remain projected, and the full observed policy/Workload set plus exact invalid credential state is locked transactionally before cleanup is staged. MCP-only certificate renewal stages a complete replacement and revokes the superseded provider serial only after installation. Focused tests cover credential lifecycle replay/expiry/revocation, secret-free audit attribution, multi-scope composition, ordinary/MCP coexistence, cutover, rollback, DomainClaim and credential revocation, generation mismatch isolation, membership cleanup, convergence, retry, replay, expiry, and stable desired identity. Compiled PostgreSQL fixtures cover typed credential-delivery lookup, stored scope/head authority, stale-policy rejection, Outbox rollback, certificate readiness, and Applied acknowledgement. Executed real PostgreSQL evidence for the newest certificate, credential, and audit paths plus joint Runtime/Gateway recovery evidence remain. |
