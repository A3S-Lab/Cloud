...
| `CELL0.4` | In progress | Component-only `C1` persists application heads and immutable canonical-ACL revisions through migration `116`, the existing A3S ORM Migrator, and the shared idempotency/Outbox/audit transaction path. `C2` registers authorization-before-replay create/revise/start/stop commands and tenant-bounded current/history queries, validates the exact existing tenant BuildRun, and reuses only the C1 repository plus shared command/query buses. Their PostgreSQL gate is checked in but has no retained pass. `C3` deployment correlation plus Workload/S0/Operations composition, `C4` Gateway publication, `C5` REST/client/CLI/Management MCP, and `C6` retained recovery evidence remain; Web stays deferred |
