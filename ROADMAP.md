...
Component-only `C0.5-MT1-C1` is also implemented as a prerequisite for the
next workload-trust slice. It introduces one explicit scope hierarchy and one
Identity-owned closed platform-role policy/binding model. The next ordering is
the remaining `MT1` support-grant/effective-decision/audit contracts and an
installation-scoped shared audit/Outbox shape, followed by `MT2` persistence
invariants. Only then may `WI1-C2` persist installation trust state; no new path
may use a synthetic Organization or `actor_is_platform_admin` as authority.
