...
- REST major version 1 publishes one unauthenticated raw OpenAPI 3.0.3 snapshot
  ...
  responses, envelopes, and compatibility metadata; current contract `1.61.0`
  ...
  versioned Variable Aggregator payload schema through the existing ACL
  transport and maintained client.
...
for List Operator, business-service, and remaining Agent/MCP/model/Tool failure semantics,
...
Current finite Execution, Connector, HumanDecision, and Subworkflow steps
retain closed, bounded owning-context evidence correlations in the existing
step projection. Exact child Execution, Operation, Connector attempt,
HumanTask, WorkflowDecision, optional accepted FormSubmission, and child
WorkflowRun URNs are reconstructed only from verified Flow history. Composite
...
runs pin runtime build `a3s-cloud-workflows@22`; the former `@1` through `@21`
...
existing Executions cleanup path. It does not implement List Operator,
business-service dispatch, compensation, or the remaining `W0.4` provider steps. The
