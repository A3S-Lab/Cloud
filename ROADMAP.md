...
5. `BX0.5` has removed the retired build executors and adds an architecture
   ...
   staying `OPEN`, with steps 1–7 enroll/OCI/deploy/health/HTTPS/logs/update
   preflight only (`enroll=not_run`, `oci=not_run`, `deploy=not_run`,
   `health=not_run`, `https=not_run`, `logs=not_run`, `update=not_run`); CI
   harness `run_bx0_clean_host_gate_ci.sh` certifies fail-closed behavior only
