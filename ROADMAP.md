...
| `AUT0.4-C1` | Implemented; component-only | `AutomationEventInvocationFactory` binds one already-normalized PluginEvent or SourceEvent identity to its exact subscription and Automation revision, validates event-key/observation ordering and the common invocation envelope, and derives policy-scoped deduplication. It owns no provider normalization/filtering, subscription persistence, queue, worker, scheduler, or P0 handoff. |
