...
| `A0.3` | In progress | One typed external-or-hosted build subject/source, optional closed Asset build admission, deterministic pinned hosted-Git input, and the common `cloud.build@5`/OCI/evidence projection foundation are implemented. Durable hosted BuildRun reservation, atomic release finalization, immutable release provenance, draft recovery, yanking, deterministic selection, and product surfaces remain |
