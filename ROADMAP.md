...
| `MCP0.3` | Cloud | Foundation in progress | Closed profile and route-policy ACL admission, migrations 052-059, typed persistence, exact release/profile/Workload binding, Runtime Service projection, credential persistence and internal one-shot issuance, complete node planning, ordinary-plus-MCP snapshot composition, atomic staging, Fleet delivery, acknowledgement, expiry cleanup, and unified certificate convergence now exist. Every ordinary publication, rollout, cutover, rollback, and certificate-repair path consumes the same node desired-state plan and preserves current MCP policy. The cursor-fair worker composes all active logical scopes for one physical Gateway, while owner-aware dispatch preserves the originating publication authority. Revoked, expired, or generation-mismatched credentials are resolved before Runtime health; only affected MCP routes are suppressed, unrelated routes remain projected, and the full observed policy/Workload set plus exact invalid credential state is locked transactionally before cleanup is staged. MCP-only certificate renewal stages a complete replacement and revokes the superseded provider serial only after installation. Focused tests cover multi-scope composition, ordinary/MCP coexistence, cutover, rollback, DomainClaim and credential revocation, generation mismatch isolation, membership cleanup, convergence, retry, replay, expiry, and stable desired identity. Compiled PostgreSQL fixtures cover stored scope/head authority, stale-policy rejection, Outbox rollback, certificate readiness, and Applied acknowledgement. Public idempotent one-time credential delivery/rotation, lifecycle surfaces, control-plane audit, executed real PostgreSQL evidence for the newest certificate and credential-cleanup paths, and joint Runtime/Gateway recovery evidence remain. |
