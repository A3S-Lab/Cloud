...
| `AUT0.2` | In progress; component-only admission boundary | Signed webhook endpoints, bounded capture, schema evaluation, exact target, replay, disable/revoke, durable recovery, and Gateway recovery remain the full gate. C1/C2 contracts and admission, plus the C3 PostgreSQL persistence boundary, are implemented; live PostgreSQL recovery evidence, listener/Gateway wiring, and public availability remain open |
