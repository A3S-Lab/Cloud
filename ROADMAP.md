...
redacted query.
`C0.3-PA2c` now has a frozen explicit-retention contract and a complete
implementation over the same query/write authority; provider and main-CI
verification are in progress. Therefore `C0.3` is still in progress rather
than fully verified.
