...
| `C0.3-RG2` | In progress; Workloads slice implemented | A typed route-metadata contract admits indirect project-owned requests only when the caller has coarse project visibility. Workloads then resolves Workload, Deployment, and workload-log IDs through its existing repository, derives the owning environment scope, and calls the shared evaluator at the application boundary. REST and Management MCP reads plus ordinary/Agent updates, rollback, Skill binding/unbinding, stop, and cancellation return the same `404` contract for denied and missing IDs; authorization runs before idempotency replay so revocation applies on the next request. Remaining Asset, Form, Workflow, BuildRun, Route, Secret, Agent execution, and Operation boundaries must follow the same pattern. No Identity-owned cross-context ownership table, presentation-only filter, or context-local grant evaluator is allowed. |
