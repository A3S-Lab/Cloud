...
immutable ACL-native repository. Eleven Application tools reuse the
project-authorized Applications CQRS and single release/session repositories:
six manage immutable Applications/releases, while five `application:write`
tools admit or read caller-owned project-member sessions, invocations, and
messages. The four release-management reads remain `cloud:read`; none copies
...
histories use replay build `a3s-cloud-workflows@12`; builds `@1` through `@11`
...
runs pin runtime build `a3s-cloud-workflows@12`; the former `@1` through `@11`
...
projection v3. Historic v1-v11 inputs retain their bytes and replay behavior.
...
composite claim false. The forty-one accepted authority decisions live under
...
| `APP0.2` | In progress; unavailable | Complete application-scoped credential and public delivery admission, public close/cancel and replay interfaces, remaining message variants, file references/citations, feedback, annotations, blocking/streaming answer parity, repeated-frame Answer ordinals, and retained delivery recovery evidence over C1-C11 |
...
10. retain the implemented `W0.1` contracts, backend `W0.2` Ontology lifecycle,
    ...
    graph Answer frames and finish Applications-owned variables,
    repeated-frame Answer ordinals, and remaining non-Execution error routes while
