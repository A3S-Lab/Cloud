...
| `AUT0.2` | In progress; component-only admission boundary | Signed webhook endpoints, bounded capture, schema evaluation, exact target, replay, disable/revoke, durable recovery, and Gateway recovery remain the full gate. C1-C4 contracts, admission, PostgreSQL persistence, and exact-version HMAC verification are implemented; live PostgreSQL recovery evidence, schema registry/evaluation, listener/Gateway wiring, and public availability remain open |
