...
| `AUT0` — Automations and Connectors | Schedule, webhook, plugin/source-event triggers and reusable outbound HTTP/business connections with exact targets, deduplication, Secret/egress policy, and recovery | In progress and unavailable; Automations C1-C6 plus AUT0.2-C7 deterministic webhook admission, AUT0.3-C1-C8 calendar/misfire/concurrency/due-window/lease/state/envelope and bounded schedule dispatch, and AUT0.4-C1/C2 normalized event-envelope and bounded fan-out components are implemented. AUT0.4-C3 adds the strict durable-ack A3S Event consumer boundary for already normalized events, and C4 composes external candidate selection, deterministic fan-out, and idempotent invocation admission. Connector C1-C13 plus Flow-owned attempt/wait, immutable-response, typed JSON interpretation, exact revision revocation, and terminal-indeterminate recovery are component foundations; live PostgreSQL recovery, production candidate/admission owners, timer/listener/Gateway wiring, and public availability remain open |
...
| `AUT0.3` | In progress; component-only calendar/misfire/concurrency/due-window/lease, durable cursor/lease, exact invocation-envelope, bounded schedule-dispatch, and P0 Task-target adaptation boundaries | Timezone-aware schedules, catch-up rules, timer ownership, and the production P0 scheduled Task handoff through the existing Boot task rail remain the full gate |
