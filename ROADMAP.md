...
| `AUT0.3-C10` | Implemented; application and PostgreSQL admission boundary | `AutomationInvocationAdmissionService` validates one exact envelope. The in-memory adapter and migration `187`/`PostgresAutomationInvocationRepository` fence invocation identity and policy-derived deduplication keys, replay exact evidence as `AlreadyAdmitted`, and atomically retain the immutable envelope, redacted Audit, and one Outbox fact only on first admission. `AutomationsDispatchServices` composes schedule and normalized-event dispatch behind this same admission port. No timer, queue, target execution, mutable revision lookup, production worker composition, or P0 handoff is introduced. |
