...
| `A0.3` | In progress | One typed external-or-hosted build subject/source, optional closed Asset build admission, deterministic pinned hosted-Git input, the common `cloud.build@5`/OCI/evidence projection, migration 063 typed persistence, concurrent hosted BuildRun reservation, and restart repair of the draft-to-operation gap are implemented. Atomic release finalization, immutable release provenance, failed-draft recovery, yanking, deterministic selection, and product surfaces remain |
