...
| `AUT0.2` | In progress; component-only admission boundary | Signed webhook endpoints, bounded capture, schema evaluation, exact target, replay, disable/revoke, durable recovery, and Gateway recovery remain the full gate. C1-C5 contracts, admission, PostgreSQL persistence, exact-version HMAC verification, and a digest-bound JSON Schema adapter are implemented; live PostgreSQL recovery evidence, schema registry selection/wiring, listener/Gateway wiring, and public availability remain open |
