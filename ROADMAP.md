...
The current `H0.2` foundation is implemented for one node and one Gateway.
...
PostgreSQL migration and recreated-repository evidence preserve this identity
across restart.
...
certificate and selector, and recovers only the replacement after restart. The
gate remains open for logical scopes beyond the current node identity,
mixed-version delivery, replicated readiness and rollout thresholds, and joint
production HA evidence.
...
| `H0.2` | Native identity/validity/readiness bridge, same-policy renewal, and command-bound revision/unit/generation target projection with PostgreSQL recovery are available; logical scopes and replicated projection remain | Explicit managed mode, native exact apply/readiness, same-digest renewal, durable journal, and rejection of local control loops are available | Single-Gateway certificate and target replacement, superseded-selector rejection, and restart recovery are verified; logical-scope, mixed-version, replicated-rollout, and production HA gates remain open |
