...
| `F0` — Foundation | Boot control plane and PostgreSQL task queue, PostgreSQL, tenancy, identity, ORM-backed Flow operations, outbox, projections, and API | In progress; re-certification required because the last complete pass used Flow `0.12.0`, while the current exact lock moves Cloud and Code to the same Flow `1.0.0-rc.1` candidate, startup-validates one exact workflow/step runtime registry, and pins one ACL `0.3.0` source with Boot `0.2.0` and ORM `0.3.0`; the remaining upstream ACL `0.2.2` constraint must converge |
...
variable before serving. Box packaging must still bind those references to
distinct database principals/grants. Retained clean-Linux
upgrade/rollback/failover evidence and broader production installation/HA
work remain.
...
Management MCP accept optional `compositeRegionsAcl`. This is policy and child
identity authority only: composite frames/exports, deterministic result
materialization, and Flow-backed Iteration/Loop dispatch remain fail-closed.
No region table, scheduler, queue, worker, event history, or Flow change was
introduced.
...
for composite frame/export and Applications-owned variable semantics,
Flow-backed Iteration and Loop dispatch, typed node error branches/fallback,
and ordered Answer frames.

The shared execution substrate now pins A3S Flow `1.0.0-rc.1`, A3S Boot `0.2.0`
...
composite claim false. The twelve accepted authority decisions live under
...
10. retain the implemented `W0.1` contracts, backend `W0.2` Ontology lifecycle,
    ...
    evidence and recertify the exact Flow `1.0.0-rc.1` compatibility lock, including
    ...
    policy/child bindings, and read-only built-in catalog discovery, then
    finish composite frames/exports, Applications-owned variables, Flow-backed
    Iteration/Loop dispatch, error branches/fallback, and Answer frames while
