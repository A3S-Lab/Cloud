...
| `AUT0` — Automations and Connectors | Schedule, webhook, plugin/source-event triggers and reusable outbound HTTP/business connections with exact targets, deduplication, Secret/egress policy, and recovery | In progress and unavailable; Automations C1-C6 plus the AUT0.3-C1 calendar, C2 misfire, C3 concurrency, C4 due-window, C5 lease, C6 durable cursor/lease, C7 deterministic due-time envelope, and AUT0.4-C1/C2 normalized event-envelope and bounded fan-out components are implemented. Connector C1-C13 plus Flow-owned attempt/wait, immutable-response, typed JSON interpretation, exact revision revocation, and terminal-indeterminate recovery are component foundations; live PostgreSQL recovery, general provider/Event-consumer wiring, and public availability remain open |
