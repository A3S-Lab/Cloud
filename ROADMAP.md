...
| `AUT0.4-C3` | Implemented; component-only | `A3sEventAutomationNormalizedEventConsumer` binds one exact durable provider subject/source to `AutomationNormalizedEventV1`. It ACKs malformed or successfully handled events, leaves handler failures unacknowledged for provider-owned redelivery, and owns no local retry, queue, subscription discovery, invocation persistence, or target execution. |
