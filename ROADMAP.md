...
| `W0` — Ontology-driven Workflow | Versioned ontologies and Workflows, deterministic goal-to-plan compilation, typed Agent/MCP/model/human steps, and Flow-based recoverable runs | In progress and unavailable (`W0.1` is implemented and `W0.2` is verified, while the `W0.3` definition/goal/deterministic-plan, native Form draft/release, minimal WorkflowRun, and internal authority-bound HumanTask decision loop are implemented; public protected submission/task surfaces and their Resource Grant enforcement, expiry/cancellation coordination, service/finite-task dispatch, typed capability steps, compensation, expanded `W0.3` cross-surface verification, and `W0.4`-`W0.5` remain) |
...
The current catalog contains 69 administrator tools and 43 read-only tools:
...
| `C0.3-RG2` | In progress; Workloads, external-source BuildRun, ordinary Route, and Secret slices implemented | A typed route-metadata contract admits indirect project-owned requests only when the caller has coarse project visibility. Workloads resolves Workload, Deployment, and workload-log IDs through its existing repository; Artifacts resolves BuildRun detail, evidence, logs, cancellation, and retry through the BuildRun repository; Edge resolves ordinary Route detail through the Edge repository; Secrets resolves detail, rotation, and version revocation through the Secret repository. Each owner derives the canonical environment scope and calls the shared evaluator at the application boundary across its existing REST and Management MCP surfaces. Denied and missing IDs share the same `404` contract, and mutation authorization runs before idempotency replay so revocation applies on the next request. Hosted Asset-release BuildRuns are organization-scoped today and therefore remain available to organization-wide roles while restricted memberships fail closed; no synthetic project ownership is inferred. MCP Route Policy, DomainClaim, and Credential remain separate Edge aggregates and must resolve their own canonical scopes rather than piggybacking on ordinary Route identity. Internal Secret materialization retains its Workload-revision/environment binding and is not replaced with an end-user grant check. Remaining Asset, Form, Workflow, Agent execution, and Operation boundaries must follow the same pattern. No Identity-owned cross-context ownership table, presentation-only filter, or context-local grant evaluator is allowed. |
...
service/finite-task dispatch, compensation, or `W0.4` provider steps. The
...
the internal accepted-submission-to-Flow-resume loop, but they do not expose a
public task product or establish end-to-end Workflow availability.
