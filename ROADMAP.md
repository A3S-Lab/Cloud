...
| `AUT0.3-C6` | Implemented; component-only PostgreSQL state boundary | Migration `184` and the in-memory/PostgreSQL schedule-state repositories persist one exact Automation revision digest, exclusive due cursor, monotonic lease generation, and owner/token/timestamp fence. Acquisition and cursor commit lock the one state row and reject stale, active, expired, non-rotated, or out-of-order fences. Migration `186` also permits an exact owner to release a fence without cursor progress. The state owner stores no definition copy, invocation, queue, retry, worker, or scheduler; live PostgreSQL recovery, invocation admission, timer ownership, and P0 handoff remain open. |
