...
| `C0.3-RG2` | In progress; Workloads read slice implemented | A typed route-metadata contract admits indirect project-owned reads only when the caller has coarse project visibility. Workloads then resolves Workload, Deployment, and workload-log IDs through its existing repository, derives the owning environment scope, and calls the shared evaluator at the application boundary; REST and Management MCP return the same `404` contract for denied and missing IDs. Remaining Asset, Form, Workflow, BuildRun, Route, Secret, Agent execution, Operation, and indirect mutation boundaries must follow the same pattern. No Identity-owned cross-context ownership table, presentation-only filter, or context-local grant evaluator is allowed. |
