...
| `MCP0` — Hosted MCP services | Modern stateless MCP release admission, Runtime Service hosting, Cloud orchestration, Gateway protocol enforcement, and joint recovery evidence | Planned |
...
| `MCP0.3` | Cloud | Foundation in progress | Closed contract values, canonical immutable Service-profile and separately mutable Edge route-policy ACL admission, migrations 052-055 plus typed A3S ORM persistence, exact AssetRelease/profile-bound WorkloadRevision identity, automatic opaque Runtime Service-spec binding, hosted credential lifecycle/persistence and an internal one-shot Argon2id issuer, scope-complete healthy-target planning, exact grant/generation authority resolution, deterministic release-bound one-route projection, and Gateway ACL compilation exist; public idempotent credential delivery/rotation, multi-route durable snapshot reconciliation/publication, lifecycle surfaces, recovery, audit, and the real PostgreSQL gate remain |
