...
| `MCP0.3` | Cloud | Foundation in progress | Closed contract values, canonical immutable Service-profile and separately mutable Edge route-policy ACL admission, migrations 052-054 plus typed A3S ORM persistence, exact AssetRelease/profile-bound WorkloadRevision identity, automatic opaque Runtime Service-spec binding, hosted credential lifecycle aggregate, scope-complete healthy-target planning, deterministic release-bound projection, and Gateway ACL compilation exist; one-time credential issuance and persistence, credential authority resolution, durable snapshot reconciliation/publication, lifecycle surfaces, recovery, audit, and the real PostgreSQL gate remain |
