...
| `C0.5-MT2-C3` | In progress | Issue one current-snapshot privileged allow through Identity after atomically resolving the active Principal, verified credential, current policy/binding, and exact optional tenant-support grant; persist the immutable decision through shared Audit and expose it only through maintained Application interfaces. |
...
approver rollback, terminal history and replay. `MT2-C3` next issues the atomic
current-snapshot privileged decision and maintained interfaces. Only then may
