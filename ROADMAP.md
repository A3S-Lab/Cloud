...
| `A1` — Heterogeneous Agent execution | Durable conversations, one provider-neutral Harness contract, semantic events, approvals, checkpoints, forks, and trajectories over existing Cloud control paths | In progress (`A1.0` verified; `A1.1` implemented; native Code `A1.2` start, cancellation, event-page, retention-gap, and recovery orchestration has retained clean Linux PostgreSQL 17 and real Box Runtime process-death evidence; dependency publication remains) |
...
- REST major version 1 publishes one unauthenticated raw OpenAPI 3.0.3 snapshot
  ...
  responses, envelopes, and compatibility metadata; current contract `1.50.0`
  retains it and extends only the closed alert-source union. The shared client
  and response headers pin the current version;
...
SMTP, tenant-scoped security investigation, audit retention/export policy, and
...
| `C0.3-N4f` | Verified on PostgreSQL 17.5 in CI (`2026-08-22`) | Edge's existing Gateway certificate reconciler gains one fixed 24-hour emergency window after normal renewal has already had time to converge. For each active logical Route and physical Gateway node, a source-specific deterministic subject and one narrow Edge-owned `GatewayCertificateExpiryRisk` projection retain only the current `at_risk`/`clear` state, exact certificate evidence, and a monotonic fact generation. The first observation of an exact active certificate whose expiry is at or before the canonical scan time plus 24 hours emits schema-v1 `edge.gateway-certificate.expiry-at-risk`; replay of that certificate is silent, while a different actually-applied certificate that is still inside the window emits one new at-risk generation without a false recovery. Only binding a different `Ready` certificate through a terminal `Applied` convergence, with expiry strictly outside the window, may clear an existing risk for that same Route-plus-node subject and emit schema-v1 `edge.gateway-certificate.expiry-risk-cleared`. The typed `GatewayCertificateExpiryRiskChanged` payload pins exact organization/project/environment, Route, Workload, hostname/path, physical Node, active Gateway revision, active certificate/expiry, fixed window, state, and—only for clearing—the previous at-risk certificate evidence; it excludes PEM, credentials, provider or acknowledgement text, commands, and private failures. Risk entry, applied-certificate clearing, projection generation, and their existing transactional Outbox facts commit atomically with optimistic concurrency. Migration `135` adds only this projection. The [successful H0 job](https://github.com/A3S-Lab/Cloud/actions/runs/32568019126/job/97019385911) proves threshold equality, normal-renewal and outside-window silence, Route/node isolation, concurrent and replay deduplication, short-lived Applied replacement behavior, Applied-only clearing, private-data exclusion, and injected at-risk/clear Outbox rollback on checksum-pinned PostgreSQL 17.5. This slice adds no alert source, policy, incident, configurable threshold, poller, timer, scheduler, queue, event rail, configuration parser, endpoint, or tool; a later slice may register `edge.gateway-certificate-expiry-risk.v1` in Notifications. |
...
Dependency publication still blocks completion, so `A1.2` remains in progress.
