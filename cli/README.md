...
Hosted-MCP credential issue and rotate require an RFC 3339 `--expires-at` and
a caller-owned idempotency key. The credential is generated by Cloud, not
accepted as a CLI argument or input value. List, get, and revoke display only
allowlisted metadata. Issue and rotate deliberately display the complete
one-time value as `ONE-TIME SECRET` in table output or `secret` in JSON so it
cannot be silently omitted; redirect it only to an approved secret sink and do
not retain command output. Every credential request and response is
non-cacheable, and the shared client rejects internal material or a secret on
metadata-only responses before the CLI can render it.
...
MCP credential list/get/revoke output contains metadata only. Issue/rotate
output includes the authoritative `replayed` value and the explicit one-time
secret; invalid response fields and sanitized upstream errors are rejected
...
Hosted-MCP credential lifecycle parity is implemented with non-cacheable
transport, fail-closed response validation, metadata-only read/revoke output,
and explicit one-time issue/rotate delivery.
