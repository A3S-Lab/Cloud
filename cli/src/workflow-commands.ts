...
export async function executeWorkflowCommand(
  ...
  switch (command) {
    ...
    case 'workflow-runs variables':
      requireReadCommand(arguments_, 'workflow-runs variables <workflow-run-id>');
      rejectWorkflowRunSpecificOptions(arguments_);
...
function isWorkflowPublication(value: unknown): value is PublishWorkflowDefinitionInput {
  ...
  if (record.semanticContracts !== undefined) {
    ...
    const keys = [...requiredKeys, 'variableDefaultsAcl', 'compositeRegionsAcl'];
    if (
      ...
      (semanticContracts.compositeRegionsAcl !== undefined &&
        typeof semanticContracts.compositeRegionsAcl !== 'string')
    ) {
      return false;
    }
  }
