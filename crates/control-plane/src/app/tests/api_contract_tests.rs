...
mod agent_response_schemas;
mod developer_workflow_response_schemas;
...
fn generated_openapi_operations_have_stable_ids_security_and_envelopes() -> Result<()> {
    ...
    assert_eq!(
        semantic_contracts["properties"]["compositeRegionsAcl"]["maxLength"],
        524_288
    );
    assert!(
    ...
    assert!(workflow_run_variables["responses"]["404"].is_object());
    assert!(workflow_run_variables["responses"]["409"].is_object());
    assert!(workflow_run_variables["responses"]["503"].is_object());
    assert_eq!(
