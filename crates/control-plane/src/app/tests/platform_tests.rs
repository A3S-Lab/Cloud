...
fn build_plan_acceptance_has_one_owner_authorized_production_composition_path() {
    ...
    for constructor in [
        ...
        assert_eq!(
            composition.matches(constructor).count(),
            1,
            "Developer Workflows BuildPlan acceptance must compose {constructor} exactly once"
        );
    }
    assert_eq!(
        composition
            .matches("crate::modules::developer_workflows::AcceptBuildPlan, _")
            .count(),
        1,
        "the canonical BuildPlan acceptance command must be registered once on the existing CQRS bus"
    );
}
