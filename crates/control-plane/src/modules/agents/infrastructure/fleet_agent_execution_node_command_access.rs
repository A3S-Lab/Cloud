...
/// Anti-corruption adapter: Agents AgentExecution Flow → Fleet Node-command store.
///
/// Translates Agents-owned enqueue/reload/ack/observation intents into Fleet
...
impl FleetAgentExecutionNodeCommandAccessAdapter {
...
impl IAgentExecutionNodeCommandPort for FleetAgentExecutionNodeCommandAccessAdapter {
    async fn enqueue_command(
        ...
        let write = self
            ...
            .await?;
        Ok(AgentExecutionNodeCommandDispatch {
    ...
    async fn find_command(
        ...
        Ok(self
            ...
            .await?
            .map(|command| AgentExecutionNodeCommandProjection {
                id: command.id,
    ...
    async fn command_acknowledgement(
        ...
        Ok(self
            ...
            .await?
            .map(|acknowledgement| AgentExecutionNodeCommandAcknowledgement {
    ...
    async fn latest_runtime_observation(
        ...
        Ok(self
            ...
            .await?
            .map(|record| AgentExecutionRuntimeObservationProjection {
