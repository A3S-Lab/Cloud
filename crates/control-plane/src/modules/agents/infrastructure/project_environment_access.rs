...
#[derive(Clone)]
pub struct ProjectsAgentsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsAgentsEnvironmentAccessAdapter {
...
#[async_trait]
impl IAgentsEnvironmentAccess for ProjectsAgentsEnvironmentAccessAdapter {
    async fn environment_exists(
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Agents environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsAgentsEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            AgentsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
