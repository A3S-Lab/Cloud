//! Anonymous-credential CQRS poll for Applications streaming observation.
//!
//! `APP0.2-C49` authorizes one opaque delivery credential, loads the exact
//! anonymous session/invocation, and projects `ApplicationStreamingObservation`
//! without a second run history, migration, SSE route, or delivery surface.
...
use super::streaming_observation_commands::load_invocation_messages;
use crate::modules::applications::domain::{
    ApplicationStreamingObservation, IApplicationDeliveryCredentialRepository,
...
/// Observe one Streaming-mode invocation for an anonymous delivery credential.
#[derive(Debug, Clone)]
pub struct ObserveAnonymousApplicationStreamingInvocation {
...
impl Query for ObserveAnonymousApplicationStreamingInvocation {
    type Output = ApplicationResult<ApplicationStreamingObservation>;
}

pub struct ObserveAnonymousApplicationStreamingInvocationHandler {
    sessions: Arc<dyn IApplicationSessionRepository>,
    credentials: Arc<dyn IApplicationDeliveryCredentialRepository>,
}

impl ObserveAnonymousApplicationStreamingInvocationHandler {
    pub fn new(
        sessions: Arc<dyn IApplicationSessionRepository>,
        credentials: Arc<dyn IApplicationDeliveryCredentialRepository>,
    ) -> Self {
...
impl QueryHandler<ObserveAnonymousApplicationStreamingInvocation>
    for ObserveAnonymousApplicationStreamingInvocationHandler
{
    fn execute(
        &self,
        query: ObserveAnonymousApplicationStreamingInvocation,
    ...
    > {
        ...
        Box::pin(async move {
            ...
            // Active so mid-stream disable still allows poll; foreign keys fail.
