//! Project-authorized CQRS poll for Applications blocking observation.
//!
//! `APP0.2-C40` authorizes one exact session/invocation, pages the existing
//! Applications-owned message sequence, and projects `ApplicationBlockingObservation`
...
use crate::modules::applications::domain::{
    ApplicationBlockingObservation, ApplicationMessage, IApplicationSessionRepository,
};
...
/// Observe one Blocking-mode invocation against its session message frames.
#[derive(Debug, Clone)]
pub struct ObserveApplicationBlockingInvocation {
...
impl Query for ObserveApplicationBlockingInvocation {
    type Output = ApplicationResult<ApplicationBlockingObservation>;
}

pub struct ObserveApplicationBlockingInvocationHandler {
    sessions: Arc<dyn IApplicationSessionRepository>,
}

impl ObserveApplicationBlockingInvocationHandler {
    pub fn new(sessions: Arc<dyn IApplicationSessionRepository>) -> Self {
        Self { sessions }
    }
}

impl QueryHandler<ObserveApplicationBlockingInvocation>
    for ObserveApplicationBlockingInvocationHandler
{
    fn execute(
        &self,
        query: ObserveApplicationBlockingInvocation,
