...
use crate::modules::shared_kernel::domain::{
    ApplicationEndUserId, ApplicationId, ApplicationSessionId, OrganizationId, PrincipalId,
...
/// Authorize anonymous delivery reads by exact credential-derived end-user id.
/// Does not grant project-member Principal authority.
pub(super) async fn anonymous_credential_session(
    sessions: &dyn IApplicationSessionRepository,
    organization_id: OrganizationId,
    project_id: ProjectId,
    application_id: ApplicationId,
    session_id: ApplicationSessionId,
    expected_end_user_id: ApplicationEndUserId,
) -> ApplicationResult<AuthorizedApplicationSession> {
    if organization_id.as_uuid().is_nil()
        || project_id.as_uuid().is_nil()
        || application_id.as_uuid().is_nil()
        || session_id.as_uuid().is_nil()
        || expected_end_user_id.as_uuid().is_nil()
    {
        return Err(ApplicationError::Invalid(
            "Anonymous Application delivery scope is invalid".into(),
        ));
    }
    let session = match sessions
        .find_session(organization_id, project_id, application_id, session_id)
    ...
    if end_user.audience != ApplicationAudience::Anonymous
        || end_user.linked_principal_id.is_some()
        || end_user.id != expected_end_user_id
    {
        return Err(session_not_found());
    }
    Ok(AuthorizedApplicationSession { end_user, session })
