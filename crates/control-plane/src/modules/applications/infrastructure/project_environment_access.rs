use crate::modules::applications::application::{
    ApplicationsEnvironmentScope, IApplicationsEnvironmentAccess,
};
...
#[derive(Clone)]
pub struct ProjectsApplicationsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsApplicationsEnvironmentAccessAdapter {
...
#[async_trait]
impl IApplicationsEnvironmentAccess for ProjectsApplicationsEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: ApplicationsEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Applications environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present = ProjectsApplicationsEnvironmentAccessAdapter::new(Arc::new(
        ...
        let scope =
            ApplicationsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
        assert!(present.environment_exists(scope).await.unwrap());

        let missing = ProjectsApplicationsEnvironmentAccessAdapter::new(Arc::new(
            StubEnvironmentRepository {
                environment: None,
                find_calls: AtomicUsize::new(0),
            },
