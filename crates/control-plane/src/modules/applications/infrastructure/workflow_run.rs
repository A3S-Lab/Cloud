...
use crate::modules::workflow::domain::{
    ...
    WORKFLOW_RUN_INPUT_SCHEMA_V22, WORKFLOW_RUN_RUNTIME_CONTRACT_REVISION_V10,
...
mod tests {
    ...
    use crate::modules::workflow::test_support::{
        ...
        application_variable_workflow_run_input, routed_application_answer_workflow_run_input,
    };
    use chrono::Duration;
    ...
    #[test]
    fn production_adapter_accepts_exact_v15_answer_failure_route_and_rejects_v14_alias() {
        ...
        input.requested_at = requested_at;
        input.deadline_at = requested_at + Duration::hours(1);
        let (run, steps) = WorkflowRun::create(input, requested_by).expect("v15 WorkflowRun");
        let record = WorkflowRunRecord { run, steps };

        WorkflowApplicationRunService::validate_record(&request, &record)
            .expect("production adapter accepts v15 Answer failure route authority");

        let mut aliased = record;
        aliased.run.execution_input.schema = WORKFLOW_RUN_INPUT_SCHEMA_V14.into();
        aliased.run.execution_input.runtime_contract_revision =
            WORKFLOW_RUN_RUNTIME_CONTRACT_REVISION_V14.into();
        aliased.run.execution_input.flow_workflow_version = WORKFLOW_RUN_FLOW_VERSION_V14.into();
        assert!(WorkflowApplicationRunService::validate_record(&request, &aliased).is_err());
    }
}
