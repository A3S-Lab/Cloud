...
fn notifications_queries_and_commands_isolate_identity_behind_one_context_owned_access_projection()
{
    ...
    let compact_access = production_access.split_whitespace().collect::<String>();
    for required in [
        "pubenumNotificationAccessScope",
        "pubstructNotificationAccess",
        "scope_is_visible",
        "environment_is_visible",
        "node_is_visible",
    ] {
        assert!(
            compact_access.contains(required),
            "Notifications lost its context-owned resource access boundary {required}"
        );
    }
    for forbidden in [
        "crate::modules::identity",
        ...
        assert!(
            !production_access.contains(forbidden),
            "Notifications resource access copied Identity authority {forbidden}"
        );
    }

    let domain = std::fs::read_to_string(root.join("notifications/domain/notification.rs"))
        .expect("read Notifications domain notification");
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Notifications domain regained Identity visibility vocabulary {forbidden}"
        );
    }

    for relative in [
        "notifications/application/get_notification.rs",
        "notifications/application/list_notifications.rs",
        "notifications/application/mark_notification_read.rs",
        "notifications/application/alert_policy.rs",
        "notifications/application/alert_policy_queries.rs",
        "notifications/application/outbound_subscription.rs",
        "notifications/application/outbound_subscription_queries.rs",
    ] {
        let source = std::fs::read_to_string(root.join(relative))
            .unwrap_or_else(|error| panic!("read {relative}: {error}"));
        let production = production_source(&source);
        assert!(
            production.contains("pub access: NotificationAccess"),
            "{relative} stopped carrying Notifications-owned access"
        );
    ...
    let compact_projection = access_projection.split_whitespace().collect::<String>();
    for required in [
        "pub(crate)fnnotification_access(",
        ...
        assert!(
            compact_projection.contains(required),
            "root anti-corruption layer lost Notifications access mapping {required}"
        );
    }
    ...
    assert!(
        ...
        "Notifications REST must project Identity into NotificationAccess"
    );
    assert!(
        !production_controller.contains("resource_access: resource_access_evaluator")
            && !production_controller.contains("resource_access,"),
        "Notifications REST must not pass ResourceAccessEvaluator into Application"
    );

    let mcp = std::fs::read_to_string(
        root.parent()
            .expect("src directory")
            .join("presentation/management_mcp/notifications.rs"),
    )
    .expect("read Notifications Management MCP adapter");
    let production_mcp = production_source(&mcp);
    assert!(
        production_mcp
            .matches("access: notification_access(&resource_access)")
            .count()
            >= 11,
        "Notifications MCP must project every resource-authorized tool into NotificationAccess"
    );
    assert!(
        !production_mcp.contains("resource_access,"),
...
fn durable_cells_queries_commands_and_admission_isolate_identity_behind_one_context_owned_access_projection()
 {
    ...
    let compact_access = production_access.split_whitespace().collect::<String>();
    for required in [
        "pubenumDurableCellAccessScope",
        "pubstructDurableCellAccess",
        "environment_is_visible",
    ] {
        assert!(
            compact_access.contains(required),
            "Durable Cells lost its context-owned resource access boundary {required}"
        );
    }
    for forbidden in [
        "crate::modules::identity",
        ...
        assert!(
            !production_access.contains(forbidden),
            "Durable Cells resource access copied Identity authority {forbidden}"
        );
    }

    for relative in [
        "durable_cells/application/queries.rs",
        "durable_cells/application/commands.rs",
        "durable_cells/application/deployment.rs",
        "durable_cells/application/route_publication.rs",
        "durable_cells/presentation/deployment_admission.rs",
    ] {
        let source = std::fs::read_to_string(root.join(relative))
            .unwrap_or_else(|error| panic!("read {relative}: {error}"));
        let production = production_source(&source);
        assert!(
            production.contains("pub access: DurableCellAccess"),
            "{relative} stopped carrying Durable Cells-owned access"
        );
        for forbidden in ["ResourceAccessEvaluator", "crate::modules::identity"] {
            assert!(
                !production.contains(forbidden),
    ...
    for required in [
        ...
        assert!(
            compact_projection.contains(required),
            "root anti-corruption layer lost Durable Cells access mapping {required}"
        );
    }

    let controller = std::fs::read_to_string(root.join("durable_cells/presentation/controller.rs"))
        .expect("read Durable Cells controller");
    let production_controller = production_source(&controller);
    assert!(
        production_controller.contains("durable_cell_access(&resource_access_evaluator("),
        "Durable Cells REST must project Identity into DurableCellAccess"
    );
    assert!(
        !production_controller.contains("resource_access: resource_access_evaluator")
            && !production_controller.contains("resource_access,"),
        "Durable Cells REST must not pass ResourceAccessEvaluator into Application"
    );

    let mcp = std::fs::read_to_string(
        root.parent()
            .expect("src directory")
            .join("presentation/management_mcp/durable_cells.rs"),
    )
    .expect("read Durable Cells Management MCP adapter");
    let production_mcp = production_source(&mcp);
    assert!(
        production_mcp
            .matches("access: durable_cell_access(&resource_access)")
            .count()
            >= 9,
        "Durable Cells MCP must project every resource-authorized tool into DurableCellAccess"
    );
    assert!(
        !production_mcp.contains("resource_access,"),
        "Durable Cells MCP must not pass ResourceAccessEvaluator into Application"
    );
}
...
#[test]
fn inference_route_and_usage_queries_isolate_identity_behind_one_context_owned_access_projection() {
    let root = module_root();

    let access = std::fs::read_to_string(root.join("inference/application/resource_access.rs"))
        .expect("read Inference resource access boundary");
    let production_access = production_source(&access);
    let compact_access = production_access.split_whitespace().collect::<String>();
    for required in [
        "pubenumInferenceAccessScope",
        "pubstructInferenceAccess",
        "environment_is_visible",
    ] {
        assert!(
            compact_access.contains(required),
            "Inference lost its context-owned resource access boundary {required}"
        );
    }
    for forbidden in [
        "crate::modules::identity",
        ...
        assert!(
            !production_access.contains(forbidden),
            "Inference resource access copied Identity authority {forbidden}"
        );
    }
    ...
    for relative in [
        ...
        let production = production_source(&source);
        assert!(
            production.contains("pub access: InferenceAccess"),
            "{relative} stopped carrying Inference-owned access"
        );
        for forbidden in ["ResourceAccessEvaluator", "crate::modules::identity"] {
            assert!(
                !production.contains(forbidden),
    ...
    for relative in [
        ...
        let production = production_source(&source);
        assert!(
            production.contains("access.environment_is_visible(")
                || production.contains(".access\n                .environment_is_visible(")
                || production
                    .split_whitespace()
                    .collect::<String>()
                    .contains("access.environment_is_visible("),
            "{relative} stopped authorizing through InferenceAccess"
        );
        for forbidden in ["ResourceAccessEvaluator", "crate::modules::identity"] {
            assert!(
                !production.contains(forbidden),
    ...
    for required in [
        ...
        assert!(
            compact_projection.contains(required),
            "root anti-corruption layer lost Inference access mapping {required}"
        );
    }
    ...
    for relative in [
        ...
        let production = production_source(&source);
        assert!(
            production.contains("inference_access(&resource_access_evaluator("),
            "{relative} must project Identity into InferenceAccess"
        );
        assert!(
            !production.contains("resource_access: resource_access_evaluator")
                && !production.contains("resource_access,"),
...
fn executions_get_and_cancel_isolate_identity_behind_one_context_owned_access_projection() {
    ...
    for relative in [
        ...
        "executions/application/commands/create_execution/command.rs",
...
#[test]
fn agents_isolate_operations_behind_owner_port() {
    let root = module_root();

    let port_path = "agents/application/agent_execution_operation_scheduler.rs";
    let port = std::fs::read_to_string(root.join(port_path))
        .expect("read Agents operation scheduling port");
    let production_port = production_source(&port);
    let compact_port = production_port.split_whitespace().collect::<String>();
    for required in [
        "pubtraitIAgentExecutionOperationScheduler:Send+Sync",
        "asyncfnschedule(&self,request:AgentExecutionOperationRequest,)->Result<AgentExecutionOperationScheduleOutcome,RepositoryError>;",
        "operation_id:OperationId",
        "organization_id:OrganizationId",
        ...
        assert!(
            compact_port.contains(required),
            "Agents operation port lost minimum interface {required}"
        );
    }
    for forbidden in [
        "crate::modules::operations",
        ...
        assert!(
            !production_port.contains(forbidden),
            "Agents operation port leaked Operations authority {forbidden}"
        );
    }

    let adapter_path = "agents/infrastructure/agent_execution_operation_scheduler.rs";
    let mut operation_sites = BTreeSet::new();
    let mut boundary_violations = BTreeSet::new();
    visit_production_sources(|relative, source| {
        if context(relative) != Some("agents") {
            return;
        }
        if source.contains("crate::modules::operations") {
            operation_sites.insert(display(relative));
    ...
    assert!(
        boundary_violations.is_empty(),
        "Agents bypassed its bounded Operations contract:\n{}",
        boundary_violations
            .into_iter()
            .collect::<Vec<_>>()
            .join("\n")
    ...
    assert_eq!(
        operation_sites,
        lines(adapter_path),
        "Agents must reach Operations through one Infrastructure adapter module"
    );

    let reconciler =
        std::fs::read_to_string(root.join("agents/application/agent_execution_reconciler.rs"))
            .expect("read Agents reconciler");
    let production_reconciler = production_source(&reconciler);
    let compact_reconciler = production_reconciler.split_whitespace().collect::<String>();
    for required in [
        "operation_scheduler:Arc<dynIAgentExecutionOperationScheduler>",
        "AgentExecutionOperationRequest::new(",
        ".operation_scheduler.schedule(request).await",
    ] {
        assert!(
            compact_reconciler.contains(required),
            "Agents reconciler lost inward scheduling boundary {required}"
        );
    }
    for forbidden in [
        "crate::modules::operations",
        ...
        assert!(
            !production_reconciler.contains(forbidden),
            "Agents reconciler rebuilt Operations authority {forbidden}"
        );
    }
    ...
    let compact_adapter = production_adapter.split_whitespace().collect::<String>();
    for required in [
        "implIAgentExecutionOperationSchedulerforOperationsAgentExecutionOperationScheduler",
        "operations:Arc<dynIOperationRepository>",
        ...
        assert!(
            compact_adapter.contains(required),
            "Agents operation adapter lost boundary behavior {required}"
        );
    }
    assert_eq!(
        production_adapter.matches(".enqueue(").count(),
        1,
        "Agents operation adapter must have one enqueue mechanism"
    );
}
...
fn edge_mcp_credentials_isolate_secrets_encryption_behind_one_owner_port() {
    ...
    let port_path = "edge/application/mcp_credential_encryption.rs";
    let port = std::fs::read_to_string(root.join(port_path))
        .expect("read Edge MCP credential encryption port");
    let production_port = production_source(&port);
    let compact_port = production_port.split_whitespace().collect::<String>();
    for required in [
        "pubtraitIEdgeMcpCredentialEncryption:Send+Sync",
        "asyncfnencrypt_delivery_receipt(",
        "asyncfnrecover_delivery(",
    ] {
        assert!(
            compact_port.contains(required),
            "Edge MCP credential encryption port lost minimum interface {required}"
        );
    }
    for forbidden in [
        "ISecretEncryptionService",
        ...
        assert!(
            !production_port.contains(forbidden),
            "Edge MCP credential encryption port leaked Secrets or concrete authority {forbidden}"
        );
    }

    for relative in [
        "edge/application/commands/create_mcp_credential/handler.rs",
        "edge/application/commands/rotate_mcp_credential/handler.rs",
        "edge/application/mcp_credential_delivery.rs",
    ] {
        let source = std::fs::read_to_string(root.join(relative))
            .unwrap_or_else(|error| panic!("read {relative}: {error}"));
        let production = production_source(&source);
        ...
        if relative.contains("handler.rs") {
            let compact = production.split_whitespace().collect::<String>();
            assert!(
                compact.contains("Arc<dynIEdgeMcpCredentialEncryption>")
                    && compact.contains(".encrypt_delivery_receipt(")
                    && compact.contains(".recover_delivery("),
                "{relative} stopped entering MCP delivery encryption through the Edge-owned port"
            );
        }
    }

    let adapter_path = "edge/infrastructure/mcp_credential_encryption.rs";
    let adapter = std::fs::read_to_string(root.join(adapter_path))
        .expect("read Edge MCP credential encryption adapter");
    let compact_adapter = production_source(&adapter)
        .split_whitespace()
        .collect::<String>();
    for required in [
        "implIEdgeMcpCredentialEncryptionforSecretsEdgeMcpCredentialEncryptionAdapter",
        "encryption:Arc<dynISecretEncryptionService>",
    ] {
        assert!(
            compact_adapter.contains(required),
            "Edge MCP credential encryption adapter lost boundary behavior {required}"
        );
    }
    for forbidden in [
        "Postgres",
        ...
        assert!(
            !production_source(&adapter).contains(forbidden),
            "Edge MCP credential encryption adapter introduced concrete state or lifecycle {forbidden}"
        );
    }
    ...
    let app = std::fs::read_to_string(root.parent().expect("src directory").join("app.rs"))
        .expect("read root composition");
    assert_eq!(
        app.matches("SecretsEdgeMcpCredentialEncryptionAdapter::new(")
            .count(),
        1,
...
#[test]
fn edge_locks_workloads_through_workloads_transaction_participant() {
    let root = module_root();
    ...
    visit_production_sources(|relative, source| {
        ...
        for forbidden in [
            ...
            if source.contains(forbidden) {
                persistence_violations.insert(format!(
                    "{path} regained Workloads table authority {forbidden}"
                ));
            }
        }
    });
    assert!(
        persistence_violations.is_empty(),
        "Edge persistence regained a local workloads mapping:\n{}",
        persistence_violations
            .into_iter()
            .collect::<Vec<_>>()
            .join("\n")
    ...
    visit_production_sources(|relative, source| {
        ...
        if source.contains("lock_running_workload_authority_for_update") {
            participant_sites.insert(path);
        }
    });
    assert!(
...
#[test]
fn agents_owns_approval_checkpoint_authorization_through_one_identity_adapter() {
    let port = std::fs::read_to_string(
        module_root().join("agents/application/approval_authorization_port.rs"),
    )
    .expect("read Agents approval authorization port");
    let compact_port = production_source(&port)
        .split_whitespace()
        .collect::<String>();
    assert!(
        compact_port.contains("traitIAgentApprovalAuthorizationPort:Send+Sync"),
        "Agents lost its consumer-owned approval authorization boundary"
    );
    assert!(
        !production_source(&port).contains("crate::modules::identity"),
        "the consumer-owned Agents approval authorization port imported Identity internals"
    );

    let handler = std::fs::read_to_string(
        module_root()
            .join("agents/application/commands/decide_agent_approval_checkpoint/handler.rs"),
    )
    .expect("read DecideAgentApprovalCheckpoint handler");
    let handler = production_source(&handler);
    assert!(handler.contains("IAgentApprovalAuthorizationPort"));
    assert_eq!(handler.matches(".authorize_decision(").count(), 1);
    assert!(
        !handler.contains("crate::modules::identity"),
        "Agents Application bypassed its approval authorization port"
    );
    assert!(
        !handler.contains("IResourceAuthorizationDecisionRepository"),
        "Agents Application still depends on Identity resource-authorization repository"
    );
    ...
    visit_production_sources(|relative, source| {
        ...
        if source.contains("impl IAgentApprovalAuthorizationPort for") {
            port_implementations.insert(display(relative));
        }
    });
    assert_eq!(
        identity_import_sites,
        BTreeSet::from(["agents/infrastructure/approval_authorization.rs".to_owned()]),
        "all Agents-to-Identity resource-authorization access must be confined to the sole consumer-side adapter"
    );
    assert_eq!(
        port_implementations,
        BTreeSet::from(["agents/infrastructure/approval_authorization.rs".to_owned()]),
        "Agent approval checkpoint authorization must have one consumer-side adapter"
    );
}
