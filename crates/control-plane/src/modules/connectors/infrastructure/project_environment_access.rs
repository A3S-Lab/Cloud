...
#[derive(Clone)]
pub struct ProjectsConnectorsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsConnectorsEnvironmentAccessAdapter {
...
#[async_trait]
impl IConnectorsEnvironmentAccess for ProjectsConnectorsEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: ConnectorsEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Connectors environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsConnectorsEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            ConnectorsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
