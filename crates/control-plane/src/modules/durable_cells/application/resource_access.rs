...
/// One Durable Cells visibility selector projected from an Identity decision.
///
...
pub enum DurableCellAccessScope {
    Project {
...
/// Durable Cells-owned projection of an already-authorized request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DurableCellAccess {
    organization_wide: bool,
    granted_scopes: BTreeSet<DurableCellAccessScope>,
}

impl DurableCellAccess {
    ...
    pub fn restricted(granted_scopes: impl IntoIterator<Item = DurableCellAccessScope>) -> Self {
        Self {
    ...
    pub fn granted_scopes(&self) -> impl Iterator<Item = DurableCellAccessScope> + '_ {
...
mod tests {
    ...
    fn organization_wide_sees_every_environment() {
        let access = DurableCellAccess::organization_wide();
    ...
    fn project_grant_covers_environments_while_environment_grant_is_exact() {
        ...
        let project_access =
            DurableCellAccess::restricted([DurableCellAccessScope::Project { project_id }]);
