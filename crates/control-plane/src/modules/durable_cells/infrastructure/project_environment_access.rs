use crate::modules::durable_cells::application::{
    DurableCellsEnvironmentScope, IDurableCellsEnvironmentAccess,
};
...
#[derive(Clone)]
pub struct ProjectsDurableCellsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsDurableCellsEnvironmentAccessAdapter {
...
#[async_trait]
impl IDurableCellsEnvironmentAccess for ProjectsDurableCellsEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: DurableCellsEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Durable Cells environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present = ProjectsDurableCellsEnvironmentAccessAdapter::new(Arc::new(
            StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            },
        ));
        let scope =
            DurableCellsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
        assert!(present.environment_exists(scope).await.unwrap());

        let missing = ProjectsDurableCellsEnvironmentAccessAdapter::new(Arc::new(
            StubEnvironmentRepository {
                environment: None,
                find_calls: AtomicUsize::new(0),
            },
        ));
        assert!(!missing.environment_exists(scope).await.unwrap());
    }
}
