...
const SELECT_GATEWAY_SCOPES: &str = "select id, organization_id, project_id, environment_id, node_id, aggregate_version, created_at, updated_at from gateway_route_scopes";
        ...
        .transaction(move |transaction| {
            Box::pin(async move {
                ...
                let inserted = execute(
                    transaction,
                    sql_query::<()>(
                        "insert into gateway_route_scopes (id, organization_id, project_id, environment_id, node_id, aggregate_version, created_at, updated_at) values (",
                    )
                    .bind(bundle.scope.id.as_uuid())
                    .append(", ")
                    .bind(bundle.scope.organization_id.as_uuid())
                    .append(", ")
                    .bind(bundle.scope.project_id.as_uuid())
                    .append(", ")
                    .bind(bundle.scope.environment_id.as_uuid())
                    .append(", ")
                    .bind(bundle.scope.node_id.as_uuid())
                    .append(", ")
                    .bind(bundle.scope.aggregate_version)
                    .append(", ")
                    .bind(bundle.scope.created_at)
                    .append(", ")
                    .bind(bundle.scope.updated_at)
                    .append(")"),
                )
                .await;
                match inserted {
...
fn validate_stored(scope: &GatewayScope) -> Result<(), RepositoryError> {
    if scope.aggregate_version == 0 || scope.updated_at < scope.created_at {
        return Err(RepositoryError::Storage(
            "stored Gateway scope is invalid".into(),
        ));
    }
    Ok(())
}
