...
#[derive(Clone)]
pub struct ProjectsEdgeEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsEdgeEnvironmentAccessAdapter {
...
#[async_trait]
impl IEdgeEnvironmentAccess for ProjectsEdgeEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: EdgeEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Edge environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsEdgeEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope = EdgeEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
