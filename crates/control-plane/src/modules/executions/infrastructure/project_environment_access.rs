...
#[derive(Clone)]
pub struct ProjectsExecutionsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsExecutionsEnvironmentAccessAdapter {
...
#[async_trait]
impl IExecutionsEnvironmentAccess for ProjectsExecutionsEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: ExecutionsEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Executions environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsExecutionsEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            ExecutionsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
