...
impl LocalKeyEncryptionService {
    pub fn load_or_create(key_path: impl Into<PathBuf>) -> Result<Self, KeyEncryptionError> {
        let key_path = key_path.into();
        let key = match fs::read(&key_path) {
            Ok(key) => key,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => create_key(&key_path)?,
            Err(error) => {
                return Err(KeyEncryptionError::Unavailable(format!(
        ...
        let cipher = Aes256Gcm::new_from_slice(&key).map_err(|error| {
            KeyEncryptionError::Rejected(format!("local encryption key is invalid: {error}"))
        })?;
        Ok(Self {
            key_path,
            key_id,
...
#[async_trait]
impl IKeyEncryptionService for LocalKeyEncryptionService {
    async fn encrypt(
        &self,
        plaintext: &[u8],
        context: &[u8],
        ...
        getrandom::fill(&mut nonce).map_err(|error| {
            KeyEncryptionError::Unavailable(format!("could not generate encryption nonce: {error}"))
        })?;
        let ciphertext = self
            .cipher
            .encrypt(
            ...
            .map_err(|_| KeyEncryptionError::Rejected("local encryption failed".into()))?;
        Ok(EncryptedValue {
    ...
    async fn decrypt(
        ...
        let nonce = parts
            .next()
            .ok_or_else(|| KeyEncryptionError::Rejected("encrypted value has no nonce".into()))
            .and_then(decode)?;
        let ciphertext = parts
            .next()
            .ok_or_else(|| KeyEncryptionError::Rejected("encrypted value has no payload".into()))
            .and_then(decode)?;
        ...
        self.cipher
            ...
            .map_err(|_| {
                KeyEncryptionError::Rejected("encrypted value authentication failed".into())
            })
    }
...
fn create_key(path: &Path) -> Result<Vec<u8>, KeyEncryptionError> {
    let parent = path.parent().ok_or_else(|| {
        KeyEncryptionError::InvalidInput("local encryption key path has no parent".into())
    })?;
    fs::create_dir_all(parent).map_err(|error| {
        KeyEncryptionError::Unavailable(format!(
    ...
    getrandom::fill(&mut key).map_err(|error| {
        KeyEncryptionError::Unavailable(format!("could not generate local encryption key: {error}"))
    })?;
    ...
    let mut file = options.open(path).map_err(|error| {
        KeyEncryptionError::Unavailable(format!("could not create local encryption key: {error}"))
    })?;
    file.write_all(&key).map_err(|error| {
        KeyEncryptionError::Unavailable(format!("could not write local encryption key: {error}"))
    })?;
    file.sync_all().map_err(|error| {
        KeyEncryptionError::Unavailable(format!("could not sync local encryption key: {error}"))
    })?;
    Ok(key)
}

fn decode(value: &str) -> Result<Vec<u8>, KeyEncryptionError> {
    URL_SAFE_NO_PAD
        .decode(value)
        .map_err(|_| KeyEncryptionError::Rejected("encrypted value contains invalid base64".into()))
}
