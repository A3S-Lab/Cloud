...
use crate::modules::identity::application::{IIdentityNodeAccess, IdentityNodeScope};
...
#[derive(Clone)]
pub struct FleetIdentityNodeAccessAdapter {
    nodes: Arc<dyn INodeRepository>,
}

impl FleetIdentityNodeAccessAdapter {
...
#[async_trait]
impl IIdentityNodeAccess for FleetIdentityNodeAccessAdapter {
    async fn node_exists(&self, scope: IdentityNodeScope) -> Result<bool, RepositoryError> {
            ...
            .await
        {
            ...
            Ok(_) => Err(RepositoryError::Storage(
                "Fleet returned inconsistent Identity node evidence".into(),
            )),
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_node_evidence() {
        ...
        let present = FleetIdentityNodeAccessAdapter::new(Arc::new(StubNodeRepository {
            node: Some(node),
            find_calls: AtomicUsize::new(0),
        }));
        let scope = IdentityNodeScope::new(organization_id, node_id).unwrap();
        assert!(present.node_exists(scope).await.unwrap());

        let missing = FleetIdentityNodeAccessAdapter::new(Arc::new(StubNodeRepository {
            node: None,
            find_calls: AtomicUsize::new(0),
        }));
