...
use crate::infrastructure::{store_audit, transaction_error, AuditWrite, PostgresPersistenceError};
...
use crate::modules::shared_kernel::domain::{
    AuthorizationDecisionRef, DecisionEvidenceRef, PrivilegedAuthorizationDecisionId,
    RepositoryError,
};
use async_trait::async_trait;
use chrono::Utc;

pub(super) struct IssuedPrivilegedAuthorization {
    pub reference: AuthorizationDecisionRef,
    pub authentication: DecisionEvidenceRef,
}

/// Issue and persist one privileged allow inside a transaction whose caller
/// already owns the Installation lock appropriate for the use case. Concrete
/// privileged mutations use this boundary so credential revocation cannot fit
/// between bearer verification, authorization, and the protected write.
pub(super) async fn issue_privileged_authorization(
    transaction: &a3s_orm::PostgresTransaction,
    request: PrivilegedAuthorizationDecisionRequest,
) -> Result<IssuedPrivilegedAuthorization, PostgresPersistenceError> {
    request
        .validate()
        .map_err(PostgresPersistenceError::Invariant)?;
    let installation_id = request.scope.installation_id();
    let decided_at = Utc::now();
    let principal = load_active_principal_for_authorization(transaction, request.principal_id)
        .await?
        .ok_or_else(|| {
            RepositoryError::Forbidden("privileged authorization principal is not active".into())
        })?;
    let credential = load_api_token_by_id_for_authorization(transaction, request.credential_id)
        .await?
        .filter(|credential| {
            credential.principal_id == request.principal_id && credential.is_active_at(decided_at)
        })
        .ok_or_else(|| {
            RepositoryError::Forbidden("privileged authorization credential is not active".into())
        })?;
    let policy = load_current_policy_for_authorization(transaction, installation_id)
        .await?
        .ok_or_else(|| {
            RepositoryError::Forbidden(
                "privileged authorization has no current platform policy".into(),
            )
        })?;
    ...
    let support_grant = match request.support_grant_id {
        Some(grant_id) => Some(
            load_grant_for_authorization(transaction, installation_id, grant_id)
                ...
                .ok_or_else(|| {
                    RepositoryError::Forbidden(
                        "privileged authorization support grant is unavailable".into(),
                    )
                })?,
        ),
        None => None,
    ...
    let reference = decision
        .reference()
        .map_err(PostgresPersistenceError::Invariant)?;
    let authentication = decision.authentication.clone();
    ...
    .await?;
    Ok(IssuedPrivilegedAuthorization {
        reference,
        authentication,
    })
}
        ...
        self.executor
            .transaction(move |transaction| {
                Box::pin(async move {
                    ...
                    Ok(issue_privileged_authorization(transaction, request)
                        .await?
                        .reference)
                })
            })
