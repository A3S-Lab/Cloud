use crate::modules::identity::application::{IIdentityProjectAccess, IdentityProjectScope};
...
#[derive(Clone)]
pub struct ProjectsIdentityProjectAccessAdapter {
    projects: Arc<dyn IProjectRepository>,
}

impl ProjectsIdentityProjectAccessAdapter {
...
#[async_trait]
impl IIdentityProjectAccess for ProjectsIdentityProjectAccessAdapter {
    async fn project_exists(&self, scope: IdentityProjectScope) -> Result<bool, RepositoryError> {
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Identity project evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_project_evidence() {
        ...
        let present = ProjectsIdentityProjectAccessAdapter::new(Arc::new(StubProjectRepository {
            project: Some(project),
            find_calls: AtomicUsize::new(0),
        }));
        let scope = IdentityProjectScope::new(organization_id, project_id).unwrap();
        assert!(present.project_exists(scope).await.unwrap());

        let missing = ProjectsIdentityProjectAccessAdapter::new(Arc::new(StubProjectRepository {
            project: None,
            find_calls: AtomicUsize::new(0),
        }));
