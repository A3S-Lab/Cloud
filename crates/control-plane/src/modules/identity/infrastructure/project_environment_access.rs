...
#[derive(Clone)]
pub struct ProjectsIdentityEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsIdentityEnvironmentAccessAdapter {
...
#[async_trait]
impl IIdentityEnvironmentAccess for ProjectsIdentityEnvironmentAccessAdapter {
    async fn environment_exists(
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Identity environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsIdentityEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            IdentityEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
