...
#[derive(Clone)]
pub struct ProjectsInferenceEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsInferenceEnvironmentAccessAdapter {
...
#[async_trait]
impl IInferenceEnvironmentAccess for ProjectsInferenceEnvironmentAccessAdapter {
    async fn environment_exists(
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Inference environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsInferenceEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            InferenceEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
