...
use crate::modules::notifications::application::{
    INotificationsNodeAccess, NotificationsNodeScope,
};
...
#[derive(Clone)]
pub struct FleetNotificationsNodeAccessAdapter {
    nodes: Arc<dyn INodeRepository>,
}

impl FleetNotificationsNodeAccessAdapter {
...
#[async_trait]
impl INotificationsNodeAccess for FleetNotificationsNodeAccessAdapter {
    async fn node_exists(&self, scope: NotificationsNodeScope) -> Result<bool, RepositoryError> {
            ...
            .await
        {
            ...
            Ok(_) => Err(RepositoryError::Storage(
                "Fleet returned inconsistent Notifications node evidence".into(),
            )),
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_node_evidence() {
        ...
        let present = FleetNotificationsNodeAccessAdapter::new(Arc::new(StubNodeRepository {
            node: Some(node),
            find_calls: AtomicUsize::new(0),
        }));
        let scope = NotificationsNodeScope::new(organization_id, node_id).unwrap();
        assert!(present.node_exists(scope).await.unwrap());

        let missing = FleetNotificationsNodeAccessAdapter::new(Arc::new(StubNodeRepository {
            node: None,
            find_calls: AtomicUsize::new(0),
        }));
