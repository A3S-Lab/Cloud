use crate::modules::notifications::application::{
    INotificationsEnvironmentAccess, NotificationsEnvironmentScope,
};
...
#[derive(Clone)]
pub struct ProjectsNotificationsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsNotificationsEnvironmentAccessAdapter {
...
#[async_trait]
impl INotificationsEnvironmentAccess for ProjectsNotificationsEnvironmentAccessAdapter {
    async fn environment_exists(
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Notifications environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present = ProjectsNotificationsEnvironmentAccessAdapter::new(Arc::new(
            StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            },
        ));
        let scope = NotificationsEnvironmentScope::new(organization_id, project_id, environment_id)
            .unwrap();
        assert!(present.environment_exists(scope).await.unwrap());

        let missing = ProjectsNotificationsEnvironmentAccessAdapter::new(Arc::new(
            StubEnvironmentRepository {
                environment: None,
                find_calls: AtomicUsize::new(0),
            },
