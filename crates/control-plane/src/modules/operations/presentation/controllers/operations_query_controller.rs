...
const OPERATION_STREAM_POLL_INTERVAL: Duration = Duration::from_secs(2);
...
fn operation_snapshot_stream(
    ...
    let options = PollingSseOptions::new(
        "operation snapshot stream",
        OPERATION_STREAM_POLL_INTERVAL,
        1,
    )?;
        ...
        move |mut state| {
            ...
            async move {
                let operations = bus
                    ...
                    .await?
                    .map_err(|_| BootError::Internal("operation snapshot query failed".into()))?
                    .into_iter()
                    .map(OperationListItemResponse::from)
                    .collect::<Vec<_>>();
                ...
                let event = if event_id == state.last_event_id {
                    None
                } else {
