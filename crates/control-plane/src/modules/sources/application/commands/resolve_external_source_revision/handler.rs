            ...
            let resolved = match resolver.resolve(&resolution_request, None).await {
                ...
                Err(SourceResolutionError::Unavailable) => {
                        ...
                        .await
                    {
                        ...
                        Err(error) => return Ok(Err(credential_error(error))),
                    };
                    match resolver
...
fn credential_error(error: SourceRepositoryCredentialError) -> ApplicationError {
    match error {
        SourceRepositoryCredentialError::Invalid(message) => ApplicationError::Invalid(message),
        SourceRepositoryCredentialError::Unavailable => private_source_unavailable(),
        SourceRepositoryCredentialError::Integrity(message)
        | SourceRepositoryCredentialError::Storage(message) => ApplicationError::Internal(message),
    }
}
