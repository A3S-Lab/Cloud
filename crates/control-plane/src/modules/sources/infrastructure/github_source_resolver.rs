...
impl GithubSourceResolver {
    ...
    async fn confirm_repository(
        ...
        let response: GithubRepositoryResponse = self
            .get_json(self.api_url(&["repos", owner, name])?)
            .await?;
    ...
    async fn resolve_reference(
        ...
        match reference {
            ...
            GitReference::Branch(branch) => {
                let response = self.git_reference(owner, name, "heads", branch).await?;
            ...
            GitReference::Tag(tag) => {
                let response = self.git_reference(owner, name, "tags", tag).await?;
                self.peel_tag(owner, name, response.object).await
            }
        }
    }
    ...
    async fn git_reference(
        ...
        let response: GithubReferenceResponse = self.get_json(self.api_url(&segments)?).await?;
    ...
    async fn get_json<T: DeserializeOwned>(&self, url: Url) -> Result<T, SourceResolutionError> {
        let mut response = self.client.get(url).send().await.map_err(|_| {
...
impl ISourceResolver for GithubSourceResolver {
    async fn resolve(
        ...
        self.confirm_repository(&request.repository).await?;
        let commit_sha = self
            .resolve_reference(&request.repository, &request.reference)
            .await?;
        Ok(ResolvedSource {
