...
pub const WORKFLOW_PLAN_SCHEMA_V12: &str = "cloud.workflow.plan.v12";
pub const WORKFLOW_PLAN_COMPILER_REVISION_V12: &str = "cloud.workflow.plan-compiler.v12";
...
impl WorkflowPlan {
    pub fn validate(&self) -> Result<(), String> {
        let version = match (self.schema.as_str(), self.compiler_revision.as_str()) {
            ...
            (WORKFLOW_PLAN_SCHEMA_V12, WORKFLOW_PLAN_COMPILER_REVISION_V12) => {
                WorkflowPlanVersion::V12
            }
        ...
        let has_branch_failure_routes = has_branch_failure_route(&workflow, &failures);
        ...
        let has_agent_failure_routes = has_agent_failure_route(&workflow);
        ...
        match version {
            ...
            WorkflowPlanVersion::V10
                if has_composite_failure_routes || has_agent_failure_routes =>
            {
                return Err(
                    "Workflow Plan v10 cannot contain a composite or Agent failure route".into(),
                )
            }
            ...
            WorkflowPlanVersion::V11 if has_agent_failure_routes => {
                return Err("Workflow Plan v11 cannot contain an Agent failure route".into())
            }
            WorkflowPlanVersion::V12 if !has_agent_failure_routes => {
                return Err(
                    "Workflow Plan v12 requires at least one descriptor-bound Agent failure route"
                        .into(),
                )
            }
            _ => {}
        }
        if version == WorkflowPlanVersion::V12 && self.environment_id.is_none() {
            return Err(
                "Workflow Plan v12 Agent failure routes require one exact environment".into(),
            );
        }
