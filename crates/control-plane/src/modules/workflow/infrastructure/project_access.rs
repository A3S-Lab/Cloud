...
#[derive(Clone)]
pub struct ProjectsWorkflowProjectAccessAdapter {
    projects: Arc<dyn IProjectRepository>,
}

impl ProjectsWorkflowProjectAccessAdapter {
...
#[async_trait]
impl IWorkflowProjectAccess for ProjectsWorkflowProjectAccessAdapter {
    async fn project_exists(&self, scope: WorkflowProjectScope) -> Result<bool, RepositoryError> {
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Workflow project evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_project_evidence() {
        ...
        let present = ProjectsWorkflowProjectAccessAdapter::new(Arc::new(StubProjectRepository {
            project: Some(project),
            find_calls: AtomicUsize::new(0),
        }));
        let scope = WorkflowProjectScope::new(organization_id, project_id).unwrap();
        assert!(present.project_exists(scope).await.unwrap());

        let missing = ProjectsWorkflowProjectAccessAdapter::new(Arc::new(StubProjectRepository {
            project: None,
            find_calls: AtomicUsize::new(0),
        }));
