...
use crate::modules::workflow::domain::{
    ...
    WorkflowAgentHookMetadata, WorkflowAgentOutcome, WorkflowAgentResumePayload,
    WorkflowAgentResumeResolution, WorkflowApplicationAnswerFailureResumePayload,
...
pub(super) fn run_workflow(invocation: WorkflowInvocation) -> Result<RuntimeCommand, FlowError> {
    ...
    if context.cancellation_request().is_some() {
        return super::cancellation::resolve_cancellation(&invocation.run_id, &input, &context);
    }
    if context
        .history()
        .last()
    ...
    for plan_step in &input.plan.steps {
        ...
        if step.plan.kind == WorkflowStepKind::Subworkflow {
            ...
            match resolution {
                super::composite::CompositeStepResolution::AwaitFrame(metadata) => {
                    return Ok(context.create_hook(
                ...
                super::composite::CompositeStepResolution::AwaitWave(metadata) => {
                    return Ok(context.create_hook(
                        metadata.flow_hook_id(),
                        metadata.flow_hook_token(),
                        serde_json::to_value(metadata)?,
        ...
        if let Some(error) = context.step_failed(&durable_step_id) {
            ...
            {
                if let Some(result) =
                    local_output_failure_route_result(&invocation.run_id, &input, step)?
                {
                    resolved.insert(step.plan.id.clone(), ResolvedState::Active(result));
                    continue;
                }
        ...
        let routed_local_failure = (matches!(
            input.schema.as_str(),
            WORKFLOW_RUN_INPUT_SCHEMA_V16
                | WORKFLOW_RUN_INPUT_SCHEMA_V17
                | WORKFLOW_RUN_INPUT_SCHEMA_V18
...
pub(super) fn agent_result(
    ...
    let (output, output_digest) = match payload.resolution {
        WorkflowAgentResumeResolution::Rejected { reason } => {
            return agent_failure_resolution(
                run_id,
                input,
                step,
                reason,
        ...
        WorkflowAgentResumeResolution::Completed {
            ...
            let failure = match &output.outcome {
                WorkflowAgentOutcome::Succeeded => None,
                WorkflowAgentOutcome::Failed { reason } => Some((
                    reason.clone(),
                    WorkflowStepFailureClassification::AgentExecutionFailed,
                )),
                WorkflowAgentOutcome::Cancelled => Some((
                    "child Agent execution was cancelled".into(),
                    WorkflowStepFailureClassification::AgentExecutionCancelled,
                )),
            };
            if let Some((error, classification)) = failure {
                return agent_failure_resolution(run_id, input, step, error, classification);
            }
...
fn agent_failure_resolution(
    ...
    if failure_route_handle(input, step)?.is_none() {
        return Ok(AgentResolution::Failed {
            error,
            routed: None,
        });
    }
    let failure = WorkflowStepFailureOutput::agent(step, classification)
        .map_err(|_| agent_payload_drift(run_id, &step.plan.id))?;
    let public_error = failure.message.clone();
    let routed = failure_route_result(run_id, input, step, failure)?;
    Ok(AgentResolution::Failed {
        error: public_error,
        routed,
    })
}
...
pub(super) fn local_output_failure_route_result(
    run_id: &str,
    input: &WorkflowRunInput,
    step: &ResolvedWorkflowRunStep,
) -> Result<Option<Box<WorkflowLocalStepResult>>, FlowError> {
    if !matches!(
        ...
        || step
            ...
            .descriptor
            .as_ref()
            .is_none_or(|descriptor| descriptor.descriptor_id != "workflow.output")
        || failure_route_handle(input, step)?.is_none()
    {
        return Ok(None);
    }
    let failure = WorkflowStepFailureOutput::local_output(step)
        .map_err(|_| failure_payload_drift(run_id, &step.plan.id))?;
    failure_route_result(run_id, input, step, failure)
}

pub(super) fn local_branch_failure_route_result(
    run_id: &str,
    input: &WorkflowRunInput,
    step: &ResolvedWorkflowRunStep,
) -> Result<Option<Box<WorkflowLocalStepResult>>, FlowError> {
...
pub(super) fn failure_route_handle(
    ...
    let Some(failure) = step.plan.failure.as_ref() else {
        return Ok(None);
    };
    if failure.error_output.is_none() {
        return Ok(None);
    }
    let expected = descriptor_failure_output(failure).map_err(FlowError::InvalidWorkflow)?;
    let handles = input
        .plan
        .edges
        .iter()
        .filter(|edge| {
            edge.source == step.plan.id
                && edge.source_handle.as_deref() == Some(expected.name.as_str())
        })
    ...
    match handles.as_slice() {
        ...
        _ => Err(FlowError::InvalidWorkflow(format!(
            "Workflow step {:?} has more than one descriptor-bound failure route",
            step.plan.id
        ))),
    }
}
