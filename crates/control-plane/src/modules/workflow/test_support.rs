...
use crate::modules::workflow::domain::{
    ...
    WORKFLOW_PLAN_SCHEMA_V8, WORKFLOW_PLAN_SCHEMA_V9, WORKFLOW_RUN_FLOW_NAME,
    WORKFLOW_RUN_FLOW_VERSION, WORKFLOW_RUN_FLOW_VERSION_V10, WORKFLOW_RUN_FLOW_VERSION_V11,
    WORKFLOW_RUN_FLOW_VERSION_V12, WORKFLOW_RUN_FLOW_VERSION_V14, WORKFLOW_RUN_FLOW_VERSION_V15,
    WORKFLOW_RUN_FLOW_VERSION_V16, WORKFLOW_RUN_FLOW_VERSION_V17, WORKFLOW_RUN_FLOW_VERSION_V18,
    WORKFLOW_RUN_FLOW_VERSION_V19, WORKFLOW_RUN_FLOW_VERSION_V2, WORKFLOW_RUN_FLOW_VERSION_V22,
    WORKFLOW_RUN_FLOW_VERSION_V23, WORKFLOW_RUN_FLOW_VERSION_V24, WORKFLOW_RUN_FLOW_VERSION_V25,
...
pub(crate) fn routed_agent_workflow_run_input() -> Result<WorkflowRunInput, String> {
    let mut input = agent_workflow_run_input()?;
    let output_step = input
        ...
        .cloned()
        .ok_or_else(|| "WorkflowRun Agent test plan lost output".to_owned())?;
    let mut failure_output_step = output_step;
    failure_output_step.id = "failure_output".into();
    failure_output_step
        .descriptor
        .as_mut()
        .ok_or_else(|| "WorkflowRun Agent failure output lost its descriptor binding".to_owned())?
        .step_id = "failure_output".into();
    input.plan.steps.insert(2, failure_output_step);
    for step in &mut input.plan.steps {
        step.failure = Some(if step.id == TEST_AGENT_STEP_ID {
    ...
    input.plan.edges = vec![
        edge("input-agent", "input", TEST_AGENT_STEP_ID, None),
        edge(
            "agent-failure",
            TEST_AGENT_STEP_ID,
            "failure_output",
        ...
        edge("agent-output", TEST_AGENT_STEP_ID, "output", None),
    ];
    input.plan.schema = WORKFLOW_PLAN_SCHEMA_V12.into();
    input.plan.compiler_revision = WORKFLOW_PLAN_COMPILER_REVISION_V12.into();
    input.plan.validate()?;
    input.plan_digest = Sha256Digest::parse(sha256_digest(&canonical_json_bounded(
        &input.plan,
        WORKFLOW_PLAN_MAX_BYTES,
        "WorkflowRun routed Agent test plan",
    )?))?;
    input.schema = WORKFLOW_RUN_INPUT_SCHEMA_V25.into();
    input.runtime_contract_revision = WORKFLOW_RUN_RUNTIME_CONTRACT_REVISION_V25.into();
    input.flow_workflow_version = WORKFLOW_RUN_FLOW_VERSION_V25.into();
    input.validate()?;
    Ok(input)
}
