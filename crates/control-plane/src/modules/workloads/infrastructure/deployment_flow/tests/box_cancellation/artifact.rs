use super::{invalid, BoxTestResult};
...
impl NodeArtifactTransport for RejectingArtifactTransport {
    async fn download(
        ...
        Err(NodeControlClientError::Invalid(
            "Artifact-free Box cancellation fixture attempted a download".into(),
        ))
    }
    ...
    async fn upload(
        ...
        Err(NodeControlClientError::Invalid(
            "Artifact-free Box cancellation fixture attempted an upload".into(),
        ))
    }
}

pub(super) fn manager(state_root: &Path, node_id: Uuid) -> BoxTestResult<Arc<NodeArtifactManager>> {
    Ok(Arc::new(
