...
use crate::modules::workloads::application::{
    IWorkloadsEnvironmentAccess, WorkloadsEnvironmentScope,
};
...
#[derive(Clone)]
pub struct ProjectsWorkloadsEnvironmentAccessAdapter {
    environments: Arc<dyn IEnvironmentRepository>,
}

impl ProjectsWorkloadsEnvironmentAccessAdapter {
...
#[async_trait]
impl IWorkloadsEnvironmentAccess for ProjectsWorkloadsEnvironmentAccessAdapter {
    async fn environment_exists(
        &self,
        scope: WorkloadsEnvironmentScope,
            ...
            .await?
        {
            ...
            Some(_) => Err(RepositoryError::Storage(
                "Projects returned inconsistent Workloads environment evidence".into(),
            )),
            None => Ok(false),
        }
    }
...
mod tests {
    ...
    async fn adapter_projects_only_exact_existing_environment_evidence() {
        ...
        let present =
            ProjectsWorkloadsEnvironmentAccessAdapter::new(Arc::new(StubEnvironmentRepository {
                environment: Some(environment),
                find_calls: AtomicUsize::new(0),
            }));
        let scope =
            WorkloadsEnvironmentScope::new(organization_id, project_id, environment_id).unwrap();
