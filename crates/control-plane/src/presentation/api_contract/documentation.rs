...
use super::knowledge_documentation::{
    component_description as knowledge_component_description,
    operation_description as knowledge_operation_description,
    operation_summary as knowledge_operation_summary,
    response_data_description as knowledge_response_data_description,
};
...
fn component_schema_description(name: &str) -> String {
    ...
    if let Some(description) = knowledge_component_description(name) {
        return description.into();
    }
...
fn special_description(method: &str, path: &str) -> Option<&'static str> {
    ...
    if let Some(description) = knowledge_operation_description(method, path) {
        return Some(description);
    }
...
fn operation_summary(method: &str, path: &str) -> String {
    ...
    if let Some(summary) = knowledge_operation_summary(method, path) {
        return summary.into();
    }
...
fn mutation_action_summary(path: &str) -> Option<&'static str> {
    for (suffix, summary) in [
        ...
        (
            ...
            "Observe an application asynchronous invocation",
        ),
        (
...
fn resource_label(segment: &str) -> Option<ResourceLabel> {
    let (singular, plural) = match segment {
        ...
        "plugin-assignments" => ("plugin assignment", "plugin assignments"),
        "plugin-plan-projections" => ("plugin plan projection", "plugin plan projections"),
        "confirmation" => ("plugin plan confirmation", "plugin plan confirmations"),
...
fn response_data_description(method: &str, path: &str, summary: &str) -> String {
    ...
    if let Some(description) = knowledge_response_data_description(method, path) {
        return description.into();
    }
