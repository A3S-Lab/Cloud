...
use crate::modules::audit::{
    AuditAttributionStatus, AuditRecordFilter, ExportAuditManifest, ExportAuditRecords,
    GetAuditRetentionStatus, ListAuditRecords, DEFAULT_AUDIT_EXPORT_MANIFEST_PAGE_SIZE,
};
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct AuditRecordManifestExportArguments {
    actor_principal_id: Option<Uuid>,
    action: Option<String>,
    aggregate_id: Option<Uuid>,
    request_id: Option<Uuid>,
...
pub async fn export_audit_manifest(
    bus: Arc<QueryBus>,
    organization_id: OrganizationId,
    arguments: AuditRecordManifestExportArguments,
    request_id: Uuid,
) -> Result<Value> {
    match bus
        ...
        .await?
    {
        Ok(bundle) => tool_result::success(
            200,
            AuditExportManifestBundleResponse::from(bundle),
            request_id,
        ),
        Err(error) => tool_result::application_error(error, request_id),
    }
