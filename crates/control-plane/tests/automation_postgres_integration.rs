...
//!
//! Kept as a separate integration binary so webhook/schedule/invocation/definition
//! persistence certification does not depend on unrelated postgres_integration
//! support modules that currently drift from production types.
...
use a3s_cloud_control_plane::modules::automations::{
    ...
    ChangeAutomationWebhookEndpoint, CommitAutomationScheduleCursor,
    CreateAutomationDefinition, CreateAutomationWebhookEndpoint, EndpointLifecycleAction,
    IAutomationDefinitionRepository, IAutomationInvocationReader,
    ...
    IAutomationWebhookSignatureVerifier, PostgresAutomationDefinitionRepository,
