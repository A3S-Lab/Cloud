//! GA-1 Gateway LIVE Code-path certification gate.
//!
//! Fail-closed: pin-matched `a3s-gateway` must carry public traffic to the
//! published Agent Service. Management-plane conversations alone are not enough.
//! BX0 Python TLS (:18444) is not Gateway LIVE.
//!
//! Module graph mirrors `a0_4_real_box_release.rs` so the A0.4 exercise can be
//! reused with a live Gateway probe hook.
...
#[ignore = "requires pin-matched a3s-gateway, real Box provider, and PostgreSQL certification environment"]
async fn ga1_gateway_live_code_path_agent_release_through_real_gateway() {
    let admin_url = std::env::var("A3S_CLOUD_TEST_POSTGRES_URL")
        .expect("GA-1 Gateway LIVE requires A3S_CLOUD_TEST_POSTGRES_URL");
    let gateway_bin = std::env::var("A3S_CLOUD_GATEWAY_BIN")
        .or_else(|_| std::env::var("A3S_CLOUD_TEST_GATEWAY_BIN"))
        .expect("GA-1 Gateway LIVE requires A3S_CLOUD_GATEWAY_BIN");
    assert!(
        !gateway_bin.contains("PLACEHOLDER"),
        "refuse PLACEHOLDER gateway binary"
    );
    assert!(
        std::path::Path::new(&gateway_bin).is_file(),
        "gateway binary missing: {gateway_bin}"
    );

    run_isolated_postgres(&admin_url, |postgres_url| {
        let gateway_bin = gateway_bin.clone();
        async move {
            ga1_gateway_live_code_path_support::exercise_ga1_gateway_live_code_path(
                postgres_url,
                &gateway_bin,
            )
            .await
    ...
    .await
    .expect("GA-1 Gateway LIVE Code-path certification gate");
}
