//! GA-1 management-plane Postgres gate (no Box / Gateway / `/dev/kvm`).
//!
//! Separate binary so it does not depend on the currently-broken
//! `postgres_integration` support surface. Proves Agents-owned
//! CreateAgentConversation → StartAgentExecution → GetAgentExecutionEvents and
//! emits `A3S_CLOUD_GA1_MANAGEMENT_PLANE_PROVEN`. Does not invent LIVE CERTIFIED.
...
async fn postgres_ga1_management_plane_conversation_execution_and_events_are_durable() {
    let Some(admin_url) = std::env::var("A3S_CLOUD_TEST_POSTGRES_URL").ok() else {
        return;
    };
    run_isolated_postgres(
        &admin_url,
        ga1_management_plane_support::exercise_ga1_management_plane_postgres,
    )
    .await
    .expect("GA-1 management-plane Postgres gate");
}
    ...
    Fut: std::future::Future<Output = Result<(), Box<dyn std::error::Error>>>,
{
    ...
    let result = AssertUnwindSafe(exercise(isolated.url().to_owned()))
        .catch_unwind()
        .await;
