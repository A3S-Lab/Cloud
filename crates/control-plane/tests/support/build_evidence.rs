...
pub(super) fn evidence_for(
    ...
    let artifact = build
        .published_artifact
        .clone()
        .ok_or("published build evidence fixture omitted its artifact")?;
    let output = build
        .output
        .as_ref()
        .ok_or("published build evidence fixture omitted its validated output")?;
    ...
    let sbom = SpdxDocument {
        ...
        name: format!("A3S Cloud PostgreSQL test build {}", build.id),
        document_namespace: format!(
            "https://a3s.dev/spdx/postgres-test-builds/{}/{}",
            build.id, build.attempt
        ),
        creation_info: SpdxCreationInfo {
            created: attested_at,
            creators: vec!["Tool: A3S Cloud PostgreSQL integration tests".into()],
        },
    ...
    let signing_key_pair = Ed25519KeyPair::from_seed_unchecked(&[7_u8; 32])
        .map_err(|_| "test Ed25519 signing seed was rejected")?;
...
fn digest_hex(value: &str) -> Result<&str, Box<dyn std::error::Error>> {
    value
        .strip_prefix("sha256:")
        .ok_or_else(|| format!("expected SHA-256 digest, got {value:?}").into())
}
