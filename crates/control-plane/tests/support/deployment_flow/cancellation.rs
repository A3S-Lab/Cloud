...
pub async fn exercise_dispatched_cancellation(
    ...
    .with_resource_inventory(Arc::new(FixedResourceInventoryAuthority { inventory }));
    ...
    let preparation_acknowledgement = command_executor.execute(preparation.clone()).await?;
    ...
    let cleanup_sequence = cleanup_command.sequence;
    ...
    let release_lease = node_repository
        .lease_commands(
            &NodeCommandLeaseRequest {
                ...
                node_id: node_id.as_uuid(),
                agent_instance_id,
                after_sequence: cleanup_sequence,
                max_commands: 10,
                wait_ms: 0,
            },
            Uuid::now_v7(),
    ...
    let release_command = release_lease
        ...
        .ok_or("deployment resource release command was not leased")?;
    let release_acknowledgement = command_executor.execute(release_command).await?;
    persist_command_result(
        &node_repository,
        node_id,
        agent_instance_id,
