...
pub(super) async fn exercise_notification_persistence(
    ...
    alert_policies::exercise_notification_alert_policy_persistence(
        executor.clone(),
        &database,
        organization_id,
        project_id,
    ...
    if let Ok(nats_url) = std::env::var("A3S_CLOUD_TEST_NATS_URL") {
        nats::exercise_notification_nats_delivery(
            nats_url,
            executor,
            &database,
            &repository,
...
#[path = "notifications/alert_policies.rs"]
mod alert_policies;

#[path = "notifications/nats.rs"]
mod nats;
