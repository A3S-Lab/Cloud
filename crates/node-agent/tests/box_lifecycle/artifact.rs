...
impl NodeArtifactTransport for RejectingArtifactTransport {
    async fn download(
        ...
        Err(NodeControlClientError::Invalid(
            "Artifact-free Box lifecycle fixture attempted a download".into(),
        ))
    }
    ...
    async fn upload(
        ...
        Err(NodeControlClientError::Invalid(
            "Artifact-free Box lifecycle fixture attempted an upload".into(),
        ))
    }
}

pub(super) fn manager(state_root: &Path, node_id: Uuid) -> TestResult<Arc<NodeArtifactManager>> {
    Ok(Arc::new(
