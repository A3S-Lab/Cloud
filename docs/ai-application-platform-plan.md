...
| `AUT0.2-C3` | Implemented component foundation; live PostgreSQL evidence pending: migration `183` and the A3S ORM `PostgresAutomationWebhookRepository` restore the exact endpoint/revision projection, immutable first-delivery capture, and append-only receipts. Lifecycle generation fences and immutable identity/revision checks are enforced in SQL; admission revalidates canonical ACL/digests and commits shared Audit/Outbox side effects atomically. The isolated PostgreSQL integration gate covers scoped lookup, first admission, exact replay, reconnect recovery, wrong-environment isolation, and concurrent lifecycle generation fencing when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. The slice stores no Secret plaintext, retry/scheduler state, or transport data and does not register a public route or worker; a live PostgreSQL result remains required | `AUT0.2-C2`, PostgreSQL/A3S ORM, shared Audit/Outbox |
...
| `AUT0.3` | In progress; component-only calendar, misfire, concurrency, due-window, lease, durable cursor/lease, exact invocation-envelope, bounded schedule-dispatch, injectable schedule-worker, digest-verified invocation-handoff, P0 Task-target adaptation, and idempotent invocation-admission boundaries exist, while catch-up rules, production candidate discovery/owner composition, worker process registration, and the production P0 scheduled Task handoff remain open | `AUT0.1`, Boot compatibility lock, P0 profile contract |
