...
| `AUT0.2-C2` | Implemented component foundation: `control-plane/modules/automations` adds one application admission service and endpoint query boundary over an atomic in-memory repository. Exact revision/endpoint binding, scoped opaque-key uniqueness and complete organization/project/environment key lookup, nil-scope rejection, restored endpoint invariant checks, generation CAS lifecycle changes, explicit signature/schema ports, delivery body-digest replay and conflict receipts, lifecycle rejection receipts, redacted audit facts, and admitted Outbox messages are covered by focused tests. C3 supplies the durable adapter; no listener, Gateway route, worker, or public availability is added | `AUT0.2-C1` |
