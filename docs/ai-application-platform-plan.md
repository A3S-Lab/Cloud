...
| `AUT0.5-C5` | Persist one immutable bounded terminal evidence fact for an exact profile/revision/attempt with request/accepted-response digests, byte counts, closed outcome, optional status and bounded Retry-After, canonical times, exact revision FK, natural attempt replay/conflict, Resource Grant-aware bounded keyset reads, redaction, and no body/credential/provider-text/retry/queue/idempotency duplication or product surface. Do not compose network execution with evidence storage before recovery is designed. | `AUT0.5-C4` |
| `AUT0.5-C6` | Add one durable attempt reservation/fencing and indeterminate-outcome recovery boundary, then compose internal authorization, exact revision loading, just-in-time Secret recheck/materialization, egress authorization, one external attempt, and terminal evidence. Evidence failure or process death cannot authorize blind provider retry; Flow or the owning A3S Event consumer retains retry/backoff/cancellation/acknowledgement. | `AUT0.5-C5`, Operations/Flow recovery contracts |
| `AUT0.5` | Complete supported management surfaces, provider wiring, and Workflow HTTP/service ports over the existing profile/revision, materializer, public-Internet egress, executor, evidence, and recovery authorities; Flow remains the retry/backoff scheduler | `AUT0.5-C6`; `W0.4` for node availability |
