...
| `APP0.2-C42` | Implemented; component-only | Freeze Applications-owned `ApplicationStreamingObservation` over one exact Streaming-mode invocation and its session Input/Answer/FinalOutput frames with cursor paging: Open while non-terminal without FinalOutput; Succeeded only with Succeeded invocation plus Input and FinalOutput; Failed/Cancelled only for those terminals without FinalOutput; ordered page frames by sequence after `after_sequence`; `next_sequence` and `has_more` for shared-cursor advancement. Blocking/asynchronous modes, foreign messages, sequence drift, cursor beyond session head, Answer after FinalOutput, and Succeeded without FinalOutput fail closed. No migration, repository, CQRS poll, REST/OpenAPI/client/CLI/MCP, Gateway, SSE, or public availability is added. ADR `0114`. Focused `cargo test -p a3s-cloud-control-plane --lib application_streaming_observation` | `APP0.2-C6`, `APP0.2-C39` |
