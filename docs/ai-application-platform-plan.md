...
| `K0.1-C2` | Implemented management/persistence slice: migration `170` and one Files repository atomically persist UserFile metadata, organization quota reservation/release, optimistic transitions, shared idempotency, audit, Outbox, and checked cleanup intent. Authorization precedes replay. REST/OpenAPI `1.77.0`, the maintained client, CLI, and five Management MCP tools expose the same CQRS without a public byte route, second upload/object authority, provider/scanner state, or cleanup queue. Retained PostgreSQL cross-surface evidence and live upload/scan/cleanup execution remain open. | `K0.1-C1`, `C0.1` |
