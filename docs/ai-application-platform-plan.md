...
| `AUT0.5` | Add reusable outbound HTTP/business connection profiles, Secret references, egress/SSRF policy, typed request/response limits, error classification, evidence, and the Workflow HTTP/service ports; Flow remains the retry/backoff scheduler | `F0`, Secrets, `C0.3`; `W0.4` for node availability |
