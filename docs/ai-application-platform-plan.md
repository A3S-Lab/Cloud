...
| `AUT0.5` | Complete execution evidence, internal execution authorization, revocation/recovery, supported management surfaces, provider wiring, and Workflow HTTP/service ports over the existing profile/revision authority, materializer, public-Internet egress authorizer, and execution port; Flow remains the retry/backoff scheduler | `AUT0.5-C4`; `W0.4` for node availability |
