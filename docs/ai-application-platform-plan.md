...
| `AUT0.3` | In progress; component-only calendar, misfire, concurrency, due-window, lease, durable cursor/lease, exact invocation-envelope, bounded schedule-dispatch, injectable schedule-worker, digest-verified invocation-handoff, P0 Task-target adaptation, idempotent invocation-admission boundaries, and the durable immutable definition/revision catalog exist, while catch-up rules, production candidate discovery/owner composition, worker process registration, and the production P0 scheduled Task handoff remain open | `AUT0.1`, Boot compatibility lock, P0 profile contract |
