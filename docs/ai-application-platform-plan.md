...
| `AUT0.2-C3` | Implemented component foundation; live PostgreSQL evidence pending: migration `183` and the A3S ORM `PostgresAutomationWebhookRepository` restore the exact endpoint/revision projection, immutable first-delivery capture, and append-only receipts. Migration `188`, `PostgresAutomationDefinitionRepository`, and the `AutomationDefinitionCatalogService` now persist one immutable definition identity, exact canonical revision ACL, digest-linked parent chain, row-locked current-head CAS, and deterministic bounded head discovery; the definition semantic value is restored from the revision rather than maintained as a second mutable policy copy. The API/worker PostgreSQL adapter family now exposes definition, webhook, invocation/read, and schedule-state ports from one role-selected factory, without enabling a route or worker. Lifecycle generation fences and immutable identity/revision checks are enforced in SQL; admission revalidates canonical ACL/digests and commits shared Audit/Outbox side effects atomically. The isolated PostgreSQL integration gate covers scoped lookup, first admission, exact replay, reconnect recovery, wrong-environment isolation, concurrent lifecycle generation fencing, and concurrent definition-head append fencing when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. The slice stores no Secret plaintext, retry/scheduler state, or transport data and does not register a public route or worker; a live PostgreSQL result remains required | `AUT0.2-C2`, PostgreSQL/A3S ORM, shared Audit/Outbox |
...
| `AUT0.3-C6` | Implemented component foundation; live PostgreSQL evidence pending: migration `184` plus in-memory/PostgreSQL schedule-state repositories persist one exact revision digest, exclusive cursor, monotonic lease generation, and owner/token/timestamp fence. Row-locked acquisition and commit reject active, stale, expired, non-rotated, and out-of-order state; migration `186` allows an exact owner to release a lease without advancing the cursor. The isolated PostgreSQL gate covers concurrent lease fencing, cursor commit, reconnect recovery, generation rotation, cursor-preserving release, and wrong-scope isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. No definition copy, invocation, queue, retry, worker, scheduler, or P0 authority is added | `AUT0.3-C5`, PostgreSQL/A3S ORM |
...
| `AUT0.3-C10` | Implemented application and PostgreSQL foundation; live evidence pending: `AutomationInvocationAdmissionService` validates one exact envelope; the in-memory adapter and migration `187`/`PostgresAutomationInvocationRepository` fence invocation identity and policy-derived deduplication keys, replay exact evidence as `AlreadyAdmitted`, and atomically retain immutable envelope, redacted Audit, and one Outbox fact only on first admission. The isolated PostgreSQL gate covers first admission, exact replay, immutable identity and deduplication conflicts, reconnect recovery, and organization isolation when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. `AutomationsDispatchServices` composes schedule and normalized-event dispatch behind this same admission port, and PostgreSQL webhook delivery admission now enters that transaction instead of maintaining a second invocation Outbox writer. No timer, queue, target execution, mutable revision lookup, production worker composition, or P0 handoff is added | `AUT0.3-C8`, `AutomationInvocationEnvelopeV1`, shared Audit/Outbox contracts |
