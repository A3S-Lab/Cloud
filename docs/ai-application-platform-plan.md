...
| `APP0.1-C2` | Implemented; component-only | Add project-authorized create/publish/get/list CQRS, exact historical idempotency replay before Workflow re-resolution, one metadata-only port over the existing immutable Workflow repository, and migration `124` PostgreSQL persistence through A3S ORM with shared idempotency, Outbox, and audit writes in one transaction. The v1 publication admission accepts exactly one Workflow Output because the release contract owns one output-schema digest; Workflow retains its broader multi-output authority | `APP0.1-C1`, `W0.3` definition/revision foundation |
| `APP0.1` | In progress | Add production composition plus maintained REST/OpenAPI, client, CLI, and Management MCP contracts over the C1/C2 authority | `APP0.1-C2`, `C0.1` |
...
1. **Freeze the parity manifest and ADRs.** Completed for the 2026-08-13 v1
   ...
   default-output folding, schema-bound typed JSON response projection, and
   descriptor-bound Connector failure routing are accepted and versioned.
...
2. **Finish the W0 semantic foundation.** Retain protected WorkflowRun and
   ...
   folding/evidence, and Plan v5 Connector failure routing; complete
