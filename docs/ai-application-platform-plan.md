...
| `AUT0.2-C8` | Planned | Expose authorized Automation webhook endpoint lifecycle (create, get, generation-fenced disable/enable/revoke) and bounded definition/revision catalog reads through REST/OpenAPI, the maintained client, CLI, and Management MCP over the existing C2/C3 admission and definition catalog authorities. Authorization precedes replay. Do not claim Gateway public receive registration, production schema-registry wiring, signed ingress availability, or product availability. | `AUT0.2-C7`, `C0.1`, `C0.3` |
