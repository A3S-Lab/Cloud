...
| `AUT0.5` | Complete remaining provider/consumer wiring beyond the first Notification NATS-to-C6 composition, revocation/recovery operations, retained PostgreSQL/integration evidence, and Workflow HTTP/service ports over the existing profile/revision, materializer, public-Internet egress, executor, attempt, evidence, recovery, and management-surface authorities; Flow or the owning A3S Event consumer remains the retry/backoff authority | `AUT0.5-C7`; `W0.4` for node availability |
