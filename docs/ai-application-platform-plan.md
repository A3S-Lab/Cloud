...
| `AUT0.4-C1` | Implemented component foundation: `AutomationEventInvocationFactory` binds one already-normalized PluginEvent or SourceEvent identity to its exact subscription and Automation revision, validates event-key/observation ordering and the common invocation envelope, and derives policy-scoped deduplication. It performs no provider normalization/filtering, subscription persistence, queue, worker execution, scheduler, or P0 owner handoff | `AUT0.1`, `AUT0.3-C7`, `U0.4` event normalization |
