...
| `AUT0.3` | In progress; component-only calendar, misfire, concurrency, due-window, lease, durable cursor/lease, exact invocation-envelope, bounded schedule-dispatch, P0 Task-target adaptation, and idempotent invocation-admission boundaries exist, while catch-up rules, timer ownership, production worker composition, and the production P0 scheduled Task handoff remain open | `AUT0.1`, Boot compatibility lock, P0 profile contract |
