...
| `AUT0.4-C1` | Implemented component foundation: `AutomationNormalizedEventV1` validates one canonical digest-bound event identity, observation time, and bounded normalized input; `AutomationEventInvocationFactory` binds it to the exact PluginEvent or SourceEvent subscription and Automation revision, validates event-key/observation ordering and the common invocation envelope, and derives policy-scoped deduplication. It performs no provider normalization/filtering, subscription persistence, queue, worker execution, scheduler, or P0 owner handoff | `AUT0.1`, `AUT0.3-C7`, `U0.4` event normalization |
