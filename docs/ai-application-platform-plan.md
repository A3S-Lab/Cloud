...
| `AUT0.4-C3` | Implemented component foundation: `A3sEventAutomationNormalizedEventConsumer` binds one exact durable provider subject/source to `AutomationNormalizedEventV1`, ACKs malformed or successfully handled events, and leaves handler failures unacknowledged for provider-owned redelivery. It performs no local retry, queue, subscription discovery, invocation persistence, target execution, or P0 owner handoff | `AUT0.4-C2`, A3S Event |
