...
| `AUT0.5-C3` | Implemented component foundation: add Resource Grant-aware create/revise/get/list/history application contracts; atomic exact-scope active Secret-version lookup owned by Secrets; race-safe admission row fencing; just-in-time redacted, non-serializable HTTP revision materialization; authorization before replay and stable replay after later Secret revoke. | `AUT0.5-C2`, Secrets, `C0.3` |
| `AUT0.5` | Complete production egress/SSRF policy, execution evidence, internal execution authorization, revocation/recovery, supported management surfaces, provider wiring, and Workflow HTTP/service ports over the existing profile/revision authority, materializer, and execution port; Flow remains the retry/backoff scheduler | `AUT0.5-C3`; `W0.4` for node availability |
