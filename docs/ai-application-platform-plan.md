...
| `AUT0.3-C6` | Implemented component foundation: migration `184` plus in-memory/PostgreSQL schedule-state repositories persist one exact revision digest, exclusive cursor, monotonic lease generation, and owner/token/timestamp fence. Row-locked acquisition and commit reject active, stale, expired, non-rotated, and out-of-order state; migration `186` allows an exact owner to release a lease without advancing the cursor. No definition copy, invocation, queue, retry, worker, scheduler, or P0 authority is added | `AUT0.3-C5`, PostgreSQL/A3S ORM |
