...
| `AUT0.4` | In progress; component-only normalized event-envelope boundary; add plugin-trigger subscriptions and normalized event dispatch while preserving Sources connection/revision authority and U0 package authority. C1 freezes the normalized event/filter port and C2 provides bounded deterministic fan-out over already selected exact revisions | `AUT0.1`, `U0.4`, applicable Sources contracts |
