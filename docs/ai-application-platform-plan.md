...
| `AUT0.5` | Complete reusable immutable outbound HTTP/business Connector profiles/revisions, A3S ACL admission, authorization, persistence, Secret references and just-in-time materialization, production egress/SSRF policy, execution evidence, revocation/recovery, and Workflow HTTP/service ports over the existing execution port; Flow remains the retry/backoff scheduler | `AUT0.5-C1`, Secrets, `C0.3`; `W0.4` for node availability |
