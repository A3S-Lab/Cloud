...
| `AUT0.2-C3` | Implemented component foundation; live PostgreSQL evidence pending: migration `183` and the A3S ORM `PostgresAutomationWebhookRepository` restore the exact endpoint/revision projection, immutable first-delivery capture, and append-only receipts. Migration `188` and `PostgresAutomationDefinitionRepository` now persist one immutable definition identity, exact canonical revision ACL, digest-linked parent chain, row-locked current-head CAS, and deterministic bounded head discovery; the definition semantic value is restored from the revision rather than maintained as a second mutable policy copy. Lifecycle generation fences and immutable identity/revision checks are enforced in SQL; admission revalidates canonical ACL/digests and commits shared Audit/Outbox side effects atomically. The isolated PostgreSQL integration gate covers scoped lookup, first admission, exact replay, reconnect recovery, wrong-environment isolation, concurrent lifecycle generation fencing, and concurrent definition-head append fencing when `A3S_CLOUD_TEST_POSTGRES_URL` is configured. The slice stores no Secret plaintext, retry/scheduler state, or transport data and does not register a public route or worker; a live PostgreSQL result remains required | `AUT0.2-C2`, PostgreSQL/A3S ORM, shared Audit/Outbox |
