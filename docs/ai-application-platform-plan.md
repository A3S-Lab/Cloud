...
| `AUT0.5` | Complete Workflow Flow scheduling, retry/wait interpretation, and immutable response-object composition over C8; finish remaining provider/consumer wiring beyond the first Notification NATS-to-C6 composition, revocation/recovery operations, and retained PostgreSQL/integration evidence. Flow or the owning A3S Event consumer remains the retry/backoff authority. | `AUT0.5-C8`; `W0.4` for node availability |
