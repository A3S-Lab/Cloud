...
| `APP0.2-C45` | Implemented; component-only | Freeze Applications-owned `ApplicationAsynchronousObservation` over one exact Asynchronous-mode invocation and its session Input/Answer/FinalOutput frames: Waiting while non-terminal without FinalOutput; Succeeded only with Succeeded invocation plus Input and FinalOutput; Failed/Cancelled only for those terminals without FinalOutput; ordered Answer identities by sequence. Blocking/Streaming modes, foreign messages, sequence drift, Answer after FinalOutput, and Succeeded without FinalOutput fail closed. No migration, repository, CQRS poll, streaming cursor, REST/OpenAPI/client/CLI/MCP, Gateway, SSE, or public availability is added. ADR `0117`. Focused `cargo test -p a3s-cloud-control-plane --lib application_asynchronous_observation` | `APP0.2-C6`, `APP0.2-C39` |
