...
| `AUT0.2-C4` | Implemented component foundation: `HmacSha256AutomationWebhookSignatureVerifier` materializes the endpoint's exact Secret version through the published Secrets port, signs the captured raw body with HMAC-SHA256, and performs constant-time comparison against the canonical lowercase-hex signature fact. Secret plaintext is transient and redacted on failure. Schema registry/evaluation, listener, Gateway route, worker, and public availability remain out of scope | `AUT0.2-C3`, Secrets exact-version materialization |
