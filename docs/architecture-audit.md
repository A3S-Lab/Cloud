...
| Module | Unique authority | Current assessment | Required optimization |
| --- | --- | --- | --- |
| Operations | User-visible long-running operation identity and progress | Correct single projection authority. Workloads and WorkflowRun create paths insert Operation requests inside caller transactions through the Operations-owned `insert_operation_request_in_transaction` participant; no non-owner context maps `operation_requests`. List queries carry Operations-owned `OperationAccess` projected at REST/MCP entry (`operation_access`); Application no longer imports Identity `ResourceAccessEvaluator`. Execution, AgentExecution, and WorkflowRun subjects resolve through owned `ExecutionAccess` / `AgentAccess` / `WorkflowAccess` without Identity bridges. | Keep the participant as the only same-transaction write path. No context may independently declare the Operations table schema. The operation engine remains Flow-backed. |
| Executions | Generic finite Task product and immutable ExecutionTemplate | Clear Task semantics and one Workflow port. Application create/workflow/template paths use Projects owner ports; reconciler schedules Operations through `IExecutionOperationScheduler`; get/cancel carry Executions-owned `ExecutionAccess` projected at REST entry (no Application `ResourceAccessEvaluator`). Operation subject visibility for `execution` subjects maps through `ExecutionAccess` in the root Operation resolver. Domain policy contains a3s-runtime protocol types. | Keep product requirements in local value objects, translate to Runtime contracts at the adapter edge, and use scope/operation owner ports. Runtime contract use is allowed only in the execution adapter published language. |
| Workloads | Service desired state, Deployment, rollout, replicas, placement groups, resource claims, and writer fencing | The correct scheduling authority, but also the largest coupling hub. Agent binding now receives a Workloads-owned `AgentReleaseAdmission`; its Domain no longer imports Artifacts or a BuildRun, and one Application anti-corruption adapter translates the Assets deployable read model through `IWorkloadAgentReleaseAdmissionPort`. Skill binding similarly receives Workloads-owned `SkillReleaseAdmission` through `IWorkloadSkillReleaseAdmissionPort`; the bind handler no longer imports `IAssetRepository`. MCP revision binding now receives Workloads-owned `McpReleaseAdmission` / `McpProfileAdmission`; Domain no longer embeds Assets `McpServiceProfile` / `McpServiceProfileBinding`, and `McpWorkloadRevisionBinding` retains owned `runtime_port` / `health_path` admission facts so postgres hydration no longer joins or maps Assets `mcp_service_profiles`. External source Workload creation receives Workloads-owned `SourceBuildAdmission` through `IWorkloadSourceBuildAdmissionPort`; the create handler no longer imports `ISourceRevisionRepository` or `IBuildRunRepository`. Verified `WI2-C2` adds the aggregate-free `a3s.cloud.bound-runtime-claim.v1` Published Language and `IBoundRuntimeClaimQueryPort`: only its owner service interprets a bound Claim plus exact ordinary or placement-group member/revision lineage, including the immutable member plan and role-specific template. Component-only `WI2-C3a` removes caller-authored execution semantics: the owner query loads one immutable Deployment admission written through migration `180`, and ordinary, placement-group, rollback, reconciliation, and evidence paths reuse the sole Runtime Spec compiler. Identity sees no Workloads repository or lifecycle. Other domain/application paths still import Assets, Fleet, Operations, Projects, Secrets, and Sources internals, while Infrastructure maps foreign Fleet, Asset, Secret, and Operation state. Deployment/stop command handlers emit `WorkloadDeploymentOperationIntent` / `WorkloadStopOperationIntent`; `DeploymentBundle` / `WorkloadStopBundle` / `ReplicaDeploymentMaterialization` return those intents (not Operations `OperationRequest`); writer-fence continuation uses `WorkloadWriterFenceContinuationIntent`; writer-fence Domain/Durable Cells Application admit owned `WorkloadRuntimeRemoveEvidence` (no Fleet `NodeCommand` entity); Infrastructure composes Operations aggregates at the persistence boundary, including placement-group replica materialization via `compose_replica_deployment_operation`, and persists those requests through the Operations-owned same-transaction participant (no Workloads `operation_requests` ORM mapping). Deployment/get/list queries enrich Operations through `IWorkloadDeploymentOperationAccess` and Fleet Runtime observations through `IWorkloadRuntimeObservationAccess`. Workload log queries read Fleet through `IWorkloadLogAccess` and project owned `WorkloadLogRecord` / presentation DTOs (no Fleet log type aliases in Application or Presentation). | Continue splitting inbound admission ports from internal aggregates. Publish WorkloadDeploymentIntent, broader RuntimeTargetObservation, and writer-fence receipts. Fleet, Secrets, and Assets must be accessed through owner ports or facts. Secret-rotation reconcile locks Secrets rows through the Secrets-owned `lock_secret_version_for_rotation` participant (no Workloads `secrets` / `secret_versions` ORM). Remove remaining foreign table mappings; retain the one Runtime compiler. |
| Fleet | Node, pool, enrollment, inventory, Claim, command journal, observation, and fencing | Strong single node-control authority. `WI2-C2` publishes aggregate-free `a3s.cloud.runtime-node-evidence.v1` through `IRuntimeNodeEvidenceQueryPort`; its owner service checks pool membership/removal/maintenance, Ready state, exact Agent session, capability digest and Runtime observation. Observation reads now retain Agent instance and first authoritative receipt time across PostgreSQL/in-memory replay. Its domain legitimately carries the versioned node command published language but exposes no provider implementation through the new fact. Gateway snapshot install/observe Node-command enqueue and observation-outcome interpretation for Edge queues is owned by `IFleetGatewaySnapshotCommandPort` / `FleetGatewaySnapshotCommandService` (Edge ACAs no longer hold `INodeControlRepository` / `NodeCommandDraft`). List nodes and node-pool query/manage carry Fleet-owned `FleetAccess` projected at REST/MCP entry (no Fleet Application `ResourceAccessEvaluator` on those paths); project/environment grants are discarded. | Make NodeCommand and remaining observation schemas explicit contract surfaces. Keep a3s-runtime translation at the Node/Runtime adapter boundary. Reuse the new node-scope query pattern for Identity/Edge instead of sharing repositories, and add Fleet-owned immutable hardware attestation only in `WI2-C4`. |
| Edge | DomainClaim, certificate, Gateway scope, Route, rollout, and complete applied snapshot | Correct live-traffic intent owner, but domain types still embed Secret shapes for some paths; MCP snapshot Workload authority CAS uses Workloads `lock_running_workload_authority_for_update` (no Edge `workloads` ORM). MCP Gateway revision projection materializes through Workloads `IWorkloadMcpActiveRevisionProjectionQueryPort` / published `ActiveMcpWorkloadRevisionProjection` (no Edge `IWorkloadRepository`). Healthy route targeting uses Workloads `IWorkloadHealthyRouteTargetCandidateQueryPort` and Edge `IEdgeRuntimeObservationAccess`. Gateway snapshot install/observe command queues consume Fleet `IFleetGatewaySnapshotCommandPort` (no Edge `INodeControlRepository` / `NodeCommandDraft`). Node organization locks for staging/cutover/certificate convergence use Fleet-owned `lock_node_organization_for_update` (no Edge `nodes` ORM). MCP route-policy Domain/Application admission uses Edge-owned `EdgeMcpServiceProfileAdmission` through `IEdgeMcpServiceProfileAccess`; stored route-policy hydration restores owned admission facts (`profile_endpoint_path` / byte and stream ceilings) without joining Assets `mcp_service_profiles`. MCP Gateway projection Domain/planners use Edge-owned `EdgeMcpServiceProfileProjectionBinding` and `EdgeMcpWorkloadRevisionProjectionBinding` mapped through Edge Application ports (`IEdgeMcpServiceProfileAccess.find_projection_binding`, `IEdgeMcpWorkloadRevisionProjectionAccess`) with Assets/Workloads quarantined in Infrastructure ACAs. Healthy route-target resolution for Gateway MCP/route planners uses Domain `IRouteTargetReader` / `RouteTarget` / `ResolvedRouteTarget*` with Workloads+Fleet quarantined in `WorkloadsFleetRouteTargetAccessAdapter`. Deployment route cutover reads Fleet Runtime observations through `IEdgeRuntimeObservationAccess` with Fleet quarantined in `FleetEdgeRuntimeObservationAccessAdapter`. Managed Gateway inference ACL staging (route cutover, certificate convergence, desired-state, rollout planner, rollout rollback, publish-route) enters through `IEdgeManagedInferenceAclAccess` with Identity/Inference projection ports quarantined in `IdentityInferenceEdgeManagedAclAccessAdapter`. MCP credential create/rotate encrypts delivery receipts through `IEdgeMcpCredentialEncryption`; Domain delivery receipts store Edge-owned `EdgeEncryptedCredentialValue` with Secrets `EncryptedSecretValue` mapped only in the encryption ACA and persistence. | Keep GatewayMember facts advancing; Edge compiles only owner-published snapshots. Gateway remains the sole applied request path. |
| Secrets | Secret/version lifecycle, authorization, encryption and exact materialization | Clear authority. Workloads secret-rotation reconcile locks one exact Secret version inside the caller transaction through Secrets-owned `lock_secret_version_for_rotation` and no longer maps `secrets` / `secret_versions`. Deployment material resolution still loads Workloads state to authorize a consumer. | Keep the rotation lock participant as the only same-transaction Secrets lock path for consumers. Replace remaining Workload repository access for materialization auth with an exact SecretConsumerAuthorization port implemented by Workloads. Material never crosses presentation or domain events. |
| Data / S0 | Object namespace contract, storage provider policy, backup/restore/retention, and writer fencing | The object namespace is correctly separate from business desired state. Current scope is incomplete for general databases/volumes. Object-namespace recovery OperationRequest composition lives in Infrastructure `ObjectNamespaceRecoveryOperationRequest`; Application keeps only owned seal/restore/delete inputs. | Keep one immutable-object client and define owner ports for exact Secret materialization. Add database/volume aggregates only under S0 gates; never absorb Durable Cell application semantics. |

### 5.4 AI and interaction products

| Module | Unique authority | Current assessment | Required optimization |
| --- | --- | --- | --- |
| Agents | Conversation, AgentExecution, semantic event sequence, approvals/checkpoints/forks trajectory | Semantic state is separate from Runtime logs and generic Executions. Start, fork, and Workflow dispatch now consume one Agents-owned `IAgentReleaseAdmissionPort`; only the consumer-owned Infrastructure adapter may compose the Assets deployable-release and Artifacts OCI-location owner interfaces, and it returns one immutable `AgentReleaseBinding`. Application imports neither owner repository/query interface nor the foreign deployable read model. An architecture fitness test rejects a second adapter, concrete persistence, Outbox/projector, command handler, or worker in this boundary. Agent execution reconciliation schedules Operations through `IAgentExecutionOperationScheduler`. Queries and mutation commands that authorize by conversation/execution carry Agents-owned `AgentAccess` projected at REST entry (no Application `ResourceAccessEvaluator`). Operation subject visibility for `agent_execution` subjects maps through `AgentAccess` in the root Operation resolver. | Keep provider-neutral Harness execution behind Agents-owned ports and the common Workloads/Fleet/Runtime path. Apply the same owner-port rule to remaining model/Tool producers; never add an Agent release repository, build query, scheduler, or retry mechanism. |
| Applications | Application/release, session, invocation, message, variable, and delivery semantics | The Application layer now has no direct Workflow module dependency. Both invocation boundaries enter timeout admission through `IApplicationWorkflowRunPort`; its sole production adapter delegates to Workflow's owning rule, the copied 30-day Domain constant is removed, public schemas reference owner constants, and migration `171` removes migration `127`'s historical database copy. An architecture fitness test rejects a direct Workflow import, a second timeout-rule entry point, or another copied bound. Queries/commands/delivery carry Applications-owned `ApplicationAccess` projected at REST/MCP entry (no Application `ResourceAccessEvaluator`); environment-only grants cannot authorize project-owned aggregates. | Preserve the anti-corruption layer: all Workflow compilation/run/effect interaction travels through Applications-owned ports and published snapshots. Keep Workflow repositories, timeout helpers, defaults, and maxima outside Applications Application and Domain code. |
| Workflow | Ontology, WorkflowDefinition, Goal, Plan, WorkflowRun, HumanTask, decision, and semantic step projections | Rich invariants and strong Flow authority tests. Workflow now owns immutable `HumanTaskSubmission` evidence and the sole historical `form_submissions` mapper. Its Application and Domain layers import no Forms internals; task coordination and submission evaluation share one consumer-owned `IHumanTaskFormPort`, implemented by one Infrastructure adapter. Migration `173` corrects the physical-table ownership description without rewriting historical bytes, IDs, URNs, or replay state. Queries, mutation commands, and hosted authoring carry Workflow-owned `WorkflowAccess` projected at REST entry (no Application `ResourceAccessEvaluator`). Operation subject visibility for `workflow_run` subjects maps through `WorkflowAccess` in the root Operation resolver. | Define internal subdomains for Definition/Planning, Run, and Human Interaction. Preserve the single Forms adapter and access Executions, Connectors, and Applications only through equivalent application ports. Flow remains the only durable execution history. |
...
| Connectors | Reusable outbound profile/revision, egress policy, exact attempt fencing/evidence, and response-object reference | Provider-neutral attempt model and Workflow port are strong. Create/revise profile admission uses Secrets published `IExactSecretVersionAccess`; infrastructure still materializes plaintext through Secrets. Queries/commands/execution/Workflow port carry Connectors-owned `ConnectorAccess` projected at REST/MCP entry (no Application `ResourceAccessEvaluator`). | Keep exact materialization in Secrets infrastructure. Connector attempts remain the sole HTTP retry/evidence authority for consumers such as Notifications. |
| Notifications | Personal inbox, subscriptions, alert policy, delivery facts, and terminal delivery receipts | Correctly reuses Connector and SMTP mechanisms, but domain types embed Connector outcomes and Identity email/evaluator types. Queries/commands carry Notifications-owned `NotificationAccess` projected at REST/MCP entry; domain `NotificationScope` no longer imports Identity evaluators; outbound Connector dispatch synthesizes Connectors-owned `ConnectorAccess` instead of Identity grants. | Consume versioned Connector and Identity published contracts. Alert sources arrive only as closed owner facts. Do not add provider retries, timers, or authorization stores. |
| Plugins | Tenant registry enrollment and exact A3S Use package assignment intent | Small, port-oriented, and correctly excludes installation/reconciliation authority. Assignment reconciliation schedules Operations through `IPluginAssignmentOperationScheduler`. | Narrow the module facade and preserve A3S Use as the only package lifecycle owner. |
...
3. Replace the Artifacts-to-Assets table write with an idempotent durable
   publication handoff.
4. ~~Replace Fleet log DTO reuse with an owner log-query port.~~ Done: `IWorkloadLogAccess` + owned `WorkloadLogRecord` / `WorkloadLogRecordResponse`.
