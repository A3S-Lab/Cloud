...
| Connectors | Reusable outbound profile/revision, egress policy, exact attempt fencing/evidence, and response-object reference | Provider-neutral attempt model and Workflow port are strong. Create/revise profile admission uses Secrets published `IExactSecretVersionAccess`; infrastructure still materializes plaintext through Secrets. | Keep exact materialization in Secrets infrastructure. Connector attempts remain the sole HTTP retry/evidence authority for consumers such as Notifications. |
| Notifications | Personal inbox, subscriptions, alert policy, delivery facts, and terminal delivery receipts | Correctly reuses Connector and SMTP mechanisms, but domain types embed Connector outcomes and Identity email/evaluator types. Queries/commands carry Notifications-owned `NotificationAccess` projected at REST/MCP entry; domain `NotificationScope` no longer imports Identity evaluators; Connectors dispatch still synthesizes Identity grants until Connectors owns its access projection. | Consume versioned Connector and Identity published contracts. Alert sources arrive only as closed owner facts. Do not add provider retries, timers, or authorization stores. |
