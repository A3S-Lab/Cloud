...
| --- | --- | --- | --- |
| shared_kernel | Stable cross-context IDs, exact scope identity/narrowing, canonical digests/timestamps, repository error, and idempotency request/result shapes | Small and mostly disciplined. `C0.5-MT1-C1` admits the closed Installation/Organization/Project/Environment `ScopeContext` because Identity, Projects, Workloads, audit/Outbox and other owner ports require identical lineage and narrowing semantics. Architecture tests forbid ambient request, Workspace, node, membership or Runtime authority from entering it. Identity- and Secrets-flavoured references still require continued admission scrutiny. | Retain the admission rule: a type enters only when at least three independent contexts need identical semantics and no business lifecycle moves with it. Keep `ScopeContext` identity-only; do not add repositories, policy services, headers or convenience adapters. |
| Identity | Organizations, Principals, Memberships, credentials, grants, authorization decisions, verified recipient contacts, platform role policy/bindings, Trust Domains, and Workload Identity Policy revisions | Existing tenant aggregates and repository ports are clear. Component-only `C0.5-MT1-C1/C2` adds one canonical platform-role-policy ACL, immutable permission ceilings, deterministic accepted revisions, binding/grant lifecycles, a canonical bounded tenant-support ACL, and one digest-bound privileged decision evidence model. ACL remains desired intent; the dynamic allow fact reuses canonical JSON and the shared decision reference. No tenant, persistence or provider authority moved into those components. Component-only `H0.4-WI1-C1` adds two canonical workload-trust ACLs, deterministic immutable revisions, predecessor-fenced repository ports, and one capability-only provider port through the Contracts published language; it adds no certificate, Secret, network, Runtime, or provider lifecycle. The existing boolean platform-administrator bypass, direct Resource Grant access to Projects/Fleet repositories and widely imported Identity presentation guards remain architectural debt. | Finish `MT1-C3`'s canonical Installation identity and explicit scope-aware audit/Outbox contract, then MT2 persistence/current-head/approver invariants and MT3 cross-surface enforcement before treating the new RBAC as authority. Replace boolean administrator bypasses, publish the broader authorization context, move shared inbound adapters to root presentation composition, and replace direct Project/Node repository access with owner ports. Persist WI1 only after installation-scoped persistence is valid, then bind WI2 evidence through owner ports. |
