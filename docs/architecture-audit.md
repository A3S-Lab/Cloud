...
`WI2-C3a` is the sole planned Workloads persistence/admission slice for one
generic execution binding bound before scheduling; every ordinary,
placement-group, reconciliation, restart and rollback path must reuse the sole
compiler, and legacy unattached Units must roll forward. `WI2-C3b` is the sole
Identity evidence-history persistence/concurrency slice. C2's transient owner
