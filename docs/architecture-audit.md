...
| Developer Workflows | Reviewable BuildPlan and workload-profile proposal/acceptance, Preview Policy revisions and lifecycle projection, owner-facing handoff intent, and later monorepo/import decisions | Domain owns canonical proposal, acceptance, profile, policy, lifecycle, event, and receipt values; `published` owns only aggregate-free committed lifecycle facts. Application owns action-scoped `IDeveloperWorkflowAuthorizationPort`, exact `IBuildPlanSourceLayoutPort`, minimal Sources subscription/evidence ports, build-outcome and target-admission ports, and Preview projection/Environment ports; foreign policy and aggregates remain private. Two Developer Workflows Infrastructure anti-corruption adapters translate only exact Sources owner evidence for accepted BuildPlans and Preview subscriptions. Separately, one Sources-owned layout adapter implements the consumer port over the existing published build-input query and the sole authorized checkout service; `P0.1-C5` authorizes before Sources/provider access, returns no credential/path/receipt, and reuses the only Git source-inventory/digest plus replay/cleanup mechanism. Workloads, Executions, Artifacts, and Projects each retain one consumer adapter that invokes the owner validation/digest contract and creates no duplicate owner record or lifecycle. Workload-profile and Preview Policy acceptance share the same authorization instance and existing repository transactions. Preview projection/handoffs reuse the shared Relay plus ordinary Projects Environment, Sources SourceRevision, and Artifacts BuildRun authorities. Executable architecture tests reject Published Language aliasing, optional production handoff, duplicate checkout/credential/source-inventory traversal, another publisher/Relay/Outbox/NATS provider/worker/retry loop, or foreign owner models outside their exact adapters. | Implement P0.1/P0.2 public interfaces, pre-acceptance source discovery, Edge and Operations adapters, production Workloads/Executions lifecycle and scheduling handoff, and expiry/cleanup. No build, scheduler, deployment, route, webhook-verification, provider delivery, or retry lifecycle moves here. |
