...
| Edge | DomainClaim, certificate, Gateway scope, Route, rollout, and complete applied snapshot | Correct live-traffic intent owner, but domain types still embed Secret shapes for some paths; infrastructure still maps some foreign tables. MCP route-policy Domain/Application admission uses Edge-owned `EdgeMcpServiceProfileAdmission` through `IEdgeMcpServiceProfileAccess`. MCP Gateway projection Domain/planners use Edge-owned `EdgeMcpServiceProfileProjectionBinding` and `EdgeMcpWorkloadRevisionProjectionBinding` mapped at Assets/Workloads ACAs / projection input reader. Healthy route-target resolution for Gateway MCP/route planners uses Domain `IRouteTargetReader` / `RouteTarget` / `ResolvedRouteTarget*` with Workloads+Fleet quarantined in `WorkloadsFleetRouteTargetAccessAdapter`. MCP credential create/rotate encrypts delivery receipts through `IEdgeMcpCredentialEncryption`. | Keep GatewayMember facts advancing; Edge compiles only owner-published snapshots. Gateway remains the sole applied request path. |
