...
| Developer Workflows | Reviewable BuildPlan, workload-profile proposal/acceptance, Preview Policy revisions and lifecycle projection, and later monorepo/import decisions | Domain owns its process, Secret-binding, resource, port, health, branch, installation-ref, canonical Preview Policy, policy-revision authority, pull-request-change proposal, lifecycle, and immutable projection-receipt values. It enters Sources only through Published Language and imports no foreign owner internals. Application owns action-scoped `IDeveloperWorkflowAuthorizationPort`, the minimal `IPreviewSourceSubscriptionQueryPort`, `IWorkloadBuildOutcomePort`, `IServiceProfileAdmissionPort`, `IScheduledTaskProfileAdmissionPort`, and `IPullRequestPreviewProjectionPort`; Identity policy, Sources aggregates/repositories, Artifacts aggregate state, and Workloads/Executions templates stay private. Migration `153` stores append-only canonical policy revisions. Component-only C4 selects new-Preview policy by fact time, freezes the exact revision across the lifecycle, and uses migration `157` for one CAS-safe local Preview plus immutable per-fact receipts. All-in-one and dedicated Relay processes compose the same projector and typed PostgreSQL family. The executable architecture test rejects another publisher, Relay, Outbox repository, NATS provider, worker spawn, or retry loop inside this context. Build outcomes still bind the exact BuildPlan ID/digest, while target admission returns one correlation-bound receipt carrying the target, exact request context, Artifact digest, and opaque owner-contract digest. | Implement Identity/Projects/Artifacts/Workloads/Executions/Edge/Operations owner adapters, production handoffs, expiry/cleanup, and interfaces. No build, scheduler, deployment, route, webhook-verification, provider delivery, or retry lifecycle moves here. |
...
one CAS transaction. BuildPlan and workload-profile acceptance ask
