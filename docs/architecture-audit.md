...
| Edge | DomainClaim, certificate, Gateway scope, Route, rollout, and complete applied snapshot | Correct live-traffic intent owner, but domain types still embed Workloads and Secret shapes for some paths; infrastructure reads foreign nodes/workloads through local table mappings, and MCP Gateway projection still consumes Assets `McpServiceProfileBinding`. MCP route-policy Domain/Application admission now uses Edge-owned `EdgeMcpServiceProfileAdmission` through `IEdgeMcpServiceProfileAccess`; Infrastructure maps Assets profiles at the ACA and Postgres load boundary. MCP credential create/rotate encrypts delivery receipts through `IEdgeMcpCredentialEncryption`. | Introduce RouteTarget, GatewayMember, and owned MCP projection inputs. Edge compiles only owner-published snapshots. Gateway remains the sole applied request path. |
