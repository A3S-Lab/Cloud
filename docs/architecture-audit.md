...
| Developer Workflows | Reviewable BuildPlan, workload-profile proposal/acceptance, Preview Policy revisions and lifecycle projection, owner-facing handoff intent, and later monorepo/import decisions | Domain owns its process, Secret-binding, resource, port, health, branch, installation-ref, canonical Preview Policy, policy-revision authority, pull-request-change proposal, lifecycle, event generation, and immutable projection-receipt values; `published` owns the aggregate-free committed lifecycle wire contract. Domain enters Sources only through Published Language. Application owns action-scoped `IDeveloperWorkflowAuthorizationPort`, the minimal `IPreviewSourceSubscriptionQueryPort`, `IWorkloadBuildOutcomePort`, `IServiceProfileAdmissionPort`, `IScheduledTaskProfileAdmissionPort`, `IPullRequestPreviewProjectionPort`, and `IPreviewEnvironmentPort`; Identity policy, Sources aggregates/repositories, Projects aggregates/repositories, and Artifacts aggregate state stay private. Exactly two read-only Infrastructure anti-corruption adapters may translate Sources owner models: one for accepted BuildPlan SourceRevision evidence and one for the exact Preview Policy subscription binding. `P0.2-C3a/C3b` confine Workloads `ServiceTemplate` and Executions `ExecutionTemplate` imports to one Infrastructure anti-corruption adapter per owner, invoke each owner validation/digest contract, and return only the consumer-owned exact receipt; neither writes an owner record. `P0.2-C3c` confines Artifacts imports to one Infrastructure adapter that consumes only the owner query and aggregate-free successful external-source BuildRun Published Language, combines it with the exact local accepted BuildPlan, and returns the existing consumer view without another lifecycle. `P0.2-C4/C5` production-compose the exact accepted-revision compilation query and authorization-first workload-profile acceptance command; C5 shares the single production authorization port instance with BuildPlan acceptance and delegates its only write to migration `147`'s repository transaction. `P0.3-C6` shares that authorization instance with Preview Policy acceptance, maps only exact validated Sources subscription evidence, and selects role-scoped instances of migration `153`'s sole append-only repository through one constructor rule. Component-only Preview C4 selects new-Preview policy by fact time, freezes the exact revision across the lifecycle, and uses migration `157` for one CAS-safe local Preview plus immutable per-fact receipts. C5a atomically stores a bounded lifecycle event with each mutation and confines Projects imports to one Infrastructure adapter that reuses the ordinary Environment repository, idempotency, and Outbox. All-in-one and dedicated Relay processes compose the same required port and projector family. Executable architecture tests reject Published Language aliasing, optional production handoff, another publisher/Relay/Outbox repository/NATS provider/worker/retry loop, or foreign owner models outside their exact adapters. Build outcomes bind the exact local BuildPlan ID/digest only after the Artifacts fact passes exact scope/source/recipe/time validation, while target admission returns one correlation-bound receipt carrying the target, exact request context, Artifact digest, and opaque owner-contract digest. | Implement P0.1/P0.2 public interfaces, source-layout acquisition, Edge and Operations adapters, production Workloads/Executions lifecycle and scheduling handoff, and expiry/cleanup. No build, scheduler, deployment, route, webhook-verification, provider delivery, or retry lifecycle moves here. |
...
lifecycle, scheduling, route, Operation, cleanup, and public interfaces remain
named follow-up work; the separate C5a Projects Environment adapter is
