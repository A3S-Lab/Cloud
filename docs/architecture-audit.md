...
| Module | Unique authority | Current assessment | Required optimization |
| --- | --- | --- | --- |
| Operations | User-visible long-running operation identity and progress | Correct single projection authority. Workloads and WorkflowRun create paths insert Operation requests inside caller transactions through the Operations-owned `insert_operation_request_in_transaction` participant; no non-owner context maps `operation_requests`. List queries carry Operations-owned `OperationAccess` projected at REST/MCP entry (`operation_access`); Application no longer imports Identity `ResourceAccessEvaluator`. | Keep the participant as the only same-transaction write path. No context may independently declare the Operations table schema. The operation engine remains Flow-backed. Replace remaining legacy Identity bridges inside the root Operation subject resolver once Executions/Agents/Workflow own access projections. |
| Executions | Generic finite Task product and immutable ExecutionTemplate | Clear Task semantics and one Workflow port. Application create/workflow/template paths use Projects owner ports; reconciler schedules Operations through `IExecutionOperationScheduler`; domain policy contains a3s-runtime protocol types. | Keep product requirements in local value objects, translate to Runtime contracts at the adapter edge, and use scope/operation owner ports. Runtime contract use is allowed only in the execution adapter published language. |
