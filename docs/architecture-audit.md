...
| Edge | DomainClaim, certificate, Gateway scope, Route, rollout, and complete applied snapshot | Correct live-traffic intent owner, but domain types still embed Workloads for MCP projection revision reads and Secret shapes for some paths; infrastructure reads foreign nodes/workloads through local table mappings. MCP route-policy Domain/Application admission uses Edge-owned `EdgeMcpServiceProfileAdmission` through `IEdgeMcpServiceProfileAccess`. MCP Gateway projection Domain/planners use Edge-owned `EdgeMcpServiceProfileProjectionBinding` mapped at the Assets ACA / projection input reader. MCP credential create/rotate encrypts delivery receipts through `IEdgeMcpCredentialEncryption`. | Introduce RouteTarget, GatewayMember, and owned Workload revision projection facts. Edge compiles only owner-published snapshots. Gateway remains the sole applied request path. |
