...
| Module | Unique authority | Current assessment | Required optimization |
| --- | --- | --- | --- |
| Operations | User-visible long-running operation identity and progress | Correct single projection authority. Workloads and WorkflowRun create paths insert Operation requests inside caller transactions through the Operations-owned `insert_operation_request_in_transaction` participant; no non-owner context maps `operation_requests`. List queries carry Operations-owned `OperationAccess` projected at REST/MCP entry (`operation_access`); Application no longer imports Identity `ResourceAccessEvaluator`. Execution subjects resolve through owned `ExecutionAccess` without an Identity bridge. | Keep the participant as the only same-transaction write path. No context may independently declare the Operations table schema. The operation engine remains Flow-backed. Replace remaining legacy Identity bridges for Agents/Workflow subjects once those contexts own access projections. |
