...
| Projects | Project, Environment, tenant attribution lineage | Compact aggregates and repositories. `create_project` admits Organization existence through `IProjectOrganizationAccess` with Identity quarantined in `IdentityProjectsOrganizationAccessAdapter` (no Application `IOrganizationRepository`). Queries still carry Identity's concrete evaluator. | Depend on a published authorization contract for list/get paths, not Identity presentation types. |
