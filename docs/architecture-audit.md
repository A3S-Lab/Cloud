...
| Identity | Organizations, Principals, Memberships, credentials, grants, authorization decisions, verified recipient contacts, platform role policy/bindings, Trust Domains, and Workload Identity Policy revisions | Existing tenant aggregates and repository ports are clear. `C0.5-MT1-C1/C2` add one canonical platform-role-policy ACL, immutable permission ceilings, deterministic accepted revisions, binding/grant lifecycles, a canonical bounded tenant-support ACL, and one digest-bound privileged decision evidence model. `MT1-C3` persists one immutable Installation identity and reuses the existing Audit/Outbox rail for exact installation and tenant scopes without a sentinel Organization. Verified `MT2-C1` adds one Identity repository and migration `177`: immutable policy history, a predecessor-fenced current head, versioned bindings, one active binding per Principal, Installation-row serialization, shared idempotency/Audit/Outbox, self-escalation denial and database-enforced active-owner recovery. ACL remains desired intent and Redis/Lane/cache/lock claims are not truth. Component-only `H0.4-WI1-C1` adds two canonical workload-trust ACLs, deterministic immutable revisions, predecessor-fenced repository ports, and one capability-only provider port through the Contracts published language; it adds no certificate, Secret, network, Runtime, or provider lifecycle. The existing boolean platform-administrator bypass, direct Resource Grant access to Projects/Fleet repositories and widely imported Identity presentation guards remain architectural debt. | Complete evidence-backed support approval/grant persistence and one atomic current-snapshot privileged-decision Application interface in `MT2-C2/C3`; then finish MT3 cross-surface enforcement before treating RBAC as general authority. Replace boolean administrator bypasses, publish the broader authorization context, move shared inbound adapters to root presentation composition, and replace direct Project/Node repository access with owner ports. WI1 may build on canonical installation-scoped persistence, but WI2 evidence must still bind through owner ports. |
