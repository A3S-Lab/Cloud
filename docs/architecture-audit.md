...
| Identity | Organizations, Principals, Memberships, credentials, grants, authorization decisions, verified recipient contacts, Trust Domains, and Workload Identity Policy revisions | Existing tenant aggregates and repository ports are clear. Component-only `H0.4-WI1-C1` adds two canonical ACL contracts, deterministic immutable revisions, predecessor-fenced repository ports, and one capability-only provider port through the Contracts published language; it adds no certificate, Secret, network, Runtime, or provider lifecycle. Resource Grant creation still directly consumes Projects and Fleet repositories; shared HTTP guards live under Identity presentation and are imported by nearly every presentation module. | Persist and expose WI1 only through authorization-first Identity Application services with atomic Outbox/audit, then bind WI2 evidence through owner ports. Publish the broader authorization context, move shared inbound adapters to root presentation composition, and replace direct Project/Node repository access with resource-scope owner ports. |
