...
signature, body, or body digest. A future Developer Workflows adapter may
translate only this Published Language into its local reducer input; it may not
import the Sources verifier, delivery entity, repository, or Inbox. Migration
`156` extends the existing Inbox rather than introducing another queue, relay,
retry mechanism, or provider lifecycle.
...
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, Preview Policy revisions, and Preview intent; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1/C2` implement canonical detection, BuildPlan/workload-profile acceptance, the pure Preview lifecycle, and authorization-first exact-Subscription-bound canonical policy revisions through migration `153`; Sources production-composes the C3 fact producer through migration `156`. Developer Workflows Domain owns local proposal and policy values and enters Sources only through Published Language; Application owns authorization, source-subscription query, verified-build-outcome, and Service/Task admission ports, returns bounded owner evidence, and imports no foreign owner model. Its fact consumer, production owner adapters/composition, Projects/Edge handoffs, individual Preview persistence, monorepos, and imports remain unavailable |
