...
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, Preview Policy revisions, and durable Preview lifecycle projection; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1/C2` implement canonical detection, BuildPlan/workload-profile acceptance, the pure Preview lifecycle, and authorization-first exact-Subscription-bound canonical policy revisions through migration `153`; Sources production-composes the C3 fact producer through migration `156`. Component-only `P0.3-C4` composes one event-time-policy-bound Developer Workflows projector in the existing Outbox Relay and migration `157` persists CAS-safe Preview state plus immutable local receipts. Domain owns local proposal, policy, lifecycle, and receipt values; Application owns authorization, source-subscription query, verified-build-outcome, Service/Task admission, and Preview projection ports, and imports no foreign owner model. Projects/Artifacts/Workloads/Executions/Edge/Operations handoffs, expiry/cleanup execution, public interfaces, monorepos, and imports remain unavailable |
