...
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, Preview Policy revisions, durable Preview lifecycle projection, and owner-facing handoff intent; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1/C2` implement canonical detection, BuildPlan/workload-profile acceptance, the pure Preview lifecycle, and authorization-first exact-Subscription-bound canonical policy revisions through migration `153`; `P0.2-C3a/C3b` confine exact web/worker and scheduled-profile translation plus Workloads `ServiceTemplate` and Executions `ExecutionTemplate` validation/digest admission to one concrete Infrastructure adapter per owner while returning only the consumer-owned receipt. `P0.2-C3c` adds the Artifacts-owned terminal successful external-source BuildRun Published Language/query and confines its consumption plus exact local BuildPlan enrichment to one Developer Workflows Infrastructure adapter. The owner value has no BuildPlan vocabulary, and the consumer imports no Artifacts aggregate, repository, or lifecycle. Sources production-composes the C3 fact producer through migration `156`. Component-only `P0.3-C4` composes one event-time-policy-bound Developer Workflows projector in the existing Outbox Relay and migration `157` persists CAS-safe Preview state plus immutable local receipts. `P0.3-C5a` atomically emits exact committed lifecycle facts and uses a required consumer-owned Application port plus one Infrastructure adapter to create only the ordinary Projects Environment through the same Relay/projector. Domain owns local proposal, policy, lifecycle, event generation, and receipt values; `published` owns the aggregate-free lifecycle wire contract; Application owns authorization, source-subscription query, verified-build-outcome, Service/Task admission, Preview projection, and Environment-handoff ports, and imports no foreign owner model. P0.2 production composition, Workloads/Executions lifecycle, scheduling, Edge, and Operations handoffs, Environment/Preview expiry cleanup, public interfaces, monorepos, and imports remain unavailable |
