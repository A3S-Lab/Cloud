...
| Durable Cells | Durable Cell application identity, immutable revision/profile, retention intent, and exact Workload/S0/Gateway deployment projection; never individual Cell state or ownership | `CELL0.1-C1/C2/C3` Service/application ACL, migration-safe revision lineage, shared fixtures, desired-state aggregate, and deterministic S0/Workloads/Operations identity mapping implemented; provider/storage/orchestration gates and `CELL0.5` availability are planned |
