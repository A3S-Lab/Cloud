...
| Notifications | Deterministic personal in-app projections of curated committed Outbox facts, exact-recipient and Resource Grant filtering, idempotent read state, a bounded deterministic outbound fact, side-effect-free signed-webhook/Slack-compatible request builders, and a NATS durable/manual-ack consumer composed with the fenced Connector application service | `C0.3-N1` is verified; component-only `C0.3-N2a` and `N2b` are implemented. Transactional delivery emission, receipts, ACL-native subscriptions, bounded rate policy, SMTP, and alert policy remain unavailable |
...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, durable exact-attempt fencing/recovery, and immutable terminal execution evidence | Component-only `AUT0.5-C1` execution plus verified `C2`-`C6` foundations are implemented; `C0.3-N2b` adds the first NATS A3S Event consumer-to-C6 composition without another retry rail. General provider wiring, Workflow ports, supported non-Web management surfaces, revocation/recovery operations, and product availability remain planned; callers never create direct HTTP clients |
