...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, durable exact-attempt fencing/recovery, and immutable terminal execution evidence | Component-only `AUT0.5-C1` execution plus verified `C2`-`C6` foundations are implemented; `C0.3-N2b` composes the first NATS A3S Event consumer with C6, while `N2c`-`N2e` keep delivery authorization, receipt, pacing, and fixed-budget decisions in Notifications by reading the same C6 evidence. General provider wiring, Workflow ports, supported non-Web management surfaces, revocation/recovery operations, and product availability remain planned; callers never create direct HTTP clients |
