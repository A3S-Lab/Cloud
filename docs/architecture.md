...
| Assets | Agent, MCP, and Skill identities, hosted Git, immutable release lifecycle, Agent deployment, and Skill-to-Agent-Workload release binding | `A0.1` and `A0.2` verified; `A0.3` through `A0.5` implemented. A0.5 has a CI REST/PostgreSQL/local-object publication gate plus a real Box node Artifact gate retaining typed Skill-bundle hydration, exact-byte, read-only mount/write-rejection, single-download, and Runtime-remove cleanup evidence. Retained external-provider and full Box Workload bind/rebind/unbind/rollback lifecycle evidence remains open |
