...
| Fleet | Nodes, enrollment, inventory, command leases, observations, claims, and fencing | Current; `C0.3-N4h` freezes a Worker-only bounded node-availability fact reconciler, migration `139` fact-head cursor, and atomic heartbeat/revoke-plus-Outbox recovery boundary before Notifications may admit an exact-node source |
