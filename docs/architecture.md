...
| Personal in-app notification projection | Notifications projects committed active-Membership creation and role-change Outbox facts for one exact recipient Principal, then applies the shared Resource Grant evaluator; Identity retains invitation and revocation lifecycle surfaces | Business desired state, source-fact mutation, a second event rail, delivery queue, provider/template/subscription authority, scheduler, or presentation-local inbox |
...
inspect the same materialization through one project-authorized
...
WorkflowRevision registry snapshot. Digest-only defaults,
composite/Application variable adapters, and public node availability remain
open.
