...
| Audit | Shared append-only `audit_records` plus one owner/admin-only bounded read projection; verified `C0.3-PA2a` adds explicit request-time Project/Environment/immutable-profile references and a closed attribution status, verified `C0.3-PA2b` adds one bounded canonical signed page, verified `C0.3-PA2c` adds one deployment policy with per-organization monotonic availability and deletion-completion watermarks through migration `144`, and verified `C0.3-PA2d` atomically returns a transient complete same-key signed manifest bundle | Agent, Gateway, inference, or MCP-specific audit stores, duplicate writers, current-pointer lookup for historical facts, scope inference from private details, public unstructured details, or a second retention scheduler/store |
...
a read cannot race a watermark advance. The Worker claims a fair bounded
tenant page with `FOR UPDATE SKIP LOCKED`, uses one global record budget for
typed ORM deletion, hides physical backlog below the logical watermark, and
...
Verified `C0.3-PA2d` extends only the same
Audit read boundary with a synchronous,
...
Applications owns exact-release `ApplicationPublicationRouteIntent` (channels, optional embed origin allowlist, rate-shaping policy profile reference) as a domain aggregate (`APP0.3-C10`) persisted through migration `210` and repository adapters (`APP0.3-C11`) with project-authorized create/get/list-by-release CQRS (`APP0.3-C12`), management REST/OpenAPI (`APP0.3-C13`), maintained cloud-client/CLI (`APP0.3-C14`), a pure Edge-facing desired-state projection mapper (`APP0.3-C15`), declare-only Gateway snapshot ACL admission through contracts + Edge ACA (`APP0.3-C16`), repository-backed desired-state intent load into planner-backed managed Gateway snapshot compile (`APP0.3-C17`), the same Edge ACA load into MCP Gateway desired-state reconciliation compile (`APP0.3-C18`), and a post-C18 empty-compile-site audit confirming no remaining production hard-coded empty intent bypasses (`APP0.3-C19` / ADR `0148`; low-level `compile*` helpers still accept caller-supplied intents; PublishRoute is not the channel owner), plus Gateway rate-shaping profile compile binding that admits declare-only profile digests against an Edge/Gateway catalog and emits a separate bound ACL block with token-bucket/GCRA scalars (`APP0.3-C20` / ADR `0149`; empty catalog fails closed; intent ACL stays declare-only; Gateway node now parses/validates those ACL blocks and installs a reloadable rate-shaping catalog (`APP0.3-C21` / ADR `0150`), Cloud wires a shared process-local catalog into production snapshot compilers with CQRS registration (`APP0.3-C22` / ADR `0151`), and optional Edge ACL `rate_shaping_profile` seeds populate that catalog at both api-worker and management boot (`APP0.3-C23` / ADR `0152`), and Gateway installs a resolvable exact-release publication route binding table from ACL intents (`APP0.3-C24` / ADR `0153`; indexed by intent id and `(release_digest, channel)`), and enforces HTTP publication admit on claimed traffic via identity headers ? resolve ? embed Origin ? catalog `try_admit` (`APP0.3-C25` / ADR `0154`; unclaimed traffic unchanged; not Delivery rate middleware), and enriches claimed admit rejects with `Retry-After` plus `x-a3s-publication-rate-profile` / `x-a3s-publication-rate-policy-revision` (`APP0.3-C26` / ADR `0155`), and applies Edge route `headers.request_headers` before publication admit so desired-state identity stamps can claim traffic (`APP0.3-C27` / ADR `0156`; inventing Delivery rate middleware or automatic path->channel mapping is refused), and persists Gateway rate-shaping profile revisions in Postgres `gateway_rate_shaping_profiles` with hydrate-then-seed into the shared process-local catalog across api-worker/management (`APP0.3-C28` / ADR `0157`; REST catalog CRUD, Redis distributed limiter certification, and inventing intent?route identity stamps stay deferred), and contract-proves managed retained snapshot compile retains publication route intent ACL plus bound rate profiles from desired state (`APP0.3-C29` / ADR `0158`), and contract-proves managed rollout rollback publications retain the same ACL through `GatewayRolloutRollbackCompiler` (`APP0.3-C30` / ADR `0159`; inventing new rollback rails or path?channel stamps stays deferred), and first-principles production-release audit (`APP0.3-C31` / ADR `0160`), and Delivery traffic-owner claim path accepting `publication.api-blocking` / `api-streaming` availability (`APP0.3-C32` / ADR `0161`; Gateway identity stamps optional for claimed admit; refuse inventing path->channel / PublishRoute channel ownership; embed/web remain unavailable; SSE/browser and Redis/REST catalog stay deferred), and explicit APP0.3 production-scope close for internal API channels with embed/web first-principles deferral (`APP0.3-C33` / ADR `0162`; parity gate `APP0.3` `implemented`; refuse inventing browser/generated-web owners).
...
The Rust control-plane binary currently supports five roles. A role selects both
background ownership and the externally registered route set:

| Role | Responsibility |
| --- | --- |
| `all` | API, application delivery, reconcilers/workers, and integration-event relay in one process |
| `api` | REST, SSE, Management MCP, and node-control endpoints (includes published application delivery via management composition today) |
| `delivery` | Published application delivery surface; anonymous `/anonymous-delivery` + authenticated `/delivery` admit/lifecycle/observation HTTP + process-status; maintained client/CLI for Principal-bound `/delivery` (`APP0.3-C9`); readiness indicator `delivery-drain` gates new session/invocation admission while observation/close/cancel continue (`APP0.3-C8`); management OpenAPI documents `/delivery` (`APP0.3-C7`) while Delivery 404s openapi; Applications owns exact-release publication route intent + rate-policy profile refs with immutable persistence, project-authorized CQRS, management REST/OpenAPI, cloud-client/CLI, Edge-facing projection, planner-backed and MCP-reconciler Gateway snapshot ACL load with post-C18 empty-site audit (`APP0.3-C10`/`C11`/`C12`/`C13`/`C14`/`C15`/`C16`/`C17`/`C18`/`C19`; Gateway rate-profile compile/runtime/HTTP admit, durable catalog, retained snapshot, and managed rollback publication ACL through C30); no Gateway apply/PublishRoute admission or Delivery worker rate authority yet (`APP0.3-C2`/`C3`/`C5`/`C6`/`C7`/`C8`/`C9`/`C10`/`C11`/`C12`/`C13`/`C14`/`C15`/`C16`/`C17`/`C18`/`C19`) |
...
source resolver consumes only the owner query port. External byte
materialization separately crosses `IExternalSourceArchivePort`, an
Artifacts-Application contract implemented by Sources Infrastructure. Sources
alone owns checkout credentials and receipts, deterministic bounded packaging,
local paths, drift revalidation, and temporary cleanup. Artifacts receives two
non-interchangeable digests (source provenance and exact archive transport), a
size, and a stream, then alone admits the bytes to its node-artifact store.
`IBuildInputPreparer` is therefore Application orchestration rather than a
Domain service. The hosted Asset staging branch remains explicit migration
debt and must move behind an equivalent Assets owner port.

`P0.1-C5` reuses that Sources authority for trusted BuildPlan input. The
internal detection query authorizes an exact action before crossing the
Developer Workflows-owned `IBuildPlanSourceLayoutPort`. Its single Sources
adapter loads the existing accepted-revision build-input snapshot, requests the
shared public/private checkout service, and maps the checkout inventory to the
bounded layout. `GitSourceCheckout` alone defines source-inventory and
whole-tree identity; the adapter reads only fixed Asset ACL evidence, validates
a strict credential-free replay that cannot recreate missing bytes, and removes
the unique transient checkout before it returns. No provider credential,
receipt, or local path crosses into Developer
Workflows, and the archive packaging walk remains transport encoding rather
than a second source-identity mechanism. SourceRevision resolution and checkout
also share one `SourceRepositoryCredentialService`; only that Application
service restores the organization connection, validates installation authority,
issues the ephemeral repository token, and redacts provider errors.

`P0.2-C6` exposes accepted WorkloadProfile intent through the same layered
boundary. The sole Application query service depends only on the existing
revision repository interface and shared action-scoped authorization port;
current, exact, and bounded-history handlers revalidate canonical Domain state
and exact tenant scope before returning it. REST and Management MCP are thin
CQRS adapters over one response projection, while OpenAPI `1.74.0`, the
maintained client, and CLI mirror that closed contract. The acceptance path
continues to parse canonical ACL only in Domain and write only through migration
`147`'s repository transaction. This adds no owner model, parser, evaluator,
compiler, persistence, event delivery, or lifecycle authority.

`P0.3-C6` production-composes Preview Policy acceptance without adding another
owner or delivery mechanism. BuildPlan, workload-profile, and Preview Policy
handlers share one production authorization port. The sole new Developer
Workflows Infrastructure adapter reads Sources through
`ISourceSubscriptionRepository`, reuses the owner aggregate's validation,
enforces exact query identity, and emits only the consumer-owned minimal
binding. Application imports no Sources internals. The typed PostgreSQL factory
selects separate management and Relay instances of the existing migration
`153` repository through one concrete-constructor rule, preserving process-role
capabilities while keeping one policy authority.

`P0.3-C7` exposes Preview management through that same layered boundary. One
`PreviewPolicyQueryService` owns current, exact-revision, and bounded continuous
history reads through the existing policy repository interface; one separate
`PullRequestPreviewQueryService` owns the exact current behavioral read through
the existing projection repository interface. Both authorize through the same
production port before private identity validation and revalidate restored
Domain state and exact scope. REST and five Management MCP tools are thin CQRS
adapters over shared closed DTOs, while OpenAPI `1.75.0`, the maintained client,
and CLI mirror the ACL-only contract and one portable pull-request integer
bound. Architecture tests require each service/handler/registration exactly
once and reject repository, parser, concrete authorization, persistence, or
owner-model access from those public adapters. C7 adds no state, event rail,
worker, lifecycle, timer, scheduler, or cleanup authority.

Pre-acceptance GitHub discovery remains inside Sources and is intentionally not
a Developer Workflows aggregate. One `GithubSourceDiscoveryQueryService`
restores the current tenant connection through `IGithubConnectionRepository`,
applies the shared `SourceRepositoryPolicy`, and delegates bounded repository
or branch/tag pages through `IGithubSourceDiscoveryProvider`. The production
provider decorator reuses `IGithubConnectionAuthorityService` immediately
before the existing installation-token issuer calls GitHub. REST/OpenAPI
`1.76.0`, the maintained client, CLI, and two Management MCP tools are thin
query adapters over the same closed DTO projection. A discovered ref is mutable
provider information until the existing source-revision acceptance command
pins a full commit; discovery therefore creates no aggregate, table, event,
cache, lifecycle, or credential authority.

Sources publishes authenticated pull-request semantics through the separate
closed `PullRequestChangeCommittedFact`, schema
`source.pull-request-change.committed@1`. The production GitHub adapter first
authenticates and normalizes provider evidence, then the existing Sources Inbox
transaction resolves exact active Subscriptions and stores one fact per match
through the shared Outbox. Its stable opaque identity makes a Subscription's
delivery observation replayable without publishing provider delivery ID,
signature, body, or body digest. The Developer Workflows anti-corruption
projector translates only this Published Language into its local Application
port; it imports no Sources verifier, delivery entity, repository, or Inbox.
Migration `156` extends the existing Inbox and migration `157` adds only the
consumer-owned lifecycle projection and immutable local receipts. Both use the
existing Outbox Relay rather than another queue, relay, retry mechanism, or
provider lifecycle.

`P0.3-C5a` extends that same pipeline by committing one bounded
`developer.pull-request-preview.lifecycle-committed@1` fact in the Preview
mutation transaction. The immutable fact type physically belongs to Developer
Workflows `published` and imports no owner Domain model. The same projector and Relay invoke the Developer
Workflows-owned `IPreviewEnvironmentPort`; only one Infrastructure adapter
imports Projects internals and translates the exact active binding to the
existing ordinary `Environment` aggregate, idempotency, repository, and
transactional Outbox. The projector requires this port at construction, and
architecture tests confine Projects imports to that adapter while continuing
to reject another projector, publisher, Outbox repository, queue, retry loop,
or worker. Cleanup facts are intentionally not Environment deletion commands.

`P0.3-C5b/C5c` continue the pipeline through owner-specific Published
Languages rather than a cross-context orchestration service. The separate
`PullRequestPreviewSourceProjector` consumes the Developer Workflows lifecycle
fact through the same Relay and invokes a Sources-owned Application port.
Sources alone validates its Subscription, creates or adopts the ordinary
external `SourceRevision`, persists the append-only migration `159` version
receipt, and atomically emits the bounded
`source.pull-request-preview-revision.lifecycle-committed@1` fact. The existing
`BuildCandidateProjector` accepts that specialized fact and invokes the
Artifacts-owned `IPreviewBuildLifecycleProjectionPort`; migration `162` stores
the Artifacts-local version/retirement fence beside immutable optional Preview
provenance on the existing candidate. Reservation reads only that local fact
projection. Retirement locks and requests cancellation on the existing
BuildRun aggregate, and an exact receipt-bound reopen permits at most one later
same-SourceRevision attempt. No context reads a foreign table, and no new
Inbox, Relay, queue, worker, scheduler, saga, candidate authority, or build
lifecycle exists.
...
| Projects | Projects, environments, tenant boundaries, and immutable attribution-profile lineage | Current; `C0.3-PA1` verified on PostgreSQL 17. Component-only `P0.3-C5a` reuses this sole Environment aggregate/repository/idempotency/Outbox authority for active Preview handoff; it adds no parallel Environment model or cleanup lifecycle |
| Sources | External source identities, revisions, authenticated webhook deliveries, subscriptions, and immutable versioned source-build-input plus committed-pull-request Published Language | Current; aggregate-to-build projection, external checkout/package adaptation, and transient pre-acceptance provider discovery are Sources-owned. `P0.3-C3` extends the single provider Inbox through migration `156` and atomically fans exact active-Subscription-bound `source.pull-request-change.committed@1` facts through the shared Outbox without exposing private delivery evidence or creating a second delivery mechanism. `P0.3-C5b` consumes the committed Preview lifecycle through one Sources-owned port, persists an ordinary SourceRevision or cleanup/suppression receipt through migration `159`, and emits one bounded specialized fact in the same transaction and Relay. The discovery query service and provider port expose repository/branch/tag pages through OpenAPI `1.76.0` without adding durable state, accepted revision, credential, or provider-response authority |
| Developer Workflows | Bounded source-layout inspection, reviewable canonical BuildPlan/workload-profile proposals, immutable acceptance, Preview Policy revisions and durable Preview projection; accepted source/build/deployment/route/schedule authority stays with existing owners | Domain owns canonical proposal, acceptance, policy, lifecycle, events, and receipts; Application owns consumer ports and action-scoped authorization; `published` carries only aggregate-free lifecycle facts; Infrastructure adapters translate exact owner evidence. `P0.1-C1` through `C6` compose authorization-first BuildPlan detection/acceptance/reads and trusted exact-revision layout acquisition through the sole Sources and migration `146` authorities, exposed by REST/OpenAPI `1.72.0`, maintained client/CLI, and four MCP tools. `P0.2-C1` through `C6` compose canonical WorkloadProfile acceptance/reads, exact compilation, and owner adapters through the same authorization plus migration `147`, exposed by REST/OpenAPI `1.74.0`, maintained client/CLI, and four additional MCP tools. Each read family has one Application query service; public adapters only dispatch CQRS and share closed response projections. Architecture tests reject ACL parser, repository, authorization, owner-model, checkout, lifecycle, or delivery bypass. `P0.3-C1` through `C7` retain their owner adapters and shared Outbox Relay, then expose policy acceptance/current/history/exact reads plus one exact behavioral Preview read through REST/OpenAPI `1.75.0`, maintained client/CLI, and five MCP tools over two narrow Application query services. Sources-owned pre-acceptance discovery is composed separately; live GitHub evidence, owner lifecycle/cleanup handoffs, monorepos, imports, retained PostgreSQL Preview cross-surface evidence, and retained WorkloadProfile certification remain open |
| Assets | Agent, MCP, and Skill identities, hosted Git, immutable release lifecycle, Agent deployment, and Skill-to-Agent-Workload release binding | `A0.1` and `A0.2` verified; `A0.3` through `A0.5` implemented. A0.5 has a CI REST/PostgreSQL/local-object publication gate plus real Box gates retaining typed Skill-bundle hydration and complete Workload bind/rebind/unbind/rollback evidence, including exact-byte, read-only mount/write-rejection, revision-history, content-addressed-download, and Runtime-remove cleanup checks. Retained external-provider evidence remains open |
| Artifacts | Immutable build candidates, sole BuildRun lifecycle, admitted bytes, receipts, evidence, and retention | Current; Domain consumes Sources only through published language, external staging crosses an Application port, and hosted Asset staging plus public Infrastructure remain migration debt. Component-only `P0.2-C3c` publishes an aggregate-free terminal successful external-source BuildRun value through one owner query for consumer anti-corruption adapters, with no BuildPlan or added lifecycle. Component-only `P0.3-C5c` consumes the exact Sources Preview-revision lifecycle fact through the existing projector, persists migration `162`'s append-only version/retirement fence, admits only the latest active candidate, atomically cancels the existing retired BuildRun, and permits only one exact receipt-bound same-revision retry without another queue or lifecycle |
...
| Files | User upload sessions, metadata, scan/quota/retention state, and typed immutable-object references | `K0.1-C1/C2` implement one UserFile aggregate, canonical ACL and exact receipts, one metadata/quota repository port, one streaming object port over the shared immutable-object client, migration `170` atomic persistence/audit/Outbox/idempotency, authorization-first CQRS, REST/OpenAPI, client/CLI, and Management MCP tools. `K0.1-C2b1` adds authorized public `PUT .../content` (OpenAPI `1.89.0`) over that same upload command with local object-store tests; `K0.1-C2b2` adds admitted-only public `GET .../content` (OpenAPI `1.90.0`) over the streaming object port; `K0.1-C2b3` adds metadata-only public `POST .../scan` admit/reject decisions (OpenAPI `1.91.0`) and a sixth Management MCP tool; `K0.1-C2b4` adds metadata-only public `POST .../expire` for awaiting uploads (OpenAPI `1.92.0`) and a seventh Management MCP tool. The retained [PostgreSQL 17 H0 persistence step](https://github.com/A3S-Lab/Cloud/actions/runs/33159659047/job/98810769471) verifies rollback, concurrent organization-quota serialization, exact lifecycle replay, quota release, and atomic side effects through the production repository ports. Live MinIO/S3 provider wiring, live scanner execution, and live object cleanup execution remain unavailable; Knowledge remains unavailable; bytes are not Build Artifacts |
| Automations | Schedule, webhook, plugin-event, and source-event definitions that create exact-release invocations with deduplication, filtering, concurrency, and misfire policy | Component-only `AUT0.1` freezes canonical definition/revision/invocation ACLs, exact ApplicationRelease/WorkflowRevision/Task targets, policy-derived deduplication, authorization snapshots, redacted Audit, and Outbox identities in `a3s-cloud-contracts`; `AUT0.3-C1` adds a bounded timezone/Cron calendar calculator that returns UTC instants without owning scheduler state, `AUT0.3-C2` applies the bounded misfire grace/coalescing policy to an ordered window without owning a cursor or invocation, `AUT0.3-C3` returns the capacity decision without mutating a counter or executing queue/cancellation, `AUT0.3-C4` composes an exclusive cursor window while returning explicit cursor progress for a future atomic owner update, `AUT0.3-C5` applies an exact half-open lease fence before and after evaluation without owning its durable row, `AUT0.3-C6` persists only the exact revision digest, cursor, and row-locked lease fence, `AUT0.3-C7` binds a selected due timestamp to the exact revision and policy-derived deduplication key without admitting or persisting an invocation, `AUT0.3-C8` composes those boundaries into one bounded lease-guarded dispatch service with stable occurrence identities, exact envelopes, idempotent admission, cursor commit, and fenced lease release, and `AUT0.3-C9` adds the P0 anti-corruption binding for exact Task profile/revision UUIDs, immutable profile digest, and canonical schedule policy beside the existing Execution template; `AUT0.3-C10` adds the application admission port, local adapter, migration `187`, PostgreSQL adapter, and `AutomationsDispatchServices` composition that validate exact envelopes, reject invocation/deduplication drift, and atomically emit first-admission Audit/Outbox facts without owning a timer, queue, target, or mutable revision lookup; PostgreSQL webhook delivery admission also enters this canonical transaction, so it does not maintain a second invocation Outbox writer. `AUT0.4-C1` validates the canonical digest-bound `AutomationNormalizedEventV1`, binds it to the exact plugin/source subscription and revision, and exposes an application port for exact `filter_digest` evaluation; `AUT0.4-C2` adds bounded deterministic fan-out over already selected exact revisions, rejecting duplicate candidate identities and returning only filter-matching envelopes; `AUT0.4-C3` adds one strict durable-ack A3S Event consumer for already normalized events, ACKing malformed/handled messages and deferring handler failures to provider redelivery; `AUT0.4-C4` composes an injected candidate provider, deterministic fan-out, and idempotent invocation-admission port while preserving external storage and target ownership; Sources/Use retain provider normalization, Automations retains filter decisions, and neither candidate storage, filter publication, event publication, queue, nor target execution is duplicated here; `AUT0.2-C1/C2` add the exact-revision endpoint/admission boundary, `C3` adds migration `183` and the PostgreSQL restore/immutability boundary, `C4` adds exact-version Secrets-backed HMAC verification over the captured raw body, `C5` adds a digest-bound self-contained JSON Schema evaluator with external retrieval disabled, `C6` adds an exact-digest registry port/adapter without taking registry ownership, and `C7` binds a captured endpoint request to one exact revision with deterministic webhook origin/deduplication without owning transport or persistence. Live PostgreSQL recovery evidence, production registry wiring, timer/scheduler/event ownership, Gateway delivery, and public availability remain later `AUT0` gates. Flow timers remain scoped to existing runs and P0 scheduled Task profiles adapt to this one invocation authority |
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions and exact revocation facts, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, durable exact-attempt fencing/recovery, immutable terminal execution evidence and recovery conclusions, one Workflow exact-attempt adapter, an immutable per-step retry budget, and typed immutable response objects | Component-only `AUT0.5-C1` execution plus verified `C2`-`C6` foundations are implemented; `C7` exposes the same profile/revision CQRS through REST/OpenAPI `1.36.0`, the maintained client, CLI, and six Management MCP tools. `C8` binds exact WorkflowRun/plan/step-attempt and Connector profile/revision/digest authority to the same C6 execution/evidence path; `C9` freezes its attempt budget and fallback delay; `C10` writes an accepted bounded response through the sole shared immutable-object authority before terminal evidence and exposes only its exact reference, digest, and length to WorkflowRun v6; `C11` authorizes exact transient reads only after matching accepted terminal evidence and revalidating the object; `C12` serializes immutable exact revision revocation with C6 dispatch admission and exposes the fact through REST/OpenAPI `1.65.0`; and `C13` atomically closes an expired dispatch only as audited body-free `Indeterminate` evidence through migration `155`, REST/OpenAPI `1.66.0`, and the maintained client. WorkflowRun and every other C6 consumer continue to receive that terminal result as indeterminate, so replay cannot call, retry, cancel, or fabricate a provider outcome. WorkflowRun v8 consumes the C8/C10/C11 path through a no-retry Flow step and persists only strict schema-validated bounded JSON as ordinary typed node output. Plan v5/Run v9 routes closed provider and response-validation failures through the exact descriptor edge as bounded v2 values while preserving the failed source projection; v8 remains fail closed without a route, v7 retains default-output semantics, v6 remains reference-only, and v5 remains digest-only. `C0.3-N2b` composes the first NATS A3S Event consumer with C6, while `N2c`-`N2e` retain Notification delivery decisions. General provider wiring, retained external-provider/recovery evidence, and product availability remain planned; callers never create direct HTTP or object-storage clients |
...
| Agents | Conversations, heterogeneous-provider Agent executions, semantic events, approvals, checkpoints, forks, and trajectories | `A1.1` implemented; the `A1.2` native Code provider, including run-scoped cancellation, retention-gap rotation, and same-generation process recovery, is verified by retained clean Linux PostgreSQL 17 and real Box Runtime process-death evidence while consuming exact published Code Core `8.0.1` and Flow `1.1.0`; component-level `A1.3` adds closed REST/client/CLI provider selection, immutable profile persistence, exact Flow recovery, fail-closed Node routing, and durable common event delivery for Code plus the non-Code reference provider. Component-level `A1.4` adds the fail-closed immutable invocation profile, digest-bound provider identity, digest-only Tool semantics, and shared audit correlation. Component-level `A1.5` adds durable approval checkpoints and exact provider-neutral resume. Component-level `A1.6` adds shared immutable logical trajectory snapshots, exact projections and Runtime telemetry, immutable fork lineage, provider-neutral fork materialization, migrations `168`-`169`, S3-only grace-delayed orphan reconciliation through the shared object client, and REST/OpenAPI `1.73.0`; retained PostgreSQL 17 and real Box evidence verifies provider restart handling, approval/replay/audit outcomes, checkpoint adoption, fork replay, and fenced orphan cleanup through a process-shared object authority, while retained checksum-pinned MinIO evidence verifies production S3-client inventory and deletion semantics. Real-provider fork execution, external HTTPS S3-compatible evidence, production model/Tool binding producers, any additional independent MCP binding, and provider/Box private checkpoint certification remain open |
...
Historic Deployment v1-v4, placement-group v1-v2, and WorkflowRun v1-v24
...
exact Application variable/Answer paths plus the closed Agent profiles are
accepted. Descriptor-bearing finite Execution requires the exact
`executions.finite` identity and semantic profile; `execution.code` cannot alias
that port. Descriptor-bearing Agent dispatch requires `agent.classic` or
`agent.release`, Agents ownership, one non-nil immutable `AgentRelease`, and
`agent.execute`; semantic-free Agent and caller-named aliases remain fenced.
Unwired MCP, model, Tool, and Memory steps fail before persistence with or
without caller descriptors. Exact Applications-generated Agent presets use the
component runtime path, while model presets remain deferred composition
...
New Operation histories also pin runtime build `a3s-cloud-workflows@27` and
...
existing `step_created` events, and runtime builds `@1`-`@26` remain explicit
...
Workflow step projections reuse their existing bounded evidence-reference
field for owning-context correlations. Verified terminal finite Execution
resumes project the exact child Execution and Operation URNs. Exact Agent steps
create or adopt one Agents-owned conversation and execution, link its Agent Flow
operation, and resume only from a matching terminal semantic event. Their
completed and cancelled projections retain exact conversation, Agent execution,
and Operation URNs; successful output also retains the immutable provider
profile and provider-run identity. When an exact Agent descriptor selects its
`error` output, Plan v12/Run v25 routes only redacted dispatch-rejected,
execution-failed, or execution-cancelled data while preserving the source's
failed projection, selected handle, and exact child evidence. Verified received
Connector observations project exact attempt URNs, with repeated observations
deduplicated and distinct retries retained. The array is canonical, sorted,
limited to 32 entries, and reconstructed solely from immutable run authority
plus Flow history. Dispatch rejection without owning-context evidence remains
empty. The references contain no provider body and grant no owner-resource
access, and no second evidence store or historical backfill is introduced.
...
APP enterprise claim path `enterprise.ha-disaster-recovery` (H0.5-C1 / ADR `0180`) freezes those Flow v2 workflow identities as the production disaster-recovery surface; Workloads autoscaling consoles and BYOK/`S0` remain deferred. H0.5-C2 / ADR `0181` then moves parity gate `H0.5` to `implemented` for that claimable APP enterprise inventory without inventing deferred autoscaling / HA-install / `S0` surfaces.
...
DAG edge. An exact Application conversation-variable Service error port emits
Plan v6; Application-composed Run v14 routes only deterministic terminal owner
rejections as redacted `cloud.workflow.step-failure.v3` data and leaves
transient or internal failures unresolved. Plans v1-v5 and Run inputs v1-v13
retain their canonical bytes and replay behavior. An exact Application Answer
Output error port emits Plan v7; Application-composed Run v15 routes the same
...
An exact Workflow-local Branch error port emits Plan v10; Run v18 executes the
deterministic selector evaluation once without retry, routes fixed redacted
`cloud.workflow.step-failure.v7` data, and leaves the source Branch failed with
the exact descriptor handle. Ordinary If / Else handles remain business routes
and cannot alias the descriptor error handle. Plans v1-v9 and Run inputs v1-v17
retain their canonical bytes and replay behavior.
An exact Workflow-owned Iteration or Loop error port emits Plan v11; Run v19
records validated child failure or immutable region-policy exhaustion through
one durable no-retry local materializer, routes fixed redacted
`cloud.workflow.step-failure.v8` data, and leaves the source Subworkflow failed
with the exact descriptor handle. Resume-authority drift remains
non-deterministic and cannot enter the ordinary DAG. Constraint-only migration
`148` admits that handle only on a failed Subworkflow projection. Plans v1-v10
and Run inputs v1-v18 retain their canonical bytes and replay behavior.
An exact Workflow-owned `workflow.variable-aggregate` Transform uses the
versioned `cloud.workflow.configuration.variable-aggregate.v1` ACL payload.
Publication proves bounded concrete groups, contiguous candidate priority, and
exact optional direct reads plus input/output descriptor and schema coverage.
Graphs containing it retain their existing Plan v2-v11 schema and emit
WorkflowRun input/runtime/Flow v20. Runtime consumes only authoritative typed
projection, skips unavailable or null candidates, selects the first value by
ordinal, validates its frozen type, and produces the exact simple or grouped
output shape. Constraint-only migration `149` widens the existing payload
schema check for this configuration and the already supported policy v2/v3
schemas without adding a table or column. Earlier WorkflowRun inputs retain
their bytes and behavior.
An exact Workflow-owned `workflow.list-operator` Transform uses the versioned
`cloud.workflow.configuration.list-operator.v1` ACL payload. Publication proves
one typed array source, bounded contiguous filter conditions, optional
one-based extraction, optional typed ordering and limit, one required direct
source read, optional direct operation reads, and exact input/output descriptor
and schema coverage. Graphs
containing it retain Plan v2-v11 and emit WorkflowRun input/runtime/Flow v21.
Runtime consumes only the authoritative typed projection, validates every
object, string, number, or boolean item, and applies filter, extract, order,
then limit in that fixed order. Empty input succeeds before dynamic operands
are resolved. Constraint-only migration `151` widens only the existing payload
schema check; Run inputs v1-v20 keep their bytes and behavior.
An Iteration whose immutable policy declares `maximumConcurrency > 1` emits
WorkflowRun input/runtime/Flow v22. Runtime partitions frames into bounded
ordinal waves, records one authority-bound wave Hook, starts or adopts each
ordinary child WorkflowRun concurrently, and resumes only after every child is
terminal and linked. Failure reduction remains ordinal-stable, parent
cancellation or timeout cancels and awaits all in-flight siblings, historical
v3-v21 Iteration replay remains serial, and Loop remains sequential.
...
v17 adds only descriptor-bound Workflow-local Output failure selection, and
v18 adds only descriptor-bound Workflow-local Branch failure selection, and
v19 adds only descriptor-bound composite-region failure selection, v20 adds
exact typed Variable Aggregation, v21 adds exact typed List Operator execution,
and v22 adds bounded-parallel Iteration waves while preserving every admitted
earlier semantic. A step
...
REST/OpenAPI `1.61.0` adds the returned
`cloud.workflow.configuration.variable-aggregate.v1` payload-schema semantic
and the maintained client union without adding a route or JSON property.
...
Flow snapshot/history pair, verifies the immutable WorkflowRun/Operation/Flow
authority, compares observed and projected sequences, and derives bounded
step/event/wait/retry/recovery/child/evidence statistics. It returns fixed
redaction-safe diagnostics and at most 256 exact evidence references with an
explicit truncation flag. PostgreSQL Workflow projections and A3S Flow remain
the only state sources; there is no diagnostics table, mutable counter,
metrics authority, copied evidence body, or parallel history.
...
proves the exact owner binding, descriptor failure contract,
declared edge, failed status, and selected handle.

Migration `143` likewise changes no ownership or runtime mechanism. It widens
that existing selected-handle constraint only for failed Output projections so
Run v15 can retain exact Application Answer routing evidence. Completed Output
...
runtime v3-v21 drives deterministic frames and ordinal reduction through one
exact Flow Hook per frame and deterministic ordinary child WorkflowRuns. The
coordinator creates or adopts the existing Goal/Plan/Run/Operation/Outbox path,
links exact child Flow identity, resumes digest-bound results, and
cancels/awaits children before parent termination. Runtime v22 uses one exact
Hook per bounded Iteration wave, starts or adopts the wave's children
concurrently, waits until all are terminal and linked, and reduces by ordinal.
Historical Iteration replay remains serial and Loop remains sequential. Exact
Applications Answer and variable
snapshot/CAS adapters are implemented through the sole Applications effect
port; unsupported or drifted owner shapes remain fail closed. No variable/region table, cache, event log, worker,
...
Runtime v14 applies the ordinary descriptor-bound failure rule to the exact
Application conversation-variable assignment Service. Its existing write Hook
resumes only `Invalid`, `NotFound`, `Conflict`, and `Forbidden` owner results as
classification-only evidence. Flow emits stable redacted failure v3, selects
the `error` handle, keeps the source Service failed, and may complete the
reachable branch. `Unavailable` and `Internal` keep the Hook active for the
existing idempotent retry path. No raw owner error, migration, public OpenAPI
change, second variable authority, or retry mechanism is introduced.

Runtime v15 applies that rule to the exact `application.answer` Output for both
root and composite-frame execution. The existing Answer Hook resumes only
`Invalid`, `NotFound`, `Conflict`, and `Forbidden` owner results as
classification-only root or frame authority. Flow emits stable redacted
failure v4, selects `error`, keeps the Answer Output failed, and may complete
the reachable branch. `Unavailable` and `Internal` keep the Hook active; child
frames retain the root effect identity and emit no child lifecycle effect. No
raw owner error, public OpenAPI change, second message authority, or retry
...
`agent-checkpoints`, `connector-responses`, and `plugin-trust-roots`
...
Cloud does not ship or select an S3 server. Its sole low-level remote adapter
is Rust `object_store` with the `aws` feature, composed through
`AmazonS3Builder` against AWS S3 or an operator-selected compatible endpoint.
The checksum-pinned MinIO process in CI is only a protocol fixture for that
adapter; it is not a production dependency or an implied provider choice. The
boundary is object storage, not a POSIX/FUSE filesystem mount.
...
and adds closed REST/client/CLI selection, creation-time profile persistence,
    exact Flow registry recovery, fail-closed Node routing, and durable event-page
    delivery for the deterministic non-Code reference Harness. A retained
    ...
    unsupported-Recovery terminal fallback with zero Recover commands, and
    cleanup. Recovery capability is checked again when observing an already
    persisted recovery successor, so an invalid pre-upgrade successor fails
    terminally without binding rotation or command enqueue and repeated
    observation is idempotent.
Component-level `A1.4` now persists one closed immutable invocation profile
before dispatch, binds its canonical digest into provider run identity, and
projects digest-only Tool request/result evidence into the semantic sequence
and shared audit transaction. Component-level `A1.5` adds durable
approval-required Tool checkpoints and exact provider-neutral resume.
Component-level `A1.6` stores bounded canonical logical trajectory snapshots
through the shared immutable-object client, persists exact digest/telemetry
projections and fork lineage, and verifies the parent snapshot before a new
execution is dispatched. Migration `169` adds short-lived PostgreSQL capture,
inventory-grace, and cleanup fences. One S3-only supervised worker reuses the
same object client, claims expired capture or observation records, and removes
only the exact object protected by its cleanup lease; malformed keys are
reported and retained. Retained PostgreSQL 17 process-death evidence verifies
logical checkpoint adoption, fork replay, and fenced orphan cleanup through a
    process-shared object authority. The same retained real Box/PostgreSQL gate
    verifies approved, denied, expired, cancelled, and provider-restart
    fail-closed approval outcomes, exact Resume replay, and digest-only audit.
    Production model and Tool binding producers, any additional independent MCP
    binding, real-provider fork execution, external HTTPS S3-compatible
    reconciliation evidence, and provider/Box private checkpoint certification
    remain open. A retained
checksum-pinned MinIO gate already verifies the production S3 client's
inventory, grace, exact cleanup lease, idempotent removal, and empty namespace
cleanup semantics.
...
The current component-level `A1.3` foundation adds:
...
The current component-level `A1.4` extension adds:

- the closed immutable `HarnessInvocationProfile` and digest-bound provider
  run identity for the currently resolved Agent, instructions,
  environment/security policy, Skill, MCP, workspace, Secret-reference, and
  provider authorities; and
- bounded digest-only Tool request/result semantic events with exact Tool
  binding admission and same-transaction shared audit correlation.

The retained PostgreSQL/reference-provider gate verifies exact Tool request
replay, shared digest-only audit correlation, and payload exclusion. Production
model and Tool binding producers and any additional independent MCP binding are
still required before `A1.4` product completion. Later A1 sub-gates add:
...
Cancellation cleanup follows the same authority. An exact policy-v4 Connector
binding emits WorkflowRun/Flow v23 and maps one completed source response to one
downstream exact Connector compensation target. Flow 1.1 owns the cancellation
request, cleanup replay, stable Hook identity, and final `Cancelled` transition;
Cloud owns typed Connector admission and reverse Plan ordering. Indeterminate
attempts fail closed, and this component slice does not claim provider-side
cancellation, arbitrary compensation, or public Workflow availability.
...
Both the public admission command and its durable internal invocation command
normalize timeouts through `IApplicationWorkflowRunPort`. The sole production
adapter delegates to Workflow's canonical timeout rule; Applications persists
the admitted seconds but owns neither Workflow's default nor its maximum. Its
Domain validates only local stored-authority and deadline invariants, and an
architecture fitness test prevents direct Workflow imports or a copied bound
from returning to the Applications Application/Domain layers. Public schemas
reference the Workflow-owned constants, and migration `171` removes migration
`127`'s historical database copy without rewriting authority rows.
...
now resolve the same exact Flow `1.1.0` release, closing the former Flow
