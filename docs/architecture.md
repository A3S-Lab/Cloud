...
verification. The build operation runtime does not yet join checkout and
execution or run the client through a Runtime Task, so production worker
enablement remains gated on that implementation. Environment-owned repository
...
tenant ownership, and optimistic conflicts. The production worker deliberately
...
This is deliberately a local-context engine boundary, not the complete G0
build operation. It binds but does not independently recompute the checkout
content digest, so the Build Flow must replay the secure checkout immediately
