...
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, and Preview intent; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1` implement canonical detection, BuildPlan/workload-profile acceptance, and the pure Preview lifecycle. Domain owns local proposal values and enters Sources only through Published Language; Application owns authorization, verified-build-outcome, and Service/Task admission ports, returns correlation-bound receipts, and imports no foreign owner model. Production owner adapters/composition and Projects/Edge handoffs, persistence/dispatch for Preview, monorepos, and imports remain unavailable |
