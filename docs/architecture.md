...
| Durable Cells | Durable Cell application identity, immutable revision/profile, retention intent, and exact Workload/S0/Gateway deployment projection; never individual Cell state or ownership | `CELL0.1-C1/C2/C3` is implemented; component-only `CELL0.2-C1` correlates the current revision with one exact S0 namespace/credential generation and policy digests without plaintext or lifecycle ownership. Recovery/provider/orchestration gates and `CELL0.5` availability remain planned |
| Data | Managed databases, immutable-object and volume provider policy, distributed volumes, backup, restore, retention, and writer fencing | Component-only `S0.1-C1` exposes the first conditional-object namespace port, opaque version tokens, destructive cleanup-verified CAS probe, and digest-locked exact Secret-version credential binding over the sole shared object client. Production provider, persistence, volume/database, backup/restore, retention, and deletion gates remain planned |
