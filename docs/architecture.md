...
input. The separate built-in discovery projection fail-closed composes the
...
WorkflowRevision registry snapshot. Authorized runtime inspection, digest-only
defaults, composite/Application variable adapters, and public node availability
remain open.
