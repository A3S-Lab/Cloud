...
| Automations | Schedule, webhook, plugin-event, and source-event definitions that create exact-release invocations with deduplication, filtering, concurrency, and misfire policy | Component-only `AUT0.1` freezes canonical definition/revision/invocation ACLs, exact ApplicationRelease/WorkflowRevision/Task targets, policy-derived deduplication, authorization snapshots, redacted Audit, and Outbox identities in `a3s-cloud-contracts`; `AUT0.2-C1/C2` add the exact-revision endpoint/admission boundary, `C3` adds migration `183` and the PostgreSQL restore/immutability boundary, and `C4` adds exact-version Secrets-backed HMAC verification over the captured raw body. Live PostgreSQL recovery evidence, schema evaluation, scheduler/event consumers, Gateway delivery, and public availability remain later `AUT0` gates. Flow timers remain scoped to existing runs and P0 scheduled Task profiles adapt to this one invocation authority |
