...
| Assets | Agent, MCP, and Skill identities, hosted Git, immutable release lifecycle, Agent deployment, and Skill-to-Agent-Workload release binding | `A0.1` and `A0.2` verified; `A0.3` through `A0.5` implemented. A0.5 now has a CI REST/PostgreSQL/local-object gate retaining exact Skill archive, receipt, replay, yank, and cleanup evidence; retained external-provider and Box lifecycle evidence remains open |
