...
| Sources | External source identities, revisions, webhooks, subscriptions, and the immutable versioned source-build-input published language | Current; aggregate-to-build projection plus external checkout/package adaptation are Sources-owned, and Artifacts Domain is contract-only |
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, Preview Policy revisions, and Preview intent; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1/C2` implement canonical detection, BuildPlan/workload-profile acceptance, the pure Preview lifecycle, and authorization-first exact-Subscription-bound canonical policy revisions through migration `153`. Domain owns local proposal and policy values and enters Sources only through Published Language; Application owns authorization, source-subscription query, verified-build-outcome, and Service/Task admission ports, returns bounded owner evidence, and imports no foreign owner model. Production owner adapters/composition and Projects/Edge handoffs, fact dispatch and individual Preview persistence, monorepos, and imports remain unavailable |
