...
same-generation provider-process recovery are also implemented locally;
dependency publication and clean Linux PostgreSQL/Runtime recovery
verification remain. Provider-neutral `A1.3` and semantic/governance `A1.4`
through `A1.6` remain planned.
