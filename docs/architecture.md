...
| Durable Cells | Durable Cell application identity, immutable revision/profile, retention intent, and exact Workload/S0/Gateway deployment projection; never individual Cell state or ownership | `CELL0.1-C1/C2/C3` is implemented; component-only `CELL0.2-C1/C2` correlate the current revision with one exact S0 namespace/credential generation and verify S0-owned recovery, retention, restore, and deletion contracts without plaintext or lifecycle ownership. Provider execution/orchestration gates and `CELL0.5` availability remain planned |
| Data | Managed databases, immutable-object and volume provider policy, distributed volumes, backup, restore, retention, and writer fencing | Component-only `S0.1-C1/C2` expose the sole-client conditional-object namespace port/probe, digest-locked exact Secret credential path, sealed recovery lineage, isolated restore evidence, bounded retention policy, and fenced grace-delayed deletion contracts. Production provider, persistence, volume/database, executable backup/restore/deletion, and retained certification evidence remain planned |
