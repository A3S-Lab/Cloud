...
| Applications | Application identities, immutable releases, six authoring/delivery projections including classic/New Agent distinction, sessions, messages/variants, conversation variables, toolkit/feedback/annotation/publication policy, and managed application delivery | Component-only `APP0.1-C1` implements strong identities, canonical immutable release ACL, exact Workflow revision/digest evidence, and release/head invariants without graph or runtime state. Persistence, authorization, interfaces, sessions, delivery, and later APP0 gates remain unavailable |
