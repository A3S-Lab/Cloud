...
| Notifications | Deterministic personal in-app projections of curated committed Outbox facts, exact-recipient and Resource Grant filtering, idempotent read state, immutable personal outbound-subscription A3S ACLs, transactional delivery authorization facts, side-effect-free signed-webhook/Slack-compatible request builders, a NATS durable/manual-ack consumer composed with the fenced Connector service, and monotonic logical terminal receipts | `C0.3-N1` is verified; component-only `C0.3-N2a`, `N2b`, and `N2c` are implemented. Bounded rate policy, SMTP, alert policy, supported management surfaces, and retained production evidence remain unavailable |
...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, durable exact-attempt fencing/recovery, and immutable terminal execution evidence | Component-only `AUT0.5-C1` execution plus verified `C2`-`C6` foundations are implemented; `C0.3-N2b` composes the first NATS A3S Event consumer with C6 and `N2c` places Notification-owned authorization and receipt persistence around it without another retry rail. General provider wiring, Workflow ports, supported non-Web management surfaces, revocation/recovery operations, and product availability remain planned; callers never create direct HTTP clients |
