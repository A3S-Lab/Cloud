...
| Deno celld-style named Durable Objects, one SQLite database per object, alarms, hibernatable WebSockets, inactive residency, object-store CAS ownership, write durability, and node handoff | `CELL0` owns immutable Durable Cell application intent and projects one ordinary Workload Service fleet; the selected Cell provider owns per-Cell execution/ownership/state in one S0 namespace | `CELL0.1-C1/C2/C3` and component-only `CELL0.2-C1/C2/C3` implement the application and S0 contracts/gate. `CELL0.3-C1/C2` bind the exact digest-pinned provider to the existing Workload Runtime Service projector/Fleet apply receipt, add one Cell-name-free operator observation on the same Fleet journal, and reuse Runtime stop/remove receipts. Component-only `C3` adds an attested celld v0.2.1 real-Box runtime-only gate with an explicit `storage=not-certified` scope. Its retained pass, recovery/deletion execution, orchestration, single-node `CELL0.5`, multi-node `CELL0.6`, and compatibility `CELL0.7` remain unavailable | celld control topology, raw configuration/deploy authority, public operator API, per-Cell Cloud rows, new Runtime class, Gateway owner lookup, or blanket Cloudflare compatibility |
...
| Durable Cells | Durable Cell application identity, immutable revision/profile, retention intent, and exact Workload/S0/Gateway deployment projection; never individual Cell state or ownership | `CELL0.1-C1/C2/C3` and component-only `CELL0.2-C1/C2/C3` correlate the current revision with exact S0 contracts and the shared provider gate. `CELL0.3-C1/C2` add only the exact provider/ordinary Runtime Service/Fleet receipt binding, one anonymous operator observation, and validation of existing stop/remove evidence. Component-only `C3` pins one upstream release and composes those contracts with the existing real Box workflow; it creates no provider runner or evidence store. `CELL0.4-C1` adds only two relational authorities through migration `116`: application heads and immutable canonical-ACL revisions, with BuildRun scope, sequential revision/state fences, and the existing A3S ORM/idempotency/Outbox/audit path. `C2` composes that authority with existing environment grants, BuildRun reads, and command/query buses; authorization precedes replay and denied/missing resources share the same boundary. Deployment rows, orchestration, interfaces, retained evidence, and `CELL0.5` availability remain planned |
