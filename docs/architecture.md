...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, and immutable terminal execution evidence | Component-only `AUT0.5-C1` execution, verified `C2` ACL-native persistence, verified `C3` authorized application/materialization, verified `C4` public-Internet DNS/SSRF enforcement, and local `C5` exact-attempt evidence persistence plus authorized bounded reads are implemented. Durable attempt reservation/fencing, internal execution authorization, indeterminate-outcome recovery, Workflow ports, provider wiring, supported management surfaces, and product availability remain planned; callers never create direct HTTP clients |
