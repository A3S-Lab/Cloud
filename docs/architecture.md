...
| Applications | Application identities, immutable releases, six authoring/delivery projections including classic/New Agent distinction, sessions, messages/variants, conversation variables, toolkit/feedback/annotation/publication policy, and managed application delivery | Component-only `APP0.1-C1/C2` implements strong identities, canonical immutable release ACL, exact Workflow revision/digest evidence, release/head invariants, and migration `124` PostgreSQL/A3S ORM persistence with atomic idempotency, audit, and Outbox facts without graph or runtime state. Authorization, interfaces, sessions, delivery, and later APP0 gates remain unavailable |
