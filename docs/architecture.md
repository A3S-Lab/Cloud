...
| Personal in-app notification projection | Notifications projects a curated closed set of committed transactional Outbox facts for one exact recipient Principal, then applies the shared Resource Grant evaluator | Business desired state, source-fact mutation, a second event rail, delivery queue, provider/template/subscription authority, scheduler, or presentation-local inbox |
...
trigger subscriptions. Migration `103` persists the revision-owned semantic
contract set, Plan v2 pins exact descriptors and variable semantics, and
WorkflowRun input/runtime/Flow v2 reconstructs supported values from immutable
input plus existing Flow history. A step with explicit reads can consume only
its typed `current` projection; a step without reads retains legacy dependency
input. Built-in catalog discovery, authorized
runtime inspection, digest-only defaults, composite/Application variable
adapters, and public node availability remain open.
