...
| Developer Workflows | Bounded source-layout inspection, reviewable canonical BuildPlan/workload-profile proposals, immutable acceptance, Preview Policy revisions and durable Preview projection; accepted source/build/deployment/route/schedule authority stays with existing owners | Domain owns canonical proposal, acceptance, policy, lifecycle, events, and receipts; Application owns consumer ports and action-scoped authorization; `published` carries only aggregate-free lifecycle facts; Infrastructure adapters translate exact owner evidence. `P0.1-C1` through `C5` compose deterministic authorization-first BuildPlan detection/acceptance and trusted exact-revision layout acquisition through the sole Sources checkout/credential/inventory authorities and migration `146` repository transaction. `P0.1-C6` exposes detection, acceptance, and immutable reads through REST/OpenAPI `1.72.0`, maintained client/CLI, and four Management MCP tools: all dispatch the same CQRS handlers, one Application query service authorizes reads, and controller/OpenAPI consume one route contract. No adapter parses ACL, reads the repository, evaluates grants, or creates BuildRun/Workload/Route/scheduler state. P0.2 compilation/acceptance and P0.3 Preview foundations retain their owner anti-corruption adapters and shared Outbox Relay. Pre-acceptance discovery, workload-profile/Preview public surfaces, owner lifecycle/cleanup handoffs, monorepos, and imports remain open |
