...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, and execution evidence | Component-only `AUT0.5-C1` execution and `AUT0.5-C2` ACL-native profile/revision persistence foundations are implemented. Authorized application surfaces, just-in-time Secret materialization, production egress/SSRF enforcement, evidence, revocation/recovery, Workflow ports, and product availability remain planned; callers never create direct HTTP clients |
