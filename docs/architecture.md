...
| Developer Workflows | Bounded source-layout inspection, versioned reviewable BuildPlan/workload-profile proposals, immutable acceptance decisions, Preview Policy revisions, durable Preview lifecycle projection, and owner-facing handoff intent; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1/C2`, `P0.2-C1/C2`, and `P0.3-C1/C2` define canonical detection/acceptance and Preview policy/lifecycle values. `P0.1-C3/C4` production-compose one deterministic detection query and one authorization-first BuildPlan acceptance command through one consumer Infrastructure adapter over Identity Membership/Resource Grant interfaces, Identity's sole `ResourceAccessEvaluator`, exact Projects Environment lookup, the existing Sources evidence adapter, and the sole migration `146` repository/Outbox transaction. `P0.1-C5` makes detection authorization-first and acquires one exact accepted-revision layout through a Developer Workflows-owned port, the existing Sources build-input query, one shared public/private checkout service, and the sole `GitSourceCheckout` source-inventory/digest plus replay/cleanup authority; credentials, receipts, and local paths do not cross the boundary. `P0.2-C3a/C3b/C3c` confine Workloads, Executions, and Artifacts translation/query contracts to one consumer Infrastructure adapter per owner; `P0.2-C4` composes their exact accepted-profile read path without owner lifecycle writes. `P0.2-C5` composes workload-profile acceptance through the exact same production authorization port and the sole migration `147` repository transaction. `P0.3-C6` composes Preview Policy acceptance through that shared port, one exact Sources subscription adapter, and role-scoped instances of the sole migration `153` repository. Sources produces exact committed PR facts through migration `156`; `P0.3-C4/C5a/C5b/C5c` reuse the existing Outbox Relay while projecting CAS-safe Preview state and handing exact versions to ordinary Projects Environment, Sources SourceRevision, and Artifacts BuildRun authorities. Domain owns local proposal, policy, lifecycle, event generation, and receipt values; `published` owns the aggregate-free lifecycle wire contract; Application owns consumer ports and imports no foreign owner model. Pre-acceptance source discovery, Workloads/Executions lifecycle, scheduling, Edge/Operations handoffs, Environment/Preview expiry cleanup, public interfaces, monorepos, and imports remain unavailable |
...
and shared audit transaction. Production model and Tool binding producers, any
additional independent MCP binding, retained PostgreSQL/real-provider recovery
and audit evidence, and governance `A1.5`/`A1.6` remain open.
