...
| Automations | Schedule, webhook, plugin-event, and source-event definitions that create exact-release invocations with deduplication, filtering, concurrency, and misfire policy | Component-only `AUT0.1` freezes canonical definition/revision/invocation ACLs, exact ApplicationRelease/WorkflowRevision/Task targets, policy-derived deduplication, authorization snapshots, redacted Audit, and Outbox identities in `a3s-cloud-contracts`; scheduler/event consumers, persistence, and public availability remain later `AUT0` gates. Flow timers remain scoped to existing runs and P0 scheduled Task profiles adapt to this one invocation authority |
