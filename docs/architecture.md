...
| Developer Workflows | Bounded source-layout inspection and versioned reviewable BuildPlan proposals; accepted source, build, deployment, route, and scheduling authority stays with existing owners | Component-only `P0.1-C1` implements canonical exact-source snapshots plus deterministic Dockerfile and A3S Asset ACL detection. It reuses the Assets-owned manifest parser and Sources-owned `BuildRecipe`; persistence, interfaces, profiles, previews, monorepos, and imports remain unavailable |
