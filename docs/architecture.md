...
| Connectors | Reusable outbound HTTP/business connection profiles, Secret bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, and execution evidence | Component-only `AUT0.5-C1` exact-revision single-attempt port and bounded HTTP executor implemented. Profile/revision persistence, A3S ACL admission, Secret materialization, production egress/SSRF enforcement, evidence, revocation/recovery, and product availability remain planned in `AUT0.5`; callers never create direct HTTP clients |
