...
| Edge | Domains, certificates, logical Gateway scopes, routes, snapshots, applied projection, and bounded typed certificate-renewal terminal facts | Current; `C0.3-N4b` freezes schema-v1 renewal failure/recovery facts per logical Route and physical Gateway node over the existing certificate reconciler and transactional Outbox; implementation and retained PostgreSQL evidence remain open |
