...
| Applications | Application identities, immutable releases, six authoring/delivery projections including classic/New Agent distinction, sessions, messages/variants, conversation variables, toolkit/feedback/annotation/publication policy, and managed application delivery | Component-only `APP0.1-C1/C2` implements strong identities, canonical immutable release ACL, exact Workflow revision/digest evidence, project-authorized CQRS, and migration `124` A3S ORM persistence with shared idempotency, Outbox, and audit writes. Historical replay reconstructs the exact release without re-resolving Workflow; no graph, run, queue, session, route, provider, credential, or runtime state is copied. Maintained interfaces, sessions, delivery, and later APP0 gates remain unavailable |
