...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, and execution evidence | Component-only `AUT0.5-C1` execution, verified `AUT0.5-C2` ACL-native persistence, verified `AUT0.5-C3` authorized application plus just-in-time materialization, and component-only `AUT0.5-C4` public-Internet DNS/SSRF enforcement with exact per-attempt address pinning are implemented. Evidence, internal execution authorization, recovery, Workflow ports, provider wiring, supported management surfaces, and product availability remain planned; callers never create direct HTTP clients |
