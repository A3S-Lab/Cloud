...
| Workflow | Ontologies, immutable ontology and Workflow revisions, goals, deterministic plan revisions, immutable step-descriptor and typed-variable semantics, Workflow runs, reachable-Output termination, HumanTasks, human decisions, finite-child coordination, and semantic step projections | `W0.1` is implemented and `W0.2` is verified; `W0.3` planning/API, revision-owned semantic contracts, exact Plan v2 pinning, Workflow-local steps, deterministic Output aggregation, HumanTask, and finite `execution` are implemented. Typed-variable runtime projection, built-in discovery, composite regions, Answer/error semantics, remaining application ports, compensation, expanded real-provider conformance, and `W0.4`-`W0.5` remain |
...
owner of invocation-only trigger subscriptions. Migration `103` persists the
revision-owned semantic contract set and Plan v2 pins exact descriptors and
variable semantics. Built-in catalog discovery, runtime materialization and
inspection, and public node availability remain open.
