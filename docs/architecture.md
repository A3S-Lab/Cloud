...
| Workflow | Ontologies, immutable ontology and Workflow revisions, goals, deterministic plan revisions, immutable step-descriptor, typed-variable, composite-region, and per-step provider-retry semantics, built-in node discovery, Workflow runs, reachable-Output termination, HumanTasks, human decisions, finite-child coordination, exact Connector capability ownership, and semantic step projections | `W0.1` is implemented and `W0.2` is verified; `W0.3` planning/API, revision-owned semantic contracts, exact Plan v2 pinning, digest-bound variable defaults, bounded composite policy/child bindings, deterministic component-only composite frame/export and ordered region reducers, initial typed-variable Flow projection, Flow-derived authorized variable inspection, the read-only 23-node catalog, Workflow-local steps, deterministic Output aggregation, HumanTask, finite `execution`, the component-only Connectors-owned exact-attempt port, and immutable policy v2 retry-budget contracts are implemented. Flow-backed Iteration/Loop/Connector dispatch and child lifecycle, Applications-owned variables, Answer/error semantics, remaining application ports, compensation, expanded real-provider conformance, and `W0.4`-`W0.5` remain |
...
WorkflowRevision registry snapshot. Composite policy is revision-bound and the
deterministic frame/export and ordinal region reducers are component-complete,
but Applications variable adapters plus Flow-backed Iteration/Loop dispatch
and child lifecycle remain open and fail closed. No variable/region table, cache,
event log, worker, scheduler, queue, or Flow mechanism was added.
