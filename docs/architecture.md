...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, durable exact-attempt fencing/recovery, and immutable terminal execution evidence | Component-only `AUT0.5-C1` execution plus verified `C2` ACL-native persistence, `C3` authorized application/materialization, `C4` public-Internet DNS/SSRF enforcement, `C5` evidence persistence/reads, and `C6` authorized one-shot execution with atomic attempt/evidence settlement are implemented. Provider/Event-consumer wiring, Workflow ports, supported non-Web management surfaces, revocation/recovery operations, and product availability remain planned; callers never create direct HTTP clients |
