...
| Audit | Shared append-only `audit_records` plus one owner/admin-only bounded read projection; `C0.3-PA2a` adds explicit request-time Project/Environment/immutable-profile references and a closed attribution status to that same authority | Agent, Gateway, inference, or MCP-specific audit stores, duplicate writers, current-pointer lookup for historical facts, scope inference from private details, or public unstructured details |
