...
| Edge | Domains, certificates, logical Gateway scopes, routes, snapshots, applied projection, bounded typed certificate-renewal terminal facts, and a frozen certificate-expiry-risk transition projection | Current; `C0.3-N4b` schema-v1 renewal failure/recovery facts per logical Route and physical Gateway node are verified over the existing certificate reconciler and transactional Outbox by the [PostgreSQL 17.5 H0 gate](https://github.com/A3S-Lab/Cloud/actions/runs/32543351641/job/96957381856). `C0.3-N4f` freezes a source-specific Route-plus-node risk subject, fixed 24-hour emergency window, monotonic Edge projection, schema-v1 at-risk/cleared owner facts, and atomic Outbox boundary; implementation, migration `135`, and retained PostgreSQL evidence remain open |
