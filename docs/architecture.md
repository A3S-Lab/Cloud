...
| Connectors | Reusable outbound HTTP/business connection profiles, immutable revisions, exact Secret-version bindings/materialization, egress policy, bounded request/response contracts, one provider-neutral execution port, and execution evidence | Component-only `AUT0.5-C1` execution, verified `AUT0.5-C2` ACL-native persistence, and `AUT0.5-C3` authorized application plus just-in-time materialization foundations are implemented. Production egress/SSRF enforcement, evidence, internal execution authorization, recovery, Workflow ports, supported management surfaces, and product availability remain planned; callers never create direct HTTP clients |
