...
`A3S_CLOUD_BX0_EXECUTE=1`, steps 1–6 may record `*=executed` only after
validated node ACL/token/`A3S_CLOUD_BX0_ENROLL_NODE_ID`,
`A3S_CLOUD_BX0_ARTIFACT_DIGEST`, `A3S_CLOUD_BX0_SERVICE_ID`,
`A3S_CLOUD_BX0_HEALTH_URL`, `A3S_CLOUD_BX0_HTTPS_URL`, and
`A3S_CLOUD_BX0_LOGS_CURSOR` from real enroll/OCI/deploy/Ready/managed-TLS/log
read (never PLACEHOLDER); prep
...
node_id/digest/service_id/health_url/https_url/logs_cursor) and may emit
