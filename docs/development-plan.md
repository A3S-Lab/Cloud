...
The current REST/OpenAPI contract is `1.45.0`. It adds versioned outbound
notification delivery budgets while retaining the caller-owned
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because user-configured suppression,
  SMTP plus alert policy, retained production evidence, security
...
- Implemented as component-only `C0.3-N2c`: one immutable personal outbound
  ...
  scheduler, or second event rail. User-configured suppression,
...
- Implemented as component-only `C0.3-N2d`: a replayed C6 `retryable` evidence
  ...
  suppression remains a separate later semantic gate.
...
- Outbound delivery product availability remains gated by the broader
  ...
  Future user-configured suppression must preserve those authorities and may
  not introduce another counter, timer, queue, scheduler, or configuration
  format. External SMTP also requires an exact Identity-owned
