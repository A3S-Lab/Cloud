...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.23.0`. Focused tests regenerate the
...
pair, and monotonic aggregate version at every boundary. REST `1.23.0`, the
...
idempotency/Outbox/audit path, and shared Resource Grant evaluator.
...
and HumanTask submission/automatic-expiry/parent-cancellation loop now consume
that foundation;
service/finite-task dispatch, compensation, multi-day
