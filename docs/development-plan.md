...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@12`. The
  former `@1` through `@11` generations are explicit replay-compatible migration entries;
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@12` keeps
  versions/builds `@1` through `@11` explicitly replayable. Migration `123`
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, forty-one accepted authority decisions, the immutable
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; retain descriptor-bound graph Answer frames and Application-
   variable snapshot/CAS semantics, and complete remaining non-Execution error routes while
...
3. retain the implemented `APP0.1`, component-only
   ...
   adapter, and component-only `APP0.2-C9/C10/C11`,
