...
| U0 | In progress; unavailable | The Cloud slice of `U0.1` pins the canonical A3S Use protocol-level-4 host contract and adds explicit capabilities, package-plan, enablement-plan, digest-only apply, and observation Fleet payloads plus one optional Node Agent adapter over the sole shared `PluginHostManager`. They reuse the existing command queue and journal. The first `U0.2` slices add the `PluginRegistry` domain, migration 083 persistence, and one typed trust-root adapter over the shared immutable-object client without copying TUF/catalog or object-storage mechanisms. Root compatibility locking, production Manager composition, registry application/interfaces, the published Use catalog adapter, assignments, and real-provider gates remain open; no user-facing capability is claimed. |
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.14.0`. Focused tests regenerate the
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 59-tool administrator and 37-tool read-only catalogs,
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  idempotency, Outbox facts, and shared audit records. REST/OpenAPI `1.14.0`,
...
package-plan/apply/enablement-plan/observation request and result types come
directly from `a3s-use-core`; Cloud does not wrap, fork, or restate them. The Use catalog,
...
`a3s-use-core` 0.2.2. `U0.1` pins its exact released Cargo dependency and Use gitlink,
...
HTTPS endpoint and content-addressed root evidence, migration 083, and A3S ORM
...
catalog or TUF metadata is persisted. The typed Plugins trust-root adapter
...
constant or TUF verifier. Enrollment application commands, the published A3S
Use catalog adapter, application queries, authorized Search projection,
REST/client/CLI/Management MCP reads, and real-provider exit evidence remain
open; the Web projection stays deferred by section 1.1.
...
changes, one authorized Search projection, REST `1.14.0`, the maintained
...
Outbox mechanisms. REST `1.14.0`, the maintained client, CLI, and ten additional
...
releases through A3S ORM. REST `1.14.0`, the maintained client, CLI, and seven
