...
| F0 | Re-certification required | The last complete evidence set covered isolated PostgreSQL migrations, tenancy, idempotency, local/NATS outbox, A3S Flow `0.12.0` history, A3S Boot `0.2.0` PostgreSQL task management, A3S ORM `0.3.0`, queue-failure readiness, and the nine-boundary persistent Build Flow `SIGKILL` gates. The current exact lock moves Cloud and Code to the same Flow `1.0.0-rc.1` candidate for the shared DAG compiler and durable execution protocol. The former Flow `0.11.0`/`0.13.1` split is closed; the candidate must still pass the same composition gates before this row returns to `Verified` |
...
The current REST/OpenAPI contract is `1.39.0`. It adds the optional Durable
Cell `storageProviderProfileAcl` input for C3b without changing the original
`1.38.0` C5 request: omission retains the earlier behavior, while the CLI
...
- Cloud pins the exact A3S Flow `1.0.0-rc.1` candidate, A3S Boot `0.2.0` with
  `queue-postgres`, and
  A3S ORM `0.3.0`-backed PostgreSQL stores. Flow events live in `a3s_flow`; Boot
  ...
  Flow candidate, so the former transitive version split is closed.
...
The shared Operations adapter now pins the exact A3S Flow `1.0.0-rc.1`
candidate with A3S Boot `0.2.0` PostgreSQL task management, isolated ORM-backed
...
migration variable before serving. Its versioned Box-hosted unit, binding
those references to separate migration and serving PostgreSQL
principals/grants, and installer orchestration remain open. The current role
...
Serving API, Worker, Relay, and `all` processes now connect through one
schema-admission path and never invoke a migrator. Admission reads the A3S ORM
ledger and requires the exact version/checksum of every migration compiled into
that serving binary; a missing or altered record fails before product
capabilities are constructed and also fails readiness. Later records are
accepted so an old process can overlap an expand-compatible release. A
...
concurrently: A3S ORM serializes them into exactly one application and one
idempotent replay. The required deployment order is PostgreSQL health,
