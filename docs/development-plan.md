...
- no `clamscan` → skip live antivirus scanner execution (metadata-only scan
  surfaces remain as already implemented).
...
| APP0 | In progress; unavailable | `APP0.1` implements strong Application/Release identities, one canonical immutable release ACL, all six experiences with immutable classic/New Agent distinction, exact Workflow revision evidence, migration `124` persistence, authorization before replay, CQRS, and REST/OpenAPI `1.42.0` plus maintained client, CLI, and six Management MCP tools over the same repository. `APP0.2-C1` through `C15` freeze and atomically persist Application-scoped end users, exact-release sessions, invocation-to-WorkflowRun correlation, monotonic input/Answer/final-output messages, optimistic immutable conversation-variable revisions, exactly-once Workflow semantic effects, and immutable invocation execution authority through migrations `125`-`127`; they also compile deterministic preset wrapper Workflows. The platform composes each exact invocation into deterministic ordinary Workflow Goal, Plan, and Run records, recovers cancellation from persisted authority, registers project-authorization-first session/invocation/cancellation and bounded cursor CQRS, exposes a Run-resolved Workflow semantic-effect consumer with exact ambiguous-commit recovery, uses v10 reconciliation to append aggregate final output before terminal observation and WorkflowRun projection persistence, uses v11 only for exact descriptor-bound Answer hooks that require committed C7 message evidence, uses v12 only for exact descriptor-bound Application variable snapshot/CAS hooks with Flow-derived inspection, uses v13 only for C13 composite root/child frame authority that maps repeated Answers to the root invocation with stable ordinals and no child lifecycle effects, uses Plan v6/Run v14 only for C14 deterministic terminal Application-variable write failure branches, and uses Plan v7/Run v15 only for C15 deterministic terminal root/frame Answer write failure branches. Migration `143` admits only failed Output selected-handle evidence. C8 exposes session open/read, invocation request/read, and ordered-message reads through REST/OpenAPI `1.43.0`; C12 exposes optimistic close/cancel and complete session replay through `1.44.0`. Both use the maintained client, CLI, and `application:write` Management MCP tools without adding another session, invocation, Workflow, or Flow authority. Focused C12 interface and C13/C14/C15 contract/compiler/runtime/coordinator/replay/inspection tests pass. The [retained PostgreSQL 17 C6-C11 recovery job](https://github.com/A3S-Lab/Cloud/actions/runs/32474020740/job/96746540732) proves the production command path across reconnect, lost Answer and variable responses, final-output/terminal replay, and exact durable counts. The [retained PostgreSQL 17 C6-C13 recovery job](https://github.com/A3S-Lab/Cloud/actions/runs/32486698014/job/96784727028) proves repeated-frame ordinal 0/1, ordinal-1 commit-before-response replay, and the complete production Applications effect chain. `APP0.2-C16` freezes Applications-owned anonymous delivery credential bindings (opaque lookup key, exact Secret version reference, generation lifecycle, stable Principal-free end-user identity). `APP0.2-C17` persists those bindings through migration `204`, one A3S ORM repository, and in-memory adapters without Identity issuance or public routes. `APP0.2-C18` wires anonymous delivery session open/replay over those bindings without Identity secret verification or public routes. `APP0.2-C19` wires anonymous delivery invocation request/replay with credential-issuer Workflow authority over C18 sessions without Identity secret verification or public routes. `APP0.2-C20` adds project-authorized Applications CQRS register/disable/enable/revoke for those bindings without Identity secret minting or public routes. `APP0.2-C21` freezes immutable ApplicationFeedback and ApplicationAnnotation aggregates for exact session releases without persistence or public routes. `APP0.2-C22` persists those aggregates through migration `205`, one A3S ORM repository plus in-memory adapters for each aggregate, and create/replay by primary key without CQRS or public routes. `APP0.2-C23` wires project-authorized Applications CQRS create and session-scoped get/list for those aggregates without Annotation Reply or public routes. `APP0.2-C24` exposes those commands through Applications management REST/OpenAPI, client, CLI, and Management MCP tools under `application:write` without Annotation Reply, anonymous delivery, or public availability. `APP0.2-C25` freezes immutable `ApplicationMessageVariant` ("More Like This") authority for exact session releases without persistence or public routes. `APP0.2-C26` persists those variants through migration `206`, one A3S ORM repository plus an in-memory adapter, and create/replay by primary key without CQRS or public routes. `APP0.2-C27` wires project-authorized Applications CQRS create and session-scoped get/list for those variants without regeneration invocation or public routes. Remaining application-scoped Identity issuance, blocking/streaming answers, remaining APP0.2 records, Gateway delivery, monitoring, and enterprise completion remain. Full public parity is a composite `APP0.6` claim and no application-platform availability exists yet. |
| K0 | In progress; unavailable | `K0.1-C1/C2` implement the canonical UserFile ACL/lifecycle, shared streaming object adapter, atomic organization quota and PostgreSQL persistence, authorization-first CQRS, shared Outbox/audit/idempotency, REST/OpenAPI, maintained client/CLI, and Management MCP tools. `K0.1-C2b1` adds authorized public `PUT .../content` (OpenAPI `1.89.0`) with local object-store tests; `K0.1-C2b2` adds admitted-only public `GET .../content` (OpenAPI `1.90.0`) over the same streaming object port; `K0.1-C2b3` adds metadata-only public `POST .../scan` (OpenAPI `1.91.0`) plus a sixth Management MCP tool; `K0.1-C2b4` adds metadata-only public `POST .../expire` (OpenAPI `1.92.0`) plus a seventh Management MCP tool; MCP remains metadata-only. The retained [PostgreSQL 17 H0 persistence step](https://github.com/A3S-Lab/Cloud/actions/runs/33159659047/job/98810769471) verifies transaction rollback, concurrent quota serialization, exact lifecycle replay, quota release, and atomic side effects through the production owner ports. `K0.1-C3` freezes KnowledgeBase/document/chunk/index/retrieval/external-binding and KnowledgePipeline release contracts with checked-in ACL fixtures. `K0.1-C4a` adds migration `201` KnowledgeBase/PipelineRelease catalogs with CAS fencing and focused PostgreSQL recovery gates without public surfaces. `K0.1-C4b1` adds application owner catalog services over those repositories. `K0.1-C4b2a` adds authorized idempotent catalog writes with audit/Outbox. `K0.1-C4b2b1` exposes that boundary over REST/OpenAPI `1.88.0`. `K0.1-C4b2b2` adds maintained client/CLI/Management MCP over the same catalog handlers. `K0.1-C5` persists immutable KnowledgeDocument/Chunk catalogs through migration `202` without public surfaces. `K0.1-C6` adds application owner document/chunk catalog services over those repositories. `K0.1-C7` adds authorized idempotent KnowledgeDocument/Chunk create writes with audit and Outbox side effects over the C5 repositories without public surfaces. `K0.1-C8` exposes that document/chunk boundary over REST/OpenAPI `1.93.0` without client/CLI/MCP or live MinIO/scanner/SEV claims. `K0.1-C9` adds maintained client, CLI, and four Management MCP tools over the same C8 handlers without bumping OpenAPI or claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C10` adds authorized bounded KnowledgeDocument/Chunk list through REST/OpenAPI `1.94.0`, client, CLI, and two Management MCP read tools without claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C11` persists immutable KnowledgeIndexRevision/RetrievalPolicyRevision/ExternalKnowledgeBinding catalogs through migration `203` with local/PostgreSQL adapters and no public surface. `K0.1-C12` adds application owner index/policy/binding catalog services over those repositories. `K0.1-C13` adds authorized idempotent index/policy/binding create writes with audit/Outbox. `K0.1-C14` exposes that index/policy/binding boundary over REST/OpenAPI `1.95.0` without client/CLI/MCP or live MinIO/scanner/SEV claims. `K0.1-C15` adds maintained client, CLI, and six Management MCP tools over the same C14 handlers without bumping OpenAPI or claiming live MinIO/S3, scanner execution, SEV ingestion, or cleanup workers. `K0.1-C16` adds authorized bounded index/policy/binding list through REST/OpenAPI `1.96.0`, client, CLI, and three Management MCP read tools without live MinIO/scanner/SEV/cleanup claims. Live MinIO/S3 provider wiring, live scanner execution, live object cleanup execution, ingestion, retrieval, and Flow-backed Knowledge Pipelines remain unavailable; on this delivery host those live-provider and SEV-backed paths are skipped at the physical machine wall and are not Verified. |
| AUT0 | In progress; unavailable | New-invocation schedules/webhooks/plugin events and reusable outbound connection profiles are specified in `ai-application-platform-plan.md`. Component-only `AUT0.2-C7` converts a captured endpoint-bound webhook request into an exact-revision invocation with deterministic origin and policy deduplication while preserving transport and persistence ownership outside Automations. Implemented management-interface `AUT0.2-C8` exposes authorized webhook endpoint lifecycle and bounded definition/revision catalog reads through REST/OpenAPI `1.97.0`, the maintained client, CLI, and eight Management MCP tools without Gateway public receive, schema-registry, or product availability claims. Component-only `AUT0.3-C8` composes exact schedule state, bounded due evaluation, stable occurrence envelopes, idempotent admission, cursor commit, and fenced lease release; it adds no timer loop or target owner. Component-only `AUT0.4-C1` binds normalized PluginEvent/SourceEvent identities to exact subscriptions and revisions, and its application boundary evaluates the exact revision `filter_digest` through an Automations-owned port. Component-only `AUT0.4-C2` adds bounded deterministic fan-out over already selected exact revisions, `AUT0.4-C3` adds a strict durable-ack A3S Event consumer for already normalized events, and `AUT0.4-C4` composes injected candidate selection with idempotent invocation admission; these preserve provider normalization, candidate storage, subscription persistence, local retry, and target ownership outside Automations. Component-only `AUT0.5-C1` supplies the exact-revision execution port and bounded HTTP executor; verified `C2`-`C6` add immutable profiles/revisions, exact Secret materialization, public-Internet egress, exact-attempt evidence, durable fencing, conservative recovery, and atomic settlement; implemented `C7` exposes the same profile/revision CQRS without resolving Secrets; component-only `C8` binds exact WorkflowRun/Plan/step-attempt and Connector authority to C6; `C9` freezes its bounded attempt budget and fallback delay; `C10` stores accepted response bytes through the sole shared immutable-object authority before terminal evidence; `C11` permits only environment-authorized, accepted-terminal-evidence-backed transient reads; `C12` serializes exact revision revocation with dispatch admission; and `C13` atomically closes an expired dispatch only as audited body-free `Indeterminate` evidence through migration `155` and REST/OpenAPI `1.66.0`. WorkflowRun v9 keeps the exact v8 hook observation, durable wait, bounded retry, fail-closed indeterminate decisions, and strict schema-bound JSON output in Flow, then routes a terminal closed classification only through an exact Plan-v5 Connector error edge; a C13 terminal replay remains indeterminate and never calls, retries, or cancels the provider. Historic v8 remains fail-closed without that interpretation, v7 retains default-output behavior, v6 stays reference-only, v5 stays digest-only, and C6 remains the sole provider-attempt authority. Component-only `C0.3-N2b` supplies the first exact-subject Notification NATS-to-C6 composition, while `N2c`-`N2e` retain Notification delivery decisions. Remaining timer/consumer/provider wiring, retained end-to-end external-provider and recovery evidence, and all Automations/Connectors product availability remain open. Management-interface `AUT0.2-C8` closed the last numbered Automations management quick-win on this host; Gateway public receive, production schema-registry wiring, and optional production process-role composition stay deferred rather than overfitted. |
...
| U0 | In progress; `U0.1`/`U0.2` verified; `U0.3` foundation certified | Verified `U0.1` pins the canonical A3S Use protocol-level-4 host contract and Node Agent `PluginHostManager` adapter. Verified `U0.2` adds `PluginRegistry`, migrations 084-085, public-HTTPS and PostgreSQL registry gates. `U0.3` foundation is certified by `run_u0_3_foundation_gate.sh` (`A3S_CLOUD_U0_3_FOUNDATION_CERTIFIED`, including Flow mutation matrix and pinned Use golden contract fixtures): assignment/plan PostgreSQL (`189`-`191`), assignment Flow crash points 1–5 and 8–9, Use recovery 6–7, Skill/Ui host converge, identity/catalog/plan/confirm/observe golden fixtures, REST/client/CLI/MCP surface parity, fail-closed/mutation filters, Node Agent Skill/Ui/uninstall/upgrade journal, and the no-second-Use-platform ratchet. Embedding full `a3s-use` remains blocked on Flow/Runtime pin alignment; product exit still requires operator-owned live Cloud↔host availability claims. |
...
The current REST/OpenAPI contract is `1.89.0`. It adds authorization-first
UserFile reservation, authorized `PUT .../content` byte transfer, bounded metadata reads, optimistic tombstone, and
...
`A3S_CLOUD_BX0_EXECUTE=1`, steps 1–9 may record `*=executed` only after
...
`A3S_CLOUD_BX0_LOGS_CURSOR`, `A3S_CLOUD_BX0_UPDATE_DIGEST`,
...
node_id/digest/service_id/health_url/https_url/logs_cursor/update_digest/rollback_digest/cleanup_instance)
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  verified and `C0.3-PA2d` complete transient manifest export is verified on PostgreSQL 17 in main CI
  ([H0](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566/job/101250990670), [complete run](https://github.com/A3S-Lab/Cloud/actions/runs/33945593566)), later cross-layer security investigation,
...
- Verified as `C0.3-PA2c`: add one
  ...
  provider resources.
- Verified on PostgreSQL 17 in main CI (`2026-09-05`) as `C0.3-PA2d`: expose one
...
| `A0.4` | Verified | Exact published Agent releases bind immutably to ordinary Workload revisions through the existing Deployment, Operation, Flow, Fleet, and Runtime path. The Code-owned final manifest now derives process, port, readiness, Secret destinations, storage policy, and the read-only `/app/.a3s` mount; caller overrides, provenance drift, archive drift, and unsupported storage fail closed. Server-side artifact injection, replay, update, rollback, Secret restart, migration 182 persistence, REST, client, and CLI projections are implemented. The retained [PostgreSQL 17/real A3S Box provider gate](https://github.com/A3S-Lab/Cloud/actions/runs/33686237668/job/100434300332) and [complete Cloud CI](https://github.com/A3S-Lab/Cloud/actions/runs/33686237772) pass against Box `65f3d3fc7c1e0e2cb1ba2d409a79f7357314f5ae` and OCI Runtime `878f8414cef3b85bef1b51fe6735017b25828252`, including published-release recovery, Secret rematerialization, cancellation, and cleanup. Hosted MCP deployment is owned by `MCP0` |
| `A0.5` | In progress | Exact Git archive publication, immutable Skill release binding/rebinding/unbinding, migration 067 persistence, read-only Runtime Artifact mounts, rollback-safe revisions, and authorized REST/client/CLI/catalog surfaces are implemented. The CI PostgreSQL gate drives REST publication against a real local immutable-object provider and retains exact archive bytes, receipt identity, replay, yanking, and temporary-bundle cleanup evidence. Real Box gates now cover typed Skill-bundle hydration and the complete Workload bind/rebind/unbind/rollback lifecycle, including the production `/a3s/skills/{asset_id}` read-only mount shape, guest write rejection, revision history, content-addressed downloads, and Runtime-remove cleanup; retained external-provider evidence still blocks verification |
...
Current `U0.3` foundation certified (`A3S_CLOUD_U0_3_FOUNDATION_CERTIFIED`): `PluginAssignment` domain entity,
`plugin.assignment.changed` event, in-memory and PostgreSQL repositories,
migration `189` (`plugin_assignments` with one live `(organization,
target_host, package_id)` uniqueness), the `SetPluginAssignment` command
handler, REST `1.84.0` put/list/get under environment-scoped
`plugin-assignments`, the typed TypeScript client methods, CLI
`plugin-assignments` list/get/set, Management MCP assignment list/get/set tools,
and `plugin:write` token scope are present. Bounded `PluginPlanProjection`
storage (migration `190`) plus Record/Get/Confirm application handlers, REST
get/confirm (`1.84.0`), client/CLI/MCP surfaces are present for digest-only Use
plan review; `cloud.plugin-assignment@1` Flow identity, lock step, Fleet
capabilities-inspect host resolution (reuse existing command across Flow polls
via `find_command` before enqueue so issued_at/not_after drift cannot conflict),
and assignment Operation enqueue reconciler are registered
(`a3s-cloud-workflows@29`); authorize-trust admits enrolled trust-root and
policy ACL bytes into the node Artifact store, enqueues
`plugin_host_authorize_trust` with find-before-enqueue command reuse, and
completes the Flow once the Agent acks exact digests; enqueue-plan loads the
exact verified catalog candidate for Install, enqueues `plugin_host_plan` with
find-before-enqueue reuse, and reaches Ready once the Agent acks; store-plan
persists the bounded `PluginPlanProjection` from the planned envelope with
deterministic projection identity and idempotent digest reuse; await-confirmation
continues immediately on policy `allow`, polls until a digest-only human
confirmation is recorded on `ask`, and fails closed on `deny`; confirmation
retains the exact confirmation envelope with its digest for Fleet apply;
enqueue-apply issues digest-only `plugin_host_apply` with find-before-enqueue
reuse and reaches Ready once the Agent acks; observe issues digest-only
`plugin_host_observe` with per-attempt find-before-enqueue reuse, polls while
Unavailable/Reconciling, and completes the Flow once observed package/surface
evidence matches the assignment; the live PostgreSQL assignment/plan
persistence gate (`189`-`191`) is certified `12/12` for transactional create/
replay, one live host/package uniqueness, tenant isolation, FK fail-closed
writes, plan-projection digest idempotency, confirmation CAS, and non-canonical
restore rejection (`A3S_CLOUD_U0_3_POSTGRES_CERTIFIED`); the Node Agent
authorize-trust path now binds Artifact downloads to the request
`binding_digest`, materializes opaque trust-root/policy blobs (no directory
extract), verifies digests, parses policy through `a3s-acl`, and admits the
root through A3S Use `inspect_bootstrap_root` before ack; assignment Flow
enqueue-plan now pre-observes the host, selects package-plan vs enablement-plan
vs already-converged from desired/observed drift, and stores enablement
operation envelopes through the same digest-only apply path; the enablement
disable path is covered end-to-end in Flow unit evidence (`16/16`, including
`enqueue_enablement_plan_selects_disable_and_accepts_planned_ack` through
store/confirm/apply/observe); Cloud Flow fail-closed/mutation matrix covers
deny-without-apply, ask confirmation timeout, already-converged short-circuit,
uninstall→Removed converge, and upgrade selection/enqueue (lock-bound upgrade
apply remains Use-owned); Cloud-side crash points 1–5 and 8–9 are covered by
assignment reconciler and Flow unit evidence (`crash_point_1_*` plus Flow
`crash_point_2_*` through `crash_point_8_9_*`: plan/apply/observe find-before-enqueue reuse and projection resume
before confirmation); Skill-only plan/apply/observe through
the Node Agent shared `PluginHostManager` journal path is covered
(`skill_only_plan_apply_observe_converges_through_shared_manager_port`);
UI-only plan/apply/observe through the same journal path is covered
(`ui_only_plan_apply_observe_converges_through_shared_manager_port`);
uninstall plan/apply/observe converges Absent/Removed through the same journal
(`uninstall_plan_apply_observe_converges_through_shared_manager_port`);
upgrade plan requests journal the exact Upgrade action and fail closed on
lock-bound apply ownership without a second channel
(`upgrade_plan_request_journals_exact_action_without_second_channel`);
Use-side crash points 6–7 are certified at the Cloud-pinned Use revision by
`tools/use-conformance/run_use_recovery_gate.sh`
(`A3S_CLOUD_U0_3_USE_RECOVERY_CERTIFIED`); real Skill/Ui converge on
`CognitivePackageHostManager` is certified at the same Use revision by
`tools/use-conformance/run_use_host_converge_gate.sh`
(`A3S_CLOUD_U0_3_USE_HOST_CERTIFIED`); Cloud crash point 1 (assignment/
Operation commit before Flow creation) is covered by
`crash_point_1_assignment_survives_before_flow_operation_enqueue`; the U0.3
source-architecture ratchet
`plugins_u0_assignment_surface_owns_no_second_use_platform` forbids a second
Use platform inside Cloud Plugins and rejects a full `a3s-use` embed until
Flow/Runtime pins align; REST/client/CLI/Management MCP assignment and plan
surfaces are catalog-pinned and certified by
`tools/use-conformance/run_assignment_surface_gate.sh`
(`A3S_CLOUD_U0_3_SURFACE_CERTIFIED`); the composed foundation meta-gate
`tools/use-conformance/run_u0_3_foundation_gate.sh` certifies recovery + host +
pinned Use golden contract fixtures + scope/path/symlink fail-closed + surface +
Cloud crash points + Flow fail-closed/mutation matrix + Node Agent
Skill/Ui/uninstall/upgrade journal together (`A3S_CLOUD_U0_3_FOUNDATION_CERTIFIED`,
including `mutation_matrix=6`, `node_agent_journal=5`,
...
Product-exit verification is `tools/use-conformance/run_u0_3_exit_audit.sh`.
It rechecks pin, architecture ratchet, golden fixtures, scope/path fail-closed,
and Node Agent enablement journal evidence, then fail-closes with
...
3. retain the implemented `APP0.1`, component-only
   ...
   and `AUT0.3-C1/C2/C3/C4/C5/C6/C7/C8/C9/C10` calendar, misfire, concurrency, due-window, lease, durable cursor/lease, deterministic due-time invocation-envelope, bounded dispatch, P0 Task-target adaptation, and application/PostgreSQL idempotent invocation-admission boundaries
...
inspect only non-secret digest-bound declarations and trust-bundle observations.
Its API-role-only
`spiffe_https_web` adapter resolves an exact content-addressed profile, uses
HTTPS with system or digest-pinned PEM trust anchors, rejects redirects,
proxies, duplicate JSON keys and oversized bodies, and canonicalizes the
observed SPIFFE JWK bundle. Operational CA paths, timeouts and body limits
remain outside the semantic profile digest. Inspection responses explicitly
separate digest-bound `declared*` policy from externally proved `observed*`
Bundle evidence. It cannot issue credentials before
`WI2` supplies exact Fleet/Runtime attestation. Migration
