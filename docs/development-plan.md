...
| MCP0 | In progress; unavailable | Closed cross-repository contracts, Runtime profile/generation fencing, Cloud canonical immutable profile plus separately mutable route-policy ACL and typed A3S ORM persistence plus Runtime/Gateway compilers, and Gateway request/auth/single-dispatch/JSON-SSE foundations pass focused tests. Cloud Workload binding, credential resolution, target reconciliation and real PostgreSQL evidence, real Box/Linux, reload/drain/telemetry, and joint conformance remain open |
...
- Cloud admits one canonical immutable Service-profile ACL, binds it to a
  ...
  revisioned, expiring Edge route-policy ACL through migration 053. Both use
  typed A3S ORM; route desired state contains authorization references but no
  Runtime endpoints or credential verifiers. Cloud also compiles the ordinary
  Runtime Service specification and deterministic Gateway ACL projection.
  WorkloadRevision binding, credential authority resolution, target
  reconciliation, the real PostgreSQL gate, lifecycle surfaces, recovery, and
  audit remain `MCP0.3`; and
