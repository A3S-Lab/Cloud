...
| --- | --- | --- |
| P0 | In progress; unavailable | `P0.1-C1` through `C5` establish bounded canonical BuildPlan detection, immutable exact-SourceRevision acceptance through migration `146`, shared authorization, and trusted accepted-revision SourceLayout acquisition. `P0.1-C6` production-composes REST/OpenAPI `1.72.0`, the maintained client/CLI, four Management MCP tools, and one authorized accepted-plan read authority over those same CQRS/domain interfaces without another parser, repository, policy evaluator, or lifecycle. P0.2 accepted workload-profile and P0.3 durable Preview foundations remain as detailed below. Workload-profile/Preview public interfaces, pre-acceptance discovery, Workloads/Executions/Edge/Operations/schedule and cleanup owner handoffs, Preview expiry execution, monorepos, and Compose import remain open. |
...
The current REST/OpenAPI contract is `1.73.0`. It adds closed logical Agent
execution checkpoint capture/list/read/snapshot, paged semantic trajectory,
and immutable fork APIs with exact object, lineage, and Runtime telemetry
evidence. It retains `1.72.0`'s closed Developer Workflows BuildPlan
detection, idempotent acceptance, exact accepted-plan list/get APIs, maintained
client and CLI, and four Management MCP tools over the same CQRS/application
authority without exposing source bytes, credentials, checkout paths, or a
downstream lifecycle. It also retains `1.71.0`'s bounded Agent
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.73.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 141-tool administrator and 80-tool read-only catalogs,
  ...
  outbound-subscription, and Developer Workflows BuildPlan extensions. The
...
object client, persists only exact projections and Runtime telemetry through
...
binding on `agent_executions`. Migrations `160`, `164`-`168` add only the named
provider, binding, approval, and checkpoint records for their implemented
component sub-gates; later A1 work cannot add another record family without an
