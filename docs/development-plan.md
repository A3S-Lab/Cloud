...
evidence. Current runtime build `a3s-cloud-workflows@21` retains `@1` through
`@20` for replay. The
...
The current REST/OpenAPI contract is `1.60.0`. It adds authorized bounded
WorkflowRun diagnostics/statistics over `1.59.0`'s complete signed audit
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@21` and patch
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.60.0`. Focused tests
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@21` keeps
  versions/builds `@1` through `@20` explicitly replayable. Migration `123`
