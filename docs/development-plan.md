...
The current REST/OpenAPI contract is `1.74.0`. It adds closed Developer
Workflows WorkloadProfile acceptance plus current, bounded-history, and exact
immutable revision reads with canonical ACL and typed Secret-material-free
intent. The maintained client, CLI, and four additional Management MCP tools
dispatch the same CQRS/application authority without adding a parser,
repository, evaluator, or downstream lifecycle. It retains `1.73.0`'s closed
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.74.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 145-tool administrator and 83-tool read-only catalogs,
  ...
  outbound-subscription, and Developer Workflows BuildPlan/WorkloadProfile
  extensions. The
