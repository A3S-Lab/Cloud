...
The current REST/OpenAPI contract is `1.76.0`. It adds bounded GitHub
installation-repository and canonical repository branch/tag discovery through
one Sources Application query authority. The maintained client, CLI, and two
additional Management MCP tools reuse the same closed response projection and
return no credential or provider body. It retains `1.75.0`'s closed Developer
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.76.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 152-tool administrator and 87-tool read-only catalogs,
  ...
  Management, and Sources-owned GitHub repository/reference discovery
  extensions. The
