...
The current REST/OpenAPI contract is `1.72.0`. It adds closed Developer
Workflows BuildPlan detection, idempotent acceptance, and exact accepted-plan
list/get APIs with canonical ACL and typed immutable evidence. The maintained
client, CLI, and four Management MCP tools dispatch the same CQRS/application
authority without exposing source bytes, credentials, checkout paths, or a
downstream lifecycle. It retains `1.71.0`'s closed, bounded Agent
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.72.0`. Focused tests
