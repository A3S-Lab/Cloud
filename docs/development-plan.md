...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@9`. The
  former `@1` through `@8` generations are explicit replay-compatible migration entries;
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  node result. WorkflowRun v9 and replay build `a3s-cloud-workflows@9` preserve
  that success path and, only for an exact Plan-v5 Connector error edge, turn a
  ...
  byte-compatible. Versions/builds `@1` through `@8` remain explicitly
  replayable. Migration `123` only admits the already wired Service projection
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, thirty-eight accepted authority decisions, the immutable
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; complete Answer frames and remaining non-Execution error routes while
   preserving historical Flow replay
   before exposing product claims;
3. retain the implemented `APP0.1`, component-only
   `APP0.2-C1/C2/C3/C4/C5/C6/C7` authorities, and the `APP0.2-C8` management
   adapter,
