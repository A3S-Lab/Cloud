...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, finite Execution, and a deterministic project-authorized 23-node discovery catalog. Migration `103` persists exact descriptor bindings, the recoverable registry snapshot, and the typed-variable contract as one immutable WorkflowRevision semantic set; compiler schema 2 emits Plan v2 with exact descriptor and semantic/variable digest pins, prevents authority downgrade, and preserves Plan v1 byte shape. WorkflowRun input/runtime/Flow v2 now freezes the variable ACL and reconstructs invocation, node-output, deterministic run-assignment, typed-read, and opaque-reference values from existing Flow history; migration `105` only expands that immutable input's bound. REST/OpenAPI `1.31.0`, client, CLI, MCP, focused domain/REST/replay/catalog tests, and PostgreSQL migration/replay/rollback/immutability plus catalog-reconnect gates cover the slice. The catalog composes the frozen parity manifest with an exact digest-bound profile ACL, has no persistence or write path, and cannot admit descriptors or claim public parity. Runtime inspection/defaults, composite and Applications-owned variables, Answer/error semantics, remaining application ports, compensation, expanded provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, nine accepted authority decisions, the immutable
