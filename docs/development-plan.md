...
| AUT0 | Planned; unavailable | New-invocation schedules/webhooks/plugin events and reusable outbound connection profiles are specified in `ai-application-platform-plan.md`. Component-only `AUT0.5-C1` supplies the exact-revision execution port and bounded HTTP executor; `AUT0.5-C2` adds canonical HTTP A3S ACL admission plus immutable environment-scoped profile/revision persistence and exact Secret-version bindings through migration `109`. Authorized application surfaces, just-in-time Secret materialization, production egress/SSRF policy, evidence, recovery, Workflow ports, and all Automations/Connectors product availability remain open. |
...
- Authorized application commands/queries, just-in-time Secret materialization
  with active-state rechecks, production egress/SSRF policy, execution evidence,
  revocation/recovery, Workflow ports, and provider wiring remain open in
  `AUT0.5`; neither component creates a product availability claim.
