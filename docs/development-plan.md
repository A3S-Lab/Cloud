...
The current REST/OpenAPI contract is `1.50.0`. It retains `1.49.0`'s immutable
personal alert-policy create/list/get/revoke extension for exact Gateway
certificate-renewal failure/recovery facts and adds the closed Workload
deployment-health failure/recovery source without another endpoint or policy
lifecycle. It also retains `1.48.0`'s complete human-readable operation, tag,
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.50.0`. Focused tests
