...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology contract foundation, exact capability references, deterministic graph validation, the ten-node outcome mapping, and architectural guards against duplicate execution mechanisms. Verified `W0.2` adds immutable Ontology revisions, deterministic migration/diff policy, and authorized Search. The `W0.3` backend now includes immutable definitions, Goals and Plans, native Forms, one Goal/Plan-bound WorkflowRun, the authority-bound HumanTask loop, and one finite `execution` capability step. Migrations `098`-`099` add Executions-owned immutable ACL-native ExecutionTemplate revisions and an exact Run/Plan/step/attempt/template/digest binding on the ordinary Execution. Workflow uses one typed Executions application port to create or adopt the child, links its existing Operation into the same A3S Flow run, resumes only from an authority-bound terminal result, and waits for the existing cleanup-first cancellation path. REST/OpenAPI `1.24.0`, the maintained client, CLI, and three Management MCP tools expose the template lifecycle through the same CQRS, grant, idempotency, A3S ORM, Outbox, and audit paths. The retained WorkflowRun `SIGKILL` fixture adds child-commit, exact-link, and terminal-resume boundaries to its four existing parent/HumanTask boundaries, while the Management MCP gate shares one ACL fixture with that test and asserts REST/MCP replay plus migration `098` database invariants. Focused tests and provider-gate sources pass local compilation/static checks; clean Linux PostgreSQL executions remain required before verification. Business-service and remaining provider steps, compensation, expanded clean `W0.3` provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
