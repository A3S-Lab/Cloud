...
- `CELL0.3` certifies a pinned provider and typed operator adapter as one
  ...
  restart-safe absence. The evidence records `storage=not-certified`; a
  retained pass and every storage-backed application/fault gate remain.
...
- `CELL0.4-C4` adds one authorization-before-replay internal composition
  ...
  route, Cell-owner lookup, or Gateway request replay. The first `C5` slice
  exposes bounded REST/OpenAPI `1.38.0` using the existing `cloud:read`,
  `workload:write`, and `route:write` permissions and canonical A3S ACLs;
  maintained client/CLI/Management MCP remain, while Web stays deferred. `C6` retains
  PostgreSQL replay/process-death evidence before the orchestration gate can
