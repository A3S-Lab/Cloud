...
`A3S_CLOUD_BX0_EXECUTE=1`, steps 1–5 may record `*=executed` only after
validated node ACL/token/`A3S_CLOUD_BX0_ENROLL_NODE_ID`,
`A3S_CLOUD_BX0_ARTIFACT_DIGEST`, `A3S_CLOUD_BX0_SERVICE_ID`,
`A3S_CLOUD_BX0_HEALTH_URL`, and `A3S_CLOUD_BX0_HTTPS_URL` from real
enroll/OCI/deploy/Ready/managed-TLS reach (never PLACEHOLDER); prep
...
execute fail-closed without node_id/digest/service_id/health_url/https_url) and
