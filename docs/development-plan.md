...
| APP0 | In progress; unavailable | Component-only `APP0.1-C1/C2/C3` freezes strong Application/Release identities, one canonical immutable release ACL, all six experiences with immutable classic/New Agent distinction, bounded delivery/audience policy, and exact Workflow revision evidence without copying graph or runtime state. Migration `124` and the A3S ORM repository persist immutable releases and sequence-fenced heads with exact Workflow content/payload checks plus atomic idempotency, audit, and Outbox facts. C3 adds authorization-before-replay create/publish/get/list CQRS and a metadata-only adapter over the existing Workflow repository. Production composition, maintained interfaces, sessions, publishing, monitoring, and enterprise completion remain. Full public parity is a composite `APP0.6` claim and no application-platform availability exists yet. |
...
The current REST/OpenAPI contract is `1.41.0`. It exposes the optional exact
Plan v3/v5 descriptor failure contracts, Plan v4 default-output port contract,
and typed projection evidence through the maintained client without adding a
failure-control surface. REST/OpenAPI `1.39.0` added the optional
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 111-tool administrator and 64-tool read-only catalogs,
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, twenty-eight accepted authority decisions, the immutable
