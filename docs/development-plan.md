...
| APP0 | In progress; unavailable | `APP0.1` implements strong Application/Release identities, one canonical immutable release ACL, all six experiences with immutable classic/New Agent distinction, exact Workflow revision evidence, migration `124` persistence, authorization before replay, CQRS, and REST/OpenAPI `1.42.0` plus maintained client, CLI, and six Management MCP tools over the same repository. Component-only `APP0.2-C1/C2/C3/C4` freezes and atomically persists Application-scoped end users, exact-release sessions, invocation-to-WorkflowRun correlation, monotonic input/Answer/final-output messages, optimistic immutable conversation-variable revisions, and exactly-once Workflow semantic effects through migration `125`; composes each exact invocation into deterministic ordinary Workflow Goal, Plan, and Run records; and compiles exact Model/Agent preset wrappers through Workflow's shared canonical publication authority. The PostgreSQL gate reconstructs persisted adapters and rejects authority drift. Remaining APP0.2 records and public interfaces, delivery, monitoring, and enterprise completion remain. Full public parity is a composite `APP0.6` claim and no application-platform availability exists yet. |
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, thirty-four accepted authority decisions, the immutable
...
3. retain the implemented `APP0.1` and component-only `APP0.2-C1/C2/C3/C4` authorities,
