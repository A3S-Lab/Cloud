...
| C0 | In progress | `C0.1`, `C0.2`, and `C0.2m` are verified. One typed TypeScript client is shared by Web and CLI; the versioned OpenAPI envelope, bounded transport, safe token handling, tenant/operational reads, replay-safe mutations, evidence, logs, diagnostics, Search, Workload/Source/Secret/Identity/Fleet/Edge parity, and compatibility checks pass focused tests. The verified pre-extension Management MCP gate proved exact 23-tool administrator and 16-tool read-only catalogs. The current 84-tool administrator and 49-tool read-only catalogs retain those tools and add fifteen Identity, one bounded tenant-administrator audit query, seven verified `W0.2` Ontology, ten `W0.3` Workflow definition/goal/plan, seven native Form lifecycle, seven WorkflowRun lifecycle, five protected HumanTask read/assignment/submission tools, three ExecutionTemplate tools, and six verified `U0.2` plugin Registry/catalog read tools; focused catalog, permission, strict-argument, lifecycle, deterministic-plan, WorkflowRun/HumanTask/ExecutionTemplate, plugin tenant, role, invitation, audit, and replay conformance pass. `C0.2m` uses the `2026-07-28` sessionless protocol with per-request metadata and `server/discover`. The backend-only `C0.3` slice implements stable human/service Principals, one explicit Principal-plus-Membership creation path, Membership roles, exact-Principal MembershipInvitations, Principal-bound credentials, exact OIDC issuer/subject links and replay-safe one-time flows, immediate role/revocation enforcement, last-owner protection, closed project/environment/node Resource Grants, A3S ORM/Outbox/audit writes, and the redacted keyset audit projection through REST/OpenAPI `1.27.0`, client, CLI, and Management MCP. OIDC discovery/JWKS/protocol surfaces, attribution, notifications, security investigation, audit retention/export, and role-focused frontend projections remain planned; `C0.3` is in progress and `C0.4` remains planned. |
...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology contract foundation, exact capability references, deterministic graph validation, the ten-node outcome mapping, and architectural guards against duplicate execution mechanisms. Verified `W0.2` adds immutable Ontology revisions, deterministic migration/diff policy, and authorized Search. The `W0.3` backend now includes immutable definitions, Goals and Plans, immutable descriptor and typed-variable domain contracts, native Forms, one Goal/Plan-bound WorkflowRun, the authority-bound HumanTask loop, deterministic reachable-Output aggregation, and one finite `execution` capability step. The variable contract freezes invocation/node/composite/run/application ownership, typed reads, deterministic assignments, explicit exports, schema ancestry, dominance, opaque Secret/object references, and optimistic Applications-port evidence without adding a variable store. Multiple declared Output sinks are terminal, settle only after every sink is active or inactive, omit inactive branches, retain the legacy scalar for one sink, and aggregate multiple active results by stable step ID under the existing byte bound. Migrations `098`-`100` add Executions-owned immutable ACL-native ExecutionTemplate revisions, an exact Run/Plan/step/attempt/template/digest binding on the ordinary Execution, and `execution` admission in the existing WorkflowStepProjection constraint. Workflow uses one typed Executions application port to create or adopt the child, links its existing Operation into the same A3S Flow run, resumes only from an authority-bound terminal result, and waits for the existing cleanup-first cancellation path. REST/OpenAPI `1.24.0`, the maintained client, CLI, and three Management MCP tools expose the template lifecycle through the same CQRS, grant, idempotency, A3S ORM, Outbox, and audit paths. The clean Linux H0 gate passes finite persistence plus the seven-boundary WorkflowRun `SIGKILL` fixture, including child-commit, exact-link, and terminal-resume boundaries. The clean C0.2 Management MCP/A3S Box/PostgreSQL gate shares the same ACL fixture and passes the exact `77/47` catalog plus its `8/8` template persistence, replay, rollback, immutability, and tenant non-disclosure result. Focused Workflow tests pass `98/98`, including ten immutable-descriptor and seventeen typed-variable contract tests plus Output aggregation, legacy replay, HumanTask, finite-child, and authority boundary tests. These verify domain contracts and focused Output and finite Execution sub-gates, not all of W0.3. Persistent contract catalog management and exact next-plan pinning, runtime variable execution/inspection, composite regions, Answer/error semantics, business-service and remaining provider steps, compensation, expanded clean `W0.3` provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
...
In the `W0` summary above, the remaining descriptor work means persistence of
the built-in catalog and exact semantic-digest pinning through an explicit next
compiler/plan revision. The immutable descriptor domain contract and its
canonical ACL conformance fixture are implemented; they do not make a public
node available.
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.27.0`. Focused tests regenerate the
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 84-tool administrator and 49-tool read-only catalogs,
  including Identity, Ontology, Workflow planning, native Form lifecycle,
  WorkflowRun lifecycle, and protected HumanTask read/assignment/submission extensions. The expanded clean A3S Box/PostgreSQL
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  Membership. REST/OpenAPI `1.27.0`, the maintained client, CLI, and six new
  ...
  `platform:write` nor self-renewing `token:write`. Provider discovery/JWKS verification and public OIDC
  surfaces remain gated and unavailable. `C0.3` remains in progress because
  the remaining OIDC protocol work, attribution, notifications, security
...
1. retain the frozen versioned ACL parity manifest, six accepted authority
   decisions, and implemented immutable step-descriptor domain contract;
   persist the catalog and pin exact semantic digests through an explicit next
   compiler/plan revision while retaining the no-duplicate authority tests;
