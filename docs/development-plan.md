...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, finite Execution plus its descriptor-bound typed failure edge and exact default-output fallback, Flow-derived authorized variable inspection, deterministic project-authorized 23-node discovery, immutable composite-region policy/bindings, deterministic frame/export and ordered reducers, Flow-backed sequential Iteration/Loop child WorkflowRun lifecycle, and Flow-owned Connector observation/wait/retry interpretation. Migrations `103`, `107`, and `108` retain exact semantic, variable-default, and composite-region material; migration `122` adds only nullable default-output projection evidence and corrects the existing selected-handle constraint for terminal Execution failures. Compiler schema 2 emits Plan v2 with exact descriptor, semantic, variable, optional composite-region, and provider-policy pins while preserving Plan v1 bytes; a graph that opts into the finite-Execution error port emits Plan v3, while the mutually exclusive exact default fallback emits Plan v4 with one typed output-port contract and policy v3 material. WorkflowRun input/runtime/Flow v2 retains non-composite typed-variable/default execution; v3 freezes composite execution; v4 preserves typed finite-Execution failure routing; historic v5 composes exact Connector attempts, durable waits, bounded retry, and body-free results; v6 adds an exact immutable response-object reference after Connectors stores accepted bytes through the shared object authority; and v7 folds the same terminal finite-Execution observation into the exact default with typed evidence. Replay build `a3s-cloud-workflows@7` keeps `@1` through `@6` explicitly compatible. REST/OpenAPI `1.41.0` exposes Plan v3/v4 failure/default contracts and projection evidence through the maintained client. `AUT0.5-C8` supplies the Connectors-owned C6 attempt adapter, `C9` freezes its retry budget, `C10` freezes response-object composition, and `C11` adds the environment-authorized terminal-evidence read boundary. Flow alone owns Connector scheduling, uses bounded `Retry-After` or the fallback delay, re-observes deferred attempts, and fails indeterminate attempts closed; public surfaces remain body-free and C6 remains the sole provider-attempt authority. These slices add no Workflow table, cache, event log, worker, scheduler, queue, retry counter, child Operation, credential authority, or provider mechanism. Typed W0.4 node consumption, Applications-owned variables, Answer and non-Execution error semantics, remaining application ports and provider runtimes, compensation, expanded provider conformance, `W0.5`, and public availability remain planned. |
...
Plan v3 descriptor failure contracts, Plan v4 default-output port contract,
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@7`. The
  former `@1` through `@6` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `AUT0.5-C9`: `cloud.workflow.policy.v2`
  ...
  queue, scheduler, or configuration language. WorkflowRun v5 through v7
  consume this policy while the HTTP Request node remains unavailable.
...
- Implemented as the component-only Workflow Connector Flow slice
  (`2026-08-20`): WorkflowRun input/runtime/Flow v6 introduced the exact
  provider-attempt interpretation, and current replay build
  `a3s-cloud-workflows@7` preserves it while binding every attempt and
  observation to one deterministic hook. The coordinator verifies hook creation
  history and delegates only to the C8 port over C6. Retryable evidence schedules one
  ...
  Accepted output and projection contain only the exact immutable response
  reference, digest, and length; raw bytes remain outside Flow. Historic v5
  output remains body-free and byte-compatible. Versions/builds `@1` through
  `@6` remain explicitly replayable. No table, migration, queue, timer worker,
  scheduler, retry counter, child Operation, credential authority, provider
  configuration, or HTTP client is added.
...
- W0.4 typed node consumption, remaining provider/Event-consumer wiring,
  revocation/recovery operations, and retained
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, twenty-three accepted authority decisions, the immutable
   ...
   and Plan v4/Run v7 exact default-output folding/evidence while retaining the
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; complete Answer frames and non-Execution error routes while
