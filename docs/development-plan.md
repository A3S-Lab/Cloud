...
| U0 | In progress; unavailable | The Cloud slice of `U0.1` pins the canonical A3S Use protocol-level-4 host contract and adds explicit capabilities, package-plan, enablement-plan, digest-only apply, and observation Fleet payloads plus one optional Node Agent adapter over the sole shared `PluginHostManager`. They reuse the existing command queue and journal. The first `U0.2` slice adds the `PluginRegistry` domain and migration 083 persistence without copying TUF/catalog state. Root compatibility locking, production Manager composition, registry application/interfaces, the published Use catalog adapter, assignments, and real-provider gates remain open; no user-facing capability is claimed. |
...
catalog or TUF metadata is persisted. Immutable root-object admission, the
published A3S Use catalog adapter, application queries, authorized Search
projection, REST/client/CLI/Management MCP reads, and real-provider exit
evidence remain open; the Web projection stays deferred by section 1.1.
