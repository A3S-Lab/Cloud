...
The current REST/OpenAPI contract is `1.59.0`. It adds the complete signed
audit manifest over `1.58.0`'s retention status, `1.57.0`'s signed page,
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.59.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 136-tool administrator and 76-tool read-only catalogs,
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   ...
   and closed, bounded finite-Execution/Connector/HumanDecision evidence
   correlations reconstructed from verified Flow history without another
   evidence store,
