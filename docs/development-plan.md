...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@16` and patch
  marker `cloud.flow.bounded-step-retries-v1`. The former `@1` through `@15`
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  attribution and `C0.3-PA2b` signed-page export are verified, later cross-layer
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@16` keeps
  versions/builds `@1` through `@15` explicitly replayable. Migration `123`
...
- Verified in main CI as `C0.3-PA2b`: expose one
  ...
  also remains green.
- After PA2b, extend the same audit authority with explicit
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, forty-five accepted authority decisions, the immutable
   ...
   and Plan v6/Run v14 descriptor-bound Application-variable failure routing
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   snapshot/CAS semantics, including v13 repeated-frame ordinals and v14
   deterministic write-failure routing, and complete other non-Execution error routes while
