...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, finite Execution plus its descriptor-bound typed failure edge, Flow-derived authorized variable inspection, deterministic project-authorized 23-node discovery, the immutable composite-region policy/binding foundation, deterministic composite frame/export and ordered region reducers, and Flow-backed sequential Iteration/Loop child WorkflowRun dispatch/linkage/cancellation/recovery. Migration `103` persists three mandatory semantic children; migration `107` permits optional exact variable-default material; migration `108` permits optional `cloud.workflow.composite-regions.v1` material without adding a table and requires new composite publication to exactly cover Iteration/Loop descriptors and child WorkflowRevision bindings. Compiler schema 2 emits Plan v2 with exact descriptor, semantic, variable, and optional composite-region digest pins, prevents authority downgrade, and preserves Plan v1 byte shape. A graph that opts into the exact finite-Execution error port emits Plan v3 with every descriptor failure contract pinned. WorkflowRun input/runtime/Flow v2 retains non-composite typed-variable/default execution; v3 freezes exact composite material and reconstructs parent and reduced composite variables from immutable input plus existing Flow history; v4 composes those semantics with bounded `cloud.workflow.step-failure.v1` routing over the same Execution hook, DAG, Flow history, and projection while migrations `105`, `107`, and `108` only expand immutable input bounds. REST/OpenAPI `1.35.0`, client, CLI, MCP, focused domain/REST/replay/catalog/default/inspection/composite/failure-route tests, and PostgreSQL migration/replay/rollback/immutability plus reconnect gates cover the foundation. `AUT0.5-C8` corrects exact Connector capability ownership and supplies a component-only Connectors-owned attempt port; `C9` freezes its bounded retry budget in the existing immutable policy payload, without scheduling the node. Variable inspection, composite binding/frame/region reduction/runtime dispatch/child adoption/cancellation, finite failure routing, the Connector attempt port, and the retry contract add no Workflow table, cache, event log, worker, scheduler, queue, retry engine, or provider mechanism; the catalog has no persistence or write path and cannot admit descriptors or claim public parity. Connector dispatch and remaining provider child lifecycle, Applications-owned variables, default-output fallback, Answer and non-Execution error semantics, remaining application ports, compensation, expanded provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@4`. The
  former `@1`, `@2`, and `@3` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `AUT0.5-C9`: `cloud.workflow.policy.v2`
  ...
  queue, scheduler, or configuration language; Flow interpretation remains
  open and the HTTP Request node remains unavailable.
...
- Workflow Flow scheduling/interpretation and immutable response-object composition,
  provider/Event-consumer wiring, revocation/recovery operations, and retained
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, nineteen accepted authority decisions, the immutable
   step-descriptor domain contract, the read-only discovery projection, Plan
   v2 exact semantic pins, and Plan v3 descriptor-bound finite-Execution
   failure routing while retaining the no-duplicate authority
   tests;
