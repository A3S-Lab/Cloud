...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because outbound notification adapters and alert
  policy, security investigation, usage-fact profile snapshots, audit
  retention/export, and role-focused frontend projections remain open.
...
- Implemented as `C0.3-N1`: the in-app Notifications adapter projects committed
  ...
  logical inbox record. The slice adds no second event rail, provider queue,
  template/subscription authority, scheduler, or configuration format.
- Add signed webhook, external SMTP, and Slack-compatible outbound adapters over
  the same projection and transactional-Outbox boundary. Their delivery must be
  deduplicated, retryable, rate-limited, and never an operation authority; they
  may not replay the business command or create another provider/configuration
  authority.
