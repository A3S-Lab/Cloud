...
| `A0.5` | In progress | Exact Git archive publication, immutable Skill release binding/rebinding/unbinding, migration 067 persistence, read-only Runtime Artifact mounts, rollback-safe revisions, and authorized REST/client/CLI/catalog surfaces are implemented. The CI PostgreSQL gate drives REST publication against a real local immutable-object provider and retains exact archive bytes, receipt identity, replay, yanking, and temporary-bundle cleanup evidence. A separate real Box gate certifies typed Skill-bundle hydration, exact bytes, the production `/a3s/skills/{asset_id}` read-only mount shape, guest write rejection, one download, and Runtime-remove cleanup; full real Box Workload binding, rebinding, unbinding, and rollback evidence still blocks verification |
