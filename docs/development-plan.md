...
| C0 | In progress | `C0.1`, `C0.2`, and `C0.2m` are verified. One typed TypeScript client is shared by Web and CLI; the versioned OpenAPI envelope, bounded transport, safe token handling, tenant/operational reads, replay-safe mutations, evidence, logs, diagnostics, Search, Workload/Source/Secret/Identity/Fleet/Edge parity, and compatibility checks pass focused tests. The verified pre-extension Management MCP gate proved exact 23-tool administrator and 16-tool read-only catalogs. The current 91-tool administrator and 54-tool read-only catalogs retain those tools and add fifteen Identity, two Project-attribution, one bounded tenant-administrator audit query, three personal-notification tools, seven verified `W0.2` Ontology, ten `W0.3` Workflow definition/goal/plan, one read-only built-in Workflow node-catalog query, seven native Form lifecycle, eight WorkflowRun lifecycle including Flow-derived variable inspection, five protected HumanTask read/assignment/submission tools, three ExecutionTemplate tools, and six verified `U0.2` plugin Registry/catalog read tools; focused catalog, permission, strict-argument, lifecycle, deterministic-plan, Workflow node-catalog, WorkflowRun/HumanTask/ExecutionTemplate, plugin tenant, role, invitation, audit, attribution, notification, and replay conformance pass. `C0.2m` uses the `2026-07-28` sessionless protocol with per-request metadata and `server/discover`. The `C0.3` slice implements stable human/service Principals, one explicit Principal-plus-Membership creation path, Membership roles, exact-Principal MembershipInvitations, Principal-bound credentials, exact OIDC issuer/subject links and replay-safe one-time flows, a bounded OIDC discovery/JWKS/ID-token adapter, production-wired login/link/callback routes and maintained-client entry points, immediate role/revocation enforcement, last-owner protection, closed project/environment/node Resource Grants, immutable versioned Project attribution, a personal in-app notification inbox, component-only signed-webhook/Slack-compatible adapters, A3S ORM/Outbox/audit writes, and the redacted keyset audit projection through REST/OpenAPI `1.32.0`, client, CLI, and Management MCP. Focused attribution and notification tests pass, and the [existing PostgreSQL 17 foundation job](https://github.com/A3S-Lab/Cloud/actions/runs/31766502180/job/94663412171) executes both persistence gates to prove lineage, replay, stale-write rejection, immutability, recipient isolation, projection deduplication, and exact transaction evidence. The notification slice reuses the transactional Outbox relay, A3S Event, shared Resource Grant evaluator, idempotency, audit, and A3S ORM migrator without another queue, provider authority, or configuration format. Durable outbound dispatch/receipts/subscriptions/rate policy and SMTP, alert policy, security investigation, usage-fact profile snapshots, audit retention/export, and role-focused frontend projections remain planned; `C0.3` is in progress and `C0.4` remains planned. |
...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, finite Execution, Flow-derived authorized variable inspection, and a deterministic project-authorized 23-node discovery catalog. Migration `103` persists three mandatory semantic children: exact descriptor bindings, the recoverable registry snapshot, and the typed-variable contract; migration `107` permits one optional immutable `cloud.workflow.variable-defaults.v1` child whose identity, coverage, canonical JSON, and digest must match the contract. Compiler schema 2 emits Plan v2 with exact descriptor and semantic/variable digest pins, prevents authority downgrade, and preserves Plan v1 byte shape. WorkflowRun input/runtime/Flow v2 freezes the variable ACL and optional default material and reconstructs invocation, node-output, defaults, deterministic run-assignment, typed-read, and opaque-reference values from immutable input plus existing Flow history; migrations `105` and `107` only expand that immutable input's bound. REST/OpenAPI `1.34.0`, client, CLI, MCP, focused domain/REST/replay/catalog/default/inspection tests, and PostgreSQL migration/replay/rollback/immutability plus catalog/inspection reconnect gates cover the slice. Variable inspection reuses the execution materializer and correlated Flow history without a table, cache, event log, or worker; the catalog composes the frozen parity manifest with an exact digest-bound profile ACL, has no persistence or write path, and cannot admit descriptors or claim public parity. Composite and Applications-owned variables, Answer/error semantics, remaining application ports, compensation, expanded provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because durable outbound notification dispatch,
  receipts, subscriptions, rate policy, and SMTP plus alert policy, security
...
- Complete outbound delivery by emitting a Notification-owned transactional
  ...
  and subscription references to shared Connectors/Secrets authority; do not
  call a provider from the source Outbox projector or add a queue or scheduler.
  External SMTP also requires an exact Identity-owned verified recipient
  contact reference and may not infer email from OIDC claims. Provider outage
  must not block unrelated integration events, replay the business command, or
  create another provider/configuration authority.
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, ten accepted authority decisions, the immutable
