...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because supported outbound-subscription management,
  user-configured suppression/delivery budgets, SMTP plus alert policy, security
...
- Implemented as component-only `C0.3-N2e`: Notifications permits at most eight
  ...
  ACK-loss replay, and the migration guard; retained PostgreSQL execution of
  migration `115` remains the final evidence gate. This slice adds no mutable
  retry counter, rate/token bucket, timer, sleep, queue, scheduler, or second
  retry policy. User-configured budgets require a later versioned ACL semantic
  gate and may not mutate the v1 subscription definition.
...
- Complete outbound delivery product availability by exposing the existing
  ...
  surfaces and retaining the NATS/PostgreSQL evidence gate. Any future
