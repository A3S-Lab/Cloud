...
| C0 | In progress | `C0.1`, `C0.2`, and `C0.2m` are verified. One typed TypeScript client is shared by Web and CLI; the versioned OpenAPI envelope, bounded transport, safe token handling, tenant/operational reads, replay-safe mutations, evidence, logs, diagnostics, Search, Workload/Source/Secret/Identity/Fleet/Edge parity, and compatibility checks pass focused tests. The verified pre-extension Management MCP gate proved exact 23-tool administrator and 16-tool read-only catalogs. The current 74-tool administrator and 45-tool read-only catalogs retain those tools and add nine Identity, seven verified `W0.2` Ontology, ten `W0.3` Workflow definition/goal/plan, seven native Form lifecycle, seven WorkflowRun lifecycle, five protected HumanTask read/assignment/submission tools, and six verified `U0.2` plugin Registry/catalog read tools; focused catalog, permission, strict-argument, lifecycle, deterministic-plan, WorkflowRun/HumanTask, plugin tenant, role, and replay conformance pass. `C0.2m` uses the `2026-07-28` sessionless protocol with per-request metadata and `server/discover`. The backend-only `C0.3` slice implements stable human/service Principals, Membership roles, Principal-bound credentials, immediate role/revocation enforcement, last-owner protection, closed project/environment/node Resource Grants, A3S ORM/Outbox/audit writes, REST/OpenAPI `1.22.0`, client, CLI, and Management MCP. Invitations, OIDC, attribution, notifications, security investigation, audit-query interfaces, and role-focused frontend projections remain planned; `C0.3` is in progress and `C0.4` remains planned. |
...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology contract foundation, exact capability references, deterministic graph validation, the ten-node outcome mapping, and architectural guards against duplicate execution mechanisms. Verified `W0.2` adds immutable Ontology revisions, deterministic migration/diff policy, and authorized Search; focused tests and the clean A3S Box/PostgreSQL cross-surface gate pass strict `12/12` persistence, rejected-write, idempotency, Outbox, audit, Search, immutability, replay, and tenant non-disclosure evidence. The `W0.3` planning slice adds immutable Workflow definitions/revisions, exact closed ACL payloads, Goals, and deterministic Plan revisions. Migration `079` adds project-scoped canonical Form drafts and immutable native owner-compiled releases. Migration `080` adds an A3S ORM-backed WorkflowRun bound to one exact Goal/Plan and correlated Operation/A3S Flow run. Migration `081` adds typed A3S ORM persistence for immutable FormSubmission and WorkflowDecision records, optimistic HumanTasks, hook Inbox evidence, and leased resume Outbox/receipts. Migration `095` adds the typed overdue-task index/query and exact `HookReceived`/`RunTimedOut` settlement evidence needed for deterministic automatic expiry. Workflow-local `input`, `transform`, `branch`, `human_decision`, and `output` execution, immutable replay validation, worker-role task/expiry coordination, lease/retry/conflict resume recovery, cancellation, deadline timeout, output, and history share the same Flow authority. Real PostgreSQL plus Flow gates cover concurrency, atomic submission/decision storage, automatic expiry after parent timeout, lease takeover, commit-before-ack recovery, replay, tenant isolation, and four API/worker `SIGKILL` boundaries without duplicating WorkflowRun, Operation, Flow history, terminal projection, or cancellation. REST `1.22.0`, the maintained client, CLI, and Management MCP add protected HumanTask reads and versioned claim/release/submission through the existing Workflow repository, domain state machine, transaction-bound idempotency/Outbox/audit path, and shared Identity Resource Grant evaluator; lists omit interaction requests and only the current claimant receives the exact native A3S Form request. Parent-run cancellation coordination, service/finite-task dispatch, typed capability steps, compensation, expanded clean `W0.3` cross-surface conformance, `W0.4`-`W0.5`, and public availability remain planned. |
...
PostgreSQL/Flow recovery evidence. Migration `095` reuses that coordinator and
Outbox for deterministic Run/Plan-bound automatic expiry, with indexed overdue
candidates and exact parent `RunTimedOut` supersession receipts. Protected
...
Parent-run cancellation coordination and an end-to-end human workflow product
surface remain open.
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.22.0`. Focused tests regenerate the
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  Workload-stop replay, A3S ORM state, and credential-free evidence. Focused
    tests prove the current 74-tool administrator and 45-tool read-only catalogs,
...
- Add invitations on top of the implemented Membership and Resource Grant
  authority. Platform administration remains a separate role and cannot be
...
Migration `095` adds a typed indexed scan for overdue non-terminal HumanTasks;
...
decision/Outbox path. The same resume worker accepts either exact
`HookReceived` evidence or, only for a matching expiry, the parent
`RunTimedOut` event, closing the deadline race without a timer service or a
second queue.
...
automatic expiry after parent Flow timeout, expired-lease takeover, Flow commit
...
pair, and monotonic aggregate version at every boundary. REST `1.22.0`, the
maintained client, `human-tasks` CLI commands, and five Management MCP tools
additionally expose bounded protected HumanTask reads and versioned claim/release/submission through
the same Workflow repository, domain state machine, transaction-bound
idempotency/Outbox/audit path, and shared Resource Grant evaluator. Parent-run
cancellation coordination, service/finite-task execution, typed capability dispatch, compensation,
...
and HumanTask submission/automatic-expiry loop now consume that foundation;
parent-run cancellation coordination, service/finite-task dispatch, compensation, multi-day
...
## 13.2 Milestone EV0: governed self-evolution
