...
revisions, runs enroll/OCI/deploy/health/HTTPS/logs/update steps 1–7
preflight only (`enroll=not_run`, `oci=not_run`, `deploy=not_run`,
`health=not_run`, `https=not_run`, `logs=not_run`, `update=not_run`), leaves
steps 8–9 OPEN, and leaves Power `UNBOUND` until PW0; product `EXIT` remains
...
paths (including stub pin missing/mismatch/match and
enroll/OCI/deploy/health/HTTPS/logs/update preflight) and may emit
