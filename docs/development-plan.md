...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.26.0`. Focused tests regenerate the
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  Membership. REST/OpenAPI `1.26.0`, the maintained client, CLI, and six new
