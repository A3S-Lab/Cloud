...
The current REST/OpenAPI contract is `1.70.0`. It adds the nullable closed
immutable Harness invocation profile to Agent executions and typed digest-only
Tool request/result event content without Tool payload or Secret material. It
retains `1.69.0`'s optional closed Agent `providerKind` selector and complete
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.70.0`. Focused tests
...
that component. Production model/Tool binding producers, any additional
independent MCP binding, and retained PostgreSQL/real-provider Tool-audit and
recovery evidence remain open. `A1.5` adds
