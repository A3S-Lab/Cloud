...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, finite Execution plus its descriptor-bound typed failure edge, Flow-derived authorized variable inspection, deterministic project-authorized 23-node discovery, immutable composite-region policy/bindings, deterministic frame/export and ordered reducers, Flow-backed sequential Iteration/Loop child WorkflowRun lifecycle, and component-only Flow-owned Connector observation/wait/retry interpretation. Compiler schema 2 emits Plan v2 with exact descriptor, semantic, variable, optional composite-region, and provider-policy pins while preserving Plan v1 bytes; a graph that opts into the finite-Execution error port emits Plan v3 with every descriptor failure contract pinned. WorkflowRun input/runtime/Flow v2 retains non-composite typed-variable/default execution; v3 freezes composite-only execution; v4 preserves Plan v3 typed failure routing; historic v5 composes exact Connector attempts, completed waits, bounded retry, and body-free results. New v6 adds an exact immutable response-object reference after Connectors stores accepted bytes through the shared object authority. Replay build `a3s-cloud-workflows@6` keeps `@1` through `@5` explicitly compatible. `AUT0.5-C8` supplies the Connectors-owned C6 attempt adapter, `C9` freezes its retry budget, and `C10` freezes response-object composition. Flow alone owns Connector scheduling, uses bounded `Retry-After` or the fallback delay, re-observes deferred attempts, and fails indeterminate attempts closed. These slices add no Workflow table, cache, event log, worker, scheduler, queue, retry counter, child Operation, credential authority, or provider mechanism. W0.4 response consumption, Applications-owned variables, default-output fallback, Answer and non-Execution error semantics, remaining application ports and provider runtimes, compensation, expanded provider conformance, `W0.5`, and public availability remain planned. |
...
| AUT0 | In progress; unavailable | New-invocation schedules/webhooks/plugin events and reusable outbound connection profiles are specified in `ai-application-platform-plan.md`. Component-only `AUT0.5-C1` supplies the exact-revision execution port and bounded HTTP executor; verified `C2`-`C6` add immutable profiles/revisions, exact Secret materialization, public-Internet egress, exact-attempt evidence, durable fencing, conservative recovery, and atomic settlement; implemented `C7` exposes the same profile/revision CQRS without resolving Secrets; component-only `C8` binds exact WorkflowRun/Plan/step-attempt and Connector authority to C6; `C9` freezes its bounded attempt budget and fallback delay; and `C10` stores accepted response bytes through the sole shared immutable-object authority before terminal evidence. WorkflowRun v6 keeps exact hook observation, durable wait, bounded retry, fail-closed indeterminate decisions, and an exact response-object reference in Flow while historic v5 remains body-free and C6 remains the sole provider-attempt authority. Component-only `C0.3-N2b` supplies the first exact-subject Notification NATS-to-C6 composition, while `N2c`-`N2e` retain Notification delivery decisions. Response consumption, remaining provider/consumer wiring, revocation/recovery operations, retained end-to-end integration evidence, and all Automations/Connectors product availability remain open. |
...
The current REST/OpenAPI contract is `1.40.0`. It exposes the optional exact
Plan v3 descriptor failure contracts through the maintained client without
adding a failure-control surface. REST/OpenAPI `1.39.0` added the optional
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@6`. The
  former `@1`, `@2`, `@3`, `@4`, and `@5` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `AUT0.5-C9`: `cloud.workflow.policy.v2`
  ...
  queue, scheduler, or configuration language. Historic WorkflowRun v5 and
  current v6 consume this policy while the HTTP Request node remains
  unavailable.
...
- Implemented as the component-only Workflow Connector Flow slice
  (`2026-08-20`): WorkflowRun input/runtime/Flow v6 and replay build
  `a3s-cloud-workflows@6` bind every exact provider attempt and observation to
  ...
  `@5` remain explicitly replayable. No table, migration, queue, timer worker,
...
- W0.4 response consumption, remaining provider/Event-consumer wiring,
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, twenty accepted authority decisions, the immutable
   ...
   routing, Run v5 Connector attempt/wait replay, and Run v6 immutable
   response-object interpretation while retaining the no-duplicate authority
   tests;
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; complete Answer frames, default-output fallback, and
   non-Execution error routes while preserving historical Flow replay
