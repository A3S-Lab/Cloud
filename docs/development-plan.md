...
- Focused tests cover canonical ACL projection, wire bounds, start/inspect
  ...
  remove. The exact Linux Box provider, real cache hit, external
  Registry/Vault, persistent-store process-death, and zero-residue gate remains
  open.
...
Exact external-provider, Registry/publication/signing, OS-process interruption
over persistent Fleet/Flow state, and final published-Workload certification
remain open.
