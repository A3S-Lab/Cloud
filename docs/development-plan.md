...
| MCP0 | In progress; unavailable | Closed cross-repository contracts, Runtime profile/generation fencing, Cloud canonical immutable profile plus separately mutable route-policy ACL, exact AssetRelease/profile-bound WorkloadRevision persistence and automatic ordinary Runtime projection, and Gateway request/auth/single-dispatch/JSON-SSE foundations pass focused tests. Cloud credential resolution, healthy target reconciliation and real PostgreSQL evidence, real Box/Linux, reload/drain/telemetry, and joint conformance remain open |
