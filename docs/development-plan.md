...
The current REST/OpenAPI contract is `1.66.0`. It adds bounded unresolved
Connector-attempt reads and one exact idempotent operator conclusion that can
close an expired dispatch only as body-free `indeterminate` evidence. It
retains `1.65.0`'s exact Connector revision revocation and closed reusable
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.66.0`. Focused tests
