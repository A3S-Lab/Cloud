...
Current runtime build `a3s-cloud-workflows@25` retains `@1` through `@24` for
...
domain-driven or multi-provider compensation, expanded provider conformance,
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@25` and patch
  marker `cloud.flow.bounded-step-retries-v1`. The former `@1` through `@24`
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@25` keeps
  versions/builds `@1` through `@24` explicitly replayable. Migration `123`
