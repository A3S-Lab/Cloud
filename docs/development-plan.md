...
| CELL0 | In progress; unavailable | `CELL0.1-C1/C2/C3` implement the canonical application foundation. Component-only `CELL0.2-C1` extends the sole shared object client with atomic conditional operations, adds the S0 namespace port and destructive cleanup-verified CAS probe, centralizes exact Secret-version identity, freezes digest-locked namespace credential lineage, and correlates one current Durable Cell revision to that exact S0 generation without plaintext or another lifecycle. Active-Secret admission/materialization, sealed recovery/retention/deletion, real S0 and Runtime/Box provider certification, Cloud orchestration, Gateway publication, retained fault evidence, and `CELL0.5` availability remain open. |
...
- `CELL0.2` consumes the one S0 object-provider/Secret path and proves
  ...
  correlation. It deliberately fails closed on the uncertified local backend;
  active Secret admission/materialization, sealed recovery, retention,
  deletion, and real provider evidence remain.
- `CELL0.3` certifies a pinned provider and typed operator adapter as one
  ...
  endpoints, health, graceful drain, adoption, and cleanup.
- `CELL0.4` adds A3S ORM persistence, idempotent commands/queries, the managed
  Workload/S0/Gateway projection, Operations, audit, and REST/client/CLI/MCP
  interfaces. Web is deferred under the active frontend freeze.
