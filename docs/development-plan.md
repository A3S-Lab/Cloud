...
| CELL0 | In progress; unavailable | `CELL0.1-C1/C2/C3` implement the canonical application foundation. Component-only `CELL0.2-C1/C2` extend the sole shared object client with atomic conditional operations and a destructive CAS probe, centralize exact Secret-version identity, reuse Secrets for active admission/JIT zeroizing materialization, freeze digest-locked namespace credentials plus sealed recovery/retention/isolated-restore/safe-deletion contracts, and correlate one current Durable Cell revision without plaintext or another lifecycle. Real S0 and Runtime/Box provider execution/certification, Cloud orchestration, Gateway publication, retained fault evidence, and `CELL0.5` availability remain open. |
...
- `CELL0.2` consumes the one S0 object-provider/Secret path and proves
  ...
  and fails closed on the uncertified local backend; real provider execution,
  retained fault evidence, and certification remain.
