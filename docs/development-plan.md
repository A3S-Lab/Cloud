...
Current runtime build `a3s-cloud-workflows@22` retains `@1` through `@21` for
...
schemas without adding a table or column. The milestone remains in progress: List Operator, business-service, and remaining
...
The current REST/OpenAPI contract is `1.61.0`. It enumerates the versioned
Variable Aggregator Workflow payload schema through the existing ACL transport
and maintained client without adding a route or JSON property. It retains
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@22` and patch
  marker `cloud.flow.bounded-step-retries-v1`. The former `@1` through `@20`
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.61.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 137-tool administrator and 77-tool read-only catalogs,
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@22` keeps
  versions/builds `@1` through `@21` explicitly replayable. Migration `123`
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, fifty-one accepted authority decisions, the
   ...
   Run v20 exact typed Workflow-local Variable Aggregation,
   and closed, bounded finite-Execution/Connector/HumanDecision/Subworkflow
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   root/frame Answer write-failure routing, and complete List Operator, business-service, and
