...
| APP0 | In progress; unavailable | `APP0.1` implements strong Application/Release identities, one canonical immutable release ACL, all six experiences with immutable classic/New Agent distinction, exact Workflow revision evidence, migration `124` persistence, authorization before replay, CQRS, and REST/OpenAPI `1.42.0` plus maintained client, CLI, and six Management MCP tools over the same repository. Component-only `APP0.2-C1/C2/C3/C4/C5` freezes and atomically persists Application-scoped end users, exact-release sessions, invocation-to-WorkflowRun correlation, monotonic input/Answer/final-output messages, optimistic immutable conversation-variable revisions, exactly-once Workflow semantic effects, and immutable invocation execution authority through migrations `125`-`126`; it also compiles deterministic preset wrapper Workflows. It composes each exact invocation into deterministic ordinary Workflow Goal, Plan, and Run records and recovers cancellation from persisted authority through the existing Workflow compilers, repositories, and state machine. The PostgreSQL gate reconstructs adapters, rejects authority drift, and proves failed authority admission rolls back atomically. Remaining APP0.2 records and public interfaces, delivery, monitoring, and enterprise completion remain. Full public parity is a composite `APP0.6` claim and no application-platform availability exists yet. |
...
3. retain the implemented `APP0.1` and component-only `APP0.2-C1/C2/C3/C4/C5` authorities,
