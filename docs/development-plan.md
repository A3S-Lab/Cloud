...
revisions, runs enroll step-1 and OCI step-2 preflight only (`enroll=not_run`,
`oci=not_run`), leaves steps 3–9 OPEN, and leaves Power `UNBOUND` until PW0;
...
(including stub pin missing/mismatch/match and enroll/OCI preflight) and may
emit `A3S_CLOUD_BX0_CLEAN_HOST_CI_CERTIFIED` only; it never unlocks product EXIT.
