...
The current REST/OpenAPI contract is `1.43.0`. It adds caller-owned
project-member Application session open/read, invocation request/read, and
bounded ordered-message reads through the `APP0.2-C8` adapter over the sole C6
authority. It retains `1.42.0`'s project-authorized Application
...
- Cloud pins the exact A3S Flow `1.0.0` release, A3S Boot `0.2.0` with
  `queue-postgres`, and
  ...
  passes that exact composition. Code `7.0.1` resolves the same exact Flow
  release, so the former transitive version split is closed.
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@11`. The
  former `@1` through `@10` generations are explicit replay-compatible migration entries;
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 122-tool administrator and 68-tool read-only catalogs,
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@11` keeps
  versions/builds `@1` through `@10` explicitly replayable. Migration `123`
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, forty accepted authority decisions, the immutable
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; retain descriptor-bound graph Answer frames and complete
   Application-variable semantics plus remaining non-Execution error routes while
...
3. retain the implemented `APP0.1`, component-only
   ...
   adapter, and component-only `APP0.2-C9/C10`,
