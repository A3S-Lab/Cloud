...
4. **Bound retries and durable activities — finite retry policy implemented;
   durable page checkpoints pending.** Flow remains the sole retry and timer
   ...
   unmarked histories retain their exact fixed retry contract. Object recovery
   and other large activities must still checkpoint deterministic pages so
   process death never restarts an unbounded namespace-sized step. No context
   adds a retry table, sleep loop, random state, or queue.
...
The current REST/OpenAPI contract is `1.46.0`. It adds nullable immutable
outbound-notification event-time suppression to the versioned delivery budgets
while retaining the caller-owned
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@14` and patch
  marker `cloud.flow.bounded-step-retries-v1`. The former `@1` through `@13`
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 125-tool administrator and 68-tool read-only catalogs,
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because SMTP plus alert policy, retained
...
- Implemented as component-only `C0.3-N2c`: one immutable personal outbound
  ...
  scheduler, or second event rail. SMTP, alert policy, and retained production
...
- Implemented as component-only `C0.3-N3b`: canonical
  ...
  pass; the updated PostgreSQL 17/NATS fixture is wired and retained CI evidence
  is pending. This admission-only policy adds no mutable silence record,
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@14` keeps
  versions/builds `@1` through `@13` explicitly replayable. Migration `123`
