...
| F0 | Re-certification required | The last complete evidence set covered isolated PostgreSQL migrations, tenancy, idempotency, local/NATS outbox, A3S Flow `0.12.0` history, A3S Boot `0.2.0` PostgreSQL task management, A3S ORM `0.3.0`, queue-failure readiness, and the nine-boundary persistent Build Flow `SIGKILL` gates. The current exact lock moves Cloud to Flow `0.13.1` for the shared DAG compiler and must pass the same composition gates before verification is restored |
