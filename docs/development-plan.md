...
| F0 | Re-certification required | The last complete evidence set covered isolated PostgreSQL migrations, tenancy, idempotency, local/NATS outbox, A3S Flow `0.12.0` history, A3S Boot `0.2.0` PostgreSQL task management, A3S ORM `0.3.0`, queue-failure readiness, and the nine-boundary persistent Build Flow `SIGKILL` gates. The current exact lock moves Cloud to Flow `0.13.1` for the shared DAG compiler, unifies ACL `0.3.0` on one exact revision, and must pass the same composition gates after the remaining upstream ACL `0.2.2` and Flow `0.11.0` constraints converge |
...
- Cloud pins A3S Flow `0.13.1`, A3S Boot `0.2.0` with `queue-postgres`, and
  ...
  lock remains under recertification. The transitive Flow `0.11.0` pulled by
  the pinned Code protocol dependency must also converge before that gate can
  return to `Verified`.
