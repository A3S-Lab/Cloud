...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@13`. The
  former `@1` through `@12` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `C0.3-N3a`: canonical
  ...
  OpenAPI snapshot, TypeScript client, and CLI tests pass; real PostgreSQL 17
  evidence remains pending. This adds no mutable counter, rate/token bucket,
...
- Implemented as the component-only Workflow Connector Flow slice
  ...
  byte-compatible. Current replay build `a3s-cloud-workflows@13` keeps
  versions/builds `@1` through `@12` explicitly replayable. Migration `123`
