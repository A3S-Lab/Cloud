...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology contract foundation, exact capability references, deterministic graph validation, the ten-node outcome mapping, and architectural guards against duplicate execution mechanisms. The backend `W0.2` implementation adds immutable A3S ORM Ontology revisions, deterministic migration/diff policy, authorized Search, REST `1.11.0`, maintained client, CLI, and seven Management MCP tools with historical idempotency replay. Focused tests pass; clean real-PostgreSQL conformance still blocks verification. `W0.3`-`W0.5` plan compilation, typed dispatch, recovery, and public availability remain planned. |
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.11.0`. Focused tests regenerate the
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  current 35-tool administrator and 21-tool read-only catalogs, annotations, all five
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  idempotency, Outbox facts, and shared audit records. REST/OpenAPI `1.11.0`,
...
Items 1 and 2 are implemented in the backend. `W0.2` uses migration `075`,
...
changes, one authorized Search projection, REST `1.11.0`, the maintained
...
pass. The clean real-PostgreSQL lifecycle and expanded cross-surface
conformance gates still block verification; no frontend was added.
