...
- `CELL0.4-C2` registers authorized idempotent create, revise, start, and stop
  ...
  application state. `C3` persists only the planned deployment correlation and composes
  the existing Workload, S0, and Operation owners. `C4` publishes only healthy
  public endpoints through existing Edge/Gateway authority. `C5` adds
  REST/OpenAPI/client/CLI/Management MCP; Web remains deferred. `C6` retains
