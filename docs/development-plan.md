...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  `C0.3` remains in progress because `C0.3-PA2a` request-time audit
  attribution is frozen but not implemented, and later cross-layer security
  investigation, product usage-fact profile snapshots, and audit
  retention/export remain open.
...
- Implemented: a bounded Project-owned attribution profile contains a business
  ...
  this slice.
  Frozen next as `C0.3-PA2a`: each new audit write must state whether Project
...
- Frozen as `C0.3-PA2a`: signed audit export must wait until every new audit
  fact carries explicit request-time attribution. Migration `142` may extend
  ...
  query gains exact Project, Environment, profile, and status filters and
  ...
  read-only Management MCP operation must share the query and retain the 133/73
  catalogs. The retained PostgreSQL 17 audit gate must prove legacy handling,
