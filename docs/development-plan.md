...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@8`. The
  former `@1` through `@7` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `AUT0.5-C9`: `cloud.workflow.policy.v2`
  ...
  queue, scheduler, or configuration language. WorkflowRun v5 through v8
...
- Implemented as the component-only Workflow Connector Flow slice
  (`2026-08-20`): WorkflowRun input/runtime/Flow v8 and replay build
  `a3s-cloud-workflows@8` bind every exact provider attempt and observation to
  ...
  node result. Raw response bytes remain outside Flow. Historic v7 retains
  default-output behavior, v6 output stays reference-only, and v5 output stays
  digest-only and byte-compatible. Versions/builds `@1` through `@7` remain
  explicitly replayable. No table,
  migration, queue, timer worker, scheduler, retry counter, child Operation,
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, twenty-four accepted authority decisions, the immutable
   ...
   Plan v4/Run v7 exact default-output folding/evidence, and Run v8 strict typed
   JSON projection while retaining the no-duplicate authority tests;
