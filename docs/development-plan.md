...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, seventeen accepted authority decisions, the immutable
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   deterministic component-only frame/export and ordered region reducers;
   complete Flow-backed Iteration/Loop dispatch and child lifecycle, Answer frames, typed
