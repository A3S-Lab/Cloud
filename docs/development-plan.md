...
The current REST/OpenAPI contract is `1.49.0`. It retains `1.48.0`'s complete human-readable
operation, tag, parameter, request, response, example, authentication, and
compatibility documentation for the entire REST surface and extends immutable personal
alert-policy create/list/get/revoke from the first closed typed DomainClaim
status source to exact Gateway certificate-renewal failure/recovery facts,
without another endpoint or policy lifecycle, while retaining `1.46.0`'s
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.49.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 129-tool administrator and 70-tool read-only catalogs,
...
- Verified as `C0.3-N4c` on PostgreSQL 17 and NATS JetStream in CI:
  ...
  REST/OpenAPI `1.49.0` and the maintained client may expose the new value
