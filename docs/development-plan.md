...
| `H0.2` | Cardinality-one complete target sets, generation-bound private service endpoints, Gateway projection, exact acknowledgement and rollback | A private endpoint becomes eligible only after workload health and the exact target-set acknowledgement; restart cannot expose a stale generation |
...
I0 retains the same E0 restriction stated above: contract work and isolated
prototypes may start early, but no production Inference tables, providers,
routes, or user-visible capability claims land before E0 passes. See
