...
| E0 | In progress | PostgreSQL-backed route ownership, healthy target resolution, complete snapshot dispatch/replay, exact acknowledgement activation, and A3S Gateway 1.0.12 route-bearing validation, atomic reload, HTTP traffic switching, and last-good preservation pass. TLS, secrets, logs, update, rollback, web, and crash gates remain |
...
- Implemented with A3S Gateway 1.0.12: exact route-bearing compiler validation,
  node-local management validation, atomic compare-and-swap reload, real HTTP
  backend switching, last-good preservation, and durable acknowledgement
  ordering.
...
| 7 | Gateway reload before acknowledgement | In progress | A3S Gateway 1.0.12 route-bearing validate/reload, live HTTP backend switching, rejected-successor preservation, atomic installed-state publication, journal replay, and Gateway-before-command acknowledgement ordering pass. PostgreSQL API tests prove exact route activation and replay; process-death injection between reload and acknowledgement remains |
...
1. Add certificate policy, a local deterministic test CA, and a real TLS
   fixture on the verified A3S Gateway 1.0.12 route path.
