...
| C0 | In progress | `C0.1`, `C0.2`, and `C0.2m` are verified. One typed TypeScript client is shared by Web and CLI; the versioned OpenAPI envelope, bounded transport, safe token handling, tenant/operational reads, replay-safe mutations, evidence, logs, diagnostics, Search, Workload/Source/Secret/Identity/Fleet/Edge parity, and compatibility checks pass focused tests. The verified pre-extension Management MCP gate proved exact 23-tool administrator and 16-tool read-only catalogs. The current 71-tool administrator and 45-tool read-only catalogs retain those tools and add nine Identity, seven verified `W0.2` Ontology, ten `W0.3` Workflow definition/goal/plan, seven native Form lifecycle, seven WorkflowRun lifecycle, two protected HumanTask reads, and six verified `U0.2` plugin Registry/catalog read tools; focused catalog, permission, strict-argument, lifecycle, deterministic-plan, WorkflowRun/HumanTask, plugin tenant, role, and replay conformance pass. `C0.2m` uses the `2026-07-28` sessionless protocol with per-request metadata and `server/discover`. The backend-only `C0.3` slice implements stable human/service Principals, Membership roles, Principal-bound credentials, immediate role/revocation enforcement, last-owner protection, closed project/environment/node Resource Grants, A3S ORM/Outbox/audit writes, REST/OpenAPI `1.19.0`, client, CLI, and Management MCP. Invitations, OIDC, attribution, notifications, security investigation, audit-query interfaces, and role-focused frontend projections remain planned; `C0.3` is in progress and `C0.4` remains planned. |
...
PostgreSQL/Flow recovery evidence. Protected HumanTask list/detail APIs now
reuse the existing Workflow repository and shared Identity Resource Grant
evaluator. Public claim/release/submission, expiry/cancellation coordination,
and an end-to-end human workflow product surface remain open.
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.19.0`. Focused tests regenerate the
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 71-tool administrator and 45-tool read-only catalogs,
  including Identity, Ontology, Workflow planning, native Form lifecycle,
  WorkflowRun lifecycle, and protected HumanTask read extensions. The expanded clean A3S Box/PostgreSQL
...
pair, and monotonic aggregate version at every boundary. REST `1.19.0`, the
maintained client, `human-tasks` CLI commands, and two read-only Management MCP
tools additionally expose bounded protected HumanTask list/detail reads through
the same Workflow repository and shared Resource Grant evaluator. Public
claim/release/Form submission, expiry/cancellation coordination,
...
mutation/submission, service/finite-task dispatch, compensation, multi-day
