...
| CELL0 | In progress; unavailable | `CELL0.1-C1/C2/C3` implement canonical Service/application ACL, fixed provider guarantees, BuildRun/bundle/profile binding, ordered Cell classes, state-version migration and rollback checks, immutable revision lineage, the versioned desired-state aggregate, digest-locked shared fixtures, and deterministic S0/Workloads/Operations identities that reuse existing owners. S0 binding, provider/Runtime/Box certification, Cloud orchestration, Gateway publication, retained fault evidence, and `CELL0.5` availability remain open. |
