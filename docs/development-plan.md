...
The current REST/OpenAPI contract is `1.68.0`. It adds Plan/compiler v12,
failure v9, and the three closed Agent failure classifications to OpenAPI and
the maintained TypeScript client, with no new route or response property. It
retains `1.67.0`'s exact `cloud.workflow.policy.v4` cancellation-compensation
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.68.0`. Focused tests
