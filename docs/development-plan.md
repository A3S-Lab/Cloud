...
The current REST/OpenAPI contract is `1.75.0`. It adds closed Developer
Workflows Preview Policy acceptance plus current, bounded-history, and exact
immutable revision reads, and one exact behavioral pull-request Preview read.
The maintained client, CLI, and five additional Management MCP tools dispatch
the same command and four queries through two narrow Application read
authorities without adding a parser, repository, evaluator, or downstream
lifecycle. It retains `1.74.0`'s WorkloadProfile acceptance/current/history/
exact-revision boundary and four Management MCP tools, plus `1.73.0`'s closed
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.75.0`. Focused tests
...
- Implemented as the operational-mutation `C0.2` slice: Workload stop and
  ...
  tests prove the current 150-tool administrator and 87-tool read-only catalogs,
  ...
  outbound-subscription, and Developer Workflows BuildPlan/WorkloadProfile/Preview Management
  extensions. The
