...
| CELL0 | In progress; unavailable | `CELL0.1-C1/C2/C3` implement the canonical application foundation. Component-only `CELL0.2-C1/C2` freeze the sole-client CAS, exact Secret, storage, recovery, retention, restore, and deletion contracts without another lifecycle. `CELL0.2-C3` checks in the shared HTTPS S3-compatible CAS/cleanup gate, credential-safe evidence script, and manual workflow while removing the former duplicate raw test client. `CELL0.3-C1` binds the exact digest-pinned provider through the existing Workload/Runtime Service projector and Fleet apply receipt. A retained real-provider pass, recovery/deletion execution, typed operator dispatch, Runtime/Box provider certification, Cloud orchestration, Gateway publication, fault evidence, and `CELL0.5` availability remain open. |
...
- `CELL0.3` certifies a pinned provider and typed operator adapter as one
  ...
  acknowledgement with both typed Runtime endpoints. It adds no Cell Runtime
  class, Service template, command journal, endpoint registry, or receipt store.
  Typed operator dispatch and real provider certification remain.
