...
| AUT0 | In progress; unavailable | New-invocation schedules/webhooks/plugin events and reusable outbound connection profiles are specified in `ai-application-platform-plan.md`. Component-only `AUT0.5-C1` supplies the exact-revision execution port and bounded HTTP executor; verified `C2`-`C6` add immutable profiles/revisions, exact Secret materialization, public-Internet egress, exact-attempt evidence, durable fencing, conservative recovery, and atomic settlement; implemented `C7` exposes the same profile/revision CQRS without resolving Secrets; component-only `C8` binds exact WorkflowRun/Plan/step-attempt and Connector authority to C6; and `C9` freezes its bounded attempt budget and fallback delay. WorkflowRun v5 now keeps exact hook observation, durable wait, bounded retry, and fail-closed indeterminate decisions in Flow while C6 remains the sole provider-attempt authority. Component-only `C0.3-N2b` supplies the first exact-subject Notification NATS-to-C6 composition, while `N2c`-`N2e` retain Notification delivery decisions. W0.4 immutable response-object composition, remaining provider/consumer wiring, revocation/recovery operations, retained end-to-end integration evidence, and all Automations/Connectors product availability remain open. |
...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@5`. The
  former `@1`, `@2`, `@3`, and `@4` generations are explicit replay-compatible migration entries;
...
- Implemented as component-only `AUT0.5-C9`: `cloud.workflow.policy.v2`
  ...
  queue, scheduler, or configuration language. WorkflowRun v5 now consumes
  this policy while the HTTP Request node remains unavailable.
- Implemented as the component-only Workflow Connector Flow slice
  (`2026-08-20`): WorkflowRun input/runtime/Flow v5 and replay build
  `a3s-cloud-workflows@5` bind every exact provider attempt and observation to
  ...
  Accepted output and projection remain body-free. Versions/builds `@1`
  through `@4` remain explicitly replayable. No table, queue, timer worker,
  scheduler, retry counter, child Operation, response store, credential
  authority, provider configuration, or HTTP client is added.
...
- W0.4 immutable response-object composition, remaining provider/Event-consumer
  wiring, revocation/recovery operations, and retained
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   ...
   routing, and Run v5 Connector attempt/wait interpretation while retaining the no-duplicate authority
   tests;
