...
- Cloud pins native `a3s-form-core` `0.1.0` at exact revision
  ...
  Cloud copy.
- New Operation histories pin runtime build `a3s-cloud-workflows@3`. The
  former `@1` and `@2` generations are explicit replay-compatible migration entries;
...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, eighteen accepted authority decisions, the immutable
...
2. retain protected `W0.3` runs, reachable-sink Output aggregation, the
   ...
   and recovery; complete Answer frames, typed
   error branches/fallback, and Flow replay preservation
