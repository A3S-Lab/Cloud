...
| MCP0 | In progress; unavailable | Closed cross-repository contracts, Runtime profile/generation fencing, Cloud Runtime/Gateway compilers, and Gateway request/auth/single-dispatch/JSON-SSE foundations pass focused tests. Real Box/Linux, persistence/reconciliation, reload/drain/telemetry, and joint conformance remain open |
...
- Runtime consumes the semantics-profile digest and rejects stale generation
  ...
  `MCP0.2` gate;
- Cloud compiles the ordinary Runtime Service specification and deterministic
  canonical Gateway ACL projection, while persistence, reconciliation,
