...
| C0 | In progress | `C0.1`, `C0.2`, and `C0.2m` are verified. One typed TypeScript client is shared by Web and CLI; the versioned OpenAPI envelope, bounded transport, safe token handling, tenant/operational reads, replay-safe mutations, evidence, logs, diagnostics, Search, Workload/Source/Secret/Identity/Fleet/Edge parity, and compatibility checks pass focused tests. The verified pre-extension Management MCP gate proved exact 23-tool administrator and 16-tool read-only catalogs. The current 84-tool administrator and 49-tool read-only catalogs retain those tools and add fifteen Identity, one bounded tenant-administrator audit query, seven verified `W0.2` Ontology, ten `W0.3` Workflow definition/goal/plan, seven native Form lifecycle, seven WorkflowRun lifecycle, five protected HumanTask read/assignment/submission tools, three ExecutionTemplate tools, and six verified `U0.2` plugin Registry/catalog read tools; focused catalog, permission, strict-argument, lifecycle, deterministic-plan, WorkflowRun/HumanTask/ExecutionTemplate, plugin tenant, role, invitation, audit, and replay conformance pass. `C0.2m` uses the `2026-07-28` sessionless protocol with per-request metadata and `server/discover`. The `C0.3` slice implements stable human/service Principals, one explicit Principal-plus-Membership creation path, Membership roles, exact-Principal MembershipInvitations, Principal-bound credentials, exact OIDC issuer/subject links and replay-safe one-time flows, a bounded OIDC discovery/JWKS/ID-token adapter, production-wired login/link/callback routes and maintained-client entry points, immediate role/revocation enforcement, last-owner protection, closed project/environment/node Resource Grants, A3S ORM/Outbox/audit writes, and the redacted keyset audit projection through REST/OpenAPI `1.28.0`, client, CLI, and Management MCP. Its local PostgreSQL 17 OIDC cross-surface gate passes and is wired into the existing foundation job. Attribution, notifications, security investigation, audit retention/export, and role-focused frontend projections remain planned; `C0.3` is in progress and `C0.4` remains planned. |
...
- The REST contract boundary serves committed `openapi/v1.json` as raw public
  ...
  and every API response pin the current contract `1.28.0`. Focused tests regenerate the
...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  Membership. REST/OpenAPI `1.28.0`, the maintained client, CLI, and six new
  ...
  through REST/OpenAPI `1.28.0`; state-scoped callback-only HttpOnly cookies
  ...
  authenticates with the returned credential after restart. `C0.3` remains in
  progress because attribution, notifications, security investigation,
  audit retention/export, and
