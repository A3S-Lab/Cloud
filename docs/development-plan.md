...
| W0 | In progress; unavailable | `W0.1` implements the closed ACL-native Workflow/Ontology foundation and `W0.2` verifies immutable Ontology revisions, deterministic migration policy, and authorized Search. `W0.3` includes immutable definitions and Goals, native Forms, Goal/Plan-bound WorkflowRuns, HumanTasks, reachable-Output aggregation, and finite Execution. Migration `103` now persists exact descriptor bindings, the recoverable registry snapshot, and the typed-variable contract as one immutable WorkflowRevision semantic set; compiler schema 2 emits Plan v2 with exact descriptor and semantic/variable digest pins, prevents authority downgrade, and preserves Plan v1 byte shape. REST/OpenAPI `1.29.0`, client, CLI, MCP, focused domain/REST tests, and a real PostgreSQL migration/replay/rollback/immutability gate cover the slice. Plan v2 runtime compilation fails closed until the typed-variable Flow adapter exists. Built-in catalog discovery, runtime inspection, composite regions, Answer/error semantics, remaining application ports, compensation, expanded provider conformance, `W0.4`-`W0.5`, and public availability remain planned. |
