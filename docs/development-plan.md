...
- Implemented as the first backend-only `C0.3` slice: one stable human or
  ...
  attribution is implemented but still awaits retained main-CI certification,
  and later cross-layer security investigation, product usage-fact profile
...
- Implemented: a bounded Project-owned attribution profile contains a business
  ...
  this slice.
  Implemented next as `C0.3-PA2a`: each new audit write states whether Project
...
- Implemented locally as `C0.3-PA2a`, with retained main-CI certification
  pending: signed audit export must wait until every new audit fact carries
  explicit request-time attribution. Migration `142` extends
  ...
  catalogs. The retained PostgreSQL 17 audit gate covers legacy handling,
