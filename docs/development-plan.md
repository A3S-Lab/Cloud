...
1. retain the frozen versioned ACL parity manifest, its exact digest-bound
   23-node profile contract, twenty-two accepted authority decisions, the immutable
   ...
   response-object interpretation, and the C11 authorized transient read
   boundary while retaining the no-duplicate authority tests;
