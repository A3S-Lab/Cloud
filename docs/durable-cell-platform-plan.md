...
**Status as of 2026-08-18: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2/C3`, `CELL0.4-C1/C2/C3/C4/C5/C6`, and `CELL0.5-C1/C2/C3a/C3b/C4a` are implemented; the `C4b` behavior and `C4c` real-Gateway checks are staged in the same joint gate. The [`CELL0.3` real-Box celld Runtime gate](https://github.com/A3S-Lab/Cloud/actions/runs/31946279906/job/95162662254) and [`CELL0.4` PostgreSQL 17 C6a/C6b recovery and lifecycle gate](https://github.com/A3S-Lab/Cloud/actions/runs/31938471588/job/95144015600) have retained passes. The credential-safe joint `CELL0.5` Box/Execution/Artifact/Secret/S0/Workloads/Gateway gate is prepared to check publication, named SQLite state, exact alarm delivery, hibernatable WebSockets, idle eviction/reactivation, RPO=0 provider-process-death recovery, and managed-TLS HTTP/WebSocket routing through the pinned Gateway, but its exact-spec preflight is blocked until the pinned Box provider advertises and certifies Runtime `Outbound`. The `CELL0.2-C3` storage provider pass, the first retained `CELL0.5-C4b/C4c` behavior/Gateway pass, and lifecycle evidence stay open, so the product is unavailable.**
...
Retained real-provider execution, Operations/Flow composition, fault evidence,
and certification remain open.

Component-only `S0.1-C4` now supplies the missing provider-neutral execution
...
restore is observed again after cleanup. This is component execution, not an availability claim:
Operations/Flow wiring, durable operation/evidence persistence, production of
the Workloads fence receipt, and a retained S3-compatible fault pass remain.
...
| `C5` | In progress: shared component-only `S0.1-C4` now covers the owning storage primitive for writer-fence-receipted seal, isolated restore, grace-delayed deletion, interruption replay, cleanup, retained-restore verification, and cross-namespace/profile negatives. Durable Cells still needs Operations/Flow composition plus retained real-provider evidence for these checks, rollout, compatible rollback or forward-only rejection, and stop | No second rollout/restore/delete lifecycle, backup engine, cleanup worker, evidence store, or product availability claim before every check passes |
