...
**Status as of 2026-08-18: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2/C3`, `CELL0.4-C1/C2/C3/C4/C5/C6`, and `CELL0.5-C1/C2/C3a/C3b/C4a` are implemented; the `C4b` behavior check is staged in the same joint gate. The [`CELL0.3` real-Box celld Runtime gate](https://github.com/A3S-Lab/Cloud/actions/runs/31946279906/job/95162662254) and [`CELL0.4` PostgreSQL 17 C6a/C6b recovery and lifecycle gate](https://github.com/A3S-Lab/Cloud/actions/runs/31938471588/job/95144015600) have retained passes. The credential-safe joint `CELL0.5` Box/Execution/Artifact/Secret/S0/Workloads gate is prepared to check publication, named SQLite state, exact alarm delivery, hibernatable WebSockets, idle eviction/reactivation, and RPO=0 provider-process-death recovery, but its exact-spec preflight is blocked until the pinned Box provider advertises and certifies Runtime `Outbound`. The `CELL0.2-C3` storage provider pass, the first retained `CELL0.5-C4` behavior pass, Gateway, and lifecycle evidence stay open, so the product is unavailable.**
...
claims remain unverified; Gateway and availability claims remain explicitly
