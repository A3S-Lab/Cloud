...
**Status as of 2026-08-17: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2/C3`, `CELL0.4-C1/C2/C3/C4/C5/C6`, and `CELL0.5-C1/C2/C3a` are implemented; the [`CELL0.3` real-Box celld Runtime gate](https://github.com/A3S-Lab/Cloud/actions/runs/31946279906/job/95162662254) and [`CELL0.4` PostgreSQL 17 C6a/C6b recovery and lifecycle gate](https://github.com/A3S-Lab/Cloud/actions/runs/31938471588/job/95144015600) have retained passes, while the `CELL0.2-C3` storage provider pass and `CELL0.5-C3b` through `C5` remain open; the product is unavailable.**
...
| `C3` | In progress; component-only `C3a` extends the existing Execution aggregate through migration `119` with an internal-only exact-node Task policy. The same Execution Operations/Flow/Fleet rail projects one finite sandboxed Runtime `Task` with outbound networking, read-only shared-artifact mounts, exact Workload-revision Cloud Secret references, and immutable authority/semantics digests; its normal cleanup still uses Runtime Remove. Public Execution create/get/list/cancel surfaces cannot admit or manipulate these tasks. `C3b` must compose this primitive after exact Workload placement, bind the C1 provider-profile ACL and C2 bundle, invoke the pinned adapter, and block Service start/advance on its successful terminal evidence | No provider-native product configuration authority, long-lived credential cache, direct Control Plane object-store write, Durable Cells task table/worker, command queue, deploy state machine, new Runtime class, or second lifecycle |
...
`C3a` is deliberately only the reusable execution foundation. It does not
select an S0 endpoint or bucket, translate provider-native deployment files,
start a publisher, or declare a Workload ready. `C3b` must load the exact
canonical A3S ACL profile and existing deployment correlation, create or adopt
the deterministic node-bound Execution after placement, and admit the Service
only after the matching Task succeeds. Failure, timeout, cancellation, replay,
and Runtime cleanup remain the existing Execution/Operations/Flow behavior.
