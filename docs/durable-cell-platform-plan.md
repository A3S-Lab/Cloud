...
**Status as of 2026-08-16: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2`, and `CELL0.4-C1/C2` are implemented; the `CELL0.2-C3` storage gate, `CELL0.3-C3` runtime-only provider gate, and `CELL0.4-C1/C2` PostgreSQL gate are checked in but have no retained passes; the product is unavailable.**
...
Component-only `C1/C2` are implemented. Their shared PostgreSQL fixture verifies
...
compact references rather than duplicated ACL bodies, and absence of
Cell/deployment/queue tables. A retained run and `C3` through `C6` remain.
...
| `CELL0.4` | In progress | Persist the frozen aggregates through A3S ORM, then add idempotent commands/queries, managed Workload projection, Gateway publication, Operations, audit, REST/client/CLI/Management MCP; Web stays deferred | Component-only `C1` implements migration `116`, typed in-memory/PostgreSQL repositories, compact exact replay, immutable revision/state fences, and shared Outbox/audit writes without deployment or per-Cell state. `C2` registers authorization-before-replay create/revise/start/stop and bounded current/history CQRS while reusing existing environment, BuildRun, and bus owners. Their retained PostgreSQL pass and `C3`-`C6` remain; dependencies stay `CELL0.1`-`CELL0.3`, `E0`, `C0.3`, and `H0.2` |
