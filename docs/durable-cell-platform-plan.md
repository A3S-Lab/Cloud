...
**Status as of 2026-08-16: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2`, and `CELL0.4-C1/C2/C3/C4` are implemented; the `CELL0.2-C3` storage gate, `CELL0.3-C3` runtime-only provider gate, and `CELL0.4-C1/C2/C3` PostgreSQL gate are checked in but have no retained passes; the product is unavailable.**
...
| `C5` | In progress: bounded REST/OpenAPI `1.38.0` reuses C2-C4 and admits deployment through three canonical A3S ACL strings; maintained client, CLI, and Management MCP remain, while Web stays deferred by the active freeze | No presentation-owned state, repository, authorization mechanism, or configuration format |
...
Component-only `C1/C2/C3/C4` and the first `C5` REST/OpenAPI slice are
...
target resolution or dispatch. A retained run plus the maintained-client,
CLI, and Management MCP remainder of `C5` and all of `C6` remain.
