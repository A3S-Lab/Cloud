...
| `C4` | In progress. Component-only `C4a` makes the existing Workloads Service consume the same exact S0 bucket/application-prefix/endpoint/region used by publication, pins the reviewed celld Service profile and image, binds public/internal listeners plus the single-node loopback advertise identity, fixes the required idle-eviction policy at 30 seconds, and rejects process/environment/namespace drift during both deployment and publication recovery. The retained real Workloads/Edge gate must still serve one named Cell and prove SQLite state, alarms, hibernatable WebSockets, idle eviction/reactivation, and RPO=0 provider-process death | No provider-native desired-state authority, per-Cell Cloud row, owner lookup, Gateway affinity, provider scheduler, alarm timer, WebSocket store, or new Runtime class |
