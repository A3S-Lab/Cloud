...
**Status as of 2026-08-17: `CELL0.1-C1` through `CELL0.1-C3`, component-only `CELL0.2-C1/C2`, `CELL0.3-C1/C2/C3`, `CELL0.4-C1/C2/C3/C4/C5/C6`, and `CELL0.5-C1/C2/C3a/C3b` are implemented; the [`CELL0.3` real-Box celld Runtime gate](https://github.com/A3S-Lab/Cloud/actions/runs/31946279906/job/95162662254) and [`CELL0.4` PostgreSQL 17 C6a/C6b recovery and lifecycle gate](https://github.com/A3S-Lab/Cloud/actions/runs/31938471588/job/95144015600) have retained passes. The credential-safe joint `CELL0.5-C3` Box/Execution/Artifact/Secret/S0 publication gate is checked in but still needs its first retained pass; the `CELL0.2-C3` storage provider pass and `CELL0.5-C4/C5` also remain open, so the product is unavailable.**
...
retained pass remains the final open C3 item.
...
single-replica loopback `--advertise` value must all match. Its environment is
empty so an input cannot disable celld's default replicate-before-response
output gate. Workloads still owns the Service and Runtime projection, S0 owns
the namespace, Secrets owns materialization, and Edge owns public routing.
This is admission evidence only; no named-state, alarm, WebSocket, eviction,
process-death, Gateway, or availability claim is made until the retained C4
gate passes. Multi-node advertising remains `CELL0.6` work and must consume an
