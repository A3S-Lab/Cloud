...
Retained real-provider execution, the owning Workloads writer-fence/enqueue
composition, fault evidence, and certification remain open.
...
| `C5` | In progress: shared `S0.1-C4` covers the owning storage primitive and exact Operations/Flow runtime composition for writer-fence-receipted seal, isolated restore, grace-delayed deletion, interruption replay, cleanup, retained-restore verification, and cross-namespace/profile negatives. Durable Cells still needs the Workloads-owned fence receipt and durable enqueue plus retained real-provider evidence for these checks, rollout, compatible rollback or forward-only rejection, and stop | No second rollout/restore/delete lifecycle, backup engine, cleanup worker, evidence store, or product availability claim before every check passes |
