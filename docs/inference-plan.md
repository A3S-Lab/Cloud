...
until their owning exit gates pass. Before E0 is complete, implementation is
limited to contract design, tests, and isolated prototypes. Production tables,
routes, providers, and capability claims remain blocked by the existing E0
delivery rule.
...
Migration numbers are assigned only when a slice lands after E0. The logical
order and ownership are:
...
- Land the smallest general Artifacts foundation, E0 Secret bindings, Inference
  model/backend catalogs, immutable resolution, cache, vLLM compiler, one
...
5. general Artifact ingest/materialization/cache and E0 Secret bindings;
...
No slice bypasses E0, creates a parallel deployment path, or marks a capability
available before its real-provider and recovery gates pass.
