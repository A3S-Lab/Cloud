...
- Shared Edge ACL contract: managed `inference` policy must declare
  ...
  immutable `policy_revision` without inventing workers. Path-scope runs before
  binding/grant admission so wrong-environment URL or missing environment fail
  closed as `NotFound` without leaking admission errors or mutating the head.
  HTTP `POST .../inference/routes/{route_id}/revisions` is certified for
  write-scope, idempotent replay, policy-revision advance, stale CAS `409`, and
  path-scope `404`. Retire likewise
  ...
  `NotFound` for org-wide callers. Get and list similarly fail closed as
  `NotFound` for a missing environment UUID (not empty success), matching
  create/publish.
  Environment visibility fails closed as
  ...
  The same write paths also fail-close through Inference-owned
  `IInferenceGrantCredentialAdmissionPort` (Identity
  `IdentityInferenceGrantCredentialAdmissionAdapter`) so every grant's
  credential must exist in the same organization/project/environment, remain
  active at `requested_at`, and match `credential_generation` exactly,
  rejecting with `ApplicationError::Invalid` /
  `INFERENCE_GRANT_CREDENTIAL_INVALID` before any catalog write.
  ...
  Cloud likewise certifies the live managed route-rollout planner path:
  `GatewayRouteRolloutPlanner::new_managed` → `plan_managed`, with the same stub
  Identity credential and Inference route ACL ports plus
  `EmptyInferenceWorkerAclProjectionPort`, produces publication ACL(s) that embed
  `inference {`, the projected `prefix`, and route grants/models while omitting
  ...
  to `revoked = true` for Gateway fail-closed. Cloud also certifies §10.3
