...
| Multi-tenancy and administrator RBAC | One explicit Installation/Organization/Project/Environment scope plus Identity-owned platform roles and bounded support grants | `C0.5-MT1-C1/C2` provide canonical role/grant intent and replayable privileged-decision evidence; `MT1-C3` provides the canonical persisted Installation and shared scoped Audit/Outbox rail; verified `MT2-C1` adds the sole policy/binding repository, immutable current-head history, version CAS, idempotency, self-escalation and database-backed recoverable-owner invariants under one Installation lock. Evidence-backed support approval/grant persistence, current-snapshot Application decisions, cross-surface enforcement and hostile-tenant evidence remain |
