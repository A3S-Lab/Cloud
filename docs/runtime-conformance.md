...
The deployment-cancellation slice is implemented in that workflow and awaits
its pull-request run. It admits one explicit headless Service because the
pinned Box revision currently advertises `NetworkMode::None` and no Runtime
health capability. Cloud projects no ports and no health probe, then requires
an authoritative `RuntimeRemove` result before the exact
`ResourceClaimRelease` and terminal `Cancelled` transition. This is lifecycle
...
1. A green exact-Box run of the deployment-cancellation gate and
   abnormal-interruption cleanup on the exact Box revision.
