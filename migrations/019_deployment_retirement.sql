...
alter table deployments
    add constraint deployments_retirement_command_fk
    foreign key (node_id, retirement_command_id)
...
alter table deployments
    add constraint deployments_status_check check (
        status in (
            ...
            'retiring', 'cancelling', 'cleanup_pending', 'active', 'failed',
            'orphaned', 'cancelled'
        )
    ),
    ...
    add constraint deployments_activation_check check (
        status = 'orphaned'
        or (status in ('retiring', 'active')) = (activated_at is not null)
    ),
    add constraint deployments_cancellation_request_check check (
        status = 'orphaned'
        or (
            (status in ('cancelling', 'cleanup_pending', 'cancelled')) =
    ...
    add constraint deployments_node_check check (
        status not in (
            'scheduled', 'applying', 'verifying', 'retiring', 'cleanup_pending',
            'active', 'orphaned'
        ) or node_id is not null
    ),
    add constraint deployments_apply_command_check check (
        status not in (
            'applying', 'verifying', 'retiring', 'cleanup_pending', 'active',
            'orphaned'
    ...
    add constraint deployments_cleanup_command_check check (
        ...
        or (
            status not in ('cleanup_pending', 'orphaned', 'cancelled')
            and cleanup_command_id is null
        )
    ),
    add constraint deployments_retirement_command_check check (
        status in ('retiring', 'active', 'orphaned')
        or retirement_command_id is null
    );
