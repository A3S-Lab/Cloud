-- Extend the sole tenant-authorized Search projection with Cloud-owned Plugin
-- Registry metadata. A3S Use remains the authority for TUF/catalog contents.
...
with registered (
    ...
    select
        ...
        null::uuid,
        null::uuid,
        'plugin_registry'::text,
        registry.id,
        registry.name,
        'Plugin registry · ' || left(registry.endpoint, 494),
        registry.state,
        registry.updated_at
    from plugin_registries as registry
...
comment on view authorized_search_projections is
    'Credential-free authoritative Cloud metadata, including Plugin Registries, for tenant-authorized global search';
