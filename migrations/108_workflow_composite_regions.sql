...
alter table workflow_revision_semantic_contracts
    ...
    check (
        (kind = 'composite_regions'
            and schema = 'cloud.workflow.composite-regions.v1'
...
declare
    ...
    required_contract_count integer;
...
begin
    ...
    into contract_count, required_contract_count
    from workflow_revision_semantic_contracts
    where organization_id = new.organization_id
      and workflow_definition_id = new.workflow_definition_id
      and workflow_revision_id = new.id;

    if required_contract_count <> 3 or contract_count not in (3, 4, 5) then
        raise exception 'Workflow compiler schema 2 requires three semantic contracts and optional default/composite material';
    end if;
    return new;
end
$$;
...
comment on table workflow_revision_semantic_contracts is
    'Immutable revision-owned descriptors, typed variables, optional defaults, and optional bounded composite-region policy; not a mutable node catalog, variable store, scheduler, or queue';
