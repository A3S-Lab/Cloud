...
export const CLOUD_API_CONTRACT_VERSION = '1.85.0';
...
export class CloudApi {
  ...
  getWorkflowRunDiagnostics(
    ...
    workflowRunId: string,
    signal?: AbortSignal
  ): Promise<WorkflowRunDiagnostics> {
    return this.get(
      `/organizations/${encodeURIComponent(organizationId)}` +
        `/workflow-runs/${encodeURIComponent(workflowRunId)}/diagnostics`,
      signal
    );
  }
  ...
  enrollPluginRegistry(
    organizationId: string,
    input: EnrollPluginRegistryInput,
    idempotencyKey: string,
    signal?: AbortSignal
  ): Promise<PluginRegistryMutationResult> {
    return this.postJson(
      `/organizations/${encodeURIComponent(organizationId)}/plugin-registries`,
      idempotencyKey,
      input,
      signal
    );
