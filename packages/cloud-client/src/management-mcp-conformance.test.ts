...
it('pins the current privileged-management, Files, Developer Workflows, source discovery, signed-audit, and retention management MCP catalogs', () => {
  expect(ADMIN_TOOLS).toHaveLength(217);
  expect(READ_ONLY_TOOLS).toHaveLength(124);
  for (const tool of [
  ...
  expect(READ_ONLY_TOOLS).not.toContain('a3s_cloud_user_files_expire');
  expect(ADMIN_TOOLS.filter((candidate) => candidate === 'a3s_cloud_user_files_scan')).toEqual([
    'a3s_cloud_user_files_scan',
  ]);
  expect(ADMIN_TOOLS.filter((candidate) => candidate === 'a3s_cloud_user_files_expire')).toEqual([
...
conformanceIt(
  ...
  async () => {
    ...
    const readOnlyToolSet = new Set<string>([
      ...
      'a3s_cloud_partner_subject_links_resolve',
      'a3s_cloud_partner_subject_links_list',
      'a3s_cloud_directory_resource_grants_list',
      'a3s_cloud_directory_resource_grants_get',
    ...
    const destructiveToolSet = new Set<string>([
      ...
      'a3s_cloud_partner_subject_links_revoke',
      'a3s_cloud_directory_resource_grants_revoke',
