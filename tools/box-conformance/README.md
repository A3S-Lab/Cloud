...
`health=not_run`), records steps 5–9 as OPEN/not-run, prints
`power_revision=UNBOUND reason=pw0_no_pin_file`, then exits 3 with
`A3S_CLOUD_BX0_CLEAN_HOST_OPEN`. Missing node-agent, OCI, control-plane, or
health probe fail-closes with exit 1. It never emits
...
Darwin-safe enroll/OCI/deploy/health preflight library cases, unarmed /
armed-without-box paths, Linux stub pin missing/mismatch/match, Runtime
pin-missing, and node-agent / OCI / control-plane / health-probe missing
fail-closed behavior, then emits `A3S_CLOUD_BX0_CLEAN_HOST_CI_CERTIFIED` only.
Product EXIT stays open.
