...
grep -Fq 'bx0_require_execute_receipts_dir' "$gate"
grep -Fq 'next_loop=collect_bx0_clean_host_evidence.sh' "$gate"
grep -Fq 'product_exit=not_claimed' "$gate"
...
ACL
  ...
  grep -Fq 'next_loop=collect_bx0_clean_host_evidence.sh' <<<"$combined_exec"
  grep -Fq "next_loop_gate_evidence_dir=$exec_ok_evidence" <<<"$combined_exec"
  grep -Fq 'next_exit=run_bx0_clean_host_exit_audit.sh' <<<"$combined_exec"
  grep -Fq 'product_exit=not_claimed' <<<"$combined_exec"
