...
import type { WorkloadLogRecord } from '../../types/api';
...
export async function consumeLogRecordStream(
  response: Response,
  label: string,
  parsePage: (data: string) => LogRecordsPage,
  onRecords: (records: WorkloadLogRecord[]) => void,
  onEventId: (eventId: string) => void
): Promise<void> {
  await consumeSseStream(response, label, (event) => {
...
export function useLogStream(
      ...
      while (!controller.signal.aborted) {
        ...
        try {
          ...
          if (response.status === 401 || response.status === 403) {
            throw new Error(`${label} authorization failed`);
          }
          setState('live');
          setError(null);
          attempt = 0;
          await consumeLogRecordStream(
            response,
            label,
            parsePage,
            (incoming) => {
  ...
  }, [api, label, parsePage, url]);

  return { records, state, error };
}

export function isLogRecord(value: unknown): value is WorkloadLogRecord {
  if (
    !isRecord(value) ||
