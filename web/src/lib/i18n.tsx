...
const ZH_CN: Record<string, string> = {
  ...
  'Sole Agent run owner': '唯一 Agent 运行所有者',
  'Sign in to A3S Cloud': '登录 A3S Cloud',
  'Open the control plane': '进入控制平面',
  ...
  'Verifying...': '正在验证...',
  'Open control plane': '进入控制平面',
  'A3S Cloud platform capabilities': 'A3S Cloud 平台能力',
  'Desired state': '期望状态',
  ...
  'A3S Cloud control': 'A3S Cloud 控制层',
  ...
  'Sole Agent execution owner': '唯一 Agent 执行所有者',
  'A3S Cloud architecture': 'A3S Cloud 架构',
  'One control path from Cloud intent to the sole A3S Code Harness. Every exported image is rendered from the HTML below.':
    '从 Cloud 意图到唯一 A3S Code Harness 的统一控制路径。所有导出图片均由下方 HTML 实时渲染。',
  'Export PNG': '导出 PNG',
  'Exporting...': '正在导出...',
  ...
  'Scrollable A3S Cloud architecture diagram': '可滚动的 A3S Cloud 架构图',
  ...
  'A3S Code Core · Sole Agent execution Harness': 'A3S Code Core · 唯一 Agent 执行 Harness',
  'Cloud only orchestrates and transports': 'Cloud 仅负责编排与传输',
  'A3S Power (planned)': 'A3S Power（规划中）',
  'Infrastructure and trust boundaries': '基础设施与信任边界',
  'Immutable object storage': '不可变对象存储',
  ...
  'Create a digest-bound Service deployment to start convergence.':
    '创建绑定摘要的 Service 部署以开始收敛。',
  ...
  'This route projection does not reference a Gateway certificate.':
    '该路由投影未引用 Gateway 证书。',
  ...
  'Create a conversation to start an immutable Agent release.':
    '创建会话以运行不可变的 Agent 发布。',
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<I18nContextValue>(() => {
    ...
    return {
      ...
      setLanguage,
      t,
      label: (input) => (language === 'zh-CN' ? (LABELS_ZH_CN[input] ?? humanizeEnglish(input)) : humanizeEnglish(input)),
